(* ============================================================================
   (N, <=) IS A LINEAR ORDER COMPATIBLE WITH ADDITION  — one driver, one run.
   Isabelle/Pure running on the Rust PolyML interpreter (polyml-rs).
   ----------------------------------------------------------------------------
   WHAT THIS PROVES.  We build, from scratch, inside Isabelle's logical Pure,
   a tiny first-order object logic (type `o`) over the naturals (type `nat`):
     * Peano constructors Zero / Suc and the recursion equations for `add`/`mult`,
     * the equality predicate `oeq` (refl + Leibniz substitution), induction,
     * the existential `Ex`, falsity `oFalse`, Peano discrimination (Suc_neq_Zero,
       Suc_inj), and the disjunction `Disj` (disjI1/disjI2/disjE),
     * the order  `le m n  ==  Ex (%p. n = m + p)`  (?p. n = m + p).
   Then, by genuine kernel inference (induction + the substitution rule), it
   derives ALL the laws that make (nat, le) a LINEAR order compatible with +:
       le_refl       :  le n n                                  (reflexivity)
       zero_le       :  le 0 n                                  (least element)
       le_add        :  le m (m + p)                            (bound)
       le_trans      :  le m n ==> le n k ==> le m k            (transitivity)
       le_antisym    :  le m n ==> le n m ==> oeq m n           (=> PARTIAL order)
       le_suc_mono   :  le m n ==> le (Suc m) (Suc n)           (monotonicity)
       le_add_mono   :  le a b ==> le (add a c) (add b c)       (+-compatibility)
       le_total      :  Disj (le m n) (le n m)                  (=> LINEAR order)
   Each is a 0-hypothesis, schematic theorem, kernel-derived, and is validated
   two ways: it must have zero hypotheses AND its proposition must be `aconv`
   the intended schematic goal (built with the same `le` abbreviation).  Only
   when BOTH hold is "OK <name>" printed; ORDER_DONE fires iff all eight OK'd.

   HOW IT WAS BUILT (merge provenance).  Four separately-verified drivers each
   proved a slice of this on its own checkpoint:
       isa_ord_foundation.sml             -> le_refl / zero_le / le_add
                                             (+ add_left_cancel, add_eq_zero_left)
       isa_ord_le_trans.sml               -> le_trans
       isa_ord_le_suc_and_add_mono.sml    -> le_suc_mono, le_add_mono
       isa_ord_le_antisym.sml             -> le_antisym
       isa_ord_le_total.sml               -> le_total (adds the Disj connective)
   This file MERGES them into one run.  The semiring/order FOUNDATION of
   isa_ord_foundation.sml is copied VERBATIM as the prefix (object logic + Peano
   add/mult + the semiring laws + Ex/oFalse/Suc_neq_Zero/Suc_inj + the `le`
   abbreviation + le_intro + add_left_cancel + add_eq_zero_left + the easy order
   lemmas), ending on its single final theory `thyEZ` / context `ctxtO`.

   THE LOAD-BEARING SUBTLETY: every const added (Ex, oFalse, Disj) EXTENDS the
   theory, and a cterm minted on an earlier theory cannot be mixed into a later
   goal (cross-theory certificate mismatch / silent no-op instantiation).  So we
   fold the le_total driver's Disj const + disjI1/disjI2/disjE axioms onto ONE
   FINAL theory extension `thyT` (built on top of the foundation's last theory
   `thyEZ`), build ONE final context `ctxtT` / `ctermT`, and re-varify + route
   EVERYTHING used downstream (le_trans, le_suc_mono, le_add_mono, le_antisym,
   le_total) through `ctxtT`.  Shared lemmas (oeq_sym/_trans, Suc_cong,
   add_left_cancel, add_eq_zero_left, the easy order lemmas) are PROVEN ONCE in
   the foundation and REUSED; the per-driver helpers (add_cong_*, exE/disjE
   discharge wrappers, etc.) are DEDUPED and re-stated once on ctxtT.
   ============================================================================ *)
...
(* ============================================================================
   UNIFIED VALIDATION : every law 0-hyp AND aconv its intended schematic goal.
   The intended goals are built with the SAME `le` abbreviation so the Ex-body
   matches exactly.  Only an OK that passes BOTH guards counts.
   ============================================================================ *)
...
(* schematic Vars (re-stated for clarity; reuse foundation's where applicable) *)
val mVx = Var (("m",0), natT);
val nVx = Var (("n",0), natT);
val kVx = Var (("k",0), natT);
val pVx = Var (("p",0), natT);
val aVx = Var (("a",0), natT);
val bVx = Var (("b",0), natT);
val cVx = Var (("c",0), natT);

(* le_refl/zero_le/le_add are foundation theorems (schematic on ctxtO); their
   props are context-independent terms, so checkT (which only inspects the term)
   validates them just as well. *)
val r_refl  = checkT ("le_refl",  le_refl,  jT (le nVx nVx));
val r_zero  = checkT ("zero_le",  zero_le,  jT (le ZeroC nVx));
val r_ladd  = checkT ("le_add",   le_add,   jT (le mVx (add mVx pVx)));
val r_trans = checkT ("le_trans", le_trans,
                Logic.mk_implies (jT (le mVx nVx),
                  Logic.mk_implies (jT (le nVx kVx), jT (le mVx kVx))));
val r_anti  = checkT ("le_antisym", le_antisym,
                Logic.list_implies ([jT (le mVx nVx), jT (le nVx mVx)], jT (oeq mVx nVx)));
val r_sucm  = checkT ("le_suc_mono", le_suc_mono,
                Logic.mk_implies (jT (le mVx nVx), jT (le (suc mVx) (suc nVx))));
val r_addm  = checkT ("le_add_mono", le_add_mono,
                Logic.mk_implies (jT (le aVx bVx), jT (le (add aVx cVx) (add bVx cVx))));
val r_total = checkT ("le_total", le_total, jT (mkDisj (le mVx nVx) (le nVx mVx)));

(* ============================================================================
   SOUNDNESS PROBES : the kernel must REJECT obviously-false variants.
   ============================================================================ *)
(* le_trans must not be the reversed (false) rule le k m. *)
val sp_trans = not ((Thm.prop_of le_trans) aconv
      Logic.mk_implies (jT (le mVx nVx),
        Logic.mk_implies (jT (le nVx kVx), jT (le kVx mVx))));
(* le_antisym must not be the false one-premise rule le m n ==> oeq m n. *)
val sp_anti = not ((Thm.prop_of le_antisym) aconv
      Logic.mk_implies (jT (le mVx nVx), jT (oeq mVx nVx)));
val () = out ("SOUNDNESS le_trans-not-reversed=" ^ Bool.toString sp_trans
            ^ " le_antisym-not-one-premise=" ^ Bool.toString sp_anti ^ "\n");

(* ============================================================================
   FINAL VERDICT : ORDER_DONE iff all EIGHT order laws OK'd (and probes sound).
   ============================================================================ *)
val all_eight = r_refl andalso r_zero andalso r_ladd andalso r_trans
                andalso r_anti andalso r_sucm andalso r_addm andalso r_total;
val () = if all_eight andalso sp_trans andalso sp_anti
         then out "ORDER_DONE\n"
         else out "ORDER_INCOMPLETE\n";
