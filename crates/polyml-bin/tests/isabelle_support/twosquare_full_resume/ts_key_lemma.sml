(* ======================================================================
   KEY ONLY-IF LEMMA for FERMAT'S TWO-SQUARE (full form).

     key_onlyif :
       |- prime2 p ==> (Ex k. p = (k+k+k+k) + 3) ==> dvd p (a*a + b*b)
            ==> Conj (dvd p a)(dvd p b)

   i.e. for a (structural) prime p == 3 (mod 4), if p | a^2 + b^2 then
   p | a AND p | b.  0-hypothesis, aconv the intended statement, with
   soundness probes (the kernel rejects dropping the p==3mod4 premise,
   dropping the divisibility premise, or weakening the conjunctive
   conclusion).  Only classical assumption = ex_middle.

   PROOF (the "-1 is not a QR mod p==3mod4" + a gcd-free square trick):
     Suppose ~(p|a).  Then ~(p|b) (else p|b^2, p|(a^2+b^2) => p|a^2 => p|a).
     Set x = a * b^(2k)  (p = 4k+3, so 2k = (p-3)/2).
     Then  b^2 * x^2 = a^2 * b^(p-1) == a^2 (mod p)   [Fermat: b^(p-1)==1],
     and   a^2 == (p-1)*b^2 (mod p)                   [from a^2+b^2==0],
     so by cancelling b^2 (mod_cancel, p does not divide b^2):
           x^2 == (p-1) == -1 (mod p).
     But "x^2 == -1" is impossible for p == 3 (mod 4): with p-1 = 2m and m
     ODD, x^(p-1) = (x^2)^m == (-1)^m == -1 (m odd), while Fermat gives
     x^(p-1) == 1, hence -1 == 1 (mod p), i.e. p | 2, contradicting p > 2.
     So p|a; and then p|b follows directly (p|a => p|a^2; p|(a^2+b^2)).

   BASE: built on the isabelle_primes_1mod4 development (the euler_criterion /
   flt base + the parity machinery: apm1, lagrange_roots, x2j, dpow_odd,
   neg1_sq, cong_d1_false), which exposes everything the contradiction needs.
   The two cited graceful-floor lemmas (this one + Brahmagupta) are the
   contracted deliverables of the full-iff campaign.
   ====================================================================== *)
(* ======================================================================
