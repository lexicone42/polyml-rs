(* ============================================================================
   FUNDAMENTAL THEOREM OF ARITHMETIC — UNIQUENESS, in Isabelle/Pure on the
   polyml-rs interpreter.  Stage 4 (FINALE) of the FTA-uniqueness arc.
   (test: isabelle_fta_unique.rs)
   ----------------------------------------------------------------------------
     fta_unique :
       |- all_prime ps ==> all_prime qs ==> product ps = product qs
            ==> !r. count r ps = count r qs
   Two prime factorisations of the same number are the SAME multiset (every
   value occurs equally often in both lists).  Together with FTA existence
   (isabelle_fta.sml), this is the full Fundamental Theorem of Arithmetic,
   proved from first principles (only classical assumption = excluded middle)
   on a Rust reimplementation of PolyML.

   The 4-stage arc that got here (each a verified checkpoint):
     1. division theorem        (isabelle_division.sml)
     2. Euclid's lemma          (isabelle_euclid_lemma.sml, Gauss descent, no gcd)
     3. Euclid's lemma for lists (isabelle_euclid_list.sml, prime|product=>in_list)
     4. UNIQUENESS              (THIS) -- count + remove1 (conditional defining
        axioms, ex_middle case-splits) + product_remove1 / count_remove1_self /
        count_remove1_other / all_prime_remove1 / mult_left_cancel /
        product_one_nil, then fta_unique by list induction on prime_in_prime_list:
        a prime p of ps is in qs (Stage 3); remove one copy from qs, cancel p
        from the product (mult_left_cancel), recurse; count_remove1_* lift the
        tail multiset-equality to the full lists.
...
(* ============================================================================
   ============================================================================
   ***  STAGE 3 FOUNDATION : natlist + product + all_prime + in_list  ***
   ----------------------------------------------------------------------------
   Re-derive (lift from isabelle_fta.sml) the list/product/all_prime machinery
   on top of the EUCLID LEMMA base, ADD a membership predicate in_list, and
   prove prime_div_eq (two primes, p|q => p=q).

   We extend the FINAL euclid theory thyS2 ONE more time (thyF) with:
     - a type natlist + Nil/Cons
     - list equality leq (leq_refl + leq_subst => sym/trans/cong)
     - structural list_induct
     - product : natlist=>nat   (product_Nil / product_Cons)
     - all_prime : natlist=>o   (all_prime_Nil + all_prime_Cons_fwd/bwd over prime2)
     - in_list : nat=>natlist=>o  (in_list_Nil_elim + in_list_Cons_fwd/bwd)
   re-init ONE final context ctxtF / ctermF, re-varify EVERY reused base lemma
   onto it, route ALL new cterms through ctermF.

   PROVED (each 0-hyp / 0-extra-hyp, validated aconv):
     leq_sym / leq_trans / leq_cong
     in_list_here / in_list_there  (membership intros)
     in_list_cong  (rewrite the member by an equality, CAPTURE-AVOIDING)
     prime_div_eq  : jT (prime2 p) ==> jT (prime2 q) ==> jT (dvd p q) ==> jT (oeq p q)
   ============================================================================ *)

val () = out "STAGE3_FOUNDATION_BEGIN\n";

(* ---- the natlist type + constructors on thyF0 ---- *)
val thyF0 = Sign.add_types_global [(Binding.name "natlist",0,NoSyn)] thyS2;
val natlistN = Sign.full_name thyF0 (Binding.name "natlist");
val natlistT = Type (natlistN,[]);
val listpredT = natlistT --> oT;       (* natlist => o *)

val thyF1 = Sign.add_consts
  [(Binding.name "Nil",     natlistT, NoSyn),
   ...
   (Binding.name "all_prime", natlistT --> oT, NoSyn),
   (Binding.name "in_list", natT --> natlistT --> oT, NoSyn)] thyF0;

fun lcnst nm T = Const (Sign.full_name thyF1 (Binding.name nm), T);
...
val in_listC = lcnst "in_list" (natT --> natlistT --> oT);
fun in_list x l = in_listC $ x $ l;

(* ---- list-equality axioms (mirror oeq_refl / oeq_subst) ---- *)
val aL = Free ("a", natlistT); val bL = Free ("b", natlistT);
val PL = Free ("P", listpredT);
val ((_,leq_refl_ax),  thyF2) = Thm.add_axiom_global (Binding.name "leq_refl",
      jT (leq aL aL)) thyF1;
val ((_,leq_subst_ax), thyF3) = Thm.add_axiom_global (Binding.name "leq_subst",
      Logic.mk_implies (jT (leq aL bL), Logic.mk_implies (jT (PL $ aL), jT (PL $ bL)))) thyF2;
...
val list_induct_prop =
  Logic.mk_implies (PhiL $ NilC,
    Logic.mk_implies (Logic.all xLI (Logic.all lLI
      ...
      PhiL $ kLI));
val ((_,list_induct_ax), thyF4) =
  Thm.add_axiom_global (Binding.name "list_induct", list_induct_prop) thyF3;

(* ---- product recursion equations (oeq on nats) ---- *)
val xAp = Free ("x", natT);  val lAp = Free ("l", natlistT);
val ((_,product_Nil_ax),  thyF5) = Thm.add_axiom_global (Binding.name "product_Nil",
      jT (oeq (product NilC) (suc ZeroC))) thyF4;
val ((_,product_Cons_ax), thyF6) = Thm.add_axiom_global (Binding.name "product_Cons",
      jT (oeq (product (cons xAp lAp)) (mult xAp (product lAp)))) thyF5;

(* ---- all_prime axioms : Nil is true; Cons is the iff (two directional Imps) ---- *)
val ((_,all_prime_Nil_ax), thyF7) = Thm.add_axiom_global (Binding.name "all_prime_Nil",
      jT (all_prime NilC)) thyF6;
val ((_,all_prime_Cons_fwd_ax), thyF8) = Thm.add_axiom_global (Binding.name "all_prime_Cons_fwd",
      Logic.mk_implies (jT (all_prime (cons xAp lAp)),
                        jT (mkConj (prime2 xAp) (all_prime lAp)))) thyF7;
val ((_,all_prime_Cons_bwd_ax), thyF9) = Thm.add_axiom_global (Binding.name "all_prime_Cons_bwd",
      Logic.mk_implies (jT (mkConj (prime2 xAp) (all_prime lAp)),
                        jT (all_prime (cons xAp lAp))) ) thyF8;

(* ---- in_list (membership) axioms ----
     in_list_Nil_elim : jT (in_list x Nil) ==> jT oFalse            (nothing in Nil)
     in_list_Cons_fwd : jT (in_list x (Cons h t))
                          ==> jT (Disj (oeq x h) (in_list x t))
     in_list_Cons_bwd : jT (Disj (oeq x h) (in_list x t))
                          ==> jT (in_list x (Cons h t))                          *)
val xIL = Free ("x", natT); val hIL = Free ("h", natT); val tIL = Free ("t", natlistT);
val ((_,in_list_Nil_elim_ax), thyF10) = Thm.add_axiom_global (Binding.name "in_list_Nil_elim",
      Logic.mk_implies (jT (in_list xIL NilC), jT oFalseC)) thyF9;
val ((_,in_list_Cons_fwd_ax), thyF11) = Thm.add_axiom_global (Binding.name "in_list_Cons_fwd",
      Logic.mk_implies (jT (in_list xIL (cons hIL tIL)),
                        jT (mkDisj (oeq xIL hIL) (in_list xIL tIL)))) thyF10;
val ((_,in_list_Cons_bwd_ax), thyF) = Thm.add_axiom_global (Binding.name "in_list_Cons_bwd",
      Logic.mk_implies (jT (mkDisj (oeq xIL hIL) (in_list xIL tIL)),
                        jT (in_list xIL (cons hIL tIL)))) thyF11;

(* ---- THE ONE FINAL CONTEXT ctxtF / ctermF ---- *)
val ctxtF  = Proof_Context.init_global thyF;
val ctermF = Thm.cterm_of ctxtF;

val () = out "STAGE3_CONSTS_READY\n";

(* ---- re-varify EVERY reused base axiom/lemma onto ctxtF (schematic, valid in thyF) ---- *)
...
val lt_irrefl_vF   = varify lt_irrefl;
val leq_refl_vF        = varify leq_refl_ax;
val leq_subst_vF       = varify leq_subst_ax;
val list_induct_vF     = varify list_induct_ax;
val product_Nil_vF     = varify product_Nil_ax;
val product_Cons_vF    = varify product_Cons_ax;
val all_prime_Nil_vF       = varify all_prime_Nil_ax;
val all_prime_Cons_fwd_vF  = varify all_prime_Cons_fwd_ax;
val all_prime_Cons_bwd_vF  = varify all_prime_Cons_bwd_ax;
val in_list_Nil_elim_vF    = varify in_list_Nil_elim_ax;
val in_list_Cons_fwd_vF    = varify in_list_Cons_fwd_ax;
val in_list_Cons_bwd_vF    = varify in_list_Cons_bwd_ax;

(* ---- connective helpers re-stated on ctxtF / ctermF ---- *)
...
fun allE_atF Pabs at hForall =
  let val inst = beta_norm (Drule.infer_instantiate ctxtF
        [(("P",0), ctermF Pabs), (("a",0), ctermF at)] allE_vF)
  in Thm.implies_elim inst hForall end;
fun oFalse_elimF_at rT = beta_norm (Drule.infer_instantiate ctxtF
      [(("R",0), ctermF rT)] oFalse_elim_vF);
fun lt_irreflF_at nT h =
  let val inst = beta_norm (Drule.infer_instantiate ctxtF [(("n",0), ctermF nT)] lt_irrefl_vF)
  in Thm.implies_elim inst h end;

(* prime2 destructors on ctxtF (re-stated, reuse ppAbs from base) *)
fun prime2_gt1_atF p hPrime =
  conjunct1_atF (lt (suc ZeroC) p, mkForall (ppAbs p)) hPrime;
fun prime2_div_atF (p, d) hPrime hDvdDP =
  let
    val faThm = conjunct2_atF (lt (suc ZeroC) p, mkForall (ppAbs p)) hPrime;
    val impAt = allE_atF (ppAbs p) d faThm;
  in mp_atF (dvd d p, mkDisj (oeq d (suc ZeroC)) (oeq d p)) impAt hDvdDP end;

(* oeq-rewrite on ctxtF : oeq x y -> jT (P x) -> jT (P y) *)
fun oeq_rewrite_atF (Pabs, xT, yT) hxy hPx =
  let val inst = beta_norm (Drule.infer_instantiate ctxtF
        [(("P",0), ctermF Pabs), (("a",0), ctermF xT), (("b",0), ctermF yT)] oeq_subst_vF)
  in (inst OF [hxy]) OF [hPx] end;

(* ---- product / all_prime ground instantiators on ctxtF ---- *)
val product_Nil_thm = product_Nil_vF;   (* oeq (product Nil) (Suc Zero) *)
fun product_Cons_at (xt,lt0) = beta_norm (Drule.infer_instantiate ctxtF
        [(("x",0), ctermF xt),(("l",0), ctermF lt0)] product_Cons_vF);
fun all_prime_Cons_fwd_at (xt,lt0) h = Thm.implies_elim
      (beta_norm (Drule.infer_instantiate ctxtF
        [(("x",0), ctermF xt),(("l",0), ctermF lt0)] all_prime_Cons_fwd_vF)) h;
fun all_prime_Cons_bwd_at (xt,lt0) h = Thm.implies_elim
      (beta_norm (Drule.infer_instantiate ctxtF
        [(("x",0), ctermF xt),(("l",0), ctermF lt0)] all_prime_Cons_bwd_vF)) h;

(* ---- in_list ground instantiators on ctxtF ---- *)
fun in_list_Nil_elim_at xt h = Thm.implies_elim
      (beta_norm (Drule.infer_instantiate ctxtF [(("x",0), ctermF xt)] in_list_Nil_elim_vF)) h;
fun in_list_Cons_fwd_at (xt,ht,tt) h = Thm.implies_elim
      (beta_norm (Drule.infer_instantiate ctxtF
        [(("x",0), ctermF xt),(("h",0), ctermF ht),(("t",0), ctermF tt)] in_list_Cons_fwd_vF)) h;
fun in_list_Cons_bwd_at (xt,ht,tt) h = Thm.implies_elim
      (beta_norm (Drule.infer_instantiate ctxtF
        [(("x",0), ctermF xt),(("h",0), ctermF ht),(("t",0), ctermF tt)] in_list_Cons_bwd_vF)) h;

val () = out "STAGE3_HELPERS_READY\n";

(* ---- a checkF analogous to checkS2, on ctxtF ---- *)
...
(* ============================================================================
   LIST EQUALITY laws : leq_sym / leq_trans / leq_cong  (mirror oeq_sym etc.)
   ============================================================================ *)
val leq_sym =
  let
    ...
    val inst = beta_norm (Drule.infer_instantiate ctxtF
          [(("P",0), ctermF Pabs), (("a",0), ctermF aF), (("b",0), ctermF bF)] leq_subst_vF);
    val refl_aa = beta_norm (Drule.infer_instantiate ctxtF [(("a",0), ctermF aF)] leq_refl_vF);
    val step = inst OF [Thm.assume (ctermF (jT (leq aF bF))), refl_aa];
  in varify (Thm.implies_intr (ctermF (jT (leq aF bF))) step) end;
...
val leq_trans =
  let
    ...
    val inst = beta_norm (Drule.infer_instantiate ctxtF
          [(("P",0), ctermF Pabs), (("a",0), ctermF bF), (("b",0), ctermF cF)] leq_subst_vF);
    val H1 = Thm.assume (ctermF (jT (leq aF bF)));
    val H2 = Thm.assume (ctermF (jT (leq bF cF)));
    val step = inst OF [H2, H1];
    val t0 = Thm.implies_intr (ctermF (jT (leq bF cF))) step;
    val t1 = Thm.implies_intr (ctermF (jT (leq aF bF))) t0;
  in varify t1 end;

(* leq_cong : leq a b ==> jT (P a) ==> jT (P b)  (this IS leq_subst, restated) *)
val leq_cong = leq_subst_vF;

(* ---- validation : leq laws ---- *)
val aVlq = Var(("a",0),natlistT); val bVlq = Var(("b",0),natlistT); val cVlq = Var(("c",0),natlistT);
val PVlq = Var(("P",0),listpredT);
val leq_sym_intended   = Logic.mk_implies (jT (leq aVlq bVlq), jT (leq bVlq aVlq));
val leq_trans_intended =
  Logic.mk_implies (jT (leq aVlq bVlq), Logic.mk_implies (jT (leq bVlq cVlq), jT (leq aVlq cVlq)));
val leq_cong_intended =
  Logic.mk_implies (jT (leq aVlq bVlq), Logic.mk_implies (jT (PVlq $ aVlq), jT (PVlq $ bVlq)));
val r_leq_sym   = checkF ("leq_sym",   leq_sym,   leq_sym_intended);
val r_leq_trans = checkF ("leq_trans", leq_trans, leq_trans_intended);
val r_leq_cong  = checkF ("leq_cong",  leq_cong,  leq_cong_intended);

(* ============================================================================
   MEMBERSHIP convenience intros : in_list_here / in_list_there
     in_list_here  : jT (oeq x h) ==> jT (in_list x (Cons h t))
     in_list_there : jT (in_list x t) ==> jT (in_list x (Cons h t))
   ============================================================================ *)
val in_list_here =
  let
    val xF = Free("x",natT); val hF = Free("h",natT); val tF = Free("t",natlistT);
    val hEq  = Thm.assume (ctermF (jT (oeq xF hF)));                       (* oeq x h *)
    val disj = disjI1F_at (oeq xF hF, in_list xF tF) hEq;                  (* Disj (oeq x h)(in_list x t) *)
    val mem  = in_list_Cons_bwd_at (xF, hF, tF) disj;                      (* in_list x (Cons h t) *)
    val disch = Thm.implies_intr (ctermF (jT (oeq xF hF))) mem;
  in varify disch end;

val in_list_there =
  let
    val xF = Free("x",natT); val hF = Free("h",natT); val tF = Free("t",natlistT);
    val hMem = Thm.assume (ctermF (jT (in_list xF tF)));                   (* in_list x t *)
    val disj = disjI2F_at (oeq xF hF, in_list xF tF) hMem;                 (* Disj (oeq x h)(in_list x t) *)
    val mem  = in_list_Cons_bwd_at (xF, hF, tF) disj;                      (* in_list x (Cons h t) *)
    val disch = Thm.implies_intr (ctermF (jT (in_list xF tF))) mem;
  in varify disch end;

val xVih = Var(("x",0),natT); val hVih = Var(("h",0),natT); val tVih = Var(("t",0),natlistT);
val in_list_here_intended =
  Logic.mk_implies (jT (oeq xVih hVih), jT (in_list xVih (cons hVih tVih)));
val in_list_there_intended =
  Logic.mk_implies (jT (in_list xVih tVih), jT (in_list xVih (cons hVih tVih)));
val r_in_list_here  = checkF ("in_list_here",  in_list_here,  in_list_here_intended);
val r_in_list_there = checkF ("in_list_there", in_list_there, in_list_there_intended);

(* ============================================================================
   in_list_cong : jT (oeq x y) ==> jT (in_list x ps) ==> jT (in_list y ps)
   Rewrite the member by an equality.  CAPTURE-AVOIDING predicate
   %z. in_list z ps built with Term.lambda over a fresh Free.
   ============================================================================ *)
val in_list_cong =
  let
    val xF = Free("x",natT); val yF = Free("y",natT); val psF = Free("ps",natlistT);
    val zF = Free("z_ic", natT);
    val Pabs = Term.lambda zF (in_list zF psF);     (* %z. in_list z ps, correct de Bruijn *)
    val hEq  = Thm.assume (ctermF (jT (oeq xF yF)));        (* oeq x y *)
    val hMem = Thm.assume (ctermF (jT (in_list xF psF)));   (* in_list x ps *)
    val res  = oeq_rewrite_atF (Pabs, xF, yF) hEq hMem;     (* in_list y ps *)
    val d1 = Thm.implies_intr (ctermF (jT (in_list xF psF))) res;
    val d2 = Thm.implies_intr (ctermF (jT (oeq xF yF))) d1;
  in varify d2 end;

val xVic = Var(("x",0),natT); val yVic = Var(("y",0),natT); val psVic = Var(("ps",0),natlistT);
val in_list_cong_intended =
  Logic.mk_implies (jT (oeq xVic yVic),
    Logic.mk_implies (jT (in_list xVic psVic), jT (in_list yVic psVic)));
val r_in_list_cong = checkF ("in_list_cong", in_list_cong, in_list_cong_intended);

(* ============================================================================
   prime_div_eq : jT (prime2 p) ==> jT (prime2 q) ==> jT (dvd p q) ==> jT (oeq p q)
   Two primes, p|q implies p=q.
     From prime2 q, allE its Forall at d:=p :  Imp (dvd p q)(Disj (oeq p 1)(oeq p q)).
     mp with dvd p q  =>  Disj (oeq p 1)(oeq p q).
     disjE: oeq p 1 contradicts prime2 p (which gives lt 1 p => lt 1 1 => lt_irrefl);
            oeq p q => done.
   ============================================================================ *)
val prime_div_eq =
  let
    val pF = Free("p",natT); val qF = Free("q",natT);
    val hPp = Thm.assume (ctermF (jT (prime2 pF)));      (* prime2 p *)
    val hPq = Thm.assume (ctermF (jT (prime2 qF)));      (* prime2 q *)
    val hDvd= Thm.assume (ctermF (jT (dvd pF qF)));      (* dvd p q *)
    val goalC = oeq pF qF;

    (* prime2 q : p | q -> Disj (oeq p 1)(oeq p q) *)
    val disjPQ = prime2_div_atF (qF, pF) hPq hDvd;       (* Disj (oeq p 1)(oeq p q) *)

    (* CASE A : oeq p 1  -> contradiction with lt 1 p (from prime2 p) *)
    val caseA =
      let
        val hp1 = Thm.assume (ctermF (jT (oeq pF (suc ZeroC))));  (* oeq p 1 *)
        val gt1 = prime2_gt1_atF pF hPp;                          (* lt 1 p = lt (Suc 0) p *)
        (* rewrite lt 1 p  to  lt 1 1  via oeq p 1, capture-avoiding predicate %z. lt 1 z *)
        val zF   = Free("z_pde", natT);
        val ltPred = Term.lambda zF (lt (suc ZeroC) zF);          (* %z. lt 1 z *)
        val lt11 = oeq_rewrite_atF (ltPred, pF, suc ZeroC) hp1 gt1;  (* lt 1 1 = lt (Suc 0)(Suc 0) *)
        val fls  = lt_irreflF_at (suc ZeroC) lt11;                (* oFalse *)
        val anyG = oFalse_elimF_at goalC;                         (* jT oFalse ==> jT (oeq p q) *)
        val res  = Thm.implies_elim anyG fls;                     (* jT (oeq p q) *)
      in Thm.implies_intr (ctermF (jT (oeq pF (suc ZeroC)))) res end;

    (* CASE B : oeq p q  -> done *)
    val caseB =
      let
        val hpq = Thm.assume (ctermF (jT (oeq pF qF)));
      in Thm.implies_intr (ctermF (jT (oeq pF qF))) hpq end;

    val concl = disjE_elimF (oeq pF (suc ZeroC), oeq pF qF, goalC) disjPQ caseA caseB;  (* oeq p q *)
    val d1 = Thm.implies_intr (ctermF (jT (dvd pF qF))) concl;
    val d2 = Thm.implies_intr (ctermF (jT (prime2 qF))) d1;
    val d3 = Thm.implies_intr (ctermF (jT (prime2 pF))) d2;
  in varify d3 end;

val pVpd = Var(("p",0),natT); val qVpd = Var(("q",0),natT);
val prime_div_eq_intended =
  Logic.mk_implies (jT (prime2 pVpd),
    Logic.mk_implies (jT (prime2 qVpd),
      Logic.mk_implies (jT (dvd pVpd qVpd), jT (oeq pVpd qVpd))));
val r_prime_div_eq = checkF ("prime_div_eq", prime_div_eq, prime_div_eq_intended);

(* ---- SOUNDNESS PROBES ---- *)
val probe_pde_needs_dvd =
  let
    val bogus = Logic.mk_implies (jT (prime2 pVpd),
                  Logic.mk_implies (jT (prime2 qVpd), jT (oeq pVpd qVpd)));  (* drops dvd p q *)
  in not ((Thm.prop_of prime_div_eq) aconv bogus) end;
val probe_pde_nontrivial =
  not ((Thm.prop_of prime_div_eq) aconv (jT (oeq pVpd qVpd)));
val () =
  if probe_pde_needs_dvd andalso probe_pde_nontrivial
  then out "PROBE_OK prime_div_eq is conditional on dvd p q / nontrivial\n"
  else out "PROBE_UNSOUND prime_div_eq collapsed!\n";

(* ============================================================================
   FINAL VALIDATION
   ============================================================================ *)
val () =
  if r_leq_sym andalso r_leq_trans andalso r_leq_cong
     andalso r_in_list_here andalso r_in_list_there andalso r_in_list_cong
     andalso r_prime_div_eq andalso probe_pde_needs_dvd andalso probe_pde_nontrivial
  then out "EUL_FOUNDATION_OK\n"
  else out "EUL_FOUNDATION_FAILED\n";

(* ============================================================================
   ============================================================================
   ***  STAGE 3 HEADLINE : prime_in_prime_list  ***
   ----------------------------------------------------------------------------
   prime_in_prime_list :
       jT (prime2 p) ==> jT (all_prime ps) ==> jT (dvd p (product ps))
         ==> jT (in_list p ps)
   A prime dividing the product of a list of primes is one of them.
   PROOF by list_induct on ps with the (capture-avoiding) predicate
       PhiL = %ps. Imp (all_prime ps) (Imp (dvd p (product ps)) (in_list p ps))
   ============================================================================ *)

val () = out "PRIME_IN_LIST_BEGIN\n";

(* ---- re-varify the order/divisibility lemmas needed onto ctxtF ---- *)
val le_trans_vF      = varify le_trans;
val dvd_le_vF        = varify dvd_le;
val euclid_lemma_vF  = varify euclid_lemma;
val le_refl_vF       = varify le_refl;
fun le_reflF_at t = beta_norm (Drule.infer_instantiate ctxtF [(("n",0), ctermF t)] le_refl_vF);

(* le_trans on ctxtF : jT (le m n) ==> jT (le n k) ==> jT (le m k) *)
fun le_trans_atF (mt, nt, kt) h1 h2 =
  let val inst = beta_norm (Drule.infer_instantiate ctxtF
        [(("m",0), ctermF mt), (("n",0), ctermF nt), (("k",0), ctermF kt)] le_trans_vF)
  in Thm.implies_elim (Thm.implies_elim inst h1) h2 end;

(* dvd_le on ctxtF : jT (dvd d n) ==> (jT (oeq n 0) ==> jT oFalse) ==> jT (le d n) *)
fun dvd_le_atF (dt, nt) hdvd hnz =
  let val inst = beta_norm (Drule.infer_instantiate ctxtF
        [(("d",0), ctermF dt), (("n",0), ctermF nt)] dvd_le_vF)
  in Thm.implies_elim (Thm.implies_elim inst hdvd) hnz end;

(* euclid_lemma on ctxtF : jT (prime2 p) ==> jT (dvd p (mult a b)) ==> jT (Disj (dvd p a)(dvd p b)) *)
fun euclid_atF (pt, at, bt) hPr hDvd =
  let val inst = beta_norm (Drule.infer_instantiate ctxtF
        [(("p",0), ctermF pt), (("a",0), ctermF at), (("b",0), ctermF bt)] euclid_lemma_vF)
  in Thm.implies_elim (Thm.implies_elim inst hPr) hDvd end;

(* impI on ctxtF : from (hImpThm : jT A ==> jT B [meta]) get jT (Imp A B) *)
...
(* dvd_cong_r on ctxtF : oeq x y -> jT (dvd p x) -> jT (dvd p y)  (capture-avoiding) *)
fun dvd_cong_rF (pT, xT, yT) hxy hdvd =
  let val zF = Free("z_dcf", natT);
      val Pabs = Term.lambda zF (dvd pT zF)
  in oeq_rewrite_atF (Pabs, xT, yT) hxy hdvd end;

val () = out "PIL_HELPERS_READY\n";

(* ============================================================================
   THE PROOF
   ============================================================================ *)
val prime_in_prime_list =
  let
    val pF = Free("p", natT);

    (* the induction predicate, capture-avoiding (in_list/dvd inject Ex binders).
       list_induct's PhiL has type natlist => PROP, so the body must be jT-wrapped. *)
    val psFresh = Free("ps_pil", natlistT);
    val PhiLbody =
      jT (mkImp (all_prime psFresh)
                (mkImp (dvd pF (product psFresh)) (in_list pF psFresh)));
    val PhiLabs = Term.lambda psFresh PhiLbody;   (* %ps. Trueprop(Imp(all_prime ps)(Imp(dvd p (product ps))(in_list p ps))) *)

    (* the OUTER fixed prime hypothesis *)
    val hPrimeP = Thm.assume (ctermF (jT (prime2 pF)));     (* prime2 p *)

    (* ------------------------------------------------------------------
       NIL CASE : prove  jT (PhiL Nil)
         = Imp (all_prime Nil) (Imp (dvd p (product Nil)) (in_list p Nil))
       Assume all_prime Nil, assume dvd p (product Nil).
       product Nil = Suc Zero  =>  dvd p (Suc Zero).
       dvd p 1 + (1 != 0)  =>  le p 1.
       prime2 p  =>  lt 1 p = le 2 p.
       le_trans (le 2 p)(le p 1) = le 2 1 = lt 1 1 ; lt_irrefl => oFalse.
       oFalse_elim => in_list p Nil.
       ------------------------------------------------------------------ *)
    val nilCase =
      let
        val hAllN  = Thm.assume (ctermF (jT (all_prime NilC)));            (* all_prime Nil *)
        val hDvdN  = Thm.assume (ctermF (jT (dvd pF (product NilC))));     (* dvd p (product Nil) *)
        (* dvd p (Suc Zero) *)
        val dvd_p_1 = dvd_cong_rF (pF, product NilC, suc ZeroC) product_Nil_thm hDvdN;
        (* nonzero discharge : jT (oeq 1 0) ==> jT oFalse
           1 = Suc 0 ; oeq (Suc 0) 0 contradicts ... use suc_neq_zero?  We instead
           derive oFalse from oeq (Suc Zero) Zero via the structural fact that we
           can rewrite lt 0 1 (= le 1 1 = le_refl) ... simplest: build the meta-imp
           by assuming oeq 1 0 and deriving oFalse through a known num fact. *)
        val nzHyp =
          let
            val hz = Thm.assume (ctermF (jT (oeq (suc ZeroC) ZeroC)));     (* Suc 0 = 0 *)
            (* rewrite  lt 0 1 (= le 1 1, le_refl)  along Suc 0 = 0  to  lt 0 0, then lt_irrefl.
               lt 0 1 = le (Suc 0) 1 ; predicate %z. lt 0 z ? No: we have oeq (Suc 0) 0,
               so rewrite the FIRST argument.  Use lt 0 (Suc 0) = le (Suc 0)(Suc 0) = le_refl(Suc 0).
               Then rewrite the Suc 0 in the *second* arg position is what oeq gives (Suc 0 -> 0):
               predicate %z. lt Zero z  applied at a:=Suc 0, b:=0 needs oeq (Suc 0) 0. *)
            val lt0_1 = le_reflF_at (suc ZeroC);          (* le (Suc 0)(Suc 0) = lt 0 (Suc 0) *)
            val zF2   = Free("z_nz", natT);
            val ltPred = Term.lambda zF2 (lt ZeroC zF2);   (* %z. lt 0 z = le (Suc 0) z *)
            val lt0_0 = oeq_rewrite_atF (ltPred, suc ZeroC, ZeroC) hz lt0_1;  (* lt 0 0 *)
            val fls   = lt_irreflF_at ZeroC lt0_0;         (* oFalse *)
          in Thm.implies_intr (ctermF (jT (oeq (suc ZeroC) ZeroC))) fls end;
        (* le p 1 *)
        val le_p_1 = dvd_le_atF (pF, suc ZeroC) dvd_p_1 nzHyp;
        (* lt 1 p = le 2 p *)
        val gt1p = prime2_gt1_atF pF hPrimeP;            (* lt (Suc 0) p = le (Suc(Suc 0)) p *)
        (* le_trans (le 2 p)(le p 1) = le 2 1 = lt 1 1 *)
        val le_2_1 = le_trans_atF (suc (suc ZeroC), pF, suc ZeroC) gt1p le_p_1;  (* le 2 1 = lt 1 1 *)
        val fls    = lt_irreflF_at (suc ZeroC) le_2_1;   (* oFalse *)
        val memNil = (oFalse_elimF_at (in_list pF NilC)) OF [fls];   (* in_list p Nil *)
        (* assemble the two object Imps *)
        val inner  = impI_atF (dvd pF (product NilC), in_list pF NilC)
                       (Thm.implies_intr (ctermF (jT (dvd pF (product NilC)))) memNil);
        val outer  = impI_atF (all_prime NilC, mkImp (dvd pF (product NilC)) (in_list pF NilC))
                       (Thm.implies_intr (ctermF (jT (all_prime NilC))) inner);
      in outer end;   (* jT (PhiL Nil) *)

    (* ------------------------------------------------------------------
       CONS STEP : !!x xs. jT (PhiL xs) ==> jT (PhiL (Cons x xs))
       Fix x, xs ; assume IH : jT (PhiL xs).
       Prove jT (PhiL (Cons x xs)) = Imp(all_prime(Cons x xs))
             (Imp(dvd p (product(Cons x xs)))(in_list p (Cons x xs))).
       Assume all_prime(Cons x xs), assume dvd p (product(Cons x xs)).
       product(Cons x xs) = mult x (product xs)  =>  dvd p (mult x (product xs)).
       euclid_lemma => Disj (dvd p x)(dvd p (product xs)). disjE:
        A) dvd p x : all_prime_Cons_fwd => Conj(prime2 x)(all_prime xs); conjunct1 => prime2 x;
           prime_div_eq (prime2 p)(prime2 x)(dvd p x) => oeq p x ;
           in_list_here (oeq p x) => in_list p (Cons x xs).
        B) dvd p (product xs) : conjunct2 => all_prime xs ; mp IH at all_prime xs then
           dvd p (product xs) => in_list p xs ; in_list_there => in_list p (Cons x xs).
       ------------------------------------------------------------------ *)
    fun consStep xF xsF (ihThm : thm) =      (* ihThm : jT (PhiL xs) = jT (Imp(all_prime xs)(Imp(dvd p (product xs))(in_list p xs))) *)
      let
        val consL  = cons xF xsF;            (* Cons x xs *)
        val hAll   = Thm.assume (ctermF (jT (all_prime consL)));               (* all_prime (Cons x xs) *)
        val hDvd   = Thm.assume (ctermF (jT (dvd pF (product consL))));         (* dvd p (product (Cons x xs)) *)
        (* product (Cons x xs) = mult x (product xs) *)
        val pcEq   = product_Cons_at (xF, xsF);     (* oeq (product (Cons x xs)) (mult x (product xs)) *)
        val dvd_pm = dvd_cong_rF (pF, product consL, mult xF (product xsF)) pcEq hDvd; (* dvd p (mult x (product xs)) *)
        (* euclid => Disj (dvd p x)(dvd p (product xs)) *)
        val disjE  = euclid_atF (pF, xF, product xsF) hPrimeP dvd_pm;           (* Disj (dvd p x)(dvd p (product xs)) *)
        (* all_prime (Cons x xs) => Conj (prime2 x)(all_prime xs) *)
        val conjPX = all_prime_Cons_fwd_at (xF, xsF) hAll;                      (* Conj (prime2 x)(all_prime xs) *)
        val prime_x = conjunct1_atF (prime2 xF, all_prime xsF) conjPX;         (* prime2 x *)
        val allxs   = conjunct2_atF (prime2 xF, all_prime xsF) conjPX;         (* all_prime xs *)

        val goalC = in_list pF consL;

        (* CASE A : dvd p x *)
        val caseA =
          let
            val hdx = Thm.assume (ctermF (jT (dvd pF xF)));                     (* dvd p x *)
            (* prime_div_eq : prime2 p ==> prime2 x ==> dvd p x ==> oeq p x *)
            val pdInst = beta_norm (Drule.infer_instantiate ctxtF
                  [(("p",0), ctermF pF), (("q",0), ctermF xF)] (varify prime_div_eq));
            val oeqpx = Thm.implies_elim (Thm.implies_elim (Thm.implies_elim pdInst hPrimeP) prime_x) hdx; (* oeq p x *)
            (* in_list_here : oeq p (head=x) ==> in_list p (Cons x xs)  (head h := x, tail t := xs) *)
            val ihInst = beta_norm (Drule.infer_instantiate ctxtF
                  [(("x",0), ctermF pF), (("h",0), ctermF xF), (("t",0), ctermF xsF)] (varify in_list_here));
            val mem = Thm.implies_elim ihInst oeqpx;                            (* in_list p (Cons x xs) *)
          in Thm.implies_intr (ctermF (jT (dvd pF xF))) mem end;

        (* CASE B : dvd p (product xs) *)
        val caseB =
          let
            val hdpx = Thm.assume (ctermF (jT (dvd pF (product xsF))));          (* dvd p (product xs) *)
            (* IH = jT (Imp (all_prime xs)(Imp (dvd p (product xs))(in_list p xs))) *)
            val ih_inner = mp_atF (all_prime xsF, mkImp (dvd pF (product xsF)) (in_list pF xsF))
                              ihThm allxs;                                       (* Imp (dvd p (product xs))(in_list p xs) *)
            val in_xs = mp_atF (dvd pF (product xsF), in_list pF xsF) ih_inner hdpx;  (* in_list p xs *)
            (* in_list_there : in_list p xs ==> in_list p (Cons x xs)  (head h := x, tail t := xs) *)
            val itInst = beta_norm (Drule.infer_instantiate ctxtF
                  [(("x",0), ctermF pF), (("h",0), ctermF xF), (("t",0), ctermF xsF)] (varify in_list_there));
            val mem = Thm.implies_elim itInst in_xs;                            (* in_list p (Cons x xs) *)
          in Thm.implies_intr (ctermF (jT (dvd pF (product xsF)))) mem end;

        val memCons = disjE_elimF (dvd pF xF, dvd pF (product xsF), goalC) disjE caseA caseB; (* in_list p (Cons x xs) *)
        (* assemble the two object Imps *)
        val inner  = impI_atF (dvd pF (product consL), in_list pF consL)
                       (Thm.implies_intr (ctermF (jT (dvd pF (product consL)))) memCons);
        val outer  = impI_atF (all_prime consL, mkImp (dvd pF (product consL)) (in_list pF consL))
                       (Thm.implies_intr (ctermF (jT (all_prime consL))) inner);
      in outer end;   (* jT (PhiL (Cons x xs)) *)

    (* ------------------------------------------------------------------
       Apply list_induct.  Predicate PhiLabs at PhiL.  The minor premises:
         minorNil : jT (PhiL Nil)                                     (nilCase)
         minorCons : !!x xs. jT (PhiL xs) ==> jT (PhiL (Cons x xs))   (consStep, forall_intr'd)
       list_induct_vF (varified) :
         P Nil ==> (!!x l. P l ==> P (Cons x l)) ==> P k
       Instantiate P := PhiLabs, k := psK (a fresh Free).
       ------------------------------------------------------------------ *)
    val psK = Free("ps", natlistT);
    val liInst = beta_norm (Drule.infer_instantiate ctxtF
          [(("PhiL",0), ctermF PhiLabs), (("k",0), ctermF psK)] list_induct_vF);
    (* liInst (beta-normal) :
         jT (Imp(all_prime Nil)...) ==>
         (!!x l. jT (Imp(all_prime l)...) ==> jT (Imp(all_prime(Cons x l))...)) ==>
         jT (Imp(all_prime psK)...)                                                   *)

    (* beta-normal form of PhiL applied to a list term *)
    fun phiBN ps = jT (mkImp (all_prime ps)
                            (mkImp (dvd pF (product ps)) (in_list pF ps)));

    (* build the minor-Cons (beta-normal) : !!x l. phiBN l ==> phiBN (Cons x l) *)
    val xCs = Free("x", natT); val lCs = Free("l", natlistT);
    val ihAssumeBN = Thm.assume (ctermF (phiBN lCs));        (* jT (Imp(all_prime l)(...)) *)
    val consBody  = consStep xCs lCs ihAssumeBN;             (* jT (Imp(all_prime(Cons x l))(...)) *)
    val minorCons0 = Thm.implies_intr (ctermF (phiBN lCs)) consBody;
    val minorCons  = Thm.forall_intr (ctermF xCs) (Thm.forall_intr (ctermF lCs) minorCons0);

    (* nilCase is already in beta-normal shape phiBN Nil ; feed both directly to liInst *)
    val phiK = Thm.implies_elim (Thm.implies_elim liInst nilCase) minorCons;  (* jT (Imp(all_prime psK)...) *)

    (* now strip the two object Imps : assume all_prime psK, dvd p (product psK) -> in_list p psK *)
    val hAllK = Thm.assume (ctermF (jT (all_prime psK)));
    val hDvdK = Thm.assume (ctermF (jT (dvd pF (product psK))));
    val innerK = mp_atF (all_prime psK, mkImp (dvd pF (product psK)) (in_list pF psK)) phiK hAllK;
    val memK   = mp_atF (dvd pF (product psK), in_list pF psK) innerK hDvdK;   (* in_list p psK *)

    (* discharge in TARGET order : dvd (innermost), all_prime, prime2 (outermost) *)
    val d1 = Thm.implies_intr (ctermF (jT (dvd pF (product psK)))) memK;
    val d2 = Thm.implies_intr (ctermF (jT (all_prime psK))) d1;
    val d3 = Thm.implies_intr (ctermF (jT (prime2 pF))) d2;
  in varify d3 end;

(* ---- validation : prime_in_prime_list 0-hyp + aconv the TARGET ---- *)
val pVpil = Var (("p",0), natT);
val psVpil = Var (("ps",0), natlistT);
val prime_in_prime_list_intended =
  Logic.mk_implies (jT (prime2 pVpil),
    Logic.mk_implies (jT (all_prime psVpil),
      Logic.mk_implies (jT (dvd pVpil (product psVpil)),
        jT (in_list pVpil psVpil))));
val r_pil = checkF ("prime_in_prime_list", prime_in_prime_list, prime_in_prime_list_intended);

(* ---- SOUNDNESS PROBES : dropping all_prime, or dropping prime2 p, must NOT be provable ---- *)
val probe_pil_needs_allprime =
  let val bogus = Logic.mk_implies (jT (prime2 pVpil),
        Logic.mk_implies (jT (dvd pVpil (product psVpil)), jT (in_list pVpil psVpil)))  (* drops all_prime *)
  in not ((Thm.prop_of prime_in_prime_list) aconv bogus) end;
val probe_pil_needs_prime =
  let val bogus = Logic.mk_implies (jT (all_prime psVpil),
        Logic.mk_implies (jT (dvd pVpil (product psVpil)), jT (in_list pVpil psVpil)))  (* drops prime2 p *)
  in not ((Thm.prop_of prime_in_prime_list) aconv bogus) end;
val () =
  if probe_pil_needs_allprime andalso probe_pil_needs_prime
  then out "PROBE_OK prime_in_prime_list needs all_prime and prime2 p\n"
  else out "PROBE_UNSOUND prime_in_prime_list collapsed!\n";

val () =
  if r_pil andalso probe_pil_needs_allprime andalso probe_pil_needs_prime
  then out "PRIME_IN_LIST_DONE\n"
  else out "PRIME_IN_LIST_FAILED\n";

(* ============================================================================
   ============================================================================
   ***  STAGE 4 FOUNDATION : count + remove1 + bridging lemmas  ***
   ----------------------------------------------------------------------------
   ADD two list operations by CONDITIONAL axioms (no native if-then-else):
     count   : nat => natlist => nat   (occurrence count)
     remove1 : nat => natlist => natlist (remove the FIRST occurrence)
   then prove the bridging lemmas needed for FTA uniqueness (Stage 4 finale).
   Everything routes through ONE final context ctxtG / ctermG.
   ============================================================================ *)

val () = out "FTU_FOUNDATION_BEGIN\n";

(* ---- extend thyF with the two new operations ---- *)
val thyG1 = Sign.add_consts
  [(Binding.name "count",   natT --> natlistT --> natT, NoSyn),
   (Binding.name "remove1", natT --> natlistT --> natlistT, NoSyn)] thyF;

val countC   = Const (Sign.full_name thyG1 (Binding.name "count"),   natT --> natlistT --> natT);
fun count r l = countC $ r $ l;
val remove1C = Const (Sign.full_name thyG1 (Binding.name "remove1"), natT --> natlistT --> natlistT);
fun remove1 x l = remove1C $ x $ l;

(* ---- count : CONDITIONAL axioms ----
     count_Nil      : oeq (count r Nil) Zero
     count_Cons_eq  : jT (oeq r h) ==> jT (oeq (count r (Cons h t)) (Suc (count r t)))
     count_Cons_neq : jT (neg (oeq r h)) ==> jT (oeq (count r (Cons h t)) (count r t))      *)
val rCnt = Free ("r", natT); val hCnt = Free ("h", natT); val tCnt = Free ("t", natlistT);
val ((_,count_Nil_ax), thyG2) = Thm.add_axiom_global (Binding.name "count_Nil",
      jT (oeq (count rCnt NilC) ZeroC)) thyG1;
val ((_,count_Cons_eq_ax), thyG3) = Thm.add_axiom_global (Binding.name "count_Cons_eq",
      Logic.mk_implies (jT (oeq rCnt hCnt),
        jT (oeq (count rCnt (cons hCnt tCnt)) (suc (count rCnt tCnt))))) thyG2;
val ((_,count_Cons_neq_ax), thyG4) = Thm.add_axiom_global (Binding.name "count_Cons_neq",
      Logic.mk_implies (jT (neg (oeq rCnt hCnt)),
        jT (oeq (count rCnt (cons hCnt tCnt)) (count rCnt tCnt)))) thyG3;

(* ---- remove1 : CONDITIONAL axioms ----
     remove1_Nil      : leq (remove1 x Nil) Nil
     remove1_Cons_eq  : jT (oeq x h) ==> jT (leq (remove1 x (Cons h t)) t)
     remove1_Cons_neq : jT (neg (oeq x h)) ==> jT (leq (remove1 x (Cons h t)) (Cons h (remove1 x t)))  *)
val xRm = Free ("x", natT); val hRm = Free ("h", natT); val tRm = Free ("t", natlistT);
val ((_,remove1_Nil_ax), thyG5) = Thm.add_axiom_global (Binding.name "remove1_Nil",
      jT (leq (remove1 xRm NilC) NilC)) thyG4;
val ((_,remove1_Cons_eq_ax), thyG6) = Thm.add_axiom_global (Binding.name "remove1_Cons_eq",
      Logic.mk_implies (jT (oeq xRm hRm),
        jT (leq (remove1 xRm (cons hRm tRm)) tRm))) thyG5;
val ((_,remove1_Cons_neq_ax), thyG) = Thm.add_axiom_global (Binding.name "remove1_Cons_neq",
      Logic.mk_implies (jT (neg (oeq xRm hRm)),
        jT (leq (remove1 xRm (cons hRm tRm)) (cons hRm (remove1 xRm tRm))))) thyG6;

(* ---- THE final context ---- *)
val ctxtG  = Proof_Context.init_global thyG;
val ctermG = Thm.cterm_of ctxtG;

val () = out "STAGE4_CONSTS_READY\n";

(* ============================================================================
   re-varify EVERY reused base axiom/lemma onto ctxtG (schematic, valid in thyG)
   ============================================================================ *)
(* foundation / FOL *)
val oeq_refl_vG    = varify oeq_refl;
val oeq_subst_vG   = varify oeq_subst;
val oeq_sym_vG     = varify oeq_sym;
val oeq_trans_vG   = varify oeq_trans;
val Suc_cong_vG    = varify Suc_cong;
val exI_vG         = varify exI_ax;
val exE_vG         = varify exE_ax;
val oFalse_elim_vG = varify oFalse_elim_ax;
val Suc_neq_Zero_vG= varify Suc_neq_Zero_ax;
val Suc_inj_vG     = varify Suc_inj_ax;
val conjI_vG       = varify conjI_ax;
val conjunct1_vG   = varify conjunct1_ax;
val conjunct2_vG   = varify conjunct2_ax;
val disjI1_vG      = varify disjI1_ax;
val disjI2_vG      = varify disjI2_ax;
val disjE_vG       = varify disjE_ax;
val mp_vG          = varify mp_ax;
val impI_vG        = varify impI_ax;
val allI_vG        = varify allI_ax;
val allE_vG        = varify allE_ax;
val ex_middle_vG   = varify ex_middle_ax;
(* peano add / mult *)
val add_0_vG       = varify add_0;
val add_Suc_vG     = varify add_Suc;
val add_0_right_vG = varify add_0_right;
val add_Suc_right_vG = varify add_Suc_right;
val add_comm_vG    = varify add_comm;
val mult_0_vG      = varify mult_0;
val mult_Suc_vG    = varify mult_Suc;
val mult_0_right_vG= varify mult_0_right;
val mult_Suc_right_vG = varify mult_Suc_right;
val mult_comm_vG   = varify mult_comm;
val mult_assoc_vG  = varify mult_assoc;
val left_distrib_vG= varify left_distrib;
(* order / cancellation *)
val add_left_cancel_vG = varify add_left_cancel;
val zero_le_vG     = varify zero_le;
val le_refl_vG     = varify le_refl;
val le_add_vG      = varify le_add;
val le_suc_mono_vG = varify le_suc_mono;
val le_trans_vG    = varify le_trans;
val le_total_vG    = varify le_total;
val lt_irrefl_vG   = varify lt_irrefl;
val disj_zero_or_suc_vG = varify disj_zero_or_suc;
(* list machinery *)
val leq_refl_vG        = varify leq_refl_ax;
val leq_subst_vG       = varify leq_subst_ax;
val leq_sym_vG         = varify leq_sym;
val leq_trans_vG       = varify leq_trans;
val list_induct_vG     = varify list_induct_ax;
val product_Nil_vG     = varify product_Nil_ax;
val product_Cons_vG    = varify product_Cons_ax;
val all_prime_Nil_vG       = varify all_prime_Nil_ax;
val all_prime_Cons_fwd_vG  = varify all_prime_Cons_fwd_ax;
val all_prime_Cons_bwd_vG  = varify all_prime_Cons_bwd_ax;
val in_list_Nil_elim_vG    = varify in_list_Nil_elim_ax;
val in_list_Cons_fwd_vG    = varify in_list_Cons_fwd_ax;
val in_list_Cons_bwd_vG    = varify in_list_Cons_bwd_ax;
(* new conditional axioms *)
val count_Nil_vG       = varify count_Nil_ax;
val count_Cons_eq_vG   = varify count_Cons_eq_ax;
val count_Cons_neq_vG  = varify count_Cons_neq_ax;
val remove1_Nil_vG     = varify remove1_Nil_ax;
val remove1_Cons_eq_vG = varify remove1_Cons_eq_ax;
val remove1_Cons_neq_vG= varify remove1_Cons_neq_ax;

(* ============================================================================
   helper functions on ctxtG / ctermG
   ============================================================================ *)
(* oeq congruence/transitivity via OF (schematic, work on any context) *)
fun oeqreflG_at t = beta_norm (Drule.infer_instantiate ctxtG [(("a",0), ctermG t)] oeq_refl_vG);

(* generic oeq rewrite : oeq x y -> jT (P x) -> jT (P y) *)
fun oeq_rw_atG (Pabs, xT, yT) hxy hPx =
  let val inst = beta_norm (Drule.infer_instantiate ctxtG
        [(("P",0), ctermG Pabs), (("a",0), ctermG xT), (("b",0), ctermG yT)] oeq_subst_vG)
  in (inst OF [hxy]) OF [hPx] end;

(* connectives *)
fun mp_atG (At, Bt) hImp hA =
  let val inst = beta_norm (Drule.infer_instantiate ctxtG
        [(("A",0), ctermG At), (("B",0), ctermG Bt)] mp_vG)
  in Thm.implies_elim (Thm.implies_elim inst hImp) hA end;
fun impI_atG (At, Bt) hImpThm =
  let val inst = beta_norm (Drule.infer_instantiate ctxtG
        [(("A",0), ctermG At), (("B",0), ctermG Bt)] impI_vG)
  in Thm.implies_elim inst hImpThm end;
fun conjI_atG (At, Bt) hA hB =
  let val inst = beta_norm (Drule.infer_instantiate ctxtG
        [(("A",0), ctermG At), (("B",0), ctermG Bt)] conjI_vG)
  in Thm.implies_elim (Thm.implies_elim inst hA) hB end;
fun conjunct1_atG (At, Bt) hConj =
  let val inst = beta_norm (Drule.infer_instantiate ctxtG
        [(("A",0), ctermG At), (("B",0), ctermG Bt)] conjunct1_vG)
  in Thm.implies_elim inst hConj end;
fun conjunct2_atG (At, Bt) hConj =
  let val inst = beta_norm (Drule.infer_instantiate ctxtG
        [(("A",0), ctermG At), (("B",0), ctermG Bt)] conjunct2_vG)
  in Thm.implies_elim inst hConj end;
fun disjI1G_at (At,Bt) h = Thm.implies_elim (beta_norm (Drule.infer_instantiate ctxtG
      [(("A",0), ctermG At), (("B",0), ctermG Bt)] disjI1_vG)) h;
fun disjI2G_at (At,Bt) h = Thm.implies_elim (beta_norm (Drule.infer_instantiate ctxtG
      [(("A",0), ctermG At), (("B",0), ctermG Bt)] disjI2_vG)) h;
fun disjE_elimG (At, Bt, Ct) dThm caseA caseB =
  let
    val inst = beta_norm (Drule.infer_instantiate ctxtG
          [(("A",0), ctermG At), (("B",0), ctermG Bt), (("C",0), ctermG Ct)] disjE_vG);
    val s1 = Thm.implies_elim inst dThm;
    val s2 = Thm.implies_elim s1 caseA;
  in Thm.implies_elim s2 caseB end;
fun oFalse_elimG_at rT = beta_norm (Drule.infer_instantiate ctxtG
      [(("R",0), ctermG rT)] oFalse_elim_vG);
fun lt_irreflG_at nT h =
  let val inst = beta_norm (Drule.infer_instantiate ctxtG [(("n",0), ctermG nT)] lt_irrefl_vG)
  in Thm.implies_elim inst h end;
fun Suc_neq_Zero_atG t = beta_norm (Drule.infer_instantiate ctxtG [(("n",0), ctermG t)] Suc_neq_Zero_vG);
fun Suc_inj_atG (uT,vT) = beta_norm (Drule.infer_instantiate ctxtG
      [(("a",0), ctermG uT),(("b",0), ctermG vT)] Suc_inj_vG);

(* ex_middle at A : Disj A (neg A) *)
fun ex_middle_atG At = beta_norm (Drule.infer_instantiate ctxtG [(("A",0), ctermG At)] ex_middle_vG);

(* exI / exE on ctxtG *)
fun exI_atG Pabs at hbody =
  let val inst = beta_norm (Drule.infer_instantiate ctxtG
        [(("P",0), ctermG Pabs), (("a",0), ctermG at)] exI_vG)
  in Thm.implies_elim inst hbody end;
fun exE_elimG (Pabs, goalC) exThm wName bodyFn =
  let
    val inst = beta_norm (Drule.infer_instantiate ctxtG
                  [(("P",0), ctermG Pabs), (("Q",0), ctermG goalC)] exE_vG);
    val s1 = Thm.implies_elim inst exThm;
    val wF = Free (wName, natT);
    val hyp = Thm.assume (ctermG (jT (betapply (Pabs, wF))));
    val body = bodyFn wF hyp;
    val minor = Thm.forall_intr (ctermG wF) (Thm.implies_intr (ctermG (jT (betapply (Pabs, wF)))) body);
  in Thm.implies_elim s1 minor end;

(* le_intro on ctxtG (uses exI_vG, le abbreviation) *)
fun le_introG (mT, nT, w) hyp =
  let
    val Pabs = Abs ("p", natT, oeq nT (add mT (Bound 0)));
    val exI_inst = beta_norm (Drule.infer_instantiate ctxtG
          [(("P",0), ctermG Pabs), (("a",0), ctermG w)] exI_vG);
  in exI_inst OF [hyp] end;

(* add / mult ground instantiators on ctxtG *)
fun add0_atG t        = beta_norm (Drule.infer_instantiate ctxtG [(("n",0), ctermG t)] add_0_vG);
fun addSuc_atG (mt,nt)= beta_norm (Drule.infer_instantiate ctxtG
                          [(("m",0), ctermG mt),(("n",0), ctermG nt)] add_Suc_vG);
fun add0r_atG t       = beta_norm (Drule.infer_instantiate ctxtG [(("n",0), ctermG t)] add_0_right_vG);
fun mult0_atG t       = beta_norm (Drule.infer_instantiate ctxtG [(("n",0), ctermG t)] mult_0_vG);
fun multSuc_atG (mt,nt)= beta_norm (Drule.infer_instantiate ctxtG
                          [(("m",0), ctermG mt),(("n",0), ctermG nt)] mult_Suc_vG);
fun mult0r_atG t      = beta_norm (Drule.infer_instantiate ctxtG [(("n",0), ctermG t)] mult_0_right_vG);
fun multSr_atG (nt,mt)= beta_norm (Drule.infer_instantiate ctxtG
                          [(("n",0), ctermG nt),(("m",0), ctermG mt)] mult_Suc_right_vG);
fun multcomm_atG (mt,nt) = beta_norm (Drule.infer_instantiate ctxtG
                          [(("m",0), ctermG mt),(("n",0), ctermG nt)] mult_comm_vG);
fun mult_assoc_atG (mt,nt,kt) = beta_norm (Drule.infer_instantiate ctxtG
                          [(("m",0), ctermG mt),(("n",0), ctermG nt),(("k",0), ctermG kt)] mult_assoc_vG);

(* add congruence on the left / right argument (capture-avoiding via Abs+Bound) *)
fun add_cong_rG (kT, pT, qT) hpq =     (* oeq p q -> oeq (add k p) (add k q) *)
  let val Pabs = Abs("z", natT, oeq (add kT pT) (add kT (Bound 0)))
  in oeq_rw_atG (Pabs, pT, qT) hpq (oeqreflG_at (add kT pT)) end;
fun add_cong_lG (pT, qT, kT) hpq =     (* oeq p q -> oeq (add p k) (add q k) *)
  let val Pabs = Abs("z", natT, oeq (add pT kT) (add (Bound 0) kT))
  in oeq_rw_atG (Pabs, pT, qT) hpq (oeqreflG_at (add pT kT)) end;
fun mult_cong_rG (kT, pT, qT) hpq =    (* oeq p q -> oeq (mult k p) (mult k q) *)
  let val Pabs = Abs("z", natT, oeq (mult kT pT) (mult kT (Bound 0)))
  in oeq_rw_atG (Pabs, pT, qT) hpq (oeqreflG_at (mult kT pT)) end;
fun mult_cong_lG (pT, qT, kT) hpq =    (* oeq p q -> oeq (mult p k) (mult q k) *)
  let val Pabs = Abs("z", natT, oeq (mult pT kT) (mult (Bound 0) kT))
  in oeq_rw_atG (Pabs, pT, qT) hpq (oeqreflG_at (mult pT kT)) end;

(* left_distrib ground : oeq (mult x (add m n)) (add (mult x m)(mult x n)) *)
fun left_distrib_atG (xT, mT, nT) = beta_norm (Drule.infer_instantiate ctxtG
      [(("x",0), ctermG xT), (("m",0), ctermG mT), (("n",0), ctermG nT)] left_distrib_vG);

(* le_total ground : Disj (le m n)(le n m) *)
fun le_total_atG (mT, nT) = beta_norm (Drule.infer_instantiate ctxtG
      [(("m",0), ctermG mT), (("n",0), ctermG nT)] le_total_vG);
(* le_suc_mono : le m n -> le (Suc m)(Suc n) *)
fun le_suc_mono_atG (mT, nT) h = Thm.implies_elim (beta_norm (Drule.infer_instantiate ctxtG
      [(("m",0), ctermG mT),(("n",0), ctermG nT)] le_suc_mono_vG)) h;
fun zero_le_atG t = beta_norm (Drule.infer_instantiate ctxtG [(("n",0), ctermG t)] zero_le_vG);
fun le_refl_atG t = beta_norm (Drule.infer_instantiate ctxtG [(("n",0), ctermG t)] le_refl_vG);
fun dzos_atG t = beta_norm (Drule.infer_instantiate ctxtG [(("p",0), ctermG t)] disj_zero_or_suc_vG);
(* le_trans : le m n -> le n k -> le m k *)
fun le_trans_atG (mT,nT,kT) h1 h2 =
  let val inst = beta_norm (Drule.infer_instantiate ctxtG
        [(("m",0), ctermG mT),(("n",0), ctermG nT),(("k",0), ctermG kT)] le_trans_vG)
  in Thm.implies_elim (Thm.implies_elim inst h1) h2 end;

(* prime2 destructor : prime2 p -> lt (Suc 0) p *)
fun prime2_gt1_atG p hPrime =
  conjunct1_atG (lt (suc ZeroC) p, mkForall (ppAbs p)) hPrime;

(* list machinery ground instantiators *)
val product_Nil_thmG = product_Nil_vG;
fun product_Cons_atG (xt,lt0) = beta_norm (Drule.infer_instantiate ctxtG
        [(("x",0), ctermG xt),(("l",0), ctermG lt0)] product_Cons_vG);
fun all_prime_Cons_fwd_atG (xt,lt0) h = Thm.implies_elim
      (beta_norm (Drule.infer_instantiate ctxtG
        [(("x",0), ctermG xt),(("l",0), ctermG lt0)] all_prime_Cons_fwd_vG)) h;
fun all_prime_Cons_bwd_atG (xt,lt0) h = Thm.implies_elim
      (beta_norm (Drule.infer_instantiate ctxtG
        [(("x",0), ctermG xt),(("l",0), ctermG lt0)] all_prime_Cons_bwd_vG)) h;

(* leq congruence on a list-predicate : leq a b -> jT (P a) -> jT (P b) *)
fun leq_rw_atG (Pabs, xT, yT) hxy hPx =
  let val inst = beta_norm (Drule.infer_instantiate ctxtG
        [(("P",0), ctermG Pabs), (("a",0), ctermG xT), (("b",0), ctermG yT)] leq_subst_vG)
  in (inst OF [hxy]) OF [hPx] end;

(* count / remove1 ground conditional instantiators *)
fun count_Nil_atG t = beta_norm (Drule.infer_instantiate ctxtG [(("r",0), ctermG t)] count_Nil_vG);
fun count_Cons_eq_atG (rt,ht,tt) h = Thm.implies_elim
      (beta_norm (Drule.infer_instantiate ctxtG
        [(("r",0), ctermG rt),(("h",0), ctermG ht),(("t",0), ctermG tt)] count_Cons_eq_vG)) h;
fun count_Cons_neq_atG (rt,ht,tt) h = Thm.implies_elim
      (beta_norm (Drule.infer_instantiate ctxtG
        [(("r",0), ctermG rt),(("h",0), ctermG ht),(("t",0), ctermG tt)] count_Cons_neq_vG)) h;
fun remove1_Nil_atG t = beta_norm (Drule.infer_instantiate ctxtG [(("x",0), ctermG t)] remove1_Nil_vG);
fun remove1_Cons_eq_atG (xt,ht,tt) h = Thm.implies_elim
      (beta_norm (Drule.infer_instantiate ctxtG
        [(("x",0), ctermG xt),(("h",0), ctermG ht),(("t",0), ctermG tt)] remove1_Cons_eq_vG)) h;
fun remove1_Cons_neq_atG (xt,ht,tt) h = Thm.implies_elim
      (beta_norm (Drule.infer_instantiate ctxtG
        [(("x",0), ctermG xt),(("h",0), ctermG ht),(("t",0), ctermG tt)] remove1_Cons_neq_vG)) h;
...
val () = out "STAGE4_HELPERS_READY\n";

(* ============================================================================
   POSITIVITY HELPERS
   ============================================================================ *)
(* pos_of_suc : oeq x (Suc y) ==> lt Zero x   (x = Suc y => 0 < x) *)
fun pos_of_suc (xT, yT) hEq =          (* hEq : oeq x (Suc y) *)
  let
    val lez   = zero_le_atG yT;                          (* le 0 y *)
    val lt0Sy = le_suc_mono_atG (ZeroC, yT) lez;         (* le (Suc 0)(Suc y) = lt 0 (Suc y) *)
    val zP    = Free("z_pos", natT);
    val Pabs  = Term.lambda zP (lt ZeroC zP);            (* %z. lt 0 z *)
    val symEq = oeq_sym_vG OF [hEq];                     (* oeq (Suc y) x *)
  in oeq_rw_atG (Pabs, suc yT, xT) symEq lt0Sy end;     (* lt 0 x *)

(* suc_of_pos : lt Zero x ==> Ex(%y. oeq x (Suc y))  (0 < x => x has a predecessor) *)
val suc_of_pos =
  let
    val xF = Free("x", natT);
    val hLt = Thm.assume (ctermG (jT (lt ZeroC xF)));    (* lt 0 x = le (Suc 0) x = Ex(%p. oeq x (add (Suc 0) p)) *)
    val Pabs = Abs("p", natT, oeq xF (add (suc ZeroC) (Bound 0)));
    val goalAbs = Abs("y", natT, oeq xF (suc (Bound 0)));
    val goalC = mkEx goalAbs;
    fun body w (hw : thm) =                               (* hw : oeq x (add (Suc 0) w) *)
      let
        val aS  = addSuc_atG (ZeroC, w);                  (* add (Suc 0) w = Suc(add 0 w) *)
        val a0  = add0_atG w;                             (* add 0 w = w *)
        val sa0 = Suc_cong_vG OF [a0];                    (* Suc(add 0 w) = Suc w *)
        val chain = oeq_trans_vG OF [aS, sa0];            (* add (Suc 0) w = Suc w *)
        val xSw = oeq_trans_vG OF [hw, chain];            (* oeq x (Suc w) *)
      in exI_atG goalAbs w xSw end;                       (* Ex(%y. oeq x (Suc y)) *)
    val concl = exE_elimG (Pabs, goalC) hLt "p_sop" body;
  in varify (Thm.implies_intr (ctermG (jT (lt ZeroC xF))) concl) end;
val suc_of_pos_vG = varify suc_of_pos;
fun suc_of_pos_atG t h = Thm.implies_elim
      (beta_norm (Drule.infer_instantiate ctxtG [(("x",0), ctermG t)] suc_of_pos_vG)) h;

(* mult_eq_zero_factor : lt Zero c ==> oeq (mult c e) Zero ==> oeq e Zero *)
val mult_eq_zero_factor =
  let
    val cF = Free("c", natT); val eF = Free("e", natT);
    val hPos = Thm.assume (ctermG (jT (lt ZeroC cF)));        (* lt 0 c *)
    val hMz  = Thm.assume (ctermG (jT (oeq (mult cF eF) ZeroC)));  (* mult c e = 0 *)
    val goalC = oeq eF ZeroC;
    val dz = dzos_atG eF;                                     (* Disj (oeq e 0)(Ex(%q. oeq e (Suc q))) *)
    (* CASE e=0 : done *)
    val caseZ =
      let val hz = Thm.assume (ctermG (jT (oeq eF ZeroC)))
      in Thm.implies_intr (ctermG (jT (oeq eF ZeroC))) hz end;
    (* CASE Ex(%q. oeq e (Suc q)) : contradiction *)
    val PabsQ = Abs("q", natT, oeq eF (suc (Bound 0)));
    val caseS =
      let
        val exq = Thm.assume (ctermG (jT (mkEx PabsQ)));
        fun qbody q (hq : thm) =                              (* hq : oeq e (Suc q) *)
          let
            (* c = Suc c0 *)
            val exc0 = suc_of_pos_atG cF hPos;                (* Ex(%y. oeq c (Suc y)) *)
            val Pc0  = Abs("y", natT, oeq cF (suc (Bound 0)));
            fun c0body c0 (hc0 : thm) =                       (* hc0 : oeq c (Suc c0) *)
              let
                (* mult c e = mult c (Suc q) *)
                val mceq = mult_cong_rG (cF, eF, suc q) hq;   (* oeq (mult c e)(mult c (Suc q)) *)
                (* mult c (Suc q) = add c (mult c q) *)
                val msr  = multSr_atG (cF, q);                (* oeq (mult c (Suc q))(add c (mult c q)) *)
                (* add c (mult c q) = add (Suc c0)(mult c q) via oeq c (Suc c0) on left *)
                val cl   = add_cong_lG (cF, suc c0, mult cF q) hc0;  (* oeq (add c ..)(add (Suc c0) ..) *)
                (* add (Suc c0)(mult c q) = Suc(add c0 (mult c q)) *)
                val aS   = addSuc_atG (c0, mult cF q);        (* oeq (add (Suc c0) ..)(Suc(add c0 ..)) *)
                val chain1 = oeq_trans_vG OF [mceq, msr];     (* oeq (mult c e)(add c (mult c q)) *)
                val chain2 = oeq_trans_vG OF [chain1, cl];    (* oeq (mult c e)(add (Suc c0)(mult c q)) *)
                val chain3 = oeq_trans_vG OF [chain2, aS];    (* oeq (mult c e)(Suc(add c0 (mult c q))) *)
                val symC = oeq_sym_vG OF [chain3];            (* oeq (Suc(add c0 ..))(mult c e) *)
                val sucEq0 = oeq_trans_vG OF [symC, hMz];     (* oeq (Suc(add c0 ..)) 0 *)
                val fls  = Thm.implies_elim (Suc_neq_Zero_atG (add c0 (mult cF q))) sucEq0; (* oFalse *)
              in Thm.implies_elim (oFalse_elimG_at goalC) fls end;  (* oeq e 0 *)
            val g = exE_elimG (Pc0, goalC) exc0 "c0w" c0body;
          in g end;
        val g = exE_elimG (PabsQ, goalC) exq "qw" qbody;
      in Thm.implies_intr (ctermG (jT (mkEx PabsQ))) g end;
    val concl = disjE_elimG (oeq eF ZeroC, mkEx PabsQ, goalC) dz caseZ caseS;
    val d1 = Thm.implies_intr (ctermG (jT (oeq (mult cF eF) ZeroC))) concl;
    val d2 = Thm.implies_intr (ctermG (jT (lt ZeroC cF))) d1;
  in varify d2 end;
val mult_eq_zero_factor_vG = varify mult_eq_zero_factor;
fun mult_eq_zero_factor_atG (cT, eT) hPos hMz =
  let val inst = beta_norm (Drule.infer_instantiate ctxtG
        [(("c",0), ctermG cT),(("e",0), ctermG eT)] mult_eq_zero_factor_vG)
  in Thm.implies_elim (Thm.implies_elim inst hPos) hMz end;

val () = out "POSITIVITY_HELPERS_READY\n";

(* ============================================================================
   mult_left_cancel : lt Zero c ==> oeq (mult c a)(mult c b) ==> oeq a b
   ============================================================================ *)
val mult_left_cancel =
  let
    val cF = Free("c", natT); val aF = Free("a", natT); val bF = Free("b", natT);
    val hPos = Thm.assume (ctermG (jT (lt ZeroC cF)));          (* lt 0 c *)
    val hEq  = Thm.assume (ctermG (jT (oeq (mult cF aF) (mult cF bF))));  (* mult c a = mult c b *)
    val goalC = oeq aF bF;
    (* one-direction helper : from le u v (v = u + e) and oeq (mult c u)(mult c v) prove oeq u v *)
    fun oneDir (uF, vF) hmce =     (* hmce : oeq (mult c u)(mult c v) ; assume le u v *)
      let
        val Pabs = Abs("p", natT, oeq vF (add uF (Bound 0)));   (* le u v = Ex(%p. oeq v (add u p)) *)
        fun body e (heq : thm) =                                 (* heq : oeq v (add u e) *)
          let
            (* mult c v = mult c (add u e) *)
            val mcv  = mult_cong_rG (cF, vF, add uF e) heq;       (* oeq (mult c v)(mult c (add u e)) *)
            val ld   = left_distrib_atG (cF, uF, e);              (* oeq (mult c (add u e))(add (mult c u)(mult c e)) *)
            val mcv2 = oeq_trans_vG OF [mcv, ld];                 (* oeq (mult c v)(add (mult c u)(mult c e)) *)
            (* mult c u = mult c v -> mult c u = add (mult c u)(mult c e) *)
            val mcu_split = oeq_trans_vG OF [hmce, mcv2];         (* oeq (mult c u)(add (mult c u)(mult c e)) *)
            (* rewrite LHS mult c u -> add (mult c u) 0 *)
            val a0r  = add0r_atG (mult cF uF);                    (* oeq (add (mult c u) 0)(mult c u) *)
            val lhs0 = oeq_trans_vG OF [a0r, mcu_split];          (* oeq (add (mult c u) 0)(add (mult c u)(mult c e)) *)
            val canc = add_left_cancel_vG OF [lhs0];              (* oeq 0 (mult c e) *)
            val mce0 = oeq_sym_vG OF [canc];                      (* oeq (mult c e) 0 *)
            val e0   = mult_eq_zero_factor_atG (cF, e) hPos mce0; (* oeq e 0 *)
            (* v = add u e = add u 0 = u *)
            val cong = add_cong_rG (uF, e, ZeroC) e0;             (* oeq (add u e)(add u 0) *)
            val au0  = add0r_atG uF;                              (* oeq (add u 0) u *)
            val veq1 = oeq_trans_vG OF [heq, cong];               (* oeq v (add u 0) *)
            val vequ = oeq_trans_vG OF [veq1, au0];               (* oeq v u *)
          in oeq_sym_vG OF [vequ] end;                            (* oeq u v *)
        val leUV = Thm.assume (ctermG (jT (le uF vF)));
      in exE_elimG (Pabs, oeq uF vF) leUV "e_mlc" body end;
    (* le_total a b *)
    val tot = le_total_atG (aF, bF);                              (* Disj (le a b)(le b a) *)
    (* CASE le a b : a = b directly *)
    val caseAB =
      let
        val proof = oneDir (aF, bF) hEq;   (* oeq a b under (le a b) assumption *)
        (* proof has hyp (le a b); discharge to get le a b ==> oeq a b *)
        val disch = Thm.implies_intr (ctermG (jT (le aF bF))) proof;
      in disch end;
    (* CASE le b a : prove oeq b a then sym *)
    val caseBA =
      let
        val hEqSym = oeq_sym_vG OF [hEq];   (* oeq (mult c b)(mult c a) *)
        val proofBA = oneDir (bF, aF) hEqSym;  (* oeq b a under (le b a) *)
        val proofAB = oeq_sym_vG OF [proofBA]; (* oeq a b *)
        val disch = Thm.implies_intr (ctermG (jT (le bF aF))) proofAB;
      in disch end;
    val concl = disjE_elimG (le aF bF, le bF aF, goalC) tot caseAB caseBA;
    val d1 = Thm.implies_intr (ctermG (jT (oeq (mult cF aF) (mult cF bF)))) concl;
    val d2 = Thm.implies_intr (ctermG (jT (lt ZeroC cF))) d1;
  in varify d2 end;

val cVml = Var(("c",0),natT); val aVml = Var(("a",0),natT); val bVml = Var(("b",0),natT);
val mult_left_cancel_intended =
  Logic.mk_implies (jT (lt ZeroC cVml),
    Logic.mk_implies (jT (oeq (mult cVml aVml) (mult cVml bVml)), jT (oeq aVml bVml)));
val r_mlc = checkG ("mult_left_cancel", mult_left_cancel, mult_left_cancel_intended);

(* ============================================================================
   mult_pos : lt Zero u ==> lt Zero v ==> lt Zero (mult u v)
   ============================================================================ *)
val mult_pos =
  let
    val uF = Free("u", natT); val vF = Free("v", natT);
    val hPu = Thm.assume (ctermG (jT (lt ZeroC uF)));   (* lt 0 u *)
    val hPv = Thm.assume (ctermG (jT (lt ZeroC vF)));   (* lt 0 v *)
    val goalC = lt ZeroC (mult uF vF);
    (* u = Suc u0 *)
    val exu = suc_of_pos_atG uF hPu;                     (* Ex(%y. oeq u (Suc y)) *)
    val Pu  = Abs("y", natT, oeq uF (suc (Bound 0)));
    fun ubody u0 (hu0 : thm) =                           (* hu0 : oeq u (Suc u0) *)
      let
        (* v = Suc v0 *)
        val exv = suc_of_pos_atG vF hPv;                 (* Ex(%y. oeq v (Suc y)) *)
        val Pv  = Abs("y", natT, oeq vF (suc (Bound 0)));
        fun vbody v0 (hv0 : thm) =                       (* hv0 : oeq v (Suc v0) *)
          let
            val c1 = mult_cong_lG (uF, suc u0, vF) hu0;      (* oeq (mult u v)(mult (Suc u0) v) *)
            val c2 = multSuc_atG (u0, vF);                   (* oeq (mult (Suc u0) v)(add v (mult u0 v)) *)
            val c3 = add_cong_lG (vF, suc v0, mult u0 vF) hv0;  (* oeq (add v ..)(add (Suc v0) ..) *)
            val c4 = addSuc_atG (v0, mult u0 vF);            (* oeq (add (Suc v0) ..)(Suc(add v0 ..)) *)
            val ch1 = oeq_trans_vG OF [c1, c2];
            val ch2 = oeq_trans_vG OF [ch1, c3];
            val ch3 = oeq_trans_vG OF [ch2, c4];            (* oeq (mult u v)(Suc(add v0 (mult u0 v))) *)
          in pos_of_suc (mult uF vF, add v0 (mult u0 vF)) ch3 end;  (* lt 0 (mult u v) *)
        val g = exE_elimG (Pv, goalC) exv "v0w" vbody;
      in g end;
    val concl = exE_elimG (Pu, goalC) exu "u0w" ubody;
    val d1 = Thm.implies_intr (ctermG (jT (lt ZeroC vF))) concl;
    val d2 = Thm.implies_intr (ctermG (jT (lt ZeroC uF))) d1;
  in varify d2 end;
val mult_pos_vG = varify mult_pos;
fun mult_pos_atG (uT, vT) hu hv =
  let val inst = beta_norm (Drule.infer_instantiate ctxtG
        [(("u",0), ctermG uT),(("v",0), ctermG vT)] mult_pos_vG)
  in Thm.implies_elim (Thm.implies_elim inst hu) hv end;

(* prime2 p => lt Zero p   (1<p so 0<p) *)
fun prime2_pos_atG p hPr =
  let
    val gt1 = prime2_gt1_atG p hPr;                          (* lt (Suc 0) p = le (Suc(Suc 0)) p *)
    val le01 = zero_le_atG (suc ZeroC);                      (* le 0 (Suc 0) *)
    val le12 = le_suc_mono_atG (ZeroC, suc ZeroC) le01;      (* le (Suc 0)(Suc(Suc 0)) *)
  in le_trans_atG (suc ZeroC, suc (suc ZeroC), p) le12 gt1 end;  (* le (Suc 0) p = lt 0 p *)

val () = out "MULT_POS_READY\n";

(* ============================================================================
   product_pos : all_prime ps ==> lt Zero (product ps)
   ============================================================================ *)
val product_pos =
  let
    val psFresh = Free("ps_pp", natlistT);
    val PhiLbody = jT (mkImp (all_prime psFresh) (lt ZeroC (product psFresh)));
    val PhiLabs = Term.lambda psFresh PhiLbody;

    (* NIL : Imp (all_prime Nil)(lt 0 (product Nil)) *)
    val nilCase =
      let
        (* lt 0 (Suc 0) *)
        val lt0s0 = pos_of_suc (suc ZeroC, ZeroC) (oeqreflG_at (suc ZeroC));  (* lt 0 (Suc 0) *)
        (* rewrite Suc 0 -> product Nil *)
        val pn_sym = oeq_sym_vG OF [product_Nil_thmG];       (* oeq (Suc 0)(product Nil) *)
        val zP = Free("z_pn", natT);
        val Pabs = Term.lambda zP (lt ZeroC zP);             (* %z. lt 0 z *)
        val lt0pn = oeq_rw_atG (Pabs, suc ZeroC, product NilC) pn_sym lt0s0;  (* lt 0 (product Nil) *)
        val hAll = Thm.assume (ctermG (jT (all_prime NilC)));
      in impI_atG (all_prime NilC, lt ZeroC (product NilC))
           (Thm.implies_intr (ctermG (jT (all_prime NilC))) lt0pn) end;

    (* CONS : !!x xs. PhiL xs ==> PhiL (Cons x xs) *)
    fun consStep xF xsF (ihThm : thm) =     (* ihThm : jT (Imp(all_prime xs)(lt 0 (product xs))) *)
      let
        val consL = cons xF xsF;
        val hAll  = Thm.assume (ctermG (jT (all_prime consL)));    (* all_prime (Cons x xs) *)
        val conjPX = all_prime_Cons_fwd_atG (xF, xsF) hAll;        (* Conj(prime2 x)(all_prime xs) *)
        val prime_x = conjunct1_atG (prime2 xF, all_prime xsF) conjPX;
        val allxs   = conjunct2_atG (prime2 xF, all_prime xsF) conjPX;
        val ihInner = mp_atG (all_prime xsF, lt ZeroC (product xsF)) ihThm allxs;  (* lt 0 (product xs) *)
        val px_pos  = prime2_pos_atG xF prime_x;                   (* lt 0 x *)
        val mpos    = mult_pos_atG (xF, product xsF) px_pos ihInner;  (* lt 0 (mult x (product xs)) *)
        (* rewrite mult x (product xs) -> product (Cons x xs) *)
        val pc      = product_Cons_atG (xF, xsF);                  (* oeq (product (Cons x xs))(mult x (product xs)) *)
        val pc_sym  = oeq_sym_vG OF [pc];                          (* oeq (mult x (product xs))(product (Cons x xs)) *)
        val zP = Free("z_pc", natT);
        val Pabs = Term.lambda zP (lt ZeroC zP);
        val ltpc = oeq_rw_atG (Pabs, mult xF (product xsF), product consL) pc_sym mpos;  (* lt 0 (product (Cons x xs)) *)
      in impI_atG (all_prime consL, lt ZeroC (product consL))
           (Thm.implies_intr (ctermG (jT (all_prime consL))) ltpc) end;

    (* apply list_induct *)
    val psK = Free("ps", natlistT);
    val liInst = beta_norm (Drule.infer_instantiate ctxtG
          [(("PhiL",0), ctermG PhiLabs), (("k",0), ctermG psK)] list_induct_vG);
    fun phiBN ps = jT (mkImp (all_prime ps) (lt ZeroC (product ps)));
    val xCs = Free("x", natT); val lCs = Free("l", natlistT);
    val ihAssumeBN = Thm.assume (ctermG (phiBN lCs));
    val consBody = consStep xCs lCs ihAssumeBN;
    val minorCons0 = Thm.implies_intr (ctermG (phiBN lCs)) consBody;
    val minorCons  = Thm.forall_intr (ctermG xCs) (Thm.forall_intr (ctermG lCs) minorCons0);
    val phiK = Thm.implies_elim (Thm.implies_elim liInst nilCase) minorCons;  (* jT (Imp(all_prime psK)(lt 0 (product psK))) *)
    (* strip the object Imp *)
    val hAllK = Thm.assume (ctermG (jT (all_prime psK)));
    val ltK   = mp_atG (all_prime psK, lt ZeroC (product psK)) phiK hAllK;
    val d1 = Thm.implies_intr (ctermG (jT (all_prime psK))) ltK;
  in varify d1 end;

val psVpp = Var(("ps",0),natlistT);
val product_pos_intended =
  Logic.mk_implies (jT (all_prime psVpp), jT (lt ZeroC (product psVpp)));
val r_pp = checkG ("product_pos", product_pos, product_pos_intended);
val product_pos_vG = varify product_pos;
fun product_pos_atG t h = Thm.implies_elim
      (beta_norm (Drule.infer_instantiate ctxtG [(("ps",0), ctermG t)] product_pos_vG)) h;

(* ============================================================================
   product_one_nil : all_prime ps ==> oeq (product ps)(Suc Zero) ==> leq ps Nil
   ============================================================================ *)
(* le_add ground on ctxtG : le m (add m p) *)
fun le_add_atG (mT, pT) = beta_norm (Drule.infer_instantiate ctxtG
      [(("m",0), ctermG mT),(("p",0), ctermG pT)] le_add_vG);

(* le_self_mult : lt Zero v ==> le x (mult x v)   (x <= x*v when v>=1) *)
val le_self_mult =
  let
    val xF = Free("x", natT); val vF = Free("v", natT);
    val hPv = Thm.assume (ctermG (jT (lt ZeroC vF)));    (* lt 0 v *)
    val goalC = le xF (mult xF vF);
    val exv = suc_of_pos_atG vF hPv;                     (* Ex(%y. oeq v (Suc y)) *)
    val Pv  = Abs("y", natT, oeq vF (suc (Bound 0)));
    fun vbody v0 (hv0 : thm) =                           (* hv0 : oeq v (Suc v0) *)
      let
        (* mult x v = mult x (Suc v0) = add x (mult x v0) *)
        val mcong = mult_cong_rG (xF, vF, suc v0) hv0;   (* oeq (mult x v)(mult x (Suc v0)) *)
        val msr   = multSr_atG (xF, v0);                 (* oeq (mult x (Suc v0))(add x (mult x v0)) *)
        val chain = oeq_trans_vG OF [mcong, msr];        (* oeq (mult x v)(add x (mult x v0)) *)
        val ladd  = le_add_atG (xF, mult xF v0);         (* le x (add x (mult x v0)) *)
        (* rewrite add x (mult x v0) -> mult x v *)
        val csym  = oeq_sym_vG OF [chain];               (* oeq (add x (mult x v0))(mult x v) *)
        val zP = Free("z_lsm", natT);
        val Pabs = Term.lambda zP (le xF zP);            (* %z. le x z *)
      in oeq_rw_atG (Pabs, add xF (mult xF v0), mult xF vF) csym ladd end;  (* le x (mult x v) *)
    val concl = exE_elimG (Pv, goalC) exv "v0lsm" vbody;
    val d1 = Thm.implies_intr (ctermG (jT (lt ZeroC vF))) concl;
  in varify d1 end;
val le_self_mult_vG = varify le_self_mult;
fun le_self_mult_atG (xT, vT) h = Thm.implies_elim
      (beta_norm (Drule.infer_instantiate ctxtG
        [(("x",0), ctermG xT),(("v",0), ctermG vT)] le_self_mult_vG)) h;

val () = out "LE_SELF_MULT_READY\n";

val product_one_nil =
  let
    val psFresh = Free("ps_pon", natlistT);
    val PhiLbody = jT (mkImp (all_prime psFresh)
                            (mkImp (oeq (product psFresh) (suc ZeroC)) (leq psFresh NilC)));
    val PhiLabs = Term.lambda psFresh PhiLbody;

    (* NIL : leq Nil Nil *)
    val nilCase =
      let
        val leqNN = beta_norm (Drule.infer_instantiate ctxtG [(("a",0), ctermG NilC)] leq_refl_vG);  (* leq Nil Nil *)
        val inner = impI_atG (oeq (product NilC)(suc ZeroC), leq NilC NilC)
                       (Thm.implies_intr (ctermG (jT (oeq (product NilC)(suc ZeroC)))) leqNN);
      in impI_atG (all_prime NilC, mkImp (oeq (product NilC)(suc ZeroC)) (leq NilC NilC))
           (Thm.implies_intr (ctermG (jT (all_prime NilC))) inner) end;

    (* CONS x xs : contradiction *)
    fun consStep xF xsF (ihThm : thm) =
      let
        val consL = cons xF xsF;
        val hAll  = Thm.assume (ctermG (jT (all_prime consL)));
        val hP1   = Thm.assume (ctermG (jT (oeq (product consL)(suc ZeroC))));  (* product (Cons x xs) = 1 *)
        val goalC = leq consL NilC;
        (* prime2 x , all_prime xs *)
        val conjPX = all_prime_Cons_fwd_atG (xF, xsF) hAll;
        val prime_x = conjunct1_atG (prime2 xF, all_prime xsF) conjPX;
        val allxs   = conjunct2_atG (prime2 xF, all_prime xsF) conjPX;
        (* le 2 x *)
        val gt1x = prime2_gt1_atG xF prime_x;             (* lt (Suc 0) x = le (Suc(Suc 0)) x *)
        (* lt 0 (product xs) *)
        val pos_xs = product_pos_atG xsF allxs;           (* lt 0 (product xs) = le (Suc 0)(product xs) *)
        (* le x (mult x (product xs)) *)
        val le_x_m = le_self_mult_atG (xF, product xsF) pos_xs;  (* le x (mult x (product xs)) *)
        (* le 2 (mult x (product xs)) *)
        val le2m = le_trans_atG (suc (suc ZeroC), xF, mult xF (product xsF)) gt1x le_x_m;
        (* product (Cons x xs) = mult x (product xs) *)
        val pc = product_Cons_atG (xF, xsF);              (* oeq (product (Cons x xs))(mult x (product xs)) *)
        (* product (Cons x xs) = 1, so mult x (product xs) = 1 *)
        val pc_sym = oeq_sym_vG OF [pc];                  (* oeq (mult x (product xs))(product (Cons x xs)) *)
        val m_eq_1 = oeq_trans_vG OF [pc_sym, hP1];       (* oeq (mult x (product xs))(Suc 0) *)
        (* rewrite le 2 (mult x (product xs)) -> le 2 (Suc 0) = le 2 1 = lt 1 1 *)
        val zP = Free("z_pon", natT);
        val Pabs = Term.lambda zP (le (suc (suc ZeroC)) zP);  (* %z. le 2 z *)
        val le2_1 = oeq_rw_atG (Pabs, mult xF (product xsF), suc ZeroC) m_eq_1 le2m;  (* le 2 1 = lt 1 1 *)
        val fls = lt_irreflG_at (suc ZeroC) le2_1;        (* oFalse *)
        val mem = Thm.implies_elim (oFalse_elimG_at goalC) fls;  (* leq (Cons x xs) Nil *)
        val inner = impI_atG (oeq (product consL)(suc ZeroC), leq consL NilC)
                       (Thm.implies_intr (ctermG (jT (oeq (product consL)(suc ZeroC)))) mem);
      in impI_atG (all_prime consL, mkImp (oeq (product consL)(suc ZeroC)) (leq consL NilC))
           (Thm.implies_intr (ctermG (jT (all_prime consL))) inner) end;

    (* list_induct *)
    val psK = Free("ps", natlistT);
    val liInst = beta_norm (Drule.infer_instantiate ctxtG
          [(("PhiL",0), ctermG PhiLabs), (("k",0), ctermG psK)] list_induct_vG);
    fun phiBN ps = jT (mkImp (all_prime ps) (mkImp (oeq (product ps)(suc ZeroC)) (leq ps NilC)));
    val xCs = Free("x", natT); val lCs = Free("l", natlistT);
    val ihAssumeBN = Thm.assume (ctermG (phiBN lCs));
    val consBody = consStep xCs lCs ihAssumeBN;
    val minorCons0 = Thm.implies_intr (ctermG (phiBN lCs)) consBody;
    val minorCons  = Thm.forall_intr (ctermG xCs) (Thm.forall_intr (ctermG lCs) minorCons0);
    val phiK = Thm.implies_elim (Thm.implies_elim liInst nilCase) minorCons;
    (* strip the two object Imps *)
    val hAllK = Thm.assume (ctermG (jT (all_prime psK)));
    val hP1K  = Thm.assume (ctermG (jT (oeq (product psK)(suc ZeroC))));
    val innerK = mp_atG (all_prime psK, mkImp (oeq (product psK)(suc ZeroC)) (leq psK NilC)) phiK hAllK;
    val memK   = mp_atG (oeq (product psK)(suc ZeroC), leq psK NilC) innerK hP1K;
    val d1 = Thm.implies_intr (ctermG (jT (oeq (product psK)(suc ZeroC)))) memK;
    val d2 = Thm.implies_intr (ctermG (jT (all_prime psK))) d1;
  in varify d2 end;

val psVon = Var(("ps",0),natlistT);
val product_one_nil_intended =
  Logic.mk_implies (jT (all_prime psVon),
    Logic.mk_implies (jT (oeq (product psVon)(suc ZeroC)), jT (leq psVon NilC)));
val r_pon = checkG ("product_one_nil", product_one_nil, product_one_nil_intended);

(* ============================================================================
   all_prime_remove1 : all_prime ps ==> all_prime (remove1 x ps)
   ============================================================================ *)
(* transfer all_prime along a leq : leq A B ==> all_prime A ==> all_prime B *)
fun allprime_transfer (At, Bt) hleq hap =
  let val Pabs = Abs("z_apt", natlistT, all_prime (Bound 0))   (* %l. all_prime l *)
  in leq_rw_atG (Pabs, At, Bt) hleq hap end;

val all_prime_remove1 =
  let
    val xF = Free("x", natT);
    val psFresh = Free("ps_apr", natlistT);
    val PhiLbody = jT (mkImp (all_prime psFresh) (all_prime (remove1 xF psFresh)));
    val PhiLabs = Term.lambda psFresh PhiLbody;

    (* NIL : all_prime (remove1 x Nil) *)
    val nilCase =
      let
        val hAll = Thm.assume (ctermG (jT (all_prime NilC)));        (* all_prime Nil *)
        val rNil = remove1_Nil_atG xF;                               (* leq (remove1 x Nil) Nil *)
        val rNilSym = leq_sym_vG OF [rNil];                          (* leq Nil (remove1 x Nil) *)
        val res = allprime_transfer (NilC, remove1 xF NilC) rNilSym hAll;  (* all_prime (remove1 x Nil) *)
      in impI_atG (all_prime NilC, all_prime (remove1 xF NilC))
           (Thm.implies_intr (ctermG (jT (all_prime NilC))) res) end;

    (* CONS h t *)
    fun consStep hF tF (ihThm : thm) =     (* ihThm : Imp(all_prime t)(all_prime (remove1 x t)) *)
      let
        val consL = cons hF tF;
        val hAll  = Thm.assume (ctermG (jT (all_prime consL)));
        val goalC = all_prime (remove1 xF consL);
        val conjHT = all_prime_Cons_fwd_atG (hF, tF) hAll;          (* Conj(prime2 h)(all_prime t) *)
        val prime_h = conjunct1_atG (prime2 hF, all_prime tF) conjHT;
        val allt    = conjunct2_atG (prime2 hF, all_prime tF) conjHT;
        val allrt   = mp_atG (all_prime tF, all_prime (remove1 xF tF)) ihThm allt;  (* all_prime (remove1 x t) *)
        (* ex_middle on oeq x h *)
        val em = ex_middle_atG (oeq xF hF);                          (* Disj (oeq x h)(neg(oeq x h)) *)
        val caseEq =
          let
            val heq = Thm.assume (ctermG (jT (oeq xF hF)));          (* oeq x h *)
            val rEq = remove1_Cons_eq_atG (xF, hF, tF) heq;          (* leq (remove1 x (Cons h t)) t *)
            val rEqSym = leq_sym_vG OF [rEq];                        (* leq t (remove1 x (Cons h t)) *)
            val res = allprime_transfer (tF, remove1 xF consL) rEqSym allt;  (* all_prime (remove1 x (Cons h t)) *)
          in Thm.implies_intr (ctermG (jT (oeq xF hF))) res end;
        val caseNeq =
          let
            val hneq = Thm.assume (ctermG (jT (neg (oeq xF hF))));   (* neg(oeq x h) *)
            val rNeq = remove1_Cons_neq_atG (xF, hF, tF) hneq;       (* leq (remove1 x (Cons h t))(Cons h (remove1 x t)) *)
            (* all_prime (Cons h (remove1 x t)) via bwd from Conj(prime2 h)(all_prime (remove1 x t)) *)
            val conjNew = conjI_atG (prime2 hF, all_prime (remove1 xF tF)) prime_h allrt;
            val apNew = all_prime_Cons_bwd_atG (hF, remove1 xF tF) conjNew;  (* all_prime (Cons h (remove1 x t)) *)
            val rNeqSym = leq_sym_vG OF [rNeq];                      (* leq (Cons h (remove1 x t))(remove1 x (Cons h t)) *)
            val res = allprime_transfer (cons hF (remove1 xF tF), remove1 xF consL) rNeqSym apNew;
          in Thm.implies_intr (ctermG (jT (neg (oeq xF hF)))) res end;
        val res = disjE_elimG (oeq xF hF, neg (oeq xF hF), goalC) em caseEq caseNeq;
      in impI_atG (all_prime consL, all_prime (remove1 xF consL))
           (Thm.implies_intr (ctermG (jT (all_prime consL))) res) end;

    (* list_induct *)
    val psK = Free("ps", natlistT);
    val liInst = beta_norm (Drule.infer_instantiate ctxtG
          [(("PhiL",0), ctermG PhiLabs), (("k",0), ctermG psK)] list_induct_vG);
    fun phiBN ps = jT (mkImp (all_prime ps) (all_prime (remove1 xF ps)));
    val hCs = Free("h", natT); val lCs = Free("l", natlistT);
    val ihAssumeBN = Thm.assume (ctermG (phiBN lCs));
    val consBody = consStep hCs lCs ihAssumeBN;
    val minorCons0 = Thm.implies_intr (ctermG (phiBN lCs)) consBody;
    val minorCons  = Thm.forall_intr (ctermG hCs) (Thm.forall_intr (ctermG lCs) minorCons0);
    val phiK = Thm.implies_elim (Thm.implies_elim liInst nilCase) minorCons;
    val hAllK = Thm.assume (ctermG (jT (all_prime psK)));
    val resK  = mp_atG (all_prime psK, all_prime (remove1 xF psK)) phiK hAllK;
    val d1 = Thm.implies_intr (ctermG (jT (all_prime psK))) resK;
  in varify d1 end;

val xVar1 = Var(("x",0),natT); val psVar1 = Var(("ps",0),natlistT);
val all_prime_remove1_intended =
  Logic.mk_implies (jT (all_prime psVar1), jT (all_prime (remove1 xVar1 psVar1)));
val r_apr = checkG ("all_prime_remove1", all_prime_remove1, all_prime_remove1_intended);

(* ============================================================================
   count_remove1_other : neg (oeq r p) ==> oeq (count r ps)(count r (remove1 p ps))
   ============================================================================ *)
(* transfer count r along a leq on the RIGHT side of an oeq:
   given hbase : oeq LHS (count r A), and hleq : leq A B,
   produce oeq LHS (count r B). *)
fun count_rw_right (rT, LHS, At, Bt) hleq hbase =
  let val Pabs = Abs("z_crr", natlistT, oeq LHS (count rT (Bound 0)))   (* %l. oeq LHS (count r l) *)
  in leq_rw_atG (Pabs, At, Bt) hleq hbase end;

val count_remove1_other =
  let
    val rF = Free("r", natT); val pF = Free("p", natT);
    val hNeqRP = Thm.assume (ctermG (jT (neg (oeq rF pF))));   (* neg(oeq r p) *)

    val psFresh = Free("ps_cro", natlistT);
    val PhiLbody = jT (oeq (count rF psFresh) (count rF (remove1 pF psFresh)));
    val PhiLabs = Term.lambda psFresh PhiLbody;

    (* NIL : oeq (count r Nil)(count r (remove1 p Nil)) *)
    val nilCase =
      let
        val rNil = remove1_Nil_atG pF;                         (* leq (remove1 p Nil) Nil *)
        val rNilSym = leq_sym_vG OF [rNil];                    (* leq Nil (remove1 p Nil) *)
        val baseRefl = oeqreflG_at (count rF NilC);            (* oeq (count r Nil)(count r Nil) *)
        (* transfer Nil -> remove1 p Nil on the right *)
      in count_rw_right (rF, count rF NilC, NilC, remove1 pF NilC) rNilSym baseRefl end;

    (* CONS h t *)
    fun consStep hF tF (ihThm : thm) =     (* ihThm : oeq (count r t)(count r (remove1 p t)) *)
      let
        val consL = cons hF tF;
        val goalC = oeq (count rF consL) (count rF (remove1 pF consL));
        val em_ph = ex_middle_atG (oeq pF hF);                 (* Disj(oeq p h)(neg(oeq p h)) *)

        (* CASE oeq p h : remove1 = t ; and neg(oeq r h) so count r (Cons h t) = count r t *)
        val casePH =
          let
            val hph = Thm.assume (ctermG (jT (oeq pF hF)));     (* oeq p h *)
            (* neg(oeq r h) : assume oeq r h -> oeq r p -> contradiction *)
            val negRH =
              let
                val hrh = Thm.assume (ctermG (jT (oeq rF hF)));      (* oeq r h *)
                val hp_sym = oeq_sym_vG OF [hph];                    (* oeq h p *)
                val rp = oeq_trans_vG OF [hrh, hp_sym];              (* oeq r p *)
                val fls = mp_atG (oeq rF pF, oFalseC) hNeqRP rp;     (* oFalse *)
              in impI_atG (oeq rF hF, oFalseC)
                   (Thm.implies_intr (ctermG (jT (oeq rF hF))) fls) end;  (* neg(oeq r h) *)
            (* count r (Cons h t) = count r t *)
            val cnt_lhs = count_Cons_neq_atG (rF, hF, tF) negRH;      (* oeq (count r (Cons h t))(count r t) *)
            (* remove1 p (Cons h t) = t *)
            val rEq = remove1_Cons_eq_atG (pF, hF, tF) hph;          (* leq (remove1 p (Cons h t)) t *)
            val rEqSym = leq_sym_vG OF [rEq];                        (* leq t (remove1 p (Cons h t)) *)
            (* oeq (count r (Cons h t))(count r t) -> oeq (count r (Cons h t))(count r (remove1 p (Cons h t))) *)
            val res = count_rw_right (rF, count rF consL, tF, remove1 pF consL) rEqSym cnt_lhs;
          in Thm.implies_intr (ctermG (jT (oeq pF hF))) res end;

        (* CASE neg(oeq p h) : remove1 = Cons h (remove1 p t) *)
        val caseNPH =
          let
            val hnph = Thm.assume (ctermG (jT (neg (oeq pF hF))));   (* neg(oeq p h) *)
            val rNeq = remove1_Cons_neq_atG (pF, hF, tF) hnph;       (* leq (remove1 p (Cons h t))(Cons h (remove1 p t)) *)
            val rNeqSym = leq_sym_vG OF [rNeq];                      (* leq (Cons h (remove1 p t))(remove1 p (Cons h t)) *)
            (* ex_middle on oeq r h *)
            val em_rh = ex_middle_atG (oeq rF hF);
            val newCons = cons hF (remove1 pF tF);                   (* Cons h (remove1 p t) *)
            (* sub-goal : oeq (count r (Cons h t))(count r (Cons h (remove1 p t))) *)
            val subGoal = oeq (count rF consL) (count rF newCons);
            val caseRH =
              let
                val hrh = Thm.assume (ctermG (jT (oeq rF hF)));      (* oeq r h *)
                val cL = count_Cons_eq_atG (rF, hF, tF) hrh;         (* oeq (count r (Cons h t))(Suc(count r t)) *)
                val cR = count_Cons_eq_atG (rF, hF, remove1 pF tF) hrh;  (* oeq (count r (Cons h (remove1 p t)))(Suc(count r (remove1 p t))) *)
                val ihSuc = Suc_cong_vG OF [ihThm];                  (* oeq (Suc(count r t))(Suc(count r (remove1 p t))) *)
                val cR_sym = oeq_sym_vG OF [cR];                     (* oeq (Suc(count r (remove1 p t)))(count r (Cons h (remove1 p t))) *)
                val ch1 = oeq_trans_vG OF [cL, ihSuc];               (* oeq (count r (Cons h t))(Suc(count r (remove1 p t))) *)
                val res = oeq_trans_vG OF [ch1, cR_sym];             (* oeq (count r (Cons h t))(count r (Cons h (remove1 p t))) *)
              in Thm.implies_intr (ctermG (jT (oeq rF hF))) res end;
            val caseNRH =
              let
                val hnrh = Thm.assume (ctermG (jT (neg (oeq rF hF))));  (* neg(oeq r h) *)
                val cL = count_Cons_neq_atG (rF, hF, tF) hnrh;       (* oeq (count r (Cons h t))(count r t) *)
                val cR = count_Cons_neq_atG (rF, hF, remove1 pF tF) hnrh;  (* oeq (count r (Cons h (remove1 p t)))(count r (remove1 p t)) *)
                val cR_sym = oeq_sym_vG OF [cR];                     (* oeq (count r (remove1 p t))(count r (Cons h (remove1 p t))) *)
                val ch1 = oeq_trans_vG OF [cL, ihThm];               (* oeq (count r (Cons h t))(count r (remove1 p t)) *)
                val res = oeq_trans_vG OF [ch1, cR_sym];             (* oeq (count r (Cons h t))(count r (Cons h (remove1 p t))) *)
              in Thm.implies_intr (ctermG (jT (neg (oeq rF hF)))) res end;
            val subRes = disjE_elimG (oeq rF hF, neg (oeq rF hF), subGoal) em_rh caseRH caseNRH;
            (* now transfer Cons h (remove1 p t) -> remove1 p (Cons h t) on the right *)
            val res = count_rw_right (rF, count rF consL, newCons, remove1 pF consL) rNeqSym subRes;
          in Thm.implies_intr (ctermG (jT (neg (oeq pF hF)))) res end;

        val res = disjE_elimG (oeq pF hF, neg (oeq pF hF), goalC) em_ph casePH caseNPH;
      in res end;
    ...
    val liInst = beta_norm (Drule.infer_instantiate ctxtG
          [(("PhiL",0), ctermG PhiLabs), (("k",0), ctermG psK)] list_induct_vG);
    fun phiBN ps = jT (oeq (count rF ps) (count rF (remove1 pF ps)));
    val hCs = Free("h", natT); val lCs = Free("l", natlistT);
    val ihAssumeBN = Thm.assume (ctermG (phiBN lCs));
    val consBody = consStep hCs lCs ihAssumeBN;
    val minorCons0 = Thm.implies_intr (ctermG (phiBN lCs)) consBody;
    val minorCons  = Thm.forall_intr (ctermG hCs) (Thm.forall_intr (ctermG lCs) minorCons0);
    val phiK = Thm.implies_elim (Thm.implies_elim liInst nilCase) minorCons;  (* oeq (count r psK)(count r (remove1 p psK)) *)
    val d1 = Thm.implies_intr (ctermG (jT (neg (oeq rF pF)))) phiK;
  in varify d1 end;

val rVco = Var(("r",0),natT); val pVco = Var(("p",0),natT); val psVco = Var(("ps",0),natlistT);
val count_remove1_other_intended =
  Logic.mk_implies (jT (neg (oeq rVco pVco)),
    jT (oeq (count rVco psVco) (count rVco (remove1 pVco psVco))));
val r_cro = checkG ("count_remove1_other", count_remove1_other, count_remove1_other_intended);

(* ============================================================================
   count_remove1_self : in_list p ps ==> oeq (count p ps)(Suc(count p (remove1 p ps)))
   ============================================================================ *)
fun in_list_Cons_fwd_atG (xt,ht,tt) h = Thm.implies_elim
      (beta_norm (Drule.infer_instantiate ctxtG
        [(("x",0), ctermG xt),(("h",0), ctermG ht),(("t",0), ctermG tt)] in_list_Cons_fwd_vG)) h;
fun in_list_Nil_elim_atG xt h = Thm.implies_elim
      (beta_norm (Drule.infer_instantiate ctxtG [(("x",0), ctermG xt)] in_list_Nil_elim_vG)) h;

val count_remove1_self =
  let
    val pF = Free("p", natT);
    val psFresh = Free("ps_crs", natlistT);
    val PhiLbody = jT (mkImp (in_list pF psFresh)
                            (oeq (count pF psFresh) (suc (count pF (remove1 pF psFresh)))));
    val PhiLabs = Term.lambda psFresh PhiLbody;

    (* NIL : in_list p Nil ==> ... ; vacuous *)
    val nilCase =
      let
        val goalBody = oeq (count pF NilC) (suc (count pF (remove1 pF NilC)));
        val hMem = Thm.assume (ctermG (jT (in_list pF NilC)));
        val fls  = in_list_Nil_elim_atG pF hMem;            (* oFalse *)
        val res  = Thm.implies_elim (oFalse_elimG_at goalBody) fls;
      in impI_atG (in_list pF NilC, goalBody)
           (Thm.implies_intr (ctermG (jT (in_list pF NilC))) res) end;

    (* CONS h t *)
    fun consStep hF tF (ihThm : thm) =     (* ihThm : Imp(in_list p t)(oeq (count p t)(Suc(count p (remove1 p t)))) *)
      let
        val consL = cons hF tF;
        val goalBody = oeq (count pF consL) (suc (count pF (remove1 pF consL)));
        val hMem = Thm.assume (ctermG (jT (in_list pF consL)));
        val disjMem = in_list_Cons_fwd_atG (pF, hF, tF) hMem;    (* Disj(oeq p h)(in_list p t) *)
        val em_ph = ex_middle_atG (oeq pF hF);

        (* CASE oeq p h *)
        val casePH =
          let
            val hph = Thm.assume (ctermG (jT (oeq pF hF)));     (* oeq p h *)
            (* count p (Cons h t) = Suc(count p t) *)
            val cL = count_Cons_eq_atG (pF, hF, tF) hph;        (* oeq (count p (Cons h t))(Suc(count p t)) *)
            (* remove1 p (Cons h t) = t *)
            val rEq = remove1_Cons_eq_atG (pF, hF, tF) hph;     (* leq (remove1 p (Cons h t)) t *)
            (* count p (remove1 p (Cons h t)) = count p t : transfer along leq *)
            val cntRefl = oeqreflG_at (count pF (remove1 pF consL));  (* oeq (count p (remove1 p (Cons h t)))(count p (remove1 p (Cons h t))) *)
            val Pabs = Abs("z_crsa", natlistT, oeq (count pF (remove1 pF consL)) (count pF (Bound 0)));
            val cntR = leq_rw_atG (Pabs, remove1 pF consL, tF) rEq cntRefl;  (* oeq (count p (remove1 p (Cons h t)))(count p t) *)
            (* Suc both : Suc(count p (remove1 p (Cons h t))) = Suc(count p t) *)
            val sucR = Suc_cong_vG OF [cntR];                   (* oeq (Suc(count p (remove1...)))(Suc(count p t)) *)
            val sucR_sym = oeq_sym_vG OF [sucR];                (* oeq (Suc(count p t))(Suc(count p (remove1...))) *)
            val res = oeq_trans_vG OF [cL, sucR_sym];           (* oeq (count p (Cons h t))(Suc(count p (remove1 p (Cons h t)))) *)
          in Thm.implies_intr (ctermG (jT (oeq pF hF))) res end;
        ...
        val caseNPH =
          let
            val hnph = Thm.assume (ctermG (jT (neg (oeq pF hF))));   (* neg(oeq p h) *)
            (* derive in_list p t from disjMem + neg(oeq p h) *)
            val in_t =
              let
                val cA =
                  let
                    val heq = Thm.assume (ctermG (jT (oeq pF hF)));
                    val fls = mp_atG (oeq pF hF, oFalseC) hnph heq;
                    val g = Thm.implies_elim (oFalse_elimG_at (in_list pF tF)) fls;
                  in Thm.implies_intr (ctermG (jT (oeq pF hF))) g end;
                val cB =
                  let val hin = Thm.assume (ctermG (jT (in_list pF tF)))
                  in Thm.implies_intr (ctermG (jT (in_list pF tF))) hin end;
              in disjE_elimG (oeq pF hF, in_list pF tF, in_list pF tF) disjMem cA cB end;  (* in_list p t *)
            val ihConcl = mp_atG (in_list pF tF, oeq (count pF tF)(suc (count pF (remove1 pF tF)))) ihThm in_t;
                                                              (* oeq (count p t)(Suc(count p (remove1 p t))) *)
            (* count p (Cons h t) = count p t *)
            val cL = count_Cons_neq_atG (pF, hF, tF) hnph;     (* oeq (count p (Cons h t))(count p t) *)
            (* remove1 p (Cons h t) = Cons h (remove1 p t) *)
            val rNeq = remove1_Cons_neq_atG (pF, hF, tF) hnph; (* leq (remove1 p (Cons h t))(Cons h (remove1 p t)) *)
            val newCons = cons hF (remove1 pF tF);
            (* count p (remove1 p (Cons h t)) = count p (Cons h (remove1 p t)) : transfer along leq *)
            val cntRefl = oeqreflG_at (count pF (remove1 pF consL));
            val Pabs = Abs("z_crsb", natlistT, oeq (count pF (remove1 pF consL)) (count pF (Bound 0)));
            val cntRnew = leq_rw_atG (Pabs, remove1 pF consL, newCons) rNeq cntRefl;
                                                              (* oeq (count p (remove1 p (Cons h t)))(count p (Cons h (remove1 p t))) *)
            (* count p (Cons h (remove1 p t)) = count p (remove1 p t) (neq) *)
            val cNew = count_Cons_neq_atG (pF, hF, remove1 pF tF) hnph;  (* oeq (count p (Cons h (remove1 p t)))(count p (remove1 p t)) *)
            (* chain : count p (remove1 p (Cons h t)) = count p (remove1 p t) *)
            val cntChain = oeq_trans_vG OF [cntRnew, cNew];    (* oeq (count p (remove1 p (Cons h t)))(count p (remove1 p t)) *)
            (* Suc both, sym : Suc(count p (remove1 p t)) = Suc(count p (remove1 p (Cons h t))) *)
            val sucChain = Suc_cong_vG OF [cntChain];          (* oeq (Suc(count p (remove1 p (Cons h t))))(Suc(count p (remove1 p t))) *)
            (* goal : oeq (count p (Cons h t))(Suc(count p (remove1 p (Cons h t)))) *)
            (* cL : count p (Cons h t) = count p t ; ihConcl : count p t = Suc(count p (remove1 p t)) *)
            val ch1 = oeq_trans_vG OF [cL, ihConcl];           (* oeq (count p (Cons h t))(Suc(count p (remove1 p t))) *)
            val sucChain_sym = oeq_sym_vG OF [sucChain];       (* oeq (Suc(count p (remove1 p t)))(Suc(count p (remove1 p (Cons h t)))) *)
            val res = oeq_trans_vG OF [ch1, sucChain_sym];     (* oeq (count p (Cons h t))(Suc(count p (remove1 p (Cons h t)))) *)
          in Thm.implies_intr (ctermG (jT (neg (oeq pF hF)))) res end;

        val res = disjE_elimG (oeq pF hF, neg (oeq pF hF), goalBody) em_ph casePH caseNPH;
      in impI_atG (in_list pF consL, goalBody)
           (Thm.implies_intr (ctermG (jT (in_list pF consL))) res) end;

    (* list_induct *)
    val psK = Free("ps", natlistT);
    val liInst = beta_norm (Drule.infer_instantiate ctxtG
          [(("PhiL",0), ctermG PhiLabs), (("k",0), ctermG psK)] list_induct_vG);
    fun phiBN ps = jT (mkImp (in_list pF ps) (oeq (count pF ps)(suc (count pF (remove1 pF ps)))));
    val hCs = Free("h", natT); val lCs = Free("l", natlistT);
    val ihAssumeBN = Thm.assume (ctermG (phiBN lCs));
    val consBody = consStep hCs lCs ihAssumeBN;
    val minorCons0 = Thm.implies_intr (ctermG (phiBN lCs)) consBody;
    val minorCons  = Thm.forall_intr (ctermG hCs) (Thm.forall_intr (ctermG lCs) minorCons0);
    val phiK = Thm.implies_elim (Thm.implies_elim liInst nilCase) minorCons;
    val hMemK = Thm.assume (ctermG (jT (in_list pF psK)));
    val resK  = mp_atG (in_list pF psK, oeq (count pF psK)(suc (count pF (remove1 pF psK)))) phiK hMemK;
    val d1 = Thm.implies_intr (ctermG (jT (in_list pF psK))) resK;
  in varify d1 end;

val pVcs = Var(("p",0),natT); val psVcs = Var(("ps",0),natlistT);
val count_remove1_self_intended =
  Logic.mk_implies (jT (in_list pVcs psVcs),
    jT (oeq (count pVcs psVcs) (suc (count pVcs (remove1 pVcs psVcs)))));
val r_crs = checkG ("count_remove1_self", count_remove1_self, count_remove1_self_intended);

(* ============================================================================
   product_remove1 : in_list p ps ==> oeq (product ps)(mult p (product (remove1 p ps)))
   ============================================================================ *)
(* mult_swap : oeq (mult h (mult p w))(mult p (mult h w))    h*(p*w) = p*(h*w) *)
fun mult_swap_atG (hT, pT, wT) =
  let
    val a1 = mult_assoc_atG (hT, pT, wT);             (* oeq ((h*p)*w)(h*(p*w)) *)
    val a1s = oeq_sym_vG OF [a1];                     (* oeq (h*(p*w))((h*p)*w) *)
    val cm  = multcomm_atG (hT, pT);                  (* oeq (h*p)(p*h) *)
    val cmc = mult_cong_lG (mult hT pT, mult pT hT, wT) cm;  (* oeq ((h*p)*w)((p*h)*w) *)
    val a2  = mult_assoc_atG (pT, hT, wT);            (* oeq ((p*h)*w)(p*(h*w)) *)
    val ch1 = oeq_trans_vG OF [a1s, cmc];             (* oeq (h*(p*w))((p*h)*w) *)
    val ch2 = oeq_trans_vG OF [ch1, a2];              (* oeq (h*(p*w))(p*(h*w)) *)
  in ch2 end;

val product_remove1 =
  let
    val pF = Free("p", natT);
    val psFresh = Free("ps_prm", natlistT);
    val PhiLbody = jT (mkImp (in_list pF psFresh)
                            (oeq (product psFresh) (mult pF (product (remove1 pF psFresh)))));
    val PhiLabs = Term.lambda psFresh PhiLbody;

    (* NIL : vacuous *)
    val nilCase =
      let
        val goalBody = oeq (product NilC) (mult pF (product (remove1 pF NilC)));
        val hMem = Thm.assume (ctermG (jT (in_list pF NilC)));
        val fls  = in_list_Nil_elim_atG pF hMem;
        val res  = Thm.implies_elim (oFalse_elimG_at goalBody) fls;
      in impI_atG (in_list pF NilC, goalBody)
           (Thm.implies_intr (ctermG (jT (in_list pF NilC))) res) end;

    (* CONS h t *)
    fun consStep hF tF (ihThm : thm) =     (* ihThm : Imp(in_list p t)(oeq (product t)(mult p (product (remove1 p t)))) *)
      let
        val consL = cons hF tF;
        val goalBody = oeq (product consL) (mult pF (product (remove1 pF consL)));
        val hMem = Thm.assume (ctermG (jT (in_list pF consL)));
        val disjMem = in_list_Cons_fwd_atG (pF, hF, tF) hMem;    (* Disj(oeq p h)(in_list p t) *)
        val em_ph = ex_middle_atG (oeq pF hF);
        (* product (Cons h t) = mult h (product t) *)
        val pc = product_Cons_atG (hF, tF);                     (* oeq (product (Cons h t))(mult h (product t)) *)
        ...
        val casePH =
          let
            ...
            (* remove1 p (Cons h t) = t *)
            val rEq = remove1_Cons_eq_atG (pF, hF, tF) hph;     (* leq (remove1 p (Cons h t)) t *)
            (* product (remove1 p (Cons h t)) = product t : transfer along leq, get oeq going right *)
            (* want : oeq (mult p (product (remove1 p (Cons h t))))(mult p (product t)) *)
            val mRefl = oeqreflG_at (mult pF (product (remove1 pF consL)));
            val Pabs = Abs("z_prma", natlistT, oeq (mult pF (product (remove1 pF consL))) (mult pF (product (Bound 0))));
            val mEq = leq_rw_atG (Pabs, remove1 pF consL, tF) rEq mRefl;
                                                              (* oeq (mult p (product (remove1 p (Cons h t))))(mult p (product t)) *)
            (* mult h (product t) = mult p (product t) via oeq p h (sym -> oeq h p) on left factor *)
            val hp_sym = oeq_sym_vG OF [hph];                  (* oeq h p *)
            val mhp = mult_cong_lG (hF, pF, product tF) hp_sym;  (* oeq (mult h (product t))(mult p (product t)) *)
            (* chain : product (Cons h t) = mult h (product t) = mult p (product t) = mult p (product (remove1 p (Cons h t))) *)
            val mEq_sym = oeq_sym_vG OF [mEq];                 (* oeq (mult p (product t))(mult p (product (remove1 p (Cons h t)))) *)
            val ch1 = oeq_trans_vG OF [pc, mhp];               (* oeq (product (Cons h t))(mult p (product t)) *)
            val res = oeq_trans_vG OF [ch1, mEq_sym];          (* oeq (product (Cons h t))(mult p (product (remove1 p (Cons h t)))) *)
          in Thm.implies_intr (ctermG (jT (oeq pF hF))) res end;
            ...
            val in_t =
              let
                ...
                val cB =
                  let val hin = Thm.assume (ctermG (jT (in_list pF tF)))
                  in Thm.implies_intr (ctermG (jT (in_list pF tF))) hin end;
              in disjE_elimG (oeq pF hF, in_list pF tF, in_list pF tF) disjMem cA cB end;
            val ihConcl = mp_atG (in_list pF tF, oeq (product tF)(mult pF (product (remove1 pF tF)))) ihThm in_t;
                                                              (* oeq (product t)(mult p (product (remove1 p t))) *)
            val wT = product (remove1 pF tF);                 (* w = product (remove1 p t) *)
            (* remove1 p (Cons h t) = Cons h (remove1 p t) *)
            val rNeq = remove1_Cons_neq_atG (pF, hF, tF) hnph; (* leq (remove1 p (Cons h t))(Cons h (remove1 p t)) *)
            val newCons = cons hF (remove1 pF tF);
            (* product (remove1 p (Cons h t)) = product (Cons h (remove1 p t)) : transfer along leq *)
            (* want oeq (mult p (product (remove1 p (Cons h t))))(mult p (product (Cons h (remove1 p t)))) *)
            val mRefl = oeqreflG_at (mult pF (product (remove1 pF consL)));
            val Pabs = Abs("z_prmb", natlistT, oeq (mult pF (product (remove1 pF consL))) (mult pF (product (Bound 0))));
            val mTrans = leq_rw_atG (Pabs, remove1 pF consL, newCons) rNeq mRefl;
                       (* oeq (mult p (product (remove1 p (Cons h t))))(mult p (product (Cons h (remove1 p t)))) *)
            (* product (Cons h (remove1 p t)) = mult h (product (remove1 p t)) = mult h w *)
            val pcNew = product_Cons_atG (hF, remove1 pF tF);  (* oeq (product (Cons h (remove1 p t)))(mult h w) *)
            (* mult p (product (Cons h (remove1 p t))) = mult p (mult h w) *)
            val mcong = mult_cong_rG (pF, product newCons, mult hF wT) pcNew;  (* oeq (mult p (product (Cons h ..)))(mult p (mult h w)) *)
            (* RHS chain : mult p (product (remove1 p (Cons h t))) = mult p (product (Cons h (rt))) = mult p (mult h w) *)
            val rhsChain = oeq_trans_vG OF [mTrans, mcong];    (* oeq (mult p (product (remove1 p (Cons h t))))(mult p (mult h w)) *)
            (* LHS : product (Cons h t) = mult h (product t) ; product t = mult p w (ihConcl) -> mult h (product t) = mult h (mult p w) *)
            val mh_ih = mult_cong_rG (hF, product tF, mult pF wT) ihConcl;  (* oeq (mult h (product t))(mult h (mult p w)) *)
            val lhsChain = oeq_trans_vG OF [pc, mh_ih];        (* oeq (product (Cons h t))(mult h (mult p w)) *)
            (* swap : mult h (mult p w) = mult p (mult h w) *)
            val sw = mult_swap_atG (hF, pF, wT);               (* oeq (mult h (mult p w))(mult p (mult h w)) *)
            val lhs2 = oeq_trans_vG OF [lhsChain, sw];         (* oeq (product (Cons h t))(mult p (mult h w)) *)
            (* now goal : oeq (product (Cons h t))(mult p (product (remove1 p (Cons h t)))) *)
            val rhsChain_sym = oeq_sym_vG OF [rhsChain];       (* oeq (mult p (mult h w))(mult p (product (remove1 p (Cons h t)))) *)
            val res = oeq_trans_vG OF [lhs2, rhsChain_sym];
          in Thm.implies_intr (ctermG (jT (neg (oeq pF hF)))) res end;
    ...
    val liInst = beta_norm (Drule.infer_instantiate ctxtG
          [(("PhiL",0), ctermG PhiLabs), (("k",0), ctermG psK)] list_induct_vG);
    fun phiBN ps = jT (mkImp (in_list pF ps) (oeq (product ps)(mult pF (product (remove1 pF ps)))));
    val hCs = Free("h", natT); val lCs = Free("l", natlistT);
    val ihAssumeBN = Thm.assume (ctermG (phiBN lCs));
    val consBody = consStep hCs lCs ihAssumeBN;
    val minorCons0 = Thm.implies_intr (ctermG (phiBN lCs)) consBody;
    ...
    val phiK = Thm.implies_elim (Thm.implies_elim liInst nilCase) minorCons;
    val hMemK = Thm.assume (ctermG (jT (in_list pF psK)));
    val resK  = mp_atG (in_list pF psK, oeq (product psK)(mult pF (product (remove1 pF psK)))) phiK hMemK;
    val d1 = Thm.implies_intr (ctermG (jT (in_list pF psK))) resK;
  in varify d1 end;

val pVpr = Var(("p",0),natT); val psVpr = Var(("ps",0),natlistT);
val product_remove1_intended =
  Logic.mk_implies (jT (in_list pVpr psVpr),
    jT (oeq (product psVpr) (mult pVpr (product (remove1 pVpr psVpr)))));
val r_prm = checkG ("product_remove1", product_remove1, product_remove1_intended);

(* ============================================================================
   SOUNDNESS PROBES + FINAL VALIDATION
   ============================================================================ *)
(* count/remove1 conditional axioms validate (0-hyp where unconditional, aconv) *)
val rVc = Var(("r",0),natT); val hVc = Var(("h",0),natT); val tVc = Var(("t",0),natlistT);
val xVc = Var(("x",0),natT);
val count_Nil_chk = (length (Thm.hyps_of count_Nil_vG) = 0) andalso
      ((Thm.prop_of count_Nil_vG) aconv (jT (oeq (count rVc NilC) ZeroC)));
val () = out ("AXIOM count_Nil ok=" ^ Bool.toString count_Nil_chk ^ "\n");
val remove1_Nil_chk = (length (Thm.hyps_of remove1_Nil_vG) = 0) andalso
      ((Thm.prop_of remove1_Nil_vG) aconv (jT (leq (remove1 xVc NilC) NilC)));
val () = out ("AXIOM remove1_Nil ok=" ^ Bool.toString remove1_Nil_chk ^ "\n");

(* soundness: mult_left_cancel must NOT hold without the positivity premise *)
val probe_mlc_needs_pos =
  let val bogus = Logic.mk_implies (jT (oeq (mult cVml aVml) (mult cVml bVml)), jT (oeq aVml bVml))
  in not ((Thm.prop_of mult_left_cancel) aconv bogus) end;
(* soundness: product_remove1 must depend on in_list (not collapse to a trivial eq) *)
val probe_prm_needs_inlist =
  not ((Thm.prop_of product_remove1) aconv (jT (oeq (product psVpr) (mult pVpr (product (remove1 pVpr psVpr))))));
(* soundness: count_remove1_self conditional on in_list *)
val probe_crs_needs_inlist =
  not ((Thm.prop_of count_remove1_self) aconv (jT (oeq (count pVcs psVcs) (suc (count pVcs (remove1 pVcs psVcs))))));
(* soundness: count_remove1_other conditional on neg(oeq r p) *)
val probe_cro_needs_neq =
  not ((Thm.prop_of count_remove1_other) aconv (jT (oeq (count rVco psVco) (count rVco (remove1 pVco psVco)))));
val () =
  if probe_mlc_needs_pos andalso probe_prm_needs_inlist
     andalso probe_crs_needs_inlist andalso probe_cro_needs_neq
  then out "PROBE_OK count/remove1 lemmas are properly conditional / nontrivial\n"
  else out "PROBE_UNSOUND a count/remove1 lemma collapsed!\n";

val () =
  if r_mlc andalso r_pp andalso r_pon andalso r_apr
     andalso r_prm andalso r_crs andalso r_cro
     andalso count_Nil_chk andalso remove1_Nil_chk
     andalso probe_mlc_needs_pos andalso probe_prm_needs_inlist
     andalso probe_crs_needs_inlist andalso probe_cro_needs_neq
  then out "FTU_FOUNDATION_OK\n"
  else out "FTU_FOUNDATION_FAILED\n";

(* ============================================================================
   ============================================================================
   ***  STAGE 4 FINALE : fta_unique  ***
   ----------------------------------------------------------------------------
   fta_unique :
     jT (all_prime ps) ==> jT (all_prime qs) ==> jT (oeq (product ps)(product qs))
       ==> jT (Forall (%r. oeq (count r ps)(count r qs)))
   Two prime factorisations of the same number have the SAME multiset.

   Proof by list_induct on ps.  The list_induct predicate is META-GENERAL in the
   SECOND list (call it QS) so the inductive hypothesis can be instantiated at
   QS := remove1 p qs :
     PhiL = %ps. !!QS. all_prime ps ==> all_prime QS ==> oeq (product ps)(product QS)
                       ==> Forall(%r. oeq (count r ps)(count r QS))
   (the body is a pure meta-prop; the inner Forall is the OBJECT universal.)
   ============================================================================ *)

val () = out "FTA_UNIQUE_BEGIN\n";

(* ---- varify the Stage-3/4 lemmas not exported with applicators, build _atG ---- *)
val prime_in_prime_list_vG = varify prime_in_prime_list;
fun prime_in_prime_list_atG (pt, pst) hPrime hAllPs hDvd =
  let val inst = beta_norm (Drule.infer_instantiate ctxtG
        [(("p",0), ctermG pt), (("ps",0), ctermG pst)] prime_in_prime_list_vG)
  in Thm.implies_elim (Thm.implies_elim (Thm.implies_elim inst hPrime) hAllPs) hDvd end;

val mult_left_cancel_vG2 = varify mult_left_cancel;
fun mult_left_cancel_atG (ct, at, bt) hPos hEq =
  let val inst = beta_norm (Drule.infer_instantiate ctxtG
        [(("c",0), ctermG ct), (("a",0), ctermG at), (("b",0), ctermG bt)] mult_left_cancel_vG2)
  in Thm.implies_elim (Thm.implies_elim inst hPos) hEq end;

val product_one_nil_vG = varify product_one_nil;
fun product_one_nil_atG pst hAllPs hOne =
  let val inst = beta_norm (Drule.infer_instantiate ctxtG
        [(("ps",0), ctermG pst)] product_one_nil_vG)
  in Thm.implies_elim (Thm.implies_elim inst hAllPs) hOne end;

val all_prime_remove1_vG = varify all_prime_remove1;
fun all_prime_remove1_atG (xt, pst) hAllPs =
  let val inst = beta_norm (Drule.infer_instantiate ctxtG
        [(("x",0), ctermG xt), (("ps",0), ctermG pst)] all_prime_remove1_vG)
  in Thm.implies_elim inst hAllPs end;

val product_remove1_vG = varify product_remove1;
fun product_remove1_atG (pt, pst) hIn =
  let val inst = beta_norm (Drule.infer_instantiate ctxtG
        [(("p",0), ctermG pt), (("ps",0), ctermG pst)] product_remove1_vG)
  in Thm.implies_elim inst hIn end;

val count_remove1_self_vG = varify count_remove1_self;
fun count_remove1_self_atG (pt, pst) hIn =
  let val inst = beta_norm (Drule.infer_instantiate ctxtG
        [(("p",0), ctermG pt), (("ps",0), ctermG pst)] count_remove1_self_vG)
  in Thm.implies_elim inst hIn end;

val count_remove1_other_vG = varify count_remove1_other;
fun count_remove1_other_atG (rt, pt, pst) hNeq =
  let val inst = beta_norm (Drule.infer_instantiate ctxtG
        [(("r",0), ctermG rt), (("p",0), ctermG pt), (("ps",0), ctermG pst)] count_remove1_other_vG)
  in Thm.implies_elim inst hNeq end;

(* allI on ctxtG : Pabs : nat=>o ; hAll : !!x. jT (Pabs x) ; -> jT (Forall Pabs) *)
fun allI_atG Pabs hAllThm =
  let val inst = beta_norm (Drule.infer_instantiate ctxtG [(("P",0), ctermG Pabs)] allI_vG)
  in Thm.implies_elim inst hAllThm end;
(* allE on ctxtG : jT (Forall Pabs) -> jT (Pabs at) *)
fun allE_atG Pabs at hForall =
  let val inst = beta_norm (Drule.infer_instantiate ctxtG
        [(("P",0), ctermG Pabs), (("a",0), ctermG at)] allE_vG)
  in Thm.implies_elim inst hForall end;

(* dvd_intro on ctxtG : oeq b (mult a w) -> jT (dvd a b)  (witness w) *)
fun dvd_intro_atG (aT, bT, w) hyp =
  let val Pabs = Abs("k_dig", natT, oeq bT (mult aT (Bound 0)))
      val exI_inst = beta_norm (Drule.infer_instantiate ctxtG
            [(("P",0), ctermG Pabs), (("a",0), ctermG w)] exI_vG)
  in Thm.implies_elim exI_inst hyp end;

val () = out "FTA_HELPERS_READY\n";
...
val fta_unique =
  let
    (* inner object-universal body, as a predicate over r (count var) for list PS, QS:
         %r. oeq (count r PS)(count r QS)        (count of bound var ok under Abs+Bound 0) *)
    fun innerPred PS QS = Abs("r", natT, oeq (count (Bound 0) PS) (count (Bound 0) QS));

    (* the META-prop body for a given (concrete or Free) ps, with QS a meta-bound Free *)
    val QSfresh = Free("QS_ftu", natlistT);
    fun phiMeta PS =                 (* !!QS. ap PS ==> ap QS ==> prod PS = prod QS ==> Forall(...) *)
      Logic.all QSfresh
        (Logic.mk_implies (jT (all_prime PS),
          Logic.mk_implies (jT (all_prime QSfresh),
            Logic.mk_implies (jT (oeq (product PS) (product QSfresh)),
              jT (mkForall (innerPred PS QSfresh))))));

    (* the list_induct predicate over ps (capture-avoiding via Term.lambda) *)
    val psFresh = Free("ps_ftu", natlistT);
    val PhiLabs = Term.lambda psFresh (phiMeta psFresh);   (* %ps. !!QS. ... *)

    (* ------------------------------------------------------------------
       NIL CASE :  !!QS. all_prime Nil ==> all_prime QS ==> oeq (product Nil)(product QS)
                     ==> Forall(%r. oeq (count r Nil)(count r QS))
       product Nil = Suc Zero = product QS  =>  product_one_nil QS => leq QS Nil.
       For r : count r QS = count r Nil = 0.
       ------------------------------------------------------------------ *)
    val nilCase =
      let
        val QSv    = Free("QSn", natlistT);
        val hAllN  = Thm.assume (ctermG (jT (all_prime NilC)));
        val hAllQ  = Thm.assume (ctermG (jT (all_prime QSv)));
        val hEqN   = Thm.assume (ctermG (jT (oeq (product NilC) (product QSv))));
        (* product Nil = Suc Zero *)
        val pn       = product_Nil_thmG;                       (* oeq (product Nil)(Suc 0) *)
        val hEqN_sym = oeq_sym_vG OF [hEqN];                   (* oeq (product QS)(product Nil) *)
        val pq_one   = oeq_trans_vG OF [hEqN_sym, pn];         (* oeq (product QS)(Suc 0) *)
        val leqQN    = product_one_nil_atG QSv hAllQ pq_one;   (* leq QS Nil *)
        (* Forall(%r. oeq (count r Nil)(count r QS)) by allI *)
        val rF       = Free("r_nil", natT);
        (* count r QS = count r Nil :  start from refl oeq (count r Nil)(count r Nil),
           rewrite SECOND Nil -> QS via leq Nil QS (sym of leqQN). *)
        val leqNQ    = leq_sym_vG OF [leqQN];                  (* leq Nil QS *)
        val cRefl    = oeqreflG_at (count rF NilC);            (* oeq (count r Nil)(count r Nil) *)
        val Pabs     = Abs("z_ftn", natlistT, oeq (count rF NilC) (count rF (Bound 0)));
        val cNQ      = leq_rw_atG (Pabs, NilC, QSv) leqNQ cRefl; (* oeq (count r Nil)(count r QS) *)
        val genR     = Thm.forall_intr (ctermG rF) cNQ;        (* !!r. jT (oeq (count r Nil)(count r QS)) *)
        val forallN  = allI_atG (innerPred NilC QSv) genR;     (* jT (Forall(%r. ...)) *)
        (* discharge the three premises as meta-implications, then generalise QS *)
        val d3 = Thm.implies_intr (ctermG (jT (oeq (product NilC)(product QSv)))) forallN;
        val d2 = Thm.implies_intr (ctermG (jT (all_prime QSv))) d3;
        val d1 = Thm.implies_intr (ctermG (jT (all_prime NilC))) d2;
        val gQ = Thm.forall_intr (ctermG QSv) d1;              (* !!QS. ... = phiMeta Nil *)
      in gQ end;

    (* ------------------------------------------------------------------
       CONS CASE : !!p ps2. (phiMeta ps2) ==> (phiMeta (Cons p ps2))
       ------------------------------------------------------------------ *)
    fun consStep pF ps2F (ihThm : thm) =     (* ihThm : phiMeta ps2  (a !!QS. ... meta-prop) *)
      let
        val consL = cons pF ps2F;
        val QSv   = Free("QSc", natlistT);   (* the QS we are proving phiMeta (Cons ..) for *)
        val hAllC = Thm.assume (ctermG (jT (all_prime consL)));     (* all_prime (Cons p ps2) *)
        val hAllQ = Thm.assume (ctermG (jT (all_prime QSv)));       (* all_prime QS *)
        val hEqC  = Thm.assume (ctermG (jT (oeq (product consL) (product QSv))));  (* product (Cons p ps2) = product QS *)

        (* prime2 p , all_prime ps2 *)
        val conjPX  = all_prime_Cons_fwd_atG (pF, ps2F) hAllC;
        val prime_p = conjunct1_atG (prime2 pF, all_prime ps2F) conjPX;
        val allps2  = conjunct2_atG (prime2 pF, all_prime ps2F) conjPX;

        (* product (Cons p ps2) = mult p (product ps2) *)
        val pc      = product_Cons_atG (pF, ps2F);
        val pc_sym  = oeq_sym_vG OF [pc];                            (* oeq (mult p (product ps2))(product (Cons p ps2)) *)
        val mEqQ    = oeq_trans_vG OF [pc_sym, hEqC];                (* oeq (mult p (product ps2))(product QS) *)

        (* dvd p (product QS) : product QS = mult p (product ps2), witness product ps2 *)
        val mEqQ_sym = oeq_sym_vG OF [mEqQ];                         (* oeq (product QS)(mult p (product ps2)) *)
        val dvd_pq   = dvd_intro_atG (pF, product QSv, product ps2F) mEqQ_sym;  (* dvd p (product QS) *)

        (* in_list p QS *)
        val in_p_qs  = prime_in_prime_list_atG (pF, QSv) prime_p hAllQ dvd_pq;  (* in_list p QS *)

        (* product QS = mult p (product (remove1 p QS)) *)
        val prq      = product_remove1_atG (pF, QSv) in_p_qs;       (* oeq (product QS)(mult p (product (remove1 p QS))) *)
        val mEqM     = oeq_trans_vG OF [mEqQ, prq];                  (* oeq (mult p (product ps2))(mult p (product (remove1 p QS))) *)

        (* p > 0 *)
        val p_pos    = prime2_pos_atG pF prime_p;                    (* lt 0 p *)
        (* product ps2 = product (remove1 p QS) *)
        val prodEq   = mult_left_cancel_atG (pF, product ps2F, product (remove1 pF QSv)) p_pos mEqM;

        (* all_prime (remove1 p QS) *)
        val allrm    = all_prime_remove1_atG (pF, QSv) hAllQ;        (* all_prime (remove1 p QS) *)

        (* IH at QS := remove1 p QS :
             ihThm : !!QS. ap ps2 ==> ap QS ==> prod ps2 = prod QS ==> Forall(%r. count r ps2 = count r QS)
           forall_elim at (remove1 p QS), then discharge premises. *)
        val rmQS    = remove1 pF QSv;
        val ihInst  = beta_norm (Thm.forall_elim (ctermG rmQS) ihThm);  (* ap ps2 ==> ap (rm) ==> prod ps2 = prod rm ==> Forall(...) *)
        val ih1     = Thm.implies_elim ihInst allps2;               (* ap (rm) ==> prod ps2 = prod rm ==> Forall(...) *)
        val ih2     = Thm.implies_elim ih1 allrm;                   (* prod ps2 = prod rm ==> Forall(...) *)
        val ihFor   = Thm.implies_elim ih2 prodEq;                  (* jT (Forall(%r. oeq (count r ps2)(count r (remove1 p QS)))) *)

        (* prove Forall(%r. oeq (count r (Cons p ps2))(count r QS)) by allI; fix r; ex_middle (oeq r p) *)
        val rF      = Free("r_cons", natT);
        val goalBodyR = oeq (count rF consL) (count rF QSv);
        (* IH instance at r : oeq (count r ps2)(count r (remove1 p QS)) *)
        val ihAtR   = allE_atG (innerPred ps2F rmQS) rF ihFor;      (* oeq (count r ps2)(count r (remove1 p QS)) *)

        val em      = ex_middle_atG (oeq rF pF);                    (* Disj (oeq r p)(neg(oeq r p)) *)

        (* CASE r = p *)
        val caseRP =
          let
            val hrp = Thm.assume (ctermG (jT (oeq rF pF)));         (* oeq r p *)
            (* count r (Cons p ps2) = Suc(count r ps2) [count_Cons_eq at oeq r p] *)
            val cL  = count_Cons_eq_atG (rF, pF, ps2F) hrp;         (* oeq (count r (Cons p ps2))(Suc(count r ps2)) *)
            (* in_list r QS : from in_list p QS rewrite p -> r via oeq r p (sym) *)
            val hrp_sym = oeq_sym_vG OF [hrp];                      (* oeq p r *)
            val Pin     = Abs("z_inr", natT, in_list (Bound 0) QSv);  (* %z. in_list z QS *)
            val in_r_qs = oeq_rw_atG (Pin, pF, rF) hrp_sym in_p_qs; (* in_list r QS *)
            (* count_remove1_self at r : count r QS = Suc(count r (remove1 r QS)) *)
            val crs_r   = count_remove1_self_atG (rF, QSv) in_r_qs; (* oeq (count r QS)(Suc(count r (remove1 r QS))) *)
            (* remove1 r QS = remove1 p QS via oeq r p on the FIRST arg of remove1 *)
            (* we want to rewrite (count r (remove1 r QS)) -> (count r (remove1 p QS)).
               build oeq (remove1 r QS)(remove1 p QS)?  remove1 is on nats: remove1 r QS vs remove1 p QS,
               equal arguments r=p.  But remove1 returns a LIST, and oeq is on nats.  Instead rewrite the
               nat term (count r (remove1 r QS)) by the oeq r p inside.  Use oeq_rw on predicate
               %z. oeq (count r QS)(Suc(count r (remove1 z QS))) at z:=r -> z:=p. *)
            val Prm = Abs("z_rmrp", natT, oeq (count rF QSv) (suc (count rF (remove1 (Bound 0) QSv))));
            val crs_p = oeq_rw_atG (Prm, rF, pF) hrp crs_r;        (* oeq (count r QS)(Suc(count r (remove1 p QS))) *)
            (* ihAtR : count r ps2 = count r (remove1 p QS) ; Suc both *)
            val sucIH = Suc_cong_vG OF [ihAtR];                     (* oeq (Suc(count r ps2))(Suc(count r (remove1 p QS))) *)
            (* chain : count r (Cons p ps2) = Suc(count r ps2) = Suc(count r (remove1 p QS)) = count r QS *)
            val ch1   = oeq_trans_vG OF [cL, sucIH];               (* oeq (count r (Cons p ps2))(Suc(count r (remove1 p QS))) *)
            val crs_p_sym = oeq_sym_vG OF [crs_p];                 (* oeq (Suc(count r (remove1 p QS)))(count r QS) *)
            val res   = oeq_trans_vG OF [ch1, crs_p_sym];         (* oeq (count r (Cons p ps2))(count r QS) *)
          in Thm.implies_intr (ctermG (jT (oeq rF pF))) res end;

        (* CASE r != p *)
        val caseRNP =
          let
            val hnrp = Thm.assume (ctermG (jT (neg (oeq rF pF))));  (* neg(oeq r p) *)
            (* count r (Cons p ps2) = count r ps2 [count_Cons_neq at neg(oeq r p)] *)
            val cL   = count_Cons_neq_atG (rF, pF, ps2F) hnrp;      (* oeq (count r (Cons p ps2))(count r ps2) *)
            (* count_remove1_other at (r,p,QS) : count r QS = count r (remove1 p QS) *)
            val cro  = count_remove1_other_atG (rF, pF, QSv) hnrp;  (* oeq (count r QS)(count r (remove1 p QS)) *)
            (* ihAtR : count r ps2 = count r (remove1 p QS) *)
            val cro_sym = oeq_sym_vG OF [cro];                      (* oeq (count r (remove1 p QS))(count r QS) *)
            val ch1  = oeq_trans_vG OF [cL, ihAtR];                (* oeq (count r (Cons p ps2))(count r (remove1 p QS)) *)
            val res  = oeq_trans_vG OF [ch1, cro_sym];            (* oeq (count r (Cons p ps2))(count r QS) *)
          in Thm.implies_intr (ctermG (jT (neg (oeq rF pF)))) res end;

        val bodyR = disjE_elimG (oeq rF pF, neg (oeq rF pF), goalBodyR) em caseRP caseRNP;  (* oeq (count r (Cons p ps2))(count r QS) *)
        val genR  = Thm.forall_intr (ctermG rF) bodyR;             (* !!r. jT (oeq (count r (Cons p ps2))(count r QS)) *)
        val forallC = allI_atG (innerPred consL QSv) genR;         (* jT (Forall(%r. ...)) *)
        (* discharge the three premises for QS, then generalise QS *)
        val d3 = Thm.implies_intr (ctermG (jT (oeq (product consL)(product QSv)))) forallC;
        val d2 = Thm.implies_intr (ctermG (jT (all_prime QSv))) d3;
        val d1 = Thm.implies_intr (ctermG (jT (all_prime consL))) d2;
        val gQ = Thm.forall_intr (ctermG QSv) d1;                  (* !!QS. ... = phiMeta (Cons p ps2) *)
      in gQ end;
    ...
    val liInst = beta_norm (Drule.infer_instantiate ctxtG
          [(("PhiL",0), ctermG PhiLabs), (("k",0), ctermG psK)] list_induct_vG);
          (* phiMeta Nil ==> (!!x l. phiMeta l ==> phiMeta (Cons x l)) ==> phiMeta psK *)
    val pCs = Free("p", natT); val lCs = Free("l", natlistT);
    val ihAssume = Thm.assume (ctermG (phiMeta lCs));
    val consBody = consStep pCs lCs ihAssume;                      (* phiMeta (Cons p l) *)
    val minorCons0 = Thm.implies_intr (ctermG (phiMeta lCs)) consBody;
    val minorCons  = Thm.forall_intr (ctermG pCs) (Thm.forall_intr (ctermG lCs) minorCons0);
    val phiK = Thm.implies_elim (Thm.implies_elim liInst nilCase) minorCons;  (* phiMeta psK = !!QS. ... *)

    (* instantiate QS := qs (the ACTUAL second list), discharge premises *)
    val qsK = Free("qs", natlistT);
    val phiKqs = beta_norm (Thm.forall_elim (ctermG qsK) phiK);    (* ap psK ==> ap qs ==> prod psK = prod qs ==> Forall(%r. count r psK = count r qs) *)
    val hAllPs = Thm.assume (ctermG (jT (all_prime psK)));
    val hAllQs = Thm.assume (ctermG (jT (all_prime qsK)));
    val hEqPQ  = Thm.assume (ctermG (jT (oeq (product psK) (product qsK))));
    val s1 = Thm.implies_elim phiKqs hAllPs;
    val s2 = Thm.implies_elim s1 hAllQs;
    val forallFinal = Thm.implies_elim s2 hEqPQ;                   (* jT (Forall(%r. oeq (count r psK)(count r qs))) *)
    val d3 = Thm.implies_intr (ctermG (jT (oeq (product psK)(product qsK)))) forallFinal;
    val d2 = Thm.implies_intr (ctermG (jT (all_prime qsK))) d3;
    val d1 = Thm.implies_intr (ctermG (jT (all_prime psK))) d2;
  in varify d1 end;

(* ---- validation : fta_unique 0-hyp + aconv the TARGET ---- *)
val psVfu = Var(("ps",0),natlistT); val qsVfu = Var(("qs",0),natlistT);
val rVfu  = Free("r_chk", natT);
val fta_unique_innerAbs = Abs("r", natT,
      oeq (count (Bound 0) psVfu) (count (Bound 0) qsVfu));
val fta_unique_intended =
  Logic.mk_implies (jT (all_prime psVfu),
    Logic.mk_implies (jT (all_prime qsVfu),
      Logic.mk_implies (jT (oeq (product psVfu) (product qsVfu)),
        jT (mkForall fta_unique_innerAbs))));
val r_ftu = checkG ("fta_unique", fta_unique, fta_unique_intended);

(* ---- SOUNDNESS PROBES : dropping all_prime ps, or product equality, must NOT be provable ---- *)
val probe_ftu_needs_allprime_ps =
  let val bogus = Logic.mk_implies (jT (all_prime qsVfu),
        Logic.mk_implies (jT (oeq (product psVfu)(product qsVfu)),
          jT (mkForall fta_unique_innerAbs)))   (* drops all_prime ps *)
  in not ((Thm.prop_of fta_unique) aconv bogus) end;
val probe_ftu_needs_prodeq =
  let val bogus = Logic.mk_implies (jT (all_prime psVfu),
        Logic.mk_implies (jT (all_prime qsVfu),
          jT (mkForall fta_unique_innerAbs)))    (* drops product equality *)
  in not ((Thm.prop_of fta_unique) aconv bogus) end;
val () =
  if probe_ftu_needs_allprime_ps andalso probe_ftu_needs_prodeq
  then out "PROBE_OK fta_unique needs all_prime ps and the product equality\n"
  else out "PROBE_UNSOUND fta_unique collapsed!\n";

val () =
  if r_ftu andalso probe_ftu_needs_allprime_ps andalso probe_ftu_needs_prodeq
  then (out "OK fta_unique\n"; out "FTA_UNIQUE_DONE\n")
  else out "FTA_UNIQUE_FAILED\n";
