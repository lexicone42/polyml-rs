(* ============================================================================
   MODULAR ARITHMETIC : congruence mod m is a CONGRUENCE RELATION  =>  Z/mZ is a
   COMMUTATIVE RING.  ONE self-contained Isabelle/Pure ML driver, proving the
   whole bundle in a single run over the warm /tmp/isabelle_pure checkpoint.

   WHAT IT PROVES (each a 0-hypothesis schematic theorem; each prints "OK <name>"):
     cong_refl   : |- cong m a a                                  (reflexive)
     cong_sym    : |- cong m a b ==> cong m b a                   (symmetric)
     cong_trans  : |- cong m a b ==> cong m b c ==> cong m a c    (transitive)
                                       => congruence mod m is an EQUIVALENCE relation
     cong_add    : |- cong m a a2 ==> cong m b b2
                       ==> cong m (add a b) (add a2 b2)           [+ compatible]
     cong_mult   : |- cong m a a2 ==> cong m b b2
                       ==> cong m (mult a b) (mult a2 b2)         [* compatible]
                                  => +,* descend to Z/mZ  =>  Z/mZ is a COMMUTATIVE RING.
   On success the driver prints all five "OK <name>" lines + "MODULAR_DONE".

   THE TWO-SIDED cong DEFINITION (the naturals have NO subtraction).  On N we
   cannot say "m | (a - b)" because a - b is not total.  Instead congruence is the
   SYMMETRIC two-sided existential
        cong m a b  :=  (?k. b = a + m*k)  \/  (?k. a = b + m*k)
   i.e. one of a,b exceeds the other by a multiple of m.  congL is the left
   disjunct (b = a + m*k) and congR the right (a = b + m*k); cong_introL/cong_introR
   inject a witnessed equality into the respective disjunct.

   NOTE on cong_add: although the STATEMENT of cong_add is purely additive, its
   proof genuinely needs the LINEAR ORDER (le_total).  The four disjE x disjE
   cases include MIXED cases (LR / RL) where a exceeds a2 by m*p while b2 exceeds
   b by m*q (or vice versa); to combine these we must compare p and q and route
   the m-multiple of their DIFFERENCE to the correct disjunct of the conclusion.
   That comparison is exactly le_total k l -- so the linear order enters the proof
   even though no inequality appears in the statement.  cong_trans's mixed cases
   (LR / RL) need le_total for the same structural reason.

   HOW IT WAS BUILT (merge, not invention):
     The foundation -- the isabelle_classical_primes.sml body (object logic o/nat
     + Trueprop + Peano add & mult + commutative-semiring lemmas + the existential
     quantifier + linear order le_total + the classical prime development), ending
     on the single final context ctxtC/ctermC, then the `cong` abbreviation +
     cong_introL/cong_introR + cong_refl + cong_sym + cong_add -- is the
     BYTE-IDENTICAL shared prefix of the two independently-verified drivers
     isa_mod_trans.sml and isa_mod_mult.sml.  It is taken ONCE.
     The cong_trans proof body is merged from isa_mod_trans.sml; the cong_mult
     proof body (with its product-expansion helpers prodR/prodL/foldM and the
     extra ctxtC product instantiators) from isa_mod_mult.sml.  Order: foundation
     -> cong_trans -> cong_mult.  Each law, after being proved, is validated 0-hyp
     AND `aconv` its intended schematic goal (goal built with the SAME `cong`
     abbreviation so the Ex-bodies match exactly).  A soundness probe confirms the
     kernel rejects a false variant of cong_mult.
   ============================================================================ *)
