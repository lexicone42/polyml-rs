(* ============================================================================
   EUCLID'S LEMMA FOR LISTS in Isabelle/Pure on the polyml-rs interpreter —
   Stage 3 of the FTA-uniqueness arc.  (test: isabelle_euclid_list.rs)
   ----------------------------------------------------------------------------
     prime_in_prime_list :
       |- prime p ==> all_prime ps ==> p | product ps ==> in_list p ps
   A prime that divides the product of a list of primes IS one of them.  A
   0-hypothesis theorem on the Isabelle/Pure logical core, only classical
   assumption = excluded middle.

   Re-derives the list machinery (natlist + product + all_prime + a membership
   predicate in_list) on the Stage-2 Euclid-lemma base, proves
     prime_div_eq : prime p ==> prime q ==> p | q ==> p = q   (two primes,
       one dividing the other are equal -- from the structural prime),
   then the headline by list induction on euclid_lemma: product (Cons h t) =
   h * product t; euclid_lemma splits p | h*(product t) into p|h (=> p=h by
   prime_div_eq => in_list p) or p | product t (=> IH => in_list p t).

   The key lemma for Stage 4 (FTA uniqueness).  Built on isabelle_euclid_lemma.sml
   by a 2-phase ultracode pipeline (wf_1b8fb713-66f): list foundation + prime_div_eq
   -> prime_in_prime_list (3 seats, all proved it).
   ============================================================================ *)
