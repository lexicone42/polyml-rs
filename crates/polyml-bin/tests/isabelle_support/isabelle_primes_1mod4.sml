(* ======================================================================
   INFINITELY MANY PRIMES ≡ 1 (mod 4), in Isabelle/Pure.

     p1m4_inf : ⊢ ∀n. ∃q. prime2 q ∧ n < q ∧ (∃t. q = 4·t + 1)

   The companion to the banked ≡3 (mod 4) result (isabelle_primes_3mod4).
   Self-contained driver (run DIRECTLY, like isabelle_twosquare/euler) — it
   MERGES the FLT/euler-criterion branch (lagrange_roots/apm1, carrying the
   modular-power machinery) with the euclid/classical-primes branch
   (prime_divisor_exists/dvd_fact/consec_coprime), re-deriving the factorial
   machinery onto the merged theory.

   KEY LEMMA (p1m4_key, the First Supplement's hard direction — the converse of
   the banked neg1_qr): ⊢ prime2 q ⟹ 2<q ⟹ ¬(q∣x) ⟹ cong q (x·x) (q−1)
   ⟹ ∃t. q = 4·t + 1. I.e. for an odd prime q∤x, x²≡−1 (mod q) ⟹ q≡1 (mod4),
   via FLT (q∤x ⟹ x^(q−1)≡1) + parity ((q−1)/2 odd if q≡3 ⟹ x^(q−1)≡−1≢1).

   INFINITUDE (p1m4_inf): Euclid construction N=(2·n!)²+1; a prime divisor q has
   (2n!)²≡−1 (mod q) so q≡1 (mod4) by the key lemma, and q>n via dvd_fact +
   consec-coprimality. Both 0-hyp, aconv-intended, soundness-probed; only
   classical assumption = ex_middle. Markers: P1M4_KEY_OK, P1M4_INF_OK.
   ultracode wf_76164d04-62a.
   ====================================================================== *)
...
(* ============================================================================
   INFINITUDE OF PRIMES == 1 (mod 4)
     p1m4_inf : |- !n. ?q. prime2 q /\ n < q /\ (?t. q = 4*t + 1)
   ----------------------------------------------------------------------------
   Built on ctxtF (= thySub + fact), which already carries:
     - the KEY LEMMA  p1m4_key  (on thySub, lifts to thyF by varify):
         prime2 q ==> 2<q ==> ~(q|x) ==> cong q (x*x)(q-1) ==> ?t. q = 4t+1
     - prime_divisor_exists, dvd_fact, consec_coprime, fact_pos  (factorial
       machinery, ported on ctxtF above)
     - cong / dvd / prime2 / sub / mult / add  (all from the euler_criterion base)

   Euclid-style construction:  N = (2*n!)^2 + 1.
     M  = 2*(n!)            (= mult 2 (fact n))
     MS = M*M               (= (2 n!)^2),  EVEN  (MS = 2 * (n! * M))
     N  = Suc MS            (= MS + 1),    ODD
   A prime divisor q of N satisfies:
     (a)  2 < q   : q prime so 1<q (=> 2<=q); q<>2 because if q=2 then 2|MS and
                    2|(Suc MS)=N, and consec_coprime(2, MS) gives oFalse.
     (b)  q | (2 n!) fails (q ndvd M) : else q|MS and q|N=Suc MS, consec_coprime.
     (c)  cong q MS (q-1)  : from q|N = Suc MS.  Suc MS = q*k => k=Suc k0 =>
                    Suc MS = q + q*k0 => MS = (q-1) + q*k0  (Suc-injective),
                    i.e. the RIGHT cong disjunct with witness k0.
   The key lemma at x:=M then yields  ?t. q = 4t+1.
   And q > n via dvd_fact + consec_coprime (q<=n => q|n! => q|MS => q|1, contra),
   exactly as in plain Euclid but with MS in place of n!.
   ============================================================================ *)

val () = out "P1M4_INF_BEGIN\n";

(* numerals on ctxtF (these match the key delta's twoC/fourC = on ctxtSub, but
   we re-state them here for clarity; ZeroC/suc are the same global constants) *)
val twoCF   = suc (suc ZeroC);
val threeCF = suc twoCF;
val fourCF  = suc threeCF;
val oneCF   = suc ZeroC;

(* ---- cong constructor on ctxtF (RIGHT disjunct):
        from hyp : oeq a (add b (mult m w))  build  jT (cong m a b)            *)
val disjI2_vFc = varify disjI2_ax;
fun disjI2Fc_at (At,Bt) h = Thm.implies_elim (beta_norm (Drule.infer_instantiate ctxtF
      [(("A",0), ctermF At), (("B",0), ctermF Bt)] disjI2_vFc)) h;
fun congR_F m a b = mkEx (Abs ("k", natT, oeq a (add b (mult m (Bound 0)))));
fun congL_F m a b = mkEx (Abs ("k", natT, oeq b (add a (mult m (Bound 0)))));
fun cong_F  m a b = mkDisj (congL_F m a b) (congR_F m a b);
fun cong_introR_F (m, a, b, w) hyp =
  let
    val RAbs = Abs ("k", natT, oeq a (add b (mult m (Bound 0))));
    val exThm = exI_atF RAbs w hyp;                         (* jT (congR m a b) *)
  in disjI2Fc_at (congL_F m a b, congR_F m a b) exThm end;  (* jT (cong m a b)  *)

(* ---- p1m4_key lifted to ctxtF.  It lives on thySub; varify re-zeroes indices
        and it lifts to thyF automatically.                                    *)
val p1m4_key_vF = varify p1m4_key;
(* p1m4_key : prime2 q ==> lt 2 q ==> neg(dvd q x) ==> cong q (mult x x)(sub q 1)
              ==> Ex (%t. oeq q (add (mult 4 t) 1))                              *)
fun p1m4_key_atF (qT, xT) hPrime hLt2 hNdvd hCong =
  let
    val inst = beta_norm (Drule.infer_instantiate ctxtF
          [(("q",0), ctermF qT), (("x",0), ctermF xT)] p1m4_key_vF)
  in (((inst OF [hPrime]) OF [hLt2]) OF [hNdvd]) OF [hCong] end;

(* ---- mult helpers on ctxtF (multSucF_at / multSrF_at / multcommF_at /
        left_distribF_at / mult_cong_lF / mult_cong_rF already in the block) ---- *)
(* mult_assoc on ctxtF : (a*b)*c = a*(b*c).  varify base mult_assoc.            *)
val mult_assoc_vFc = varify mult_assoc;
fun mult_assocF_at (aT,bT,cT) = beta_norm (Drule.infer_instantiate ctxtF
      [(("x",0), ctermF aT),(("y",0), ctermF bT),(("z",0), ctermF cT)] mult_assoc_vFc);
(* mult_0_right on ctxtF *)
val mult_0_right_vFc = varify mult_0_right;
fun mult0rFc_at t = beta_norm (Drule.infer_instantiate ctxtF [(("n",0), ctermF t)] mult_0_right_vFc);

(* ---- prime2 conjuncts on ctxtF (prime2 p = Conj (lt 1 p)(Forall (ppAbs p))) ---- *)
fun prime2_gt1_atF p hPrime =
  conjunct1_atF (lt (suc ZeroC) p, mkForall (ppAbs p)) hPrime;   (* jT (lt 1 p) = jT (le 2 p) *)

(* ---- le_neq_lt / neg-intro already lifted in block: le_neq_ltF_at, negI_atF ---- *)

(* ---- dvd_intro on ctxtF (block has dvd_introF) ; dvd_self_mult in block ---- *)

(* ---- substitution into a predicate on ctxtF (oeq_subst) ---- *)
fun substPredF (Pabs, aT, bT) hab hPa =
  let val inst = beta_norm (Drule.infer_instantiate ctxtF
        [(("P",0), ctermF Pabs), (("a",0), ctermF aT), (("b",0), ctermF bT)] oeq_subst_vF)
  in inst OF [hab, hPa] end;

(* ---- sub instantiators directly on ctxtF (avoid cross-context theory merges) ---- *)
val sub_n_0_vFc     = varify sub_n_0_ax;
val sub_Suc_Suc_vFc = varify sub_Suc_Suc_ax;
fun subN0F_at t       = beta_norm (Drule.infer_instantiate ctxtF [(("n",0), ctermF t)] sub_n_0_vFc);
fun subSSF_at (nt,kt) = beta_norm (Drule.infer_instantiate ctxtF
                          [(("n",0), ctermF nt),(("k",0), ctermF kt)] sub_Suc_Suc_vFc);

val () = out "P1M4_INF_HELPERS_READY\n";

(* ============================================================================
   le_one_two_F is le_one_two (already built : le 1 2).
   lt_one_two_F : lt 1 2 = le 2 2 = le_refl 2.                                  *)
val lt_one_two_F = le_reflF_at twoCF;     (* jT (le 2 2) = jT (lt 1 2) *)

(* mult two W = W + W  (two_mult_F) :
     mult 2 W = mult (Suc(Suc 0)) W = add W (mult (Suc 0) W)      [multSucF]
              = add W (add W (mult 0 W))                          [multSucF]
              = add W (add W 0)                                   [mult0F]
              = add W W                                           [add0r]            *)
fun two_mult_F W =
  let
    val s1 = multSucF_at (suc ZeroC, W);              (* mult (Suc(Suc 0)) W = add W (mult (Suc 0) W) *)
    val s2 = multSucF_at (ZeroC, W);                  (* mult (Suc 0) W = add W (mult 0 W) *)
    val m0 = mult0F_at W;                             (* mult 0 W = 0 *)
    val inner0 = add_cong_rF (W, mult ZeroC W, ZeroC) m0;   (* add W (mult 0 W) = add W 0 *)
    val innerW = oeq_trans OF [inner0, add0rF_at W];        (* add W (mult 0 W) = W *)
    val s2W    = oeq_trans OF [s2, innerW];                 (* mult (Suc 0) W = W *)
    val cong   = add_cong_rF (W, mult (suc ZeroC) W, W) s2W;(* add W (mult (Suc 0) W) = add W W *)
  in oeq_trans OF [s1, cong] end;                    (* mult 2 W = add W W *)

(* ============================================================================
   THE CORE : for a fixed Free n, build
     jT (Ex (%q. Conj (Conj (prime2 q)(lt n q)) (Ex (%t. oeq q (add (mult 4 t) 1)))))
   ============================================================================ *)
(* the final result predicate body :  %q. Conj (Conj (prime2 q)(lt n q)) (mod4 q)
   where mod4 q = Ex(%t. oeq q (add (mult 4 t) 1)).                              *)
fun mod4Body q = mkEx (Abs("t", natT, oeq q (add (mult fourCF (Bound 0)) oneCF)));
fun innerResAbs nt =
  let val qF = Free("q_e", natT)
  in Term.lambda qF (mkConj (mkConj (prime2 qF) (ltF nt qF)) (mod4Body qF)) end;

val () = out "P1M4_INF_CORE_READY\n";
...
val p1m4_inf =
  let
    fun coreFor nF =
      let
        val factN = fact nF;
        val M     = mult twoCF factN;                 (* M = 2 * n! *)
        val MS    = mult M M;                          (* MS = (2 n!)^2 *)
        val NN    = suc MS;                            (* N  = MS + 1 *)

        (* ---- MS even : MS = mult 2 Weven  with Weven = mult (fact n) M ---- *)
        val Weven   = mult factN M;
        val ms_assoc= mult_assocF_at (twoCF, factN, M);   (* (2 * n!) * M = 2 * (n! * M) *)
        (* MS = mult M M = mult (2*n!) M = (2*n!)*M ; ms_assoc : (2*n!)*M = 2*(n!*M) = mult 2 Weven *)
        val MS_2W   = ms_assoc;                           (* oeq MS (mult 2 Weven) : MS = mult M M = mult (mult 2 n!) M *)
        (* NB MS is literally mult M M = mult (mult twoCF factN) M, and ms_assoc states
           oeq (mult (mult twoCF factN) M) (mult twoCF (mult factN M)).  Term-identical LHS. *)

        (* ---- le 2 N : from fact_pos (le 1 n!) by le_suc_mono on (1, n!) ... but N = Suc MS,
                not Suc(fact n).  We need le 2 (Suc MS) = le 1 MS lifted.  MS >= 1 since
                MS = mult M M and M >= 1 (M = 2*n!, n! >= 1).  Simplest: le 1 MS from
                MS = 2*Weven >= ... actually easier: N = Suc MS, and le 2 (Suc MS) needs MS >= 1.
                We get MS >= 1 from: MS = mult 2 Weven ; and Weven >= 1? Not directly needed.
                Cleanest: le 2 (Suc MS) <- le 1 MS <- (MS = Suc(something)) OR via le_suc_mono
                from le 1 MS.  le 1 MS : MS = mult M M, M = 2*n! = Suc(Suc(...)) since n!>=1
                so 2*n! >= 2 >= 1, hence M >= 1, hence M*M >= 1.  We prove le 1 MS by
                dvd_self_mult-free route:  le 1 MS  <=  le 1 M  &  ...  Simpler still:
                le 2 N is only used to feed prime_divisor_exists (needs le 2 N).
                Establish le 1 MS from mult_le_mono :  le 1 M ==> le (mult M 1) (mult M M)
                and mult M 1 = M, le 1 M => le 1 (mult M M) by le_trans.  Hmm needs le 1 M.
                le 1 M : M = 2*n!, fact_pos gives le 1 n!, mult_le_mono(2,1,n!) ... messy.
                EASIEST: le 1 MS via "MS = mult 2 Weven" + (Weven could be 0!). If n=0, n!=1,
                M=2, MS=4, fine. Weven = n!*M = 1*2 = 2 >0. In general Weven = n!*(2 n!) >= 1.
                But proving Weven>=1 is as hard.  Use: le 1 MS  <-  le 1 (mult M M) with
                le 1 M and mult_le_mono.  le 1 M : *)
        (* le 1 M  :  M = 2*n!,  le 1 (fact n) [fact_pos],  mult_le_mono(2,1,n!) :
             le 1 n! ==> le (mult 2 1)(mult 2 n!) = le (mult 2 1) M ;
             mult 2 1 = 2 ; le 2 M ; le_trans(1,2,M) with le_one_two => le 1 M.       *)
        val le1fn   = fact_pos_atF nF;                    (* le 1 (fact n) *)
        val le_21_2nf = mult_le_monoF_at (twoCF, oneCF, factN) le1fn;  (* le (2*1)(2*n!) = le (mult 2 1) M *)
        (* mult 2 1 = 2 :  mult 2 (Suc 0) = add 2 (mult 2 0) [multSrF] = add 2 0 [mult0r] = 2 *)
        val m21a    = multSrF_at (twoCF, ZeroC);          (* mult 2 (Suc 0) = add 2 (mult 2 0) *)
        val m21b    = mult0rFc_at twoCF;                  (* mult 2 0 = 0 *)
        val m21c    = add_cong_rF (twoCF, mult twoCF ZeroC, ZeroC) m21b;  (* add 2 (mult 2 0) = add 2 0 *)
        val m21d    = oeq_trans OF [m21c, add0rF_at twoCF];   (* add 2 (mult 2 0) = 2 *)
        val m21     = oeq_trans OF [m21a, m21d];          (* mult 2 (Suc 0) = 2 *)
        (* rewrite le (mult 2 1)(M) -> le 2 M via subst (mult 2 1 -> 2) in (le _ M) *)
        val zF1     = Free("z_m21", natT);
        val Psub1   = Term.lambda zF1 (le zF1 M);         (* %z. le z M *)
        val le_2_M  = substPredF (Psub1, mult twoCF oneCF, twoCF) m21 le_21_2nf;  (* le 2 M *)
        val le_1_M  = le_transF_at (oneCF, twoCF, M) le_one_two le_2_M;  (* le 1 M *)
        (* le 1 MS :  mult_le_mono(M, 1, M) : le 1 M ==> le (mult M 1)(mult M M) = le (mult M 1) MS ;
             mult M 1 = M ; le M MS ; le_trans(1,M,MS) with le 1 M.                  *)
        val le_M1_MS = mult_le_monoF_at (M, oneCF, M) le_1_M;  (* le (mult M 1)(mult M M) = le (mult M 1) MS *)
        val mM1a    = multSrF_at (M, ZeroC);              (* mult M (Suc 0) = add M (mult M 0) *)
        val mM1b    = mult0rFc_at M;                      (* mult M 0 = 0 *)
        val mM1c    = add_cong_rF (M, mult M ZeroC, ZeroC) mM1b;  (* add M (mult M 0) = add M 0 *)
        val mM1d    = oeq_trans OF [mM1c, add0rF_at M];   (* add M (mult M 0) = M *)
        val mM1     = oeq_trans OF [mM1a, mM1d];          (* mult M (Suc 0) = M *)
        val zF2     = Free("z_mM1", natT);
        val Psub2   = Term.lambda zF2 (le zF2 MS);        (* %z. le z MS ; MS = mult M M *)
        val le_M_MS = substPredF (Psub2, mult M oneCF, M) mM1 le_M1_MS;  (* le M MS *)
        val le_1_MS = le_transF_at (oneCF, M, MS) le_1_M le_M_MS;        (* le 1 MS *)
        val le_2_N  = le_suc_monoF_at (oneCF, MS) le_1_MS;               (* le 2 (Suc MS) = le 2 N *)

        (* ---- prime_divisor_exists at N -> Ex (%q. Conj (prime2 q)(dvd q N)) ---- *)
        val pdeEx   = prime_divisor_exists_atF NN le_2_N;
        val pdeBody = resultBodyAbs NN;                   (* %q. Conj (prime2 q)(dvd q N) *)
        val goalEx  = mkEx (innerResAbs nF);

        fun afterQ q (hConj : thm) =                      (* hConj : Conj (prime2 q)(dvd q N) *)
          let
            val hPrime = conjunct1_atF (prime2 q, dvd q NN) hConj;   (* prime2 q *)
            val hDvdqN = conjunct2_atF (prime2 q, dvd q NN) hConj;   (* dvd q N = dvd q (Suc MS) *)
            val lt1q   = prime2_gt1_atF q hPrime;                    (* lt 1 q = le 2 q *)
            val le1q   = le_transF_at (oneCF, twoCF, q) le_one_two lt1q;  (* le 1 q *)

            (* dvd q MS :  via afterMS using q | (Suc MS) ; we need MS and Suc MS together.
               We'll exE the dvd q N witness ONCE and reuse for (a),(b),(c).             *)
            val PdvdN = Abs("k", natT, oeq NN (mult q (Bound 0)));   (* body of dvd q N *)

            fun afterK kW (hk : thm) =       (* hk : oeq (Suc MS) (mult q k) *)
              let
                (* dzos on k : k=0 -> Suc MS = q*0 = 0 -> Suc_neq_Zero ;
                               k=Suc k0 -> Suc MS = q*(Suc k0) = q + q*k0.                *)
                val dzk = dzosF_at kW;        (* Disj (oeq k 0)(Ex(%k0. oeq k (Suc k0))) *)
                (* the body produces, for k = Suc k0 :  hk0 : oeq MS (add (sub q 1)(mult q k0))
                   then everything (a),(b),(c) below.  For k=0 we derive oFalse and lift to goal. *)
                val goalC = goalEx;

                val caseK0 =
                  let
                    val hk0 = Thm.assume (ctermF (jT (oeq kW ZeroC)));   (* k = 0 *)
                    (* Suc MS = mult q k = mult q 0 = 0 *)
                    val cong0 = mult_cong_rF (q, kW, ZeroC) hk0;         (* mult q k = mult q 0 *)
                    val mq0   = mult0rFc_at q;                           (* mult q 0 = 0 *)
                    val mqk0  = oeq_trans OF [cong0, mq0];               (* mult q k = 0 *)
                    val sms0  = oeq_trans OF [hk, mqk0];                 (* Suc MS = 0 *)
                    val fls   = (Suc_neq_ZeroF_at MS) OF [sms0];         (* oFalse *)
                    val gEx   = (oFalse_elimF_at goalC) OF [fls];        (* goalEx *)
                  in Thm.implies_intr (ctermF (jT (oeq kW ZeroC))) gEx end;

                val PsucK = Abs("k0", natT, oeq kW (suc (Bound 0)));
                val caseKS =
                  let
                    val exk0 = Thm.assume (ctermF (jT (mkEx PsucK)));   (* Ex k0. k = Suc k0 *)
                    fun bodyK0 k0W (hk0 : thm) =                        (* hk0 : oeq k (Suc k0) *)
                      let
                        (* Suc MS = mult q k = mult q (Suc k0) = add q (mult q k0)   [mult_Suc_right] *)
                        val congK = mult_cong_rF (q, kW, suc k0W) hk0;  (* mult q k = mult q (Suc k0) *)
                        val msr   = multSrF_at (q, k0W);                (* mult q (Suc k0) = add q (mult q k0) *)
                        val mqk   = oeq_trans OF [congK, msr];          (* mult q k = add q (mult q k0) *)
                        val sms_q : thm = oeq_trans OF [hk, mqk];       (* Suc MS = add q (mult q k0) *)

                        (* ===========================================================
                           (a)  2 < q   (q <> 2) via consec_coprime when q = 2.
                           We prove  neg (oeq twoCF q)  then le_neq_lt (2,q) with le 2 q.
                           q = 2 case:  dvd 2 MS  and  dvd 2 (Suc MS) = dvd q N (subst),
                           consec_coprime(2, MS) => oFalse.
                           =========================================================== *)
                        (* dvd q MS first (needed for (a) and is general):
                           from sms_q : Suc MS = add q (mult q k0).
                           MS = (q-1) + (mult q k0)  [Suc-injective + suc(q-1)=q].
                           We need suc(sub q 1) = q.  q >= 1 (le 1 q).  Use le1q witness:
                             le 1 q : Ex w. oeq q (add 1 w).  exE w, hw: q = add (Suc 0) w = Suc(0+w)=Suc w.
                             then sub q 1 = sub (Suc w) (Suc 0) = sub w 0 = w  [sub_Suc_Suc, sub_n_0]
                             and suc(sub q 1) = Suc w = q.                                            *)
                        val Pq1 = Abs("w", natT, oeq q (add oneCF (Bound 0)));   (* body of le 1 q *)
                        fun bodyW wW (hw : thm) =        (* hw : oeq q (add (Suc 0) w) *)
                          let
                            (* q = Suc w : add (Suc 0) w = Suc(0 + w) = Suc w *)
                            val a1w  = addSucF_at (ZeroC, wW);     (* add (Suc 0) w = Suc(0 + w) *)
                            val a0w  = add0F_at wW;                (* 0 + w = w *)
                            val sucw = oeq_trans OF [a1w, Suc_cong OF [a0w]];  (* add (Suc 0) w = Suc w *)
                            val q_Sw = oeq_trans OF [hw, sucw];    (* q = Suc w *)
                            (* sub q 1 = w :  sub (Suc w)(Suc 0) = sub w 0 = w *)
                            (* subst q -> Suc w in (sub _ 1) via predicate %z. oeq (sub z 1) w ?
                               Easier: build oeq (sub q 1) w by rewriting q. *)
                            val subSS = subSSF_at (wW, ZeroC);     (* sub (Suc w)(Suc 0) = sub w 0 *)
                            val subN0 = subN0F_at wW;              (* sub w 0 = w *)
                            val subSw = oeq_trans OF [subSS, subN0];  (* sub (Suc w)(Suc 0) = w *)
                            (* sub q 1 = sub (Suc w) 1 [subst q=Suc w] = w *)
                            val zsub  = Free("z_subq", natT);
                            val Psubq = Term.lambda zsub (oeq (sub zsub oneCF) wW);  (* %z. oeq (sub z 1) w *)
                            (* need oeq (Suc w) q to subst (Suc w)->q ; or subst q->Suc w: use q_Sw *)
                            val subq1_w = substPredF (Psubq, suc wW, q) (oeq_sym OF [q_Sw]) subSw;
                                                                   (* oeq (sub q 1) w *)
                            (* suc(sub q 1) = Suc w = q *)
                            val Sd_Sw = Suc_cong OF [subq1_w];     (* Suc(sub q 1) = Suc w *)
                            val Sd_q  = oeq_trans OF [Sd_Sw, oeq_sym OF [q_Sw]];  (* Suc(sub q 1) = q *)

                            (* MS = (sub q 1) + (mult q k0) :
                               sms_q : Suc MS = add q (mult q k0)
                               rewrite q -> Suc(sub q 1) [Sd_q sym] on the RHS first operand:
                                 add q (mult q k0) = add (Suc(sub q 1))(mult q k0) = Suc(add (sub q 1)(mult q k0))
                               so Suc MS = Suc(add (sub q 1)(mult q k0)) ; Suc_inj => MS = add (sub q 1)(mult q k0). *)
                            val q_Sd  = oeq_sym OF [Sd_q];         (* q = Suc(sub q 1) *)
                            val cR    = add_cong_lF (q, suc (sub q oneCF), mult q k0W) q_Sd;
                                                                   (* add q (mult q k0) = add (Suc(sub q 1))(mult q k0) *)
                            val aSr   = addSucF_at (sub q oneCF, mult q k0W);
                                                                   (* add (Suc(sub q 1))(mult q k0) = Suc(add (sub q 1)(mult q k0)) *)
                            val rhs_S = oeq_trans OF [cR, aSr];    (* add q (mult q k0) = Suc(add (sub q 1)(mult q k0)) *)
                            val sms_S = oeq_trans OF [sms_q, rhs_S];   (* Suc MS = Suc(add (sub q 1)(mult q k0)) *)
                            val ms_eq = (Suc_inj_atF (MS, add (sub q oneCF) (mult q k0W))) OF [sms_S];
                                                                   (* oeq MS (add (sub q 1)(mult q k0)) *)

                            (* ----- (c) cong q MS (sub q 1) : RIGHT disjunct, witness k0, body ms_eq ----- *)
                            val hCong = cong_introR_F (q, MS, sub q oneCF, k0W) ms_eq;  (* jT (cong q MS (sub q 1)) *)

                            (* ----- (b) q ndvd M (neg (dvd q M)) :
                               if dvd q M then dvd q MS = dvd q (mult M M) [dvd_self_mult], and
                               dvd q N = dvd q (Suc MS); consec_coprime(q, MS) with lt 1 q => oFalse. ----- *)
                            val hNdvdM =
                              let
                                val dvdHypP = jT (dvd q M);
                                val dvdHyp  = Thm.assume (ctermF dvdHypP);   (* dvd q M *)
                                (* dvd q M => dvd q (mult M M) = dvd q MS by dvd_trans (q | M, M | M*M).
                                   dvd_self_mult gives M | (mult M M). *)
                                (* instantiate dvd_self_mult at a:=M, b:=M -> dvd M (mult M M) = dvd M MS *)
                                val dvd_M_MS_at =
                                  beta_norm (Drule.infer_instantiate ctxtF
                                    [(("a",0), ctermF M), (("b",0), ctermF M)] (varify dvd_self_mult));
                                val dvd_q_MS = dvd_trans_atF (q, M, MS) dvdHyp dvd_M_MS_at;  (* dvd q MS *)
                                (* consec_coprime(q, MS) lt1q (dvd q MS)(dvd q (Suc MS)=hDvdqN) => oFalse *)
                                val fls = consec_coprime_atF (q, MS) lt1q dvd_q_MS hDvdqN;   (* oFalse *)
                                val impB = Thm.implies_intr (ctermF dvdHypP) fls;            (* dvd q M ==> oFalse *)
                              in negI_atF (dvd q M) impB end;       (* neg (dvd q M) *)

                            (* ----- (a) 2 < q :  q <> 2 ; if q = 2 then dvd 2 MS and dvd 2 (Suc MS),
                               consec_coprime(2, MS) lt(1)(2) => oFalse.  Then le_neq_lt(2,q) with le 2 q. ----- *)
                            val hLt2q =
                              let
                                (* le 2 q = lt 1 q = lt1q. *)
                                val le2q = lt1q;                  (* jT (le 2 q) *)
                                (* neg (oeq 2 q) : assume oeq 2 q, derive oFalse. *)
                                val neq2q =
                                  let
                                    val eqHypP = jT (oeq twoCF q);
                                    val eqHyp  = Thm.assume (ctermF eqHypP);   (* 2 = q *)
                                    (* dvd 2 MS : MS = mult 2 Weven [MS_2W] ; dvd 2 (mult 2 Weven) [dvd_self_mult];
                                       subst (mult 2 Weven) -> MS gives dvd 2 MS. *)
                                    val dvd_2_2W =
                                      beta_norm (Drule.infer_instantiate ctxtF
                                        [(("a",0), ctermF twoCF), (("b",0), ctermF Weven)] (varify dvd_self_mult));
                                                                  (* dvd 2 (mult 2 Weven) *)
                                    val zdv   = Free("z_dvd2", natT);
                                    val Pdv   = Term.lambda zdv (dvd twoCF zdv);   (* %z. dvd 2 z *)
                                    (* MS = mult 2 Weven [MS_2W] ; subst (mult 2 Weven)->MS needs oeq (mult 2 Weven) MS = sym MS_2W *)
                                    val dvd_2_MS = substPredF (Pdv, mult twoCF Weven, MS) (oeq_sym OF [MS_2W]) dvd_2_2W;
                                                                  (* dvd 2 MS *)
                                    (* dvd 2 (Suc MS) : from hDvdqN (dvd q N) with q = 2 [subst q->2 i.e. oeq q 2 = sym eqHyp].
                                       dvd q (Suc MS) ; subst q -> 2. *)
                                    val zdv2  = Free("z_dvdN", natT);
                                    val Pdv2  = Term.lambda zdv2 (dvd zdv2 NN);     (* %z. dvd z N *)
                                    val dvd_2_N = substPredF (Pdv2, q, twoCF) (oeq_sym OF [eqHyp]) hDvdqN;
                                                                  (* dvd 2 (Suc MS) *)
                                    (* lt 1 2 = le 2 2 = le_refl 2 *)
                                    val lt12 = lt_one_two_F;       (* jT (le 2 2) = jT (lt 1 2) *)
                                    val fls  = consec_coprime_atF (twoCF, MS) lt12 dvd_2_MS dvd_2_N;  (* oFalse *)
                                    val impB = Thm.implies_intr (ctermF eqHypP) fls;  (* oeq 2 q ==> oFalse *)
                                  in negI_atF (oeq twoCF q) impB end;   (* neg (oeq 2 q) *)
                                val lt2q = le_neq_ltF_at (twoCF, q) le2q neq2q;  (* lt 2 q = le 3 q *)
                              in lt2q end;

                            (* ----- apply the KEY LEMMA at x := M ----- *)
                            val hMod4 = p1m4_key_atF (q, M) hPrime hLt2q hNdvdM hCong;
                                                              (* Ex(%t. oeq q (add (mult 4 t) 1)) *)

                            (* ----- q > n (lt n q) :  same as Euclid, with MS in place of n!.
                               le_total(Suc n, q) ; case lt n q done ; case le q (Suc n):
                                 le_cases(q,n) -> Disj (le q n)(oeq q (Suc n)) ;
                                 sub le q n : dvd_fact(le1q)(le q n)->dvd q (fact n) ;
                                   dvd q MS [dvd q n! => dvd q (n! * M) ... wait MS = M*M = (2 n!)*(2 n!),
                                   and n! | M (M=2 n!), so n!|M and q|n! => q|M => q|MS => with q|N consec.
                                   Simplest: q|n! and n!|MS?  MS = (2 n!)^2, n! | (2 n!) | MS.  q|n! => q|MS
                                   by dvd_trans (q|n!, n!|MS).  Then consec_coprime(q, MS)(q|MS)(q|Suc MS).
                                 sub oeq q (Suc n) : q = Suc n -> lt n q = le (Suc n) q = le_refl + subst. ----- *)
                            val ltnq =
                              let
                                  ...
                                  let
                                    ...
                                    val subLeqn =
                                      let
                                        ...
                                        (* n! | MS :  MS = (2 n!)^2 ; n! | (2 n!) = M [dvd_self_mult2: a | (b*a), here n! | (2 * n!)],
                                           and M | MS [dvd_self_mult: M | (M*M)], dvd_trans => n! | MS. *)
                                        val dvd_fn_M =
                                          beta_norm (Drule.infer_instantiate ctxtF
                                            [(("a",0), ctermF factN), (("b",0), ctermF twoCF)] (varify dvd_self_mult2));
                                                                          (* dvd (fact n) (mult 2 (fact n)) = dvd n! M *)
                                        val dvd_M_MS_at =
                                          beta_norm (Drule.infer_instantiate ctxtF
                                            [(("a",0), ctermF M), (("b",0), ctermF M)] (varify dvd_self_mult));
                                                                          (* dvd M (mult M M) = dvd M MS *)
                                        val dvd_fn_MS = dvd_trans_atF (factN, M, MS) dvd_fn_M dvd_M_MS_at;  (* dvd n! MS *)
                                        val dvd_q_MS  = dvd_trans_atF (q, factN, MS) dvdqfn dvd_fn_MS;      (* dvd q MS *)
                                        val fls = consec_coprime_atF (q, MS) lt1q dvd_q_MS hDvdqN;          (* oFalse *)
                                        val g   = (oFalse_elimF_at (ltF nF q)) OF [fls];                    (* lt n q *)
                                      in Thm.implies_intr (ctermF (jT (le q nF))) g end;
                                    val subEq =
                                      let
                                        val heq = Thm.assume (ctermF (jT (oeq q (suc nF))));   (* q = Suc n *)
                                        val leSnSn = le_reflF_at (suc nF);                    (* le (Suc n)(Suc n) *)
                                        val zF   = Free("z_eu", natT);
                                        val Psub = Term.lambda zF (le (suc nF) zF);           (* %z. le (Suc n) z *)
                                        val Sn_q = oeq_sym OF [heq];                          (* Suc n = q *)
                                        val g = substPredF (Psub, suc nF, q) Sn_q leSnSn;     (* le (Suc n) q = lt n q *)
                            ...
                            (* ----- assemble Conj (Conj (prime2 q)(lt n q)) (mod4 q) ; exI ----- *)
                            val conjPL = conjI_atF (prime2 q, ltF nF q) hPrime ltnq;    (* Conj (prime2 q)(lt n q) *)
                            val conjFull = conjI_atF (mkConj (prime2 q) (ltF nF q), mod4Body q) conjPL hMod4;
                                                              (* Conj (Conj (prime2 q)(lt n q))(mod4 q) *)
                            val ex = exI_atF (innerResAbs nF) q conjFull;   (* goalEx *)
                          in ex end;
                        val gW = exE_elimF (Pq1, goalC) le1q "w_q1" bodyW;
                      in gW end;
                    val gK0 = exE_elimF (PsucK, goalC) exk0 "k0_inf" bodyK0;
                  in Thm.implies_intr (ctermF (jT (mkEx PsucK))) gK0 end;

                val combinedK = disjE_elimF (oeq kW ZeroC, mkEx PsucK, goalC) dzk caseK0 caseKS;
              in combinedK end;   (* goalEx *)

            val resK = exE_elimF (PdvdN, goalEx) hDvdqN "kdvd_inf" afterK;
          in resK end;   (* goalEx *)

        val resEx = exE_elimF (pdeBody, goalEx) pdeEx "q_inf" afterQ;
      in resEx end;   (* jT (Ex (%q. Conj (Conj (prime2 q)(lt n q))(mod4 q))) *)

    (* ---- allI over n ---- *)
    val nGen = Free("n", natT);
    val coreThm = coreFor nGen;
    val PallAbs =
      let
        val nF = Free("n_all", natT)
        val qF = Free("q_e", natT)
      in Term.lambda nF (mkEx (Term.lambda qF (mkConj (mkConj (prime2 qF) (ltF nF qF)) (mod4Body qF)))) end;
    val allThm  = Thm.forall_intr (ctermF nGen) coreThm;
    val gAll    = allI_atF PallAbs allThm;
  in varify gAll end;

val () = out "P1M4_INF_PROVED\n";

(* ============================================================================
   VALIDATION : 0-hyp + aconv intended.
   ============================================================================ *)
val p1m4_inf_intended =
  let
    val nF = Free("n_all", natT)
    val qF = Free("q_e", natT)
    val Pall = Term.lambda nF (mkEx (Term.lambda qF
                 (mkConj (mkConj (prime2 qF) (ltF nF qF)) (mod4Body qF))))
  in jT (mkForall Pall) end;

val inf_nhyp  = length (Thm.hyps_of p1m4_inf);
val inf_aconv = (Thm.prop_of p1m4_inf) aconv p1m4_inf_intended;
val () = out ("INF hyps=" ^ Int.toString inf_nhyp ^ " aconv=" ^ Bool.toString inf_aconv ^ "\n");

(* soundness probe : the conclusion genuinely carries the (?t. q = 4t+1) conjunct
   (not the weaker "?t. q = 4t" and not dropping it).                              *)
val inf_probe_mod4 =
  not ((Thm.prop_of p1m4_inf) aconv
       (let
          val nF = Free("n_all", natT)
          val qF = Free("q_e", natT)
          val Pall = Term.lambda nF (mkEx (Term.lambda qF
                       (mkConj (mkConj (prime2 qF) (ltF nF qF))
                               (mkEx (Abs("t", natT, oeq qF (mult fourCF (Bound 0))))))))
        in jT (mkForall Pall) end));

val () =
  if inf_nhyp = 0 andalso inf_aconv andalso inf_probe_mod4
  then out "P1M4_INF_OK\n"
  else out ("P1M4_INF_FAILED nhyp=" ^ Int.toString inf_nhyp
            ^ " aconv=" ^ Bool.toString inf_aconv
            ^ " pmod4=" ^ Bool.toString inf_probe_mod4 ^ "\n");
