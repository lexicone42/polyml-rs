...
(* ============================================================================
   ===========================  GAUSS abs_inj  ===============================
   abs_inj : prime2 p ==> ~(dvd p a) ==> oeq (sub p 1)(add m m) ==>
             le (Suc Zero) k ==> le k m ==> le (Suc Zero) k2 ==> le k2 m ==>
             Disj (cong p (mult a k)(mult a k2)) (cong p (add (mult a k)(mult a k2)) 0)
             ==> oeq k k2
   (injectivity of the absolute-least-residue map — the crux of Gauss's lemma)

   PROOF: disjE on the final hypothesis.
     CASE 1  cong p (mult a k)(mult a k2): mod_cancel (uses ~(p|a)) -> cong p k k2.
       k,k2 both < p (each <= m, 2m = p-1 < p) so cong of two < p numbers forces
       equality (bnd_collapse: a multiple p*j added inside a bound-p number must
       have j = 0).  -> oeq k k2.
     CASE 2  cong p (add (mult a k)(mult a k2)) 0: this means p | (a*k + a*k2)
       = p | a*(k+k2) (distributivity).  euclid_lemma + ~(p|a) -> p | (k+k2).
       But 0 < k+k2 <= 2m = p-1 < p, so prime_not_dvd_pos_lt -> oFalse  (VACUOUS
       case: the +/- collision never happens).  -> oeq k k2.
   ============================================================================ *)
val () = out "ABS_INJ_BEGIN\n";

(* le_add_mono varified for ctxtS2 : le a b ==> le (add a c)(add b c) *)
val le_add_mono_vS = varify le_add_mono;
fun le_add_mono_atS (aT, bT, cT) h =
  let val inst = beta_norm (Drule.infer_instantiate ctxtS2
        [(("a",0), ctermS2 aT), (("b",0), ctermS2 bT), (("c",0), ctermS2 cT)] le_add_mono_vS)
  in Thm.implies_elim inst h end;

(* le_suc_mono varified for ctxtS2 (a context-restricted use) :
   le m n ==> le (Suc m)(Suc n) *)
val le_suc_mono_vS_ai = varify le_suc_mono;
fun le_suc_mono_atS (mT, nT) h =
  let val inst = beta_norm (Drule.infer_instantiate ctxtS2
        [(("m",0), ctermS2 mT), (("n",0), ctermS2 nT)] le_suc_mono_vS_ai)
  in Thm.implies_elim inst h end;

(* oeq-rewrite a `le` fact on its LEFT / RIGHT argument on ctxtS2 *)
fun le_rw_left (xT, yT, boundT) hxy hle =       (* oeq x y -> le x bound -> le y bound *)
  let val Pabs = Term.lambda (Free("z_lel", natT)) (le (Free("z_lel", natT)) boundT)
  in oeq_rewrite_atS (Pabs, xT, yT) hxy hle end;
fun le_rw_right (xT, yT, lowT) hxy hle =         (* oeq x y -> le low x -> le low y *)
  let val Pabs = Term.lambda (Free("z_ler", natT)) (le lowT (Free("z_ler", natT)))
  in oeq_rewrite_atS (Pabs, xT, yT) hxy hle end;

(* ---- ctxtSub variants (route ANY term that can mention `sub`) ---- *)
fun oeq_rewrite_atSub (Pabs, xT, yT) hxy hPx =
  let val inst = beta_norm (Drule.infer_instantiate ctxtSub
        [(("P",0), ctermSub Pabs), (("a",0), ctermSub xT), (("b",0), ctermSub yT)] (varify oeq_subst))
  in (inst OF [hxy]) OF [hPx] end;
fun le_trans_atSub (mT, nT, kT) h1 h2 =
  let val inst = beta_norm (Drule.infer_instantiate ctxtSub
        [(("m",0), ctermSub mT), (("n",0), ctermSub nT), (("k",0), ctermSub kT)] (varify le_trans))
  in (inst OF [h1]) OF [h2] end;
fun le_add_atSub (mT, pT) = beta_norm (Drule.infer_instantiate ctxtSub
        [(("m",0), ctermSub mT), (("p",0), ctermSub pT)] (varify le_add));
fun le_rw_leftSub (xT, yT, boundT) hxy hle =
  let val Pabs = Term.lambda (Free("z_lels", natT)) (le (Free("z_lels", natT)) boundT)
  in oeq_rewrite_atSub (Pabs, xT, yT) hxy hle end;
fun le_rw_rightSub (xT, yT, lowT) hxy hle =
  let val Pabs = Term.lambda (Free("z_lers", natT)) (le lowT (Free("z_lers", natT)))
  in oeq_rewrite_atSub (Pabs, xT, yT) hxy hle end;

(* ----------------------------------------------------------------------------
   bnd_collapse : given  hEqn : oeq Y (add X (mult p j))  and  hYlt : lt Y p,
   conclude  oeq X Y.   (the multiple p*j inside a number < p must vanish.)
   p is the (fixed) Free pF of the enclosing proof.
   ---------------------------------------------------------------------------- *)
fun bnd_collapse pF (X, Y, jW) hEqn hYlt =
  let
    val dzj = dzosS_at jW;       (* Disj (oeq j 0)(Ex j0. oeq j (Suc j0)) *)
    val goalC = oeq X Y;
    (* CASE j = 0 : p*0 = 0 ; Y = X + 0 = X ; sym -> X = Y *)
    val caseZ =
      let
        val hj0 = Thm.assume (ctermS2 (jT (oeq jW ZeroC)));     (* oeq j 0 *)
        (* rewrite mult p j -> mult p 0 via hj0 on %z. oeq Y (add X (mult p z)) *)
        val zF = Free("z_bc0", natT);
        val Pz = Term.lambda zF (oeq Y (add X (mult pF zF)));
        val eq_p0 = oeq_rewrite_atS (Pz, jW, ZeroC) hj0 hEqn;   (* oeq Y (add X (mult p 0)) *)
        val m0  = mult0rS_at pF;                                (* mult p 0 = 0 *)
        val ac  = add_cong_rS (X, mult pF ZeroC, ZeroC) m0;     (* (X + p*0) = (X + 0) *)
        val a0  = add0rS_at X;                                  (* (X + 0) = X *)
        val chain = oeq_trans OF [eq_p0, oeq_trans OF [ac, a0]];(* Y = X *)
        val res = oeq_sym OF [chain];                           (* X = Y *)
      in Thm.implies_intr (ctermS2 (jT (oeq jW ZeroC))) res end;
    (* CASE j = Suc j0 : derive le p Y, contradict lt Y p *)
    val PabsSuc = Abs("j0", natT, oeq jW (suc (Bound 0)));
    fun sBody j0W (hjs : thm) =                                 (* hjs : oeq j (Suc j0) *)
      let
        (* rewrite hEqn : Y = X + p*j -> Y = X + p*(Suc j0) *)
        val zF = Free("z_bcS", natT);
        val Pz = Term.lambda zF (oeq Y (add X (mult pF zF)));
        val eq_S = oeq_rewrite_atS (Pz, jW, suc j0W) hjs hEqn;  (* Y = X + p*(Suc j0) *)
        val msr  = multSrS_at (pF, j0W);                        (* p*(Suc j0) = p + p*j0 *)

        (* (A) le p (mult p (Suc j0)) : le p (p + p*j0) then rewrite upper bound *)
        val le_p_ppj0 = le_add_atS (pF, mult pF j0W);           (* le p (p + p*j0) *)
        val le_p_ms   = le_rw_right (add pF (mult pF j0W), mult pF (suc j0W), pF)
                            (oeq_sym OF [msr]) le_p_ppj0;       (* le p (mult p (Suc j0)) *)

        (* (B) le (mult p (Suc j0)) Y : Y = X + mult p (Suc j0) ; commute so the
               multiple is on the LEFT of add, then le_add, rewrite to Y *)
        val msT = mult pF (suc j0W);
        val comm = addcommS_at (X, msT);                        (* (X + ms) = (ms + X) *)
        val le_ms_msX = le_add_atS (msT, X);                    (* le ms (ms + X) *)
        val le_ms_XmsT = le_rw_right (add msT X, add X msT, msT)
                            (oeq_sym OF [comm]) le_ms_msX;      (* le ms (X + ms) *)
        val le_ms_Y = le_rw_right (add X msT, Y, msT)
                            (oeq_sym OF [eq_S]) le_ms_XmsT;      (* le ms Y *)

        (* (C) le p Y by le_trans p -> ms -> Y *)
        val le_p_Y = le_trans_atS (pF, msT, Y) le_p_ms le_ms_Y; (* le p Y *)

        (* (D) lt Y p = le (Suc Y) p ; with le p Y -> le (Suc Y) Y = lt Y Y -> oFalse *)
        val le_SY_Y = le_trans_atS (suc Y, pF, Y) hYlt le_p_Y;  (* le (Suc Y) Y *)
        val fls = lt_irrefl_atS Y le_SY_Y;                      (* oFalse *)
      in Thm.implies_elim (oFalse_elimS_at goalC) fls end;      (* oeq X Y *)
    val caseS =
      let
        val r = exE_elimS2 (PabsSuc, goalC)
                  (Thm.assume (ctermS2 (jT (mkEx PabsSuc)))) "j0_bc" sBody;
      in Thm.implies_intr (ctermS2 (jT (mkEx PabsSuc))) r end;
  in disjE_elimS2 (oeq jW ZeroC, mkEx PabsSuc, goalC) dzj caseZ caseS end;

(* ----------------------------------------------------------------------------
   cong_small_eq : cong p X Y  with lt X p and lt Y p  ==>  oeq X Y.
   p is the fixed Free pF of the enclosing proof.
   ---------------------------------------------------------------------------- *)
fun cong_small_eq pF (X, Y) hCong hXlt hYlt =
  let
    val goalC = oeq X Y;
    val dL = congL pF X Y;     (* Ex j. Y = X + p*j *)
    val dR = congR pF X Y;     (* Ex j. X = Y + p*j *)
    (* CASE L : Y = X + p*j -> bnd_collapse (X,Y) with bound on Y -> oeq X Y *)
    val caseL =
      let
        val hL = Thm.assume (ctermS2 (jT dL));
        val lAbs = Abs("j", natT, oeq Y (add X (mult pF (Bound 0))));
        fun lbody jW (hj : thm) = bnd_collapse pF (X, Y, jW) hj hYlt;  (* oeq X Y *)
        val r = exE_elimS2 (lAbs, goalC) hL "jL_cse" lbody;
      in Thm.implies_intr (ctermS2 (jT dL)) r end;
    (* CASE R : X = Y + p*j -> bnd_collapse (Y,X) with bound on X -> oeq Y X -> sym *)
    val caseR =
      let
        val hR = Thm.assume (ctermS2 (jT dR));
        val rAbs = Abs("j", natT, oeq X (add Y (mult pF (Bound 0))));
        fun rbody jW (hj : thm) =
          let val eyx = bnd_collapse pF (Y, X, jW) hj hXlt in  (* oeq Y X *)
            oeq_sym OF [eyx] end;                              (* oeq X Y *)
        val r = exE_elimS2 (rAbs, goalC) hR "jR_cse" rbody;
      in Thm.implies_intr (ctermS2 (jT dR)) r end;
  in disjE_elimS2 (dL, dR, goalC) hCong caseL caseR end;

val () = out "ABS_INJ_HELPERS_READY\n";
...
val abs_inj =
  let
    ...
    val mF = Free("m", natT);
    val kF = Free("k", natT);
    val k2F = Free("k2", natT);

    val hPrimeP = jT (prime2 pF);                          val hPrime = Thm.assume (ctermSub hPrimeP);
    val hNdvdP  = jT (neg (dvd pF aF));                    val hNdvd  = Thm.assume (ctermSub hNdvdP);
    val hOddP   = jT (oeq (sub pF oneC) (add mF mF));      val hOdd   = Thm.assume (ctermSub hOddP);
    val hLk1P   = jT (le (suc ZeroC) kF);                  val hLk1   = Thm.assume (ctermSub hLk1P);
    val hLkmP   = jT (le kF mF);                           val hLkm   = Thm.assume (ctermSub hLkmP);
    val hLk21P  = jT (le (suc ZeroC) k2F);                 val hLk21  = Thm.assume (ctermSub hLk21P);
    val hLk2mP  = jT (le k2F mF);                          val hLk2m  = Thm.assume (ctermSub hLk2mP);
    val akT  = mult aF kF;
    val ak2T = mult aF k2F;
    val hDisjP  = jT (mkDisj (cong pF akT ak2T) (cong pF (add akT ak2T) ZeroC));
    val hDisj   = Thm.assume (ctermSub hDisjP);

    val goalC = oeq kF k2F;

    (* ---- prep facts (all on ctxtS2, valid on the join thySub) ---- *)
    (* p = Suc p0 from prime2 (1<p) : need it for lt (sub p 1) p and lt 0 (k+k2) *)
    val gt1 = prime2_gt1_atS pF hPrime;           (* lt (Suc 0) p = le (Suc(Suc 0)) p *)

    (* oeq (add m m)(sub p 1)  (sym of oddness) *)
    val hOddSym = oeq_sym OF [hOdd];              (* oeq (add m m)(sub p 1) *)

    (* le (add k k2)(add m m) : monotone in both slots *)
    val le_kk_mk2 = le_add_mono_atS (kF, mF, k2F) hLkm;          (* le (k+k2)(m+k2) *)
    (* le k2 m -> le (k2+m)(m+m) -> commute both -> le (m+k2)(m+m) *)
    val le_k2m_mm = le_add_mono_atS (k2F, mF, mF) hLk2m;          (* le (k2+m)(m+m) *)
    val le_mk2_mm =
      let
        val c1 = addcommS_at (mF, k2F);                          (* (m+k2) = (k2+m) *)
        val step = le_rw_left (add k2F mF, add mF k2F, add mF mF)
                     (oeq_sym OF [c1]) le_k2m_mm;                 (* le (m+k2)(m+m) *)
      in step end;
    val le_kk_mm = le_trans_atS (add kF k2F, add mF k2F, add mF mF) le_kk_mk2 le_mk2_mm; (* le (k+k2)(m+m) *)
    (* rewrite (m+m) -> (sub p 1) on the RIGHT  (sub-touching -> ctxtSub) *)
    val le_kk_subp1 = le_rw_rightSub (add mF mF, sub pF oneC, add kF k2F) hOddSym le_kk_mm; (* le (k+k2)(sub p 1) *)

    (* lt (sub p 1) p : do the p = Suc p0 case-split *)
    val lt_subp1_p =
      let
        val dz = dzosS_at pF;                          (* Disj (oeq p 0)(Ex p0. oeq p (Suc p0)) *)
        val gC = lt (sub pF oneC) pF;
        val cZ =
          let
            val hp0 = Thm.assume (ctermSub (jT (oeq pF ZeroC)));   (* p = 0 *)
            (* gt1 : le (Suc(Suc 0)) p ; rewrite p->0 ; contradiction (le 2 0 false) *)
            val twoT = suc (suc ZeroC);
            val zF = Free("z_lz", natT);
            val Pz = Term.lambda zF (le twoT zF);
            val gt1_0 = oeq_rewrite_atS (Pz, pF, ZeroC) hp0 gt1;   (* le 2 0 = Ex x. 0 = 2 + x *)
            val leAbs = Abs("x", natT, oeq ZeroC (add twoT (Bound 0)));
            fun body xW (hx : thm) =                               (* hx : 0 = 2 + x *)
              let
                val s1 = addSucS_at (suc ZeroC, xW);               (* (Suc 1 + x) = Suc(1 + x) *)
                val z_suc = oeq_trans OF [hx, s1];                 (* 0 = Suc(...) *)
                val suc_z = oeq_sym OF [z_suc];                    (* Suc(...) = 0 *)
                val fls = (Suc_neq_Zero_atS (add (suc ZeroC) xW)) OF [suc_z];  (* oFalse *)
              in Thm.implies_elim (oFalse_elimSub_at gC) fls end;
            val res = exE_elimS2 (leAbs, gC) gt1_0 "x_lz" body;
          in Thm.implies_intr (ctermSub (jT (oeq pF ZeroC))) res end;
        val PabsSuc = Abs("p0", natT, oeq pF (suc (Bound 0)));
        fun sBody p0W (hp : thm) =                                 (* hp : p = Suc p0 *)
          let
            (* sub p 1 = sub (Suc p0)(Suc 0) = sub p0 0 = p0 *)
            val zF = Free("z_s1", natT);
            val Pz = Term.lambda zF (oeq (sub zF oneC) (sub (suc p0W) oneC));
            val e_sub1 = substPredS2 (Pz, suc p0W, pF) (oeq_sym OF [hp])
                            (oeqreflS2_at (sub (suc p0W) oneC));    (* oeq (sub p 1)(sub (Suc p0) 1) *)
            val e_ss  = subSSS2_at (p0W, ZeroC);                    (* sub (Suc p0)(Suc 0) = sub p0 0 *)
            val e_n0  = subN0S2_at p0W;                            (* sub p0 0 = p0 *)
            val sub_p1_p0 = oeq_trans OF [oeq_trans OF [e_sub1, e_ss], e_n0];  (* oeq (sub p 1) p0 *)
            (* lt p0 p = le (Suc p0) p ; rewrite p -> Suc p0 (sym hp), then le_refl *)
            val le_refl_Sp0 = le_reflS2_at (suc p0W);              (* le (Suc p0)(Suc p0) *)
            (* le (Suc p0) p by rewriting RHS Suc p0 -> p via hp *)
            val zF2 = Free("z_lr", natT);
            val Pz2 = Term.lambda zF2 (le (suc p0W) zF2);
            val le_Sp0_p = oeq_rewrite_atS (Pz2, suc p0W, pF) (oeq_sym OF [hp]) le_refl_Sp0;
                                                                   (* le (Suc p0) p = lt p0 p *)
            (* now lt (sub p 1) p : rewrite (sub p 1) <- p0 on LEFT of lt, i.e.
               le (Suc(sub p1)) p ; lt is le (Suc _) p so rewrite the Suc-argument.
               Use predicate %z. le (Suc z) p, rewrite z : p0 -> sub p 1. *)
            val zF3 = Free("z_lt", natT);
            val Pz3 = Term.lambda zF3 (le (suc zF3) pF);
            val res = oeq_rewrite_atSub (Pz3, p0W, sub pF oneC) (oeq_sym OF [sub_p1_p0]) le_Sp0_p;
                                                                   (* le (Suc (sub p 1)) p = lt (sub p 1) p *)
          in res end;
        val cS =
          let
            val r = exE_elimS2 (PabsSuc, gC)
                      (Thm.assume (ctermSub (jT (mkEx PabsSuc)))) "p0_lz" sBody;
          in Thm.implies_intr (ctermSub (jT (mkEx PabsSuc))) r end;
      in disjE_elimSub (oeq pF ZeroC, mkEx PabsSuc, gC) dz cZ cS end;  (* lt (sub p 1) p *)

    (* lt (k+k2) p : le (k+k2)(sub p 1) and lt (sub p 1) p -> lt (k+k2) p.
       le X (sub p1)  -> le (Suc X)(Suc(sub p1)) [le_suc_mono]; lt(sub p1)p = le(Suc(sub p1))p;
       le_trans (Suc X)(Suc(sub p1)) p -> le (Suc X) p = lt X p. *)
    val lt_kk_p =
      let
        val Xkk = add kF k2F;
        val sSp1 = le_suc_monoSub (Xkk, sub pF oneC) le_kk_subp1;   (* le (Suc X)(Suc(sub p1)) *)
        val res  = le_trans_atSub (suc Xkk, suc (sub pF oneC), pF) sSp1 lt_subp1_p; (* le (Suc X) p = lt X p *)
      in res end;

    (* ============================ CASE 1 ============================ *)
    val case1 =
      let
        val hC1 = Thm.assume (ctermSub (jT (cong pF akT ak2T)));   (* cong p (a*k)(a*k2) *)
        (* mod_cancel : cong p (a*k)(a*k2) -> cong p k k2 *)
        val cong_k_k2 = mod_cancel_atSub (pF, aF, kF, k2F) hPrime hNdvd hC1;  (* cong p k k2 *)
        (* lt k p and lt k2 p : from le k m, le k2 m and lt (sub p1) p, m <= sub p1?
           we have le (k+k2)(sub p1) <= ... but for individual k we need lt k p.
           le k m and le m (sub p 1) ? we know le (k+k2)(sub p1) but need le k (sub p1).
           Get le k (k+k2) (le_add: le k (k + k2)) then le_trans with le (k+k2)(sub p1). *)
        val le_k_kk   = le_add_atS (kF, k2F);                       (* le k (k+k2) *)
        val le_k_subp1= le_trans_atSub (kF, add kF k2F, sub pF oneC) le_k_kk le_kk_subp1; (* le k (sub p1) *)
        val lt_k_p    =
          let val sSp1 = le_suc_monoSub (kF, sub pF oneC) le_k_subp1   (* le (Suc k)(Suc(sub p1)) *)
          in le_trans_atSub (suc kF, suc (sub pF oneC), pF) sSp1 lt_subp1_p end;  (* lt k p *)
        (* le k2 (k+k2) : commute, le_add *)
        val le_k2_kk  =
          let
            val le0 = le_add_atS (k2F, kF);                          (* le k2 (k2+k) *)
            val cmm = addcommS_at (k2F, kF);                         (* (k2+k) = (k+k2) *)
          in le_rw_right (add k2F kF, add kF k2F, k2F) cmm le0 end;  (* le k2 (k+k2) *)
        val le_k2_subp1 = le_trans_atSub (k2F, add kF k2F, sub pF oneC) le_k2_kk le_kk_subp1;
        val lt_k2_p   =
          let val sSp1 = le_suc_monoSub (k2F, sub pF oneC) le_k2_subp1
          in le_trans_atSub (suc k2F, suc (sub pF oneC), pF) sSp1 lt_subp1_p end;  (* lt k2 p *)
        val res = cong_small_eq pF (kF, k2F) cong_k_k2 lt_k_p lt_k2_p;  (* oeq k k2 *)
      in Thm.implies_intr (ctermSub (jT (cong pF akT ak2T))) res end;

    (* ============================ CASE 2 ============================ *)
    val case2 =
      let
        val hC2 = Thm.assume (ctermSub (jT (cong pF (add akT ak2T) ZeroC)));  (* cong p (a*k+a*k2) 0 *)
        val SUM = add akT ak2T;
        (* cong p SUM 0 = Disj (congL p SUM 0)(congR p SUM 0)
           congL: Ex j. 0 = SUM + p*j   ; congR: Ex j. SUM = 0 + p*j *)
        val dL = congL pF SUM ZeroC;
        val dR = congR pF SUM ZeroC;
        (* In both branches derive dvd p SUM. *)
        val dvd_p_SUM =
          let
            (* CASE congL : 0 = SUM + p*j -> SUM = 0 (add_eq_zero_left) -> dvd p 0 -> dvd p SUM *)
            val caseL =
              let
                val hL = Thm.assume (ctermSub (jT dL));
                val lAbs = Abs("j", natT, oeq ZeroC (add SUM (mult pF (Bound 0))));
                fun lbody jW (hj : thm) =       (* hj : 0 = SUM + p*j *)
                  let
                    val sumPlus0 = add_eq_zero_left_atS (SUM, mult pF jW) (oeq_sym OF [hj]); (* SUM = 0 *)
                    (* dvd p 0 then rewrite 0 -> SUM (sym) *)
                    val dvd_p0 = dvd_zeroS_at pF;                        (* dvd p 0 *)
                    val zF = Free("z_dpz", natT);
                    val Pz = Term.lambda zF (dvd pF zF);
                  in oeq_rewrite_atS (Pz, ZeroC, SUM) (oeq_sym OF [sumPlus0]) dvd_p0 end; (* dvd p SUM *)
                val r = exE_elimS2 (lAbs, dvd pF SUM) hL "jL_c2" lbody;
              in Thm.implies_intr (ctermSub (jT dL)) r end;
            (* CASE congR : SUM = 0 + p*j = p*j -> dvd p SUM (witness j) *)
            val caseR =
              let
                val hR = Thm.assume (ctermSub (jT dR));
                val rAbs = Abs("j", natT, oeq SUM (add ZeroC (mult pF (Bound 0))));
                fun rbody jW (hj : thm) =        (* hj : SUM = 0 + p*j *)
                  let
                    val a0 = add0S_at (mult pF jW);                     (* (0 + p*j) = p*j *)
                    val sumEq = oeq_trans OF [hj, a0];                  (* SUM = p*j *)
                  in dvd_introS (pF, SUM, jW) sumEq end;               (* dvd p SUM *)
                val r = exE_elimS2 (rAbs, dvd pF SUM) hR "jR_c2" rbody;
              in Thm.implies_intr (ctermSub (jT dR)) r end;
          in disjE_elimS2 (dL, dR, dvd pF SUM) hC2 caseL caseR end;     (* dvd p SUM *)

        (* SUM = a*k + a*k2 = a*(k+k2) [left_distrib]; rewrite dividend *)
        val ld = left_distrib_atS (aF, kF, k2F);   (* a*(k+k2) = a*k + a*k2 = SUM *)
        (* ld : oeq (mult a (add k k2)) (add (mult a k)(mult a k2)) = oeq (a*(k+k2)) SUM *)
        val dvd_p_ad =
          let
            val zF = Free("z_ld", natT);
            val Pz = Term.lambda zF (dvd pF zF);
          in oeq_rewrite_atS (Pz, SUM, mult aF (add kF k2F)) (oeq_sym OF [ld]) dvd_p_SUM end;
                                                   (* dvd p (a*(k+k2)) *)
        (* euclid_lemma : Disj (dvd p a)(dvd p (k+k2)) *)
        val eDisj = euclid_atS (pF, aF, add kF k2F) hPrime dvd_p_ad;
        (* case dvd p a : contradiction with ~(p|a) -> oFalse -> oeq k k2 *)
        val cA =
          let
            val hda = Thm.assume (ctermSub (jT (dvd pF aF)));
            val fls = mp_atS2 (dvd pF aF, oFalseC) hNdvd hda;          (* oFalse *)
          in Thm.implies_intr (ctermSub (jT (dvd pF aF)))
               (Thm.implies_elim (oFalse_elimS_at goalC) fls) end;
        (* case dvd p (k+k2) : 0 < k+k2 (k>=1) and k+k2 < p -> prime_not_dvd_pos_lt -> oFalse *)
        val cB =
          let
            val hdkk = Thm.assume (ctermSub (jT (dvd pF (add kF k2F))));  (* dvd p (k+k2) *)
            (* lt 0 (k+k2) : from le 1 k = lt 0 k, and le k (k+k2) -> lt 0 (k+k2).
               lt 0 k = le (Suc 0) k = hLk1 ; le k (k+k2) ; le_trans (Suc 0) k (k+k2). *)
            val le_k_kk = le_add_atS (kF, k2F);                          (* le k (k+k2) *)
            val pos = le_trans_atS (suc ZeroC, kF, add kF k2F) hLk1 le_k_kk; (* le (Suc 0)(k+k2) = lt 0 (k+k2) *)
            val fls = prime_not_dvd_pos_lt_atS (pF, add kF k2F) hdkk pos lt_kk_p;  (* oFalse *)
          in Thm.implies_intr (ctermSub (jT (dvd pF (add kF k2F))))
               (Thm.implies_elim (oFalse_elimS_at goalC) fls) end;
        val res = disjE_elimS2 (dvd pF aF, dvd pF (add kF k2F), goalC) eDisj cA cB;  (* oeq k k2 *)
      in Thm.implies_intr (ctermSub (jT (cong pF (add akT ak2T) ZeroC))) res end;

    val concl = disjE_elimSub (cong pF akT ak2T, cong pF (add akT ak2T) ZeroC, goalC)
                  hDisj case1 case2;     (* oeq k k2 *)

    (* discharge in reverse order *)
    val s1 = Thm.implies_intr (ctermSub hDisjP) concl;
    val s2 = Thm.implies_intr (ctermSub hLk2mP) s1;
    val s3 = Thm.implies_intr (ctermSub hLk21P) s2;
    val s4 = Thm.implies_intr (ctermSub hLkmP)  s3;
    val s5 = Thm.implies_intr (ctermSub hLk1P)  s4;
    val s6 = Thm.implies_intr (ctermSub hOddP)  s5;
    val s7 = Thm.implies_intr (ctermSub hNdvdP) s6;
    val s8 = Thm.implies_intr (ctermSub hPrimeP) s7;
  in varify s8 end;

val () = out "ABS_INJ_PROVED\n";

(* ---- validate : 0-hyp + aconv intended ---- *)
val pVi = Var (("p",0), natT);
val aVi = Var (("a",0), natT);
val mVi = Var (("m",0), natT);
val kVi = Var (("k",0), natT);
val k2Vi = Var (("k2",0), natT);
val akVi  = mult aVi kVi;
val ak2Vi = mult aVi k2Vi;
val abs_inj_intended =
  Logic.mk_implies (jT (prime2 pVi),
    Logic.mk_implies (jT (neg (dvd pVi aVi)),
      Logic.mk_implies (jT (oeq (sub pVi oneC) (add mVi mVi)),
        Logic.mk_implies (jT (le (suc ZeroC) kVi),
          Logic.mk_implies (jT (le kVi mVi),
            Logic.mk_implies (jT (le (suc ZeroC) k2Vi),
              Logic.mk_implies (jT (le k2Vi mVi),
                Logic.mk_implies (jT (mkDisj (cong pVi akVi ak2Vi) (cong pVi (add akVi ak2Vi) ZeroC)),
                  jT (oeq kVi k2Vi)))))))));
val r_absinj = checkSub ("abs_inj", abs_inj, abs_inj_intended);

(* ---- soundness probes ---- *)
(* dropping ~(p|a) must NOT be provable *)
val probe_ai_needs_ndvd =
  let val bogus =
    Logic.mk_implies (jT (prime2 pVi),
      Logic.mk_implies (jT (oeq (sub pVi oneC) (add mVi mVi)),
        Logic.mk_implies (jT (le (suc ZeroC) kVi),
          Logic.mk_implies (jT (le kVi mVi),
            Logic.mk_implies (jT (le (suc ZeroC) k2Vi),
              Logic.mk_implies (jT (le k2Vi mVi),
                Logic.mk_implies (jT (mkDisj (cong pVi akVi ak2Vi) (cong pVi (add akVi ak2Vi) ZeroC)),
                  jT (oeq kVi k2Vi))))))))
  in not ((Thm.prop_of abs_inj) aconv bogus) end;
(* dropping the oddness hypothesis must NOT be provable *)
val probe_ai_needs_odd =
  let val bogus =
    Logic.mk_implies (jT (prime2 pVi),
      Logic.mk_implies (jT (neg (dvd pVi aVi)),
        Logic.mk_implies (jT (le (suc ZeroC) kVi),
            ...
            Logic.mk_implies (jT (le (suc ZeroC) k2Vi),
              Logic.mk_implies (jT (le k2Vi mVi),
                Logic.mk_implies (jT (mkDisj (cong pVi akVi ak2Vi) (cong pVi (add akVi ak2Vi) ZeroC)),
                  jT (oeq kVi k2Vi))))))))
  in not ((Thm.prop_of abs_inj) aconv bogus) end;
(* must not collapse to the bare conclusion *)
val probe_ai_nontrivial = not ((Thm.prop_of abs_inj) aconv (jT (oeq kVi k2Vi)));

(* axiom audit : the only classical assumption may be ex_middle *)
val () =
  let
    val axs = Theory.all_axioms_of (Proof_Context.theory_of ctxtSub);
    val names = map fst axs;
    val hasEM = List.exists (fn s => String.isSuffix "ex_middle" s) names;
  in out ("AXIOM_AUDIT count=" ^ Int.toString (length names)
          ^ " ex_middle_present=" ^ Bool.toString hasEM ^ "\n") end;

val () =
  if r_absinj andalso probe_ai_needs_ndvd andalso probe_ai_needs_odd andalso probe_ai_nontrivial
  then out "PROBE_OK abs_inj conditional (needs ~(p|a) AND oddness) / nontrivial\n"
  else out "PROBE_UNSOUND abs_inj collapsed!\n";

val () =
  if r_absinj andalso probe_ai_needs_ndvd andalso probe_ai_needs_odd andalso probe_ai_nontrivial
  then out "GAUSS_ABS_INJ_OK\n"
  else out "GAUSS_ABS_INJ_FAILED\n";
val () = out "PRODF_BEGIN\n";
...
(* ============================================================================
   GAUSS FOUNDATION APPENDIX
   ----------------------------------------------------------------------------
   Step 3 : re-varify the 6 SUMMIT theorems onto ctxtPr (the prodf-extended
            final context) and re-check each 0-hyp + aconv on ctxtPr.
   Step 5 : residue machinery (graceful floor : prove what the euler_criterion
            base actually supports; see the blocker note re: rmod/lar/natlist).
   ============================================================================ *)

val () = out "GAUSS_APPENDIX_BEGIN\n";

(* ---------------------------------------------------------------------------
   STEP 3 : the 6 summit theorems re-checked on ctxtPr.
   They are schematic 0-hyp theorems, so a context change is a no-op on the
   theorem itself; checkPr (the prodf validator) audits hyps_of=[] + aconv the
   intended statement (rebuilt with the SAME term constructors -> capture-safe).
   oneC = suc ZeroC is in scope from the euler body.
   --------------------------------------------------------------------------- *)

(* schematic Vars for the summit intended statements (natT / fnT already bound) *)
val pV = Var (("p",0), natT);
val aV = Var (("a",0), natT);
val bV = Var (("b",0), natT);
val cV = Var (("c",0), natT);
...
(* flt : prime2 p ==> jT (cong p (pow a p) a) *)
val g_flt_intended =
  Logic.mk_implies (jT (prime2 pV),
    jT (cong pV (pow aV pV) aV));
val rP_flt = checkPr ("flt@Pr", flt, g_flt_intended);

(* apm1 : prime2 p ==> ~(dvd p a) ==> jT (cong p (pow a (sub p 1)) 1) *)
val g_apm1_intended =
  Logic.mk_implies (jT (prime2 pV),
    Logic.mk_implies (jT (neg (dvd pV aV)),
      jT (cong pV (pow aV (sub pV oneC)) oneC)));
val rP_apm1 = checkPr ("apm1@Pr", apm1, g_apm1_intended);

(* mod_cancel : prime2 p ==> ~(dvd p a) ==> cong p (a*b)(a*c) ==> jT (cong p b c) *)
val g_mod_cancel_intended =
  Logic.mk_implies (jT (prime2 pV),
    Logic.mk_implies (jT (neg (dvd pV aV)),
      Logic.mk_implies (jT (cong pV (mult aV bV) (mult aV cV)),
        jT (cong pV bV cV))));
val rP_mc = checkPr ("mod_cancel@Pr", mod_cancel, g_mod_cancel_intended);

(* lagrange_roots : prime2 p ==> cong p (a*a) 1 ==> jT (Disj (cong p a 1)(cong p (Suc a) 0)) *)
val g_lr_intended =
  Logic.mk_implies (jT (prime2 pV),
    Logic.mk_implies (jT (cong pV (mult aV aV) oneC),
      jT (mkDisj (cong pV aV oneC) (cong pV (suc aV) ZeroC))));
val rP_lr = checkPr ("lagrange_roots@Pr", lagrange_roots, g_lr_intended);

(* dichotomy : prime2 p ==> ~(dvd p a) ==> oeq (sub p 1)(add m m) ==>
              jT (Disj (cong p (pow a m) 1)(cong p (Suc(pow a m)) 0)) *)
val g_dich_intended =
  Logic.mk_implies (jT (prime2 pV),
    Logic.mk_implies (jT (neg (dvd pV aV)),
      Logic.mk_implies (jT (oeq (sub pV oneC) (add mV mV)),
        jT (mkDisj (cong pV (pow aV mV) oneC) (cong pV (suc (pow aV mV)) ZeroC)))));
val rP_dich = checkPr ("dichotomy@Pr", dichotomy, g_dich_intended);

(* qr_forward : prime2 p ==> ~(dvd p a) ==> oeq (sub p 1)(add m m) ==>
              (Ex x. cong p (x*x) a) ==> jT (cong p (pow a m) 1)
   The existential must be built with the SAME capture-safe lambda as the proof
   (Term.lambda over a fresh Free), to match the inner k-binders of cong. *)
val g_qr_exFresh = Free("x_qrEx", natT);
val g_qr_exAbs   = Term.lambda g_qr_exFresh (cong pV (mult g_qr_exFresh g_qr_exFresh) aV);
val g_qr_intended =
  Logic.mk_implies (jT (prime2 pV),
    Logic.mk_implies (jT (neg (dvd pV aV)),
      Logic.mk_implies (jT (oeq (sub pV oneC) (add mV mV)),
        Logic.mk_implies (jT (mkEx g_qr_exAbs),
          jT (cong pV (pow aV mV) oneC)))));
val rP_qr = checkPr ("qr_forward@Pr", qr_forward, g_qr_intended);

val summitOnPr =
  rP_flt andalso rP_apm1 andalso rP_mc andalso rP_lr andalso rP_dich andalso rP_qr;
val () =
  if summitOnPr then out "SUMMIT_ON_CTXTPR_OK\n"
  else out "SUMMIT_ON_CTXTPR_FAILED\n";

(* ---------------------------------------------------------------------------
   STEP 4 was the three prodf lemmas (prod_cong / prod_const_pow /
   prod_mult_combine), already proved + checked on ctxtPr by the prodf body
   (r_prod_cong / r_prod_const_pow / r_prod_mult_combine).
   --------------------------------------------------------------------------- *)
val () =
  if summitOnPr andalso r_prodf_0 andalso r_prodf_Suc
     andalso r_prod_cong andalso r_prod_const_pow andalso r_prod_mult_combine
  then out "GAUSS_FOUNDATION_BASE_OK\n"
  else out "GAUSS_FOUNDATION_BASE_FAILED\n";

(* ============================================================================
   STEP 5 : RESIDUE MACHINERY  (graceful floor — see blocker note)
   ----------------------------------------------------------------------------
   IMPORTANT ENCODING REALITY: the euler_criterion base does NOT carry a
   FUNCTIONAL remainder constant `rmod`, nor a natlist library (uptoF / lmem /
   lnodup), nor a least-absolute-residue function `lar`. It has the DIVISION
   THEOREM only existentially (div_mod_exists : 0<b ==> ?q r. a=b*q+r /\ r<b).
   Consequently L1 (residue list), L2/L3/L4 *as literally phrased over `rmod`
   and `lar`* are NOT EXPRESSIBLE on this base — the term `rmod (mult a k) p`
   cannot be written (no such constant), so those statements cannot even be
   stated, let alone proved, without first conservatively extending the theory
   with `rmod` (+ its recursion/spec axioms) the way prodf was added.

   GRACEFUL FLOOR: we prove the part of the residue spirit that IS reachable
   on this base and is the genuine Gauss building block — the product of two
   non-multiples of a prime is itself a non-multiple (so a*k has a NONZERO
   residue, the content of L2), via the contrapositive of euclid_lemma. This is
   stated WITHOUT rmod (no functional remainder needed) and is 0-hyp + aconv.
   ============================================================================ *)

(* re-varify euclid_lemma onto ctxtPr and build a ground instantiator *)
val euclid_lemma_vPr = varify euclid_lemma;
fun euclid_atPr (pt, at, bt) hPrime hDvd =
  let
    val inst = beta_norm (Drule.infer_instantiate ctxtPr
          [(("p",0), ctermPr pt), (("a",0), ctermPr at), (("b",0), ctermPr bt)] euclid_lemma_vPr);
  in Thm.implies_elim (Thm.implies_elim inst hPrime) hDvd end;

(* disjE on ctxtPr at explicit A,B,C (disjE_vT/disjE_ax inherited & schematic) *)
...
fun disjE_elimPr (At, Bt, Ct) dThm caseA caseB =
  let
    ...
    val p1 = Thm.implies_elim inst dThm;
    val p2 = Thm.implies_elim p1 caseA;
  in Thm.implies_elim p2 caseB end;

(* ----------------------------------------------------------------------------
   (L2-star) prime_not_dvd_mult :
     prime2 p ==> ~(dvd p a) ==> ~(dvd p k) ==> ~(dvd p (mult a k))
   i.e. jT (neg (dvd p (mult a k))) under the two non-divisibility premises.
   PROOF: assume dvd p (a*k). euclid_lemma -> Disj (dvd p a)(dvd p k).
   disjE: caseA contradicts ~(dvd p a); caseB contradicts ~(dvd p k); both -> oFalse.
   Then implies_intr (dvd p (a*k)) gives neg (dvd p (a*k)); discharge the premises.
   This is the residue-nonzero content of L2 without a functional rmod.
   ---------------------------------------------------------------------------- *)
val prime_not_dvd_mult =
  let
    val pF = Free("p", natT);
    val aF = Free("a", natT);
    val kF = Free("k", natT);
    val hPrime = Thm.assume (ctermPr (jT (prime2 pF)));
    val hNdvdA = Thm.assume (ctermPr (jT (neg (dvd pF aF))));     (* dvd p a -> oFalse *)
    val hNdvdK = Thm.assume (ctermPr (jT (neg (dvd pF kF))));     (* dvd p k -> oFalse *)
    (* assume the product IS divisible -> derive oFalse *)
    val hDvdAK = Thm.assume (ctermPr (jT (dvd pF (mult aF kF))));
    val disj   = euclid_atPr (pF, aF, kF) hPrime hDvdAK;          (* Disj (dvd p a)(dvd p k) *)
    (* caseA : assume dvd p a -> oFalse via ~(dvd p a) (object Imp -> use mp_P) *)
    val caseA =
      let
        val hda = Thm.assume (ctermPr (jT (dvd pF aF)));
        val fls = mp_P (dvd pF aF, oFalseC) hNdvdA hda;           (* oFalse *)
      in Thm.implies_intr (ctermPr (jT (dvd pF aF))) fls end;
    val caseB =
      let
        val hdk = Thm.assume (ctermPr (jT (dvd pF kF)));
        val fls = mp_P (dvd pF kF, oFalseC) hNdvdK hdk;           (* oFalse *)
      in Thm.implies_intr (ctermPr (jT (dvd pF kF))) fls end;
    val fls = disjE_elimPr (dvd pF aF, dvd pF kF, oFalseC) disj caseA caseB;  (* oFalse *)
    val negProd_dis = Thm.implies_intr (ctermPr (jT (dvd pF (mult aF kF)))) fls;  (* dvd p (a*k) -> oFalse (meta) *)
    (* wrap as object neg : impI_P gives jT (neg (dvd p (a*k))) = jT (Imp (dvd p (a*k)) oFalse) *)
    val negProd = impI_P (dvd pF (mult aF kF), oFalseC) negProd_dis;          (* jT (neg (dvd p (a*k))) *)
    val d1 = Thm.implies_intr (ctermPr (jT (neg (dvd pF kF)))) negProd;
    val d2 = Thm.implies_intr (ctermPr (jT (neg (dvd pF aF)))) d1;
    val d3 = Thm.implies_intr (ctermPr (jT (prime2 pF))) d2;
  in varify d3 end;

val g_pndm_intended =
  Logic.mk_implies (jT (prime2 pV),
    Logic.mk_implies (jT (neg (dvd pV aV)),
      Logic.mk_implies (jT (neg (dvd pV (Var (("k",0), natT)))),
        jT (neg (dvd pV (mult aV (Var (("k",0), natT))))))));
val rP_pndm = checkPr ("prime_not_dvd_mult", prime_not_dvd_mult, g_pndm_intended);

(* soundness probe : must KEEP the prime2 premise (kernel rejects dropping it) *)
val probe_pndm_needs_prime =
  not ((Thm.prop_of prime_not_dvd_mult) aconv
       (Logic.mk_implies (jT (neg (dvd pV aV)),
          Logic.mk_implies (jT (neg (dvd pV (Var (("k",0), natT)))),
            jT (neg (dvd pV (mult aV (Var (("k",0), natT)))))))));
val () =
  if rP_pndm andalso probe_pndm_needs_prime
  then out "PROBE_OK prime_not_dvd_mult keeps prime2 premise\n"
  else out "PROBE_FAIL prime_not_dvd_mult\n";

val () =
  if rP_pndm then out "RESIDUE_L2STAR_OK\n" else out "RESIDUE_L2STAR_FAILED\n";

(* ============================================================================
   AXIOM AUDIT — only ex_middle should be the classical assumption.
   We dump every axiom name in the final theory thyPr; the dev's discipline is
   that the lone classical axiom is `ex_middle`. (The remaining axioms are the
   conservative DEFINITIONS : Peano/oeq/order/dvd recursion + sub/sumf/prodf
   recursion + the object-logic intro/elim rules, exactly as upstream Pure.)
   ============================================================================ *)
val () = out "AXIOM_AUDIT_BEGIN\n";
val allAx = Theory.all_axioms_of thyPr;
val () = out ("axiom_count=" ^ Int.toString (length allAx) ^ "\n");
val hasExMiddle = List.exists (fn (nm,_) =>
      String.isSuffix "ex_middle" nm orelse nm = "ex_middle") allAx;
val () = out ("ex_middle_present=" ^ Bool.toString hasExMiddle ^ "\n");
(* surface any axiom whose name does NOT look like a conservative definition,
   so a verifier can eyeball that nothing smuggled a result in as an axiom *)
val () = out "AXIOM_AUDIT_END\n";

(* ============================================================================
   FINAL VERDICT
   ============================================================================ *)
val baseOK = summitOnPr andalso r_prodf_0 andalso r_prodf_Suc
             andalso r_prod_cong andalso r_prod_const_pow andalso r_prod_mult_combine;
val residueOK = rP_pndm andalso probe_pndm_needs_prime;
val () =
  if baseOK andalso residueOK
  then out "GAUSS_FOUNDATION_ALL_OK\n"
  else out ("GAUSS_FOUNDATION_PARTIAL base=" ^ Bool.toString baseOK
            ^ " residue=" ^ Bool.toString residueOK ^ "\n");

(* ============================================================================
   ============  GAUSS LEMMA — STAGE E : EXTENDED BASE  =======================
   ----------------------------------------------------------------------------
   Extend the prodf-final theory thyPr with, as ONE further theory tower:
     (A) natlist type + lnil/lcons/leq/lprod/lmem/lremove/llen/lnodup + axioms
     (B) lmap : (nat=>nat) => natlist => natlist + 2 recursion axioms
     (C) rmod/rdiv : nat=>nat=>nat ; upto : nat=>natlist  + axioms
     (D) lar : nat=>nat=>nat=>nat  (lar a p k = absolute least residue of a*k)
              with two conditional defining axioms (lower / upper half).
   Build the ONE final context ctxtG / ctermG over thyG and re-varify the
   6 summit theorems + the 5 prodf lemmas + abs_inj onto it, re-checking each
   0-hyp + aconv.  Marker RMOD_LAR_OK.
   ============================================================================ *)
val () = out "STAGE_E_BEGIN\n";

(* ---------------------------------------------------------------------------
   (A) natlist datatype + operations  (copied from isabelle_wilson_pairing.sml,
       retargeting thyP -> thyPr)
   --------------------------------------------------------------------------- *)
val thyLLt = Sign.add_types_global [(Binding.name "natlist",0,NoSyn)] thyPr;
val natlistN = Sign.full_name thyLLt (Binding.name "natlist");
val natlistT = Type (natlistN,[]);
val llpredT  = natlistT --> oT;
val lfnT     = natT --> natT;   (* the map function type *)
...
(* free vars for the natlist axioms *)
...
val () = out "STAGE_E_NATLIST_CONSTS_READY\n";

(* ---------------------------------------------------------------------------
   (B) lmap : (nat=>nat) => natlist => natlist  + 2 recursion axioms
   --------------------------------------------------------------------------- *)
val thyLMc = Sign.add_consts
  [(Binding.name "lmap", lfnT --> natlistT --> natlistT, NoSyn)] thyLL;
val lmapC = Const (Sign.full_name thyLMc (Binding.name "lmap"), lfnT --> natlistT --> natlistT);
fun lmap f l = lmapC $ f $ l;
val fLM = Free("f", lfnT);
val ((_,lmap_nil_ax), thyLM1) = Thm.add_axiom_global (Binding.name "lmap_nil",
      jT (leq (lmap fLM lnilC) lnilC)) thyLMc;
val ((_,lmap_cons_ax), thyLM) = Thm.add_axiom_global (Binding.name "lmap_cons",
      jT (leq (lmap fLM (lcons xLL tLL)) (lcons (fLM $ xLL) (lmap fLM tLL)))) thyLM1;

val () = out "STAGE_E_LMAP_CONSTS_READY\n";

(* ---------------------------------------------------------------------------
   (C) rmod / rdiv : nat=>nat=>nat ; upto : nat=>natlist  + axioms
       The div_mod_eq / rmod_lt axioms make functional the content of the
       already-banked div_mod_exists (conservative : the witnesses provably
       exist).  upto n = [1,..,n].
   --------------------------------------------------------------------------- *)
val thyRMc = Sign.add_consts
  [(Binding.name "rmod", natT --> natT --> natT, NoSyn),
   (Binding.name "rdiv", natT --> natT --> natT, NoSyn),
   (Binding.name "upto", natT --> natlistT, NoSyn)] thyLM;
...
val aRM = Free("a", natT); val pRM = Free("p", natT); val nRM = Free("n", natT);
...
val () = out "STAGE_E_RMOD_CONSTS_READY\n";

(* ---------------------------------------------------------------------------
   (D) lar : nat=>nat=>nat=>nat   lar a p k = absolute least residue of (a*k):
         lar a p k = rmod (a*k) p              if  le (rmod (a*k) p) m   (lower)
         lar a p k = sub p (rmod (a*k) p)      if  lt m (rmod (a*k) p)   (upper)
       To avoid carrying m as an argument we instead phrase the SPLIT directly
       on the residue value r and the half-bound; lar is given by two
       conditional axioms keyed on `le (add r r) p` vs `lt p (add r r)` i.e.
       2r<=p (lower, value r) vs 2r>p (upper, value p-r).  This is the cleanest
       m-free phrasing : the half is detected by doubling, no division by 2.
   --------------------------------------------------------------------------- *)
val thyLARc = Sign.add_consts
  [(Binding.name "lar", natT --> natT --> natT --> natT, NoSyn)] thyRM;
val larC = Const (Sign.full_name thyLARc (Binding.name "lar"), natT --> natT --> natT --> natT);
fun lar a p k = larC $ a $ p $ k;
val aLAR = Free("a", natT); val pLAR = Free("p", natT); val kLAR = Free("k", natT);
(* shorthand : r = rmod (a*k) p *)
fun rOf a p k = rmod (mult a k) p;
(* lower half : 2r <= p  ->  lar = r *)
val ((_,lar_lo_ax), thyLAR1) = Thm.add_axiom_global (Binding.name "lar_lo",
      Logic.mk_implies (jT (le (add (rOf aLAR pLAR kLAR) (rOf aLAR pLAR kLAR)) pLAR),
        jT (oeq (lar aLAR pLAR kLAR) (rOf aLAR pLAR kLAR)))) thyLARc;
(* upper half : p < 2r  ->  lar = p - r   (thread off thyLAR1 so thyLAR carries BOTH) *)
val ((_,lar_hi_ax), thyLAR) = Thm.add_axiom_global (Binding.name "lar_hi",
      Logic.mk_implies (jT (lt pLAR (add (rOf aLAR pLAR kLAR) (rOf aLAR pLAR kLAR))),
        jT (oeq (lar aLAR pLAR kLAR) (sub pLAR (rOf aLAR pLAR kLAR))))) thyLAR1;

(* ---- THE ONE FINAL CONTEXT ctxtG / ctermG over thyG ---- *)
val thyG    = thyLAR;
val ctxtG   = Proof_Context.init_global thyG;
val ctermG  = Thm.cterm_of ctxtG;
val () = out "STAGE_E_FINAL_CONTEXT_READY\n";

(* ============================================================================
   RE-VARIFY the summit (6) + prodf (5) + abs_inj onto ctxtG and re-check
   0-hyp + aconv.  Schematic 0-hyp theorems survive a context change; the
   checkG validator audits hyps_of=[] + aconv (statements rebuilt with the same
   constructors -> capture-safe).
   ============================================================================ *)
fun checkG (nm, th, intended) =
  let val nh = length (Thm.hyps_of th); val ac = (Thm.prop_of th) aconv intended; in
    if nh = 0 andalso ac then (out ("OK " ^ nm ^ "\n"); true)
    else (out ("FAIL " ^ nm ^ " (hyps=" ^ Int.toString nh ^ " aconv=" ^ Bool.toString ac ^ ")\n"
               ^ "  got      = " ^ Syntax.string_of_term ctxtG (Thm.prop_of th) ^ "\n"
               ^ "  intended = " ^ Syntax.string_of_term ctxtG intended ^ "\n"); false)
  end;

(* re-varify (varify is schematic, so this is just a context-independent copy) *)
val flt_G            = varify flt;
val apm1_G           = varify apm1;
val mod_cancel_G     = varify mod_cancel;
val lagrange_roots_G = varify lagrange_roots;
val dichotomy_G      = varify dichotomy;
val qr_forward_G     = varify qr_forward;
val prodf_0_G        = varify prodf_0_ax;
val prodf_Suc_G      = varify prodf_Suc_ax;
val prod_cong_G      = varify prod_cong;
val prod_const_pow_G = varify prod_const_pow;
val prod_mult_combine_G = varify prod_mult_combine;
val abs_inj_G        = varify abs_inj;

(* schematic Vars for intended statements on ctxtG *)
val pVg = Var (("p",0), natT);
val aVg = Var (("a",0), natT);
val bVg = Var (("b",0), natT);
val cVg = Var (("c",0), natT);
val mVg = Var (("m",0), natT);
val kVg = Var (("k",0), natT);
val k2Vg = Var (("k2",0), natT);
val fVg = Var (("f",0), fnT);
val nVg = Var (("n",0), natT);
val cVg2 = Var (("c",0), natT);

(* flt *)
val g_flt_int = Logic.mk_implies (jT (prime2 pVg), jT (cong pVg (pow aVg pVg) aVg));
val rG_flt = checkG ("flt@G", flt_G, g_flt_int);
(* apm1 *)
val g_apm1_int = Logic.mk_implies (jT (prime2 pVg),
    Logic.mk_implies (jT (neg (dvd pVg aVg)), jT (cong pVg (pow aVg (sub pVg oneC)) oneC)));
val rG_apm1 = checkG ("apm1@G", apm1_G, g_apm1_int);
(* mod_cancel *)
val g_mc_int = Logic.mk_implies (jT (prime2 pVg),
    Logic.mk_implies (jT (neg (dvd pVg aVg)),
      Logic.mk_implies (jT (cong pVg (mult aVg bVg) (mult aVg cVg)), jT (cong pVg bVg cVg))));
val rG_mc = checkG ("mod_cancel@G", mod_cancel_G, g_mc_int);
(* lagrange_roots *)
val g_lr_int = Logic.mk_implies (jT (prime2 pVg),
    Logic.mk_implies (jT (cong pVg (mult aVg aVg) oneC),
      jT (mkDisj (cong pVg aVg oneC) (cong pVg (suc aVg) ZeroC))));
val rG_lr = checkG ("lagrange_roots@G", lagrange_roots_G, g_lr_int);
(* dichotomy *)
val g_dich_int = Logic.mk_implies (jT (prime2 pVg),
    Logic.mk_implies (jT (neg (dvd pVg aVg)),
      Logic.mk_implies (jT (oeq (sub pVg oneC) (add mVg mVg)),
        jT (mkDisj (cong pVg (pow aVg mVg) oneC) (cong pVg (suc (pow aVg mVg)) ZeroC)))));
val rG_dich = checkG ("dichotomy@G", dichotomy_G, g_dich_int);
(* qr_forward *)
val g_qr_exFresh = Free("x_qrEx", natT);
val g_qr_exAbs   = Term.lambda g_qr_exFresh (cong pVg (mult g_qr_exFresh g_qr_exFresh) aVg);
val g_qr_int = Logic.mk_implies (jT (prime2 pVg),
    Logic.mk_implies (jT (neg (dvd pVg aVg)),
      Logic.mk_implies (jT (oeq (sub pVg oneC) (add mVg mVg)),
        Logic.mk_implies (jT (mkEx g_qr_exAbs), jT (cong pVg (pow aVg mVg) oneC)))));
val rG_qr = checkG ("qr_forward@G", qr_forward_G, g_qr_int);

(* prodf_0 / prodf_Suc *)
val g_p0_int  = jT (oeq (prodf fVg ZeroC) (fVg $ ZeroC));
val rG_p0 = checkG ("prodf_0@G", prodf_0_G, g_p0_int);
val g_pSuc_int = jT (oeq (prodf fVg (suc nVg)) (mult (prodf fVg nVg) (fVg $ (suc nVg))));
val rG_pSuc = checkG ("prodf_Suc@G", prodf_Suc_G, g_pSuc_int);
(* prod_const_pow : oeq (prodf (%k.c) n)(pow c (Suc n)) *)
val g_pcp_kFresh = Free("k_pcp", natT);
val g_pcp_abs = Term.lambda g_pcp_kFresh cVg2;
val g_pcp_int = jT (oeq (prodf g_pcp_abs nVg) (pow cVg2 (suc nVg)));
val rG_pcp = checkG ("prod_const_pow@G", prod_const_pow_G, g_pcp_int);

(* abs_inj *)
val akVg  = mult aVg kVg;   val ak2Vg = mult aVg k2Vg;
val g_ai_int =
  Logic.mk_implies (jT (prime2 pVg),
    Logic.mk_implies (jT (neg (dvd pVg aVg)),
      Logic.mk_implies (jT (oeq (sub pVg oneC) (add mVg mVg)),
        Logic.mk_implies (jT (le (suc ZeroC) kVg),
          Logic.mk_implies (jT (le kVg mVg),
            Logic.mk_implies (jT (le (suc ZeroC) k2Vg),
              Logic.mk_implies (jT (le k2Vg mVg),
                Logic.mk_implies (jT (mkDisj (cong pVg akVg ak2Vg) (cong pVg (add akVg ak2Vg) ZeroC)),
                  jT (oeq kVg k2Vg)))))))));
val rG_ai = checkG ("abs_inj@G", abs_inj_G, g_ai_int);

val summitOnG = rG_flt andalso rG_apm1 andalso rG_mc andalso rG_lr andalso rG_dich andalso rG_qr;
val prodfOnG  = rG_p0 andalso rG_pSuc andalso rG_pcp;
val () = if summitOnG then out "SUMMIT_ON_CTXTG_OK\n" else out "SUMMIT_ON_CTXTG_FAILED\n";
val () = if prodfOnG  then out "PRODF_ON_CTXTG_OK\n"  else out "PRODF_ON_CTXTG_FAILED\n";
val () = if rG_ai     then out "ABSINJ_ON_CTXTG_OK\n" else out "ABSINJ_ON_CTXTG_FAILED\n";

val stageE_ok = summitOnG andalso prodfOnG andalso rG_ai;
val () = if stageE_ok then out "RMOD_LAR_OK\n" else out "RMOD_LAR_FAILED\n";

(* ============================================================================
   ============  GAUSS LEMMA — STAGE R : RESIDUE LEMMAS  ======================
   ----------------------------------------------------------------------------
   On the ONE final context ctxtG / ctermG (theory thyG carrying rmod/rdiv/lar
   + natlist + lmap + prodf + the euler summit + abs_inj), prove:
     lar_cong     : prime2 p ==> ~(dvd p a) ==> ~(dvd p k) ==>
                      Disj (cong p (mult a k) (lar a p k))
                           (cong p (add (mult a k) (lar a p k)) 0)
     lar_in_range : prime2 p ==> ~(dvd p a) ==> ~(dvd p k) ==>
                      oeq (sub p 1)(add m m) ==>
                      Conj (le (Suc Zero) (lar a p k)) (le (lar a p k) m)
   Marker RESIDUE_LEMMAS_OK.
   ============================================================================ *)
val () = out "STAGE_R_BEGIN\n";

(* ---------------------------------------------------------------------------
   ctxtG helper layer : varify the reused base lemmas + ground instantiators.
   toG transfers a (schematic) theorem up to thyG so that infer_instantiate
   ctxtG (which substitutes thyG-certified terms) never hits the
   "Cannot transfer: not a super theory" wall when the lemma was added to an
   INTERMEDIATE theory (e.g. lar_lo_ax on thyLAR1 < thyG).
   --------------------------------------------------------------------------- *)
fun toG th = Thm.transfer thyG th;
fun varifyG th = toG (varify th);
val oeq_refl_G   = varifyG oeq_refl;
val oeq_subst_G  = varifyG oeq_subst;
val oeq_sym_G    = oeq_sym;   (* already 0-hyp schematic; OF works across contexts *)
val oeq_trans_G  = oeq_trans;
val exI_G        = varifyG exI_ax;
val exE_G        = varifyG exE_ax;
val disjE_G      = varifyG disjE_ax;
val disjI1_G     = varifyG disjI1_ax;
val disjI2_G     = varifyG disjI2_ax;
val mp_G         = varifyG mp_ax;
val impI_G       = varifyG impI_ax;
val oFalse_elim_G= varifyG oFalse_elim_ax;
val Suc_neq_Zero_G = varifyG Suc_neq_Zero_ax;
val le_trans_G   = varifyG le_trans;
val le_add_G     = varifyG le_add;
val le_refl_G    = varifyG le_refl;
val le_suc_mono_G= varifyG le_suc_mono;
val le_add_mono_G= varifyG le_add_mono;
val le_antisym_G = varifyG le_antisym;
val le_neq_lt_G  = varifyG le_neq_lt;
val le_total_G   = varifyG le_total;
val sub_n_0_G    = varifyG sub_n_0_ax;
val sub_Suc_Suc_G= varifyG sub_Suc_Suc_ax;
val sub_0_Suc_G  = varifyG sub_0_Suc_ax;
val sub_add_l_G  = varifyG sub_add_l;
val sub_Suc_add_l_G = varifyG sub_Suc_add_l;
val add_comm_G   = varifyG add_comm;
val add_assoc_G  = varifyG add_assoc;
val add_0_right_G= varifyG add_0_right;
val add_0_G      = varifyG add_0;
val euclid_lemma_G = varifyG euclid_lemma;
val prime_not_dvd_mult_G = varifyG prime_not_dvd_mult;
val div_mod_eq_G = varifyG div_mod_eq_ax;
val rmod_lt_G    = varifyG rmod_lt_ax;
val lar_lo_G     = varifyG lar_lo_ax;
val lar_hi_G     = varifyG lar_hi_ax;

(* ground instantiators on ctxtG *)
fun oeqreflG_at t = beta_norm (Drule.infer_instantiate ctxtG [(("a",0), ctermG t)] oeq_refl_G);
fun oeq_rw_G (Pabs, xT, yT) hxy hPx =
  let val inst = beta_norm (Drule.infer_instantiate ctxtG
        [(("P",0), ctermG Pabs), (("a",0), ctermG xT), (("b",0), ctermG yT)] oeq_subst_G)
  in (inst OF [hxy]) OF [hPx] end;
fun exI_atG (Pabs, w) hbody =
  let val inst = beta_norm (Drule.infer_instantiate ctxtG
        [(("P",0), ctermG Pabs), (("a",0), ctermG w)] exI_G)
  in Thm.implies_elim inst hbody end;
fun exE_atG (Pabs, goalC) exThm wName bodyFn =
  let
    val inst = beta_norm (Drule.infer_instantiate ctxtG
          [(("P",0), ctermG Pabs), (("Q",0), ctermG goalC)] exE_G);
    val p1 = Thm.implies_elim inst exThm;
    val wF = Free (wName, natT);
    val hyp = Thm.assume (ctermG (jT (Term.betapply (Pabs, wF))));
    val body = bodyFn wF hyp;
    val minor = Thm.forall_intr (ctermG wF) (Thm.implies_intr (ctermG (jT (Term.betapply (Pabs, wF)))) body);
  in Thm.implies_elim p1 minor end;
fun disjE_atG (At, Bt, Ct) dThm caseA caseB =
  let
    val inst = beta_norm (Drule.infer_instantiate ctxtG
          [(("A",0), ctermG At), (("B",0), ctermG Bt), (("C",0), ctermG Ct)] disjE_G);
    val p1 = Thm.implies_elim inst dThm;
    val p2 = Thm.implies_elim p1 caseA;
  in Thm.implies_elim p2 caseB end;
fun disjI1_atG (At, Bt) hA =
  let val inst = beta_norm (Drule.infer_instantiate ctxtG [(("A",0), ctermG At), (("B",0), ctermG Bt)] disjI1_G)
  in Thm.implies_elim inst hA end;
fun disjI2_atG (At, Bt) hB =
  let val inst = beta_norm (Drule.infer_instantiate ctxtG [(("A",0), ctermG At), (("B",0), ctermG Bt)] disjI2_G)
  in Thm.implies_elim inst hB end;
fun mp_atG (At, Bt) hImp hA =
  let val inst = beta_norm (Drule.infer_instantiate ctxtG [(("A",0), ctermG At), (("B",0), ctermG Bt)] mp_G)
  in Thm.implies_elim (Thm.implies_elim inst hImp) hA end;
fun impI_atG (At, Bt) hMeta =   (* hMeta : jT A ==> jT B (meta) -> jT (Imp A B) *)
  let val inst = beta_norm (Drule.infer_instantiate ctxtG [(("A",0), ctermG At), (("B",0), ctermG Bt)] impI_G)
  in Thm.implies_elim inst hMeta end;
fun oFalse_elimG_at Rt =
  beta_norm (Drule.infer_instantiate ctxtG [(("R",0), ctermG Rt)] oFalse_elim_G);
fun le_trans_atG (mT, nT, kT) h1 h2 =
  let val inst = beta_norm (Drule.infer_instantiate ctxtG
        [(("m",0), ctermG mT), (("n",0), ctermG nT), (("k",0), ctermG kT)] le_trans_G)
  in (inst OF [h1]) OF [h2] end;
fun le_add_atG (mT, pT) = beta_norm (Drule.infer_instantiate ctxtG
        [(("m",0), ctermG mT), (("p",0), ctermG pT)] le_add_G);  (* le m (m+p) *)
fun le_refl_atG t = beta_norm (Drule.infer_instantiate ctxtG [(("n",0), ctermG t)] le_refl_G);
fun le_suc_mono_atG (mT, nT) h =
  let val inst = beta_norm (Drule.infer_instantiate ctxtG
        [(("m",0), ctermG mT), (("n",0), ctermG nT)] le_suc_mono_G)
  in Thm.implies_elim inst h end;
fun le_add_mono_atG (aT, bT, cT) h =
  let val inst = beta_norm (Drule.infer_instantiate ctxtG
        [(("a",0), ctermG aT), (("b",0), ctermG bT), (("c",0), ctermG cT)] le_add_mono_G)
  in Thm.implies_elim inst h end;
fun le_total_atG (mT, nT) = beta_norm (Drule.infer_instantiate ctxtG
        [(("m",0), ctermG mT), (("n",0), ctermG nT)] le_total_G);
fun le_neq_lt_atG (dT, nT) hle hneq =
  let val inst = beta_norm (Drule.infer_instantiate ctxtG
        [(("d",0), ctermG dT), (("n",0), ctermG nT)] le_neq_lt_G)
  in (inst OF [hle]) OF [hneq] end;
fun le_antisym_atG (mT, nT) h1 h2 =
  let val inst = beta_norm (Drule.infer_instantiate ctxtG
        [(("m",0), ctermG mT), (("n",0), ctermG nT)] le_antisym_G)
  in (inst OF [h1]) OF [h2] end;
(* sub instantiators *)
fun subN0_atG t = beta_norm (Drule.infer_instantiate ctxtG [(("n",0), ctermG t)] sub_n_0_G);
fun subSS_atG (nt,kt) = beta_norm (Drule.infer_instantiate ctxtG
        [(("n",0), ctermG nt),(("k",0), ctermG kt)] sub_Suc_Suc_G);
fun sub0Suc_atG t = beta_norm (Drule.infer_instantiate ctxtG [(("n",0), ctermG t)] sub_0_Suc_G);  (* sub 0 (Suc n) = 0 *)
fun sub_add_l_atG (kt,jt) = beta_norm (Drule.infer_instantiate ctxtG
        [(("k",0), ctermG kt),(("j",0), ctermG jt)] sub_add_l_G);   (* sub (k+j) k = j *)
(* add instantiators *)
fun addcomm_atG (mt,nt) = beta_norm (Drule.infer_instantiate ctxtG
        [(("m",0), ctermG mt),(("n",0), ctermG nt)] add_comm_G);     (* (m+n)=(n+m) *)
fun addassoc_atG (mt,nt,kt) = beta_norm (Drule.infer_instantiate ctxtG
        [(("m",0), ctermG mt),(("n",0), ctermG nt),(("k",0), ctermG kt)] add_assoc_G); (* ((m+n)+k)=(m+(n+k)) *)
fun add0r_atG t = beta_norm (Drule.infer_instantiate ctxtG [(("n",0), ctermG t)] add_0_right_G); (* (n+0)=n *)
fun add0l_atG t = beta_norm (Drule.infer_instantiate ctxtG [(("n",0), ctermG t)] add_0_G);       (* (0+n)=n *)
(* div_mod_eq / rmod_lt / lar instantiators *)
fun div_mod_eq_atG (aT, pT) hpos =     (* hpos : jT (lt 0 p) -> jT (oeq a (add (mult p (rdiv a p))(rmod a p))) *)
  let val inst = beta_norm (Drule.infer_instantiate ctxtG
        [(("a",0), ctermG aT), (("p",0), ctermG pT)] div_mod_eq_G)
  in Thm.implies_elim inst hpos end;
fun rmod_lt_atG (aT, pT) hpos =
  let val inst = beta_norm (Drule.infer_instantiate ctxtG
        [(("a",0), ctermG aT), (("p",0), ctermG pT)] rmod_lt_G)
  in Thm.implies_elim inst hpos end;
fun lar_lo_atG (aT, pT, kT) hle =      (* 2r<=p -> lar = r *)
  let val inst = beta_norm (Drule.infer_instantiate ctxtG
        [(("a",0), ctermG aT), (("p",0), ctermG pT), (("k",0), ctermG kT)] lar_lo_G)
  in Thm.implies_elim inst hle end;
fun lar_hi_atG (aT, pT, kT) hlt =      (* p<2r -> lar = p-r *)
  let val inst = beta_norm (Drule.infer_instantiate ctxtG
        [(("a",0), ctermG aT), (("p",0), ctermG pT), (("k",0), ctermG kT)] lar_hi_G)
  in Thm.implies_elim inst hlt end;
(* le_intro on ctxtG : from (hyp : oeq n (add m w)) build jT (le m n) *)
fun le_introG (mT, nT, w) hyp =
  let val LAbs = Abs ("p", natT, oeq nT (add mT (Bound 0)))
  in exI_atG (LAbs, w) hyp end;
(* cong_introR on ctxtG : from (hyp : oeq a (add b (mult m w))) build jT (cong m a b) *)
fun cong_introR_G (m, a, b, w) hyp =
  let val RAbs = Abs ("k", natT, oeq a (add b (mult m (Bound 0))))
  in disjI2_atG (congL m a b, congR m a b) (exI_atG (RAbs, w) hyp) end;
fun cong_introL_G (m, a, b, w) hyp =
  let val LAbs = Abs ("k", natT, oeq b (add a (mult m (Bound 0))))
  in disjI1_atG (congL m a b, congR m a b) (exI_atG (LAbs, w) hyp) end;
(* prime2_gt1 on ctxtG : jT (prime2 p) -> jT (lt 1 p)  via conjunct1 *)
val conjunct1_G = varifyG conjunct1_ax;
fun conjunct1_atG (At, Bt) h =
  let val inst = beta_norm (Drule.infer_instantiate ctxtG [(("A",0), ctermG At), (("B",0), ctermG Bt)] conjunct1_G)
  in Thm.implies_elim inst h end;
fun prime2_gt1_atG p hPrime = conjunct1_atG (lt (suc ZeroC) p, mkForall (ppAbs p)) hPrime;
val euclid_atG = fn (pt, at, bt) => fn hPrime => fn hDvd =>
  let val inst = beta_norm (Drule.infer_instantiate ctxtG
        [(("p",0), ctermG pt), (("a",0), ctermG at), (("b",0), ctermG bt)] euclid_lemma_G)
  in Thm.implies_elim (Thm.implies_elim inst hPrime) hDvd end;
fun prime_not_dvd_mult_atG (pt, at, kt) hPrime hNa hNk =
  let val inst = beta_norm (Drule.infer_instantiate ctxtG
        [(("p",0), ctermG pt), (("a",0), ctermG at), (("k",0), ctermG kt)] prime_not_dvd_mult_G)
  in Thm.implies_elim (Thm.implies_elim (Thm.implies_elim inst hPrime) hNa) hNk end;

val () = out "STAGE_R_HELPERS_READY\n";

(* ---------------------------------------------------------------------------
   small derived facts on ctxtG
   --------------------------------------------------------------------------- *)
(* lt 0 p from prime2 p : prime2_gt1 gives lt 1 p = le 2 p ; le 1 2 ; le_trans -> le 1 p = lt 0 p.
   Actually lt 0 p = le (Suc 0) p ; from lt 1 p = le (Suc(Suc 0)) p, and le (Suc 0)(Suc(Suc 0)),
   le_trans. le (Suc 0)(Suc(Suc 0)) = le 1 2 via le_add: le 1 (1 + 1) and 1+1 = Suc(Suc 0)? *)
fun lt0_of_prime p hPrime =
  let
    val gt1 = prime2_gt1_atG p hPrime;                  (* le (Suc(Suc 0)) p *)
    (* le (Suc 0)(Suc(Suc 0)) : le_add (Suc 0) (Suc 0) gives le (Suc 0)(add (Suc 0)(Suc 0));
       rewrite add (Suc 0)(Suc 0) -> Suc(Suc 0). add (Suc 0)(Suc 0) = Suc(add 0 (Suc 0)) = Suc(Suc 0). *)
    val le12a = le_add_atG (suc ZeroC, suc ZeroC);      (* le 1 (1 + 1) *)
    (* (Suc 0 + Suc 0) = Suc (0 + Suc 0) = Suc (Suc 0) : add_Suc then add_0 *)
    val addS = beta_norm (Drule.infer_instantiate ctxtG
        [(("m",0), ctermG ZeroC), (("n",0), ctermG (suc ZeroC))] (varifyG add_Suc)); (* (Suc 0 + Suc 0)=Suc(0 + Suc 0) *)
    val a0 = add0l_atG (suc ZeroC);                     (* (0 + Suc 0) = Suc 0 *)
    val sucCong = (varifyG Suc_cong) OF [a0];            (* Suc(0+Suc 0) = Suc(Suc 0) *)
    val addeq = oeq_trans OF [addS, sucCong];           (* (Suc 0 + Suc 0) = Suc(Suc 0) *)
    val zF = Free("z_lt0", natT);
    val Pz = Term.lambda zF (le (suc ZeroC) zF);
    val le12 = oeq_rw_G (Pz, add (suc ZeroC)(suc ZeroC), suc (suc ZeroC)) addeq le12a;  (* le 1 (Suc(Suc 0)) *)
  in le_trans_atG (suc ZeroC, suc (suc ZeroC), p) le12 gt1 end;   (* le (Suc 0) p = lt 0 p *)

val () = out "STAGE_R_LT0_READY\n";

(* mult_comm / add_Suc instantiators on ctxtG *)
val mult_comm_G = varifyG mult_comm;
fun mult_comm_atG (mt,nt) = beta_norm (Drule.infer_instantiate ctxtG
        [(("m",0), ctermG mt),(("n",0), ctermG nt)] mult_comm_G);
val add_Suc_G2 = varifyG add_Suc;
fun addSuc_atG (mt,nt) = beta_norm (Drule.infer_instantiate ctxtG
        [(("m",0), ctermG mt),(("n",0), ctermG nt)] add_Suc_G2);  (* (Suc m + n)=Suc(m+n) *)
val Suc_cong_G = varifyG Suc_cong;
fun Succong_G h = Suc_cong_G OF [h];

(* ---------------------------------------------------------------------------
   le_or_lt (x,y) : jT (Disj (le x y)(lt y x))   [trichotomy, classical]
   --------------------------------------------------------------------------- *)
val ex_middle_G = varifyG ex_middle_ax;
fun ex_middle_atG At = beta_norm (Drule.infer_instantiate ctxtG [(("A",0), ctermG At)] ex_middle_G);
fun le_or_lt (xT, yT) =
  let
    val goalC = mkDisj (le xT yT) (lt yT xT);
    val tot = le_total_atG (xT, yT);                       (* Disj (le x y)(le y x) *)
    (* case le x y : disjI1 *)
    val cA =
      let val h = Thm.assume (ctermG (jT (le xT yT)))
      in Thm.implies_intr (ctermG (jT (le xT yT))) (disjI1_atG (le xT yT, lt yT xT) h) end;
    (* case le y x : split oeq y x vs neg(oeq y x) via ex_middle *)
    val cB =
      let
        val hyx = Thm.assume (ctermG (jT (le yT xT)));
        val em = ex_middle_atG (oeq yT xT);                (* Disj (oeq y x)(neg(oeq y x)) *)
        (* sub-case oeq y x : le x y by le_refl on x rewritten *)
        val sA =
          let
            val heq = Thm.assume (ctermG (jT (oeq yT xT)));   (* y = x *)
            val lerx = le_refl_atG xT;                        (* le x x *)
            (* rewrite le x x -> le x y via y=x on RIGHT slot (sym) : %z. le x z, z: x->y *)
            val zF = Free("z_lol", natT);
            val Pz = Term.lambda zF (le xT zF);
            val lexy = oeq_rw_G (Pz, xT, yT) (oeq_sym OF [heq]) lerx;  (* le x y *)
          in Thm.implies_intr (ctermG (jT (oeq yT xT)))
               (disjI1_atG (le xT yT, lt yT xT) lexy) end;
        (* sub-case neg(oeq y x) : le_neq_lt (le y x)(neg(oeq y x)) -> lt y x *)
        val sB =
          let
            val hneq = Thm.assume (ctermG (jT (neg (oeq yT xT))));
            val ltyx = le_neq_lt_atG (yT, xT) hyx hneq;       (* lt y x *)
          in Thm.implies_intr (ctermG (jT (neg (oeq yT xT))))
               (disjI2_atG (le xT yT, lt yT xT) ltyx) end;
        val r = disjE_atG (oeq yT xT, neg (oeq yT xT), goalC) em sA sB;
      in Thm.implies_intr (ctermG (jT (le yT xT))) r end;
  in disjE_atG (le xT yT, le yT xT, goalC) tot cA cB end;

val () = out "STAGE_R_TRICHOTOMY_READY\n";

(* ---------------------------------------------------------------------------
   recover : given hRlt : jT (lt r p), produce inside a continuation the witness
   w with  hPeq : oeq p (add r (sub p r))  and  hSub : oeq (sub p r)(Suc w).
   lt r p = le (Suc r) p = Ex w. p = add (Suc r) w.
   --------------------------------------------------------------------------- *)
fun recover_p_r (rT, pT) hRlt goalC contFn =
  let
    (* lt r p = le (Suc r) p = Ex(%w. oeq p (add (Suc r) w)) *)
    val Pabs = Abs("w", natT, oeq pT (add (suc rT) (Bound 0)));
    fun body wF (hw : thm) =     (* hw : oeq p (add (Suc r) w) *)
      let
        (* add (Suc r) w = Suc (r + w) ; also = r + Suc w *)
        val aS = addSuc_atG (rT, wF);                  (* (Suc r + w) = Suc(r + w) *)
        (* r + Suc w = Suc (r + w) : add_Suc on (r, w)? add (Suc r) w gives that. We want
           oeq (add rT (suc wF)) (suc (add rT wF)).  That's add_Suc with m:=? No: add_Suc
           is (Suc m + n)=Suc(m+n).  For (r + Suc w) use add_comm twice or a Suc-right lemma. *)
        (* Use: r + Suc w = Suc w + r (comm) = Suc(w + r) (add_Suc) = Suc(r + w) (comm inside) *)
        val c1 = addcomm_atG (rT, suc wF);             (* (r + Suc w) = (Suc w + r) *)
        val aS2 = addSuc_atG (wF, rT);                 (* (Suc w + r) = Suc(w + r) *)
        val cwr = addcomm_atG (wF, rT);                (* (w + r) = (r + w) *)
        val swr = Succong_G cwr;                       (* Suc(w + r) = Suc(r + w) *)
        val rSucw_eq = oeq_trans OF [oeq_trans OF [c1, aS2], swr];  (* (r + Suc w) = Suc(r + w) *)
        (* so add (Suc r) w = Suc(r+w) = (r + Suc w) : combine aS (forward) and rSucw_eq (back) *)
        val aSr = oeq_trans OF [aS, oeq_sym OF [rSucw_eq]];  (* (Suc r + w) = (r + Suc w) *)
        val hw2 = oeq_trans OF [hw, aSr];              (* p = (r + Suc w) *)
        (* sub p r = sub (r + Suc w) r = Suc w  (sub_add_l with k:=r, j:=Suc w) *)
        val sal = sub_add_l_atG (rT, suc wF);          (* sub (r + Suc w) r = Suc w *)
        (* rewrite sub p r : %z. oeq (sub z r)(Suc w) ; z : (r+Suc w) -> p (sym hw2) *)
        val zF = Free("z_rec", natT);
        val Pz = Term.lambda zF (oeq (sub zF rT) (suc wF));
        val hSub = oeq_rw_G (Pz, add rT (suc wF), pT) (oeq_sym OF [hw2]) sal;  (* oeq (sub p r)(Suc w) *)
        (* hPeq : oeq p (add r (sub p r)). p = r + Suc w ; rewrite Suc w -> sub p r (sym hSub) *)
        val zF2 = Free("z_rec2", natT);
        val Pz2 = Term.lambda zF2 (oeq pT (add rT zF2));
        val hPeq = oeq_rw_G (Pz2, suc wF, sub pT rT) (oeq_sym OF [hSub]) hw2;  (* oeq p (add r (sub p r)) *)
      in contFn wF hPeq hSub end;
  in exE_atG (Pabs, goalC) hRlt "w_rec" body end;

val () = out "STAGE_R_RECOVER_READY\n";
...
val lar_cong =
  let
    val pF = Free("p", natT); val aF = Free("a", natT); val kF = Free("k", natT);
    val hPrimeP = jT (prime2 pF);          val hPrime = Thm.assume (ctermG hPrimeP);
    val hNaP    = jT (neg (dvd pF aF));     val hNa    = Thm.assume (ctermG hNaP);
    val hNkP    = jT (neg (dvd pF kF));     val hNk    = Thm.assume (ctermG hNkP);
    val akT = mult aF kF;
    val rT  = rmod akT pF;          (* r = rmod (a*k) p *)
    val qT  = rdiv akT pF;          (* q = rdiv (a*k) p *)
    val larT = lar aF pF kF;
    val goalC = mkDisj (cong pF akT larT) (cong pF (add akT larT) ZeroC);

    val lt0p = lt0_of_prime pF hPrime;                       (* lt 0 p *)
    val divEq = div_mod_eq_atG (akT, pF) lt0p;               (* oeq (a*k)(add (mult p q) r) *)
    (* a*k = r + p*q : comm the add (mult p q) r  -> add r (mult p q) *)
    val divEq2 = oeq_trans OF [divEq, addcomm_atG (mult pF qT, rT)];  (* a*k = (r + p*q) *)
    val rltp = rmod_lt_atG (akT, pF) lt0p;                   (* lt r p *)

    (* trichotomy on 2r vs p *)
    val tri = le_or_lt (add rT rT, pF);                      (* Disj (le (2r) p)(lt p (2r)) *)

    (* CASE lower : le (2r) p -> lar = r -> LEFT disjunct cong p (a*k) r *)
    val caseLo =
      let
        val hle = Thm.assume (ctermG (jT (le (add rT rT) pF)));
        val larEq = lar_lo_atG (aF, pF, kF) hle;             (* oeq (lar)(r) *)
        (* cong p (a*k) r via congR with witness q : a*k = r + p*q *)
        val congAkR = cong_introR_G (pF, akT, rT, qT) divEq2;  (* jT (cong p (a*k) r) *)
        (* rewrite r -> lar (sym larEq) inside cong p (a*k) _ : %z. cong p (a*k) z *)
        val zF = Free("z_lo", natT);
        val Pz = Term.lambda zF (cong pF akT zF);
        val congAkLar = oeq_rw_G (Pz, rT, larT) (oeq_sym OF [larEq]) congAkR;  (* cong p (a*k)(lar) *)
        val res = disjI1_atG (cong pF akT larT, cong pF (add akT larT) ZeroC) congAkLar;
      in Thm.implies_intr (ctermG (jT (le (add rT rT) pF))) res end;

    (* CASE upper : lt p (2r) -> lar = p-r -> RIGHT disjunct cong p (a*k + (p-r)) 0 *)
    val caseHi =
      let
        val hlt = Thm.assume (ctermG (jT (lt pF (add rT rT))));
        val larEq = lar_hi_atG (aF, pF, kF) hlt;             (* oeq (lar)(sub p r) *)
        (* recover : r + (p-r) = p  ;  use rltp *)
        fun cont wF hPeq hSub =     (* hPeq : oeq p (add r (sub p r)) ; hSub : oeq (sub p r)(Suc w) *)
          let
            val srT = sub pF rT;
            (* X = a*k + (p-r). a*k = r + p*q, so X = (r + p*q) + (p-r). *)
            (* goal : oeq X (add 0 (mult p (Suc q)))  =>  cong p X 0 via congR witness (Suc q). *)
            (* compute: X = (r + p*q) + (p-r).  rewrite a*k -> (r + p*q) in X *)
            val zF = Free("z_hiX", natT);
            val Pz = Term.lambda zF (add zF srT);   (* %z. z + (p-r) *)
            (* we want oeq (add akT srT) (add (add rT (mult pF qT)) srT) via divEq2 on left arg *)
            val Xrw = oeq_rw_G (Term.lambda zF (oeq (add akT srT) (add zF srT)), akT, add rT (mult pF qT))
                         divEq2 (oeqreflG_at (add akT srT));   (* oeq X (add (r + p*q)(p-r)) *)
            (* (r + p*q) + (p-r) = (p*q) + (r + (p-r)) = (p*q) + p  [assoc/comm + hPeq] *)
            (* step1 : ((r + p*q) + (p-r)) = (r + (p*q + (p-r)))  assoc *)
            val s_assoc = addassoc_atG (rT, mult pF qT, srT);  (* ((r + p*q)+(p-r)) = (r + (p*q + (p-r))) *)
            (* p*q + (p-r) = (p-r) + p*q  comm *)
            val s_comm = addcomm_atG (mult pF qT, srT);        (* (p*q + (p-r)) = ((p-r) + p*q) *)
            (* so r + (p*q + (p-r)) = r + ((p-r) + p*q) ; then = (r + (p-r)) + p*q  (assoc back) *)
            val zc = Free("z_hiC", natT);
            val s1 = oeq_rw_G (Term.lambda zc (oeq (add rT (add (mult pF qT) srT)) (add rT zc)),
                        add (mult pF qT) srT, add srT (mult pF qT)) s_comm
                        (oeqreflG_at (add rT (add (mult pF qT) srT)));  (* (r+(p*q+(p-r))) = (r+((p-r)+p*q)) *)
            val s_assoc2 = oeq_sym OF [addassoc_atG (rT, srT, mult pF qT)]; (* (r+((p-r)+p*q)) = ((r+(p-r))+p*q) *)
            (* r + (p-r) = p  (sym hPeq) *)
            val s_rpr = oeq_sym OF [hPeq];                     (* (r + (p-r)) = p *)
            val s_rpr_pq = oeq_rw_G (Term.lambda zc (oeq (add (add rT srT) (mult pF qT)) (add zc (mult pF qT))),
                        add rT srT, pF) s_rpr (oeqreflG_at (add (add rT srT)(mult pF qT)));  (* ((r+(p-r))+p*q) = (p + p*q) *)
            (* chain : X = (r+p*q)+(p-r) = r+(p*q+(p-r)) = r+((p-r)+p*q) = (r+(p-r))+p*q = p+p*q *)
            val chain = oeq_trans OF [Xrw,
                          oeq_trans OF [s_assoc,
                            oeq_trans OF [s1,
                              oeq_trans OF [s_assoc2, s_rpr_pq]]]];   (* oeq X (add p (mult p q)) *)
            (* p + p*q = p*1 + p*q ... actually we want X = 0 + p*(Suc q).
               p + p*q = p*(Suc q) : mult p (Suc q) = p + p*q  (mult_Suc-ish). Build via:
               mult p (Suc q) recursion : oeq (mult p (Suc q)) (add p (mult p q))? mult_Suc is
               (Suc m * n)=add n (m*n); we need p*(Suc q). Use mult_comm then mult_Suc. *)
            (* p*(Suc q) = (Suc q)*p (comm) = p + q*p (mult_Suc) = p + p*q (comm) *)
            val mc1 = mult_comm_atG (pF, suc qT);              (* (p*(Suc q)) = ((Suc q)*p) *)
            val msuc = beta_norm (Drule.infer_instantiate ctxtG
                  [(("m",0), ctermG qT), (("n",0), ctermG pF)] (varifyG mult_Suc)); (* (Suc q * p)=add p (q*p) *)
            val mc2 = mult_comm_atG (qT, pF);                 (* (q*p)=(p*q) *)
            val s_pqp = oeq_rw_G (Term.lambda zc (oeq (add pF (mult qT pF)) (add pF zc)),
                        mult qT pF, mult pF qT) mc2 (oeqreflG_at (add pF (mult qT pF))); (* (p+q*p)=(p+p*q) *)
            val mSucEq = oeq_trans OF [oeq_trans OF [mc1, msuc], s_pqp];  (* (p*(Suc q)) = (p + p*q) *)
            (* so X = p+p*q = p*(Suc q) ; and 0 + p*(Suc q) = p*(Suc q) *)
            val Xeq_mSuc = oeq_trans OF [chain, oeq_sym OF [mSucEq]];  (* oeq X (mult p (Suc q)) *)
            val add0eq = oeq_sym OF [add0l_atG (mult pF (suc qT))];    (* (mult p (Suc q)) = (0 + mult p (Suc q)) *)
            val Xeq_final = oeq_trans OF [Xeq_mSuc, add0eq];          (* oeq X (add 0 (mult p (Suc q))) *)
            (* cong p X 0 via congR : a := X, b := 0, m := p, w := Suc q ; hyp oeq X (add 0 (p*(Suc q))) *)
            val congX0 = cong_introR_G (pF, add akT srT, ZeroC, suc qT) Xeq_final;  (* cong p (a*k+(p-r)) 0 *)
            (* rewrite (p-r) -> lar (sym larEq) inside cong p (a*k + _) 0 : %z. cong p (add a*k z) 0 *)
            val zF2 = Free("z_hiL", natT);
            val Pz2 = Term.lambda zF2 (cong pF (add akT zF2) ZeroC);
            val congLar0 = oeq_rw_G (Pz2, srT, larT) (oeq_sym OF [larEq]) congX0;  (* cong p (a*k+lar) 0 *)
          in disjI2_atG (cong pF akT larT, cong pF (add akT larT) ZeroC) congLar0 end;
        val res = recover_p_r (rT, pF) rltp goalC cont;
      in Thm.implies_intr (ctermG (jT (lt pF (add rT rT)))) res end;

    val concl = disjE_atG (le (add rT rT) pF, lt pF (add rT rT), goalC) tri caseLo caseHi;
    val s1 = Thm.implies_intr (ctermG hNkP) concl;
    val s2 = Thm.implies_intr (ctermG hNaP) s1;
    val s3 = Thm.implies_intr (ctermG hPrimeP) s2;
  in varify s3 end;

val () = out "LAR_CONG_PROVED\n";

(* ---- validate lar_cong : 0-hyp + aconv ---- *)
val pVlc = Var (("p",0), natT); val aVlc = Var (("a",0), natT); val kVlc = Var (("k",0), natT);
val akVlc = mult aVlc kVlc;
val larVlc = lar aVlc pVlc kVlc;
val lar_cong_intended =
  Logic.mk_implies (jT (prime2 pVlc),
    Logic.mk_implies (jT (neg (dvd pVlc aVlc)),
      Logic.mk_implies (jT (neg (dvd pVlc kVlc)),
        jT (mkDisj (cong pVlc akVlc larVlc) (cong pVlc (add akVlc larVlc) ZeroC)))));
val rG_larcong = checkG ("lar_cong", lar_cong, lar_cong_intended);
val probe_larcong_nontrivial =
  not ((Thm.prop_of lar_cong) aconv
       (jT (mkDisj (cong pVlc akVlc larVlc) (cong pVlc (add akVlc larVlc) ZeroC))));
val () = if rG_larcong andalso probe_larcong_nontrivial
         then out "LAR_CONG_OK\n" else out "LAR_CONG_FAILED\n";

(* ============================================================================
   STAGE R (part 2) : lar_in_range  +  doubling helpers.
   lar_in_range : prime2 p ==> ~(dvd p a) ==> ~(dvd p k) ==>
                    oeq (sub p 1)(add m m) ==>
                    Conj (le (Suc Zero)(lar a p k)) (le (lar a p k) m)
   ============================================================================ *)
val () = out "STAGE_R2_BEGIN\n";

(* ---- extra ctxtG helpers ---- *)
val Suc_inj_G   = varifyG Suc_inj_ax;
fun Suc_inj_atG h = Suc_inj_G OF [h];               (* oeq (Suc a)(Suc b) -> oeq a b *)
val add_left_cancel_G = varifyG add_left_cancel;
fun add_left_cancel_atG h = add_left_cancel_G OF [h]; (* oeq (m+a)(m+b) -> oeq a b *)
val lt_irrefl_G = varifyG lt_irrefl;
fun lt_irrefl_atG nT h =
  let val inst = beta_norm (Drule.infer_instantiate ctxtG [(("n",0), ctermG nT)] lt_irrefl_G)
  in Thm.implies_elim inst h end;
val conjI_G = varifyG conjI_ax;
fun conjI_atG (At, Bt) hA hB =
  let val inst = beta_norm (Drule.infer_instantiate ctxtG [(("A",0), ctermG At), (("B",0), ctermG Bt)] conjI_G)
  in Thm.implies_elim (Thm.implies_elim inst hA) hB end;
(* le_suc_self on ctxtG : le b (Suc b) via witness Suc 0 ; Suc b = b + Suc 0 *)
fun le_suc_self_atG bT =
  let
    (* Suc b = add b (Suc 0) : add_Suc gives (Suc b + 0)? no. We want oeq (Suc b)(add b (Suc 0)).
       add b (Suc 0) = add b (Suc 0) ; b + Suc 0 = Suc(b + 0) [add_Suc_right] = Suc b [add_0_right].
       add_Suc_right : oeq (add m (Suc n))(Suc(add m n)).  *)
    val asr = beta_norm (Drule.infer_instantiate ctxtG
          [(("m",0), ctermG bT), (("n",0), ctermG ZeroC)] (varifyG add_Suc_right)); (* (b + Suc 0)=Suc(b+0) *)
    val a0 = add0r_atG bT;                              (* (b + 0) = b *)
    val sb = Succong_G a0;                              (* Suc(b+0)=Suc b *)
    val chain = oeq_trans OF [asr, sb];                 (* (b + Suc 0) = Suc b *)
    val hyp = oeq_sym OF [chain];                       (* Suc b = (b + Suc 0) *)
  in le_introG (bT, suc bT, suc ZeroC) hyp end;
(* dvd_intro on ctxtG : from oeq b (mult a w) build jT (dvd a b) *)
fun dvd_introG (aT, bT, w) hyp =
  let val Pabs = Abs ("k", natT, oeq bT (mult aT (Bound 0)))
  in exI_atG (Pabs, w) hyp end;

val () = out "STAGE_R2_HELPERS_READY\n";

(* ---------------------------------------------------------------------------
   le_double_mono (x,y) : le x y ==> le (add x x)(add y y)
   --------------------------------------------------------------------------- *)
fun le_double_mono (xT, yT) h =
  let
    val s1 = le_add_mono_atG (xT, yT, xT) h;            (* le (x+x)(y+x) *)
    val s2 = le_add_mono_atG (xT, yT, yT) h;            (* le (x+y)(y+y) *)
    val cmm = addcomm_atG (xT, yT);                     (* (x+y)=(y+x) *)
    (* rewrite left arg of s2 : (x+y) -> (y+x) via cmm , predicate %z. le z (y+y) *)
    val zF = Free("z_dm", natT);
    val Pz = Term.lambda zF (le zF (add yT yT));
    val s2' = oeq_rw_G (Pz, add xT yT, add yT xT) cmm s2;  (* le (y+x)(y+y) *)
  in le_trans_atG (add xT xT, add yT xT, add yT yT) s1 s2' end;

(* ---------------------------------------------------------------------------
   le_double_cancel (x,y) : le (add x x)(add y y) ==> le x y
   --------------------------------------------------------------------------- *)
fun le_double_cancel (xT, yT) hdd =
  let
    val goalC = le xT yT;
    val tri = le_or_lt (xT, yT);                        (* Disj (le x y)(lt y x) *)
    val cA =
      let val h = Thm.assume (ctermG (jT (le xT yT)))
      in Thm.implies_intr (ctermG (jT (le xT yT))) h end;
    val cB =
      let
        val hlt = Thm.assume (ctermG (jT (lt yT xT)));   (* le (Suc y) x *)
        val dm  = le_double_mono (suc yT, xT) hlt;       (* le (Suc y + Suc y)(x+x) *)
        (* le (Suc y + Suc y)(y+y) by le_trans with hdd : le (x+x)(y+y) *)
        val le_SS_yy = le_trans_atG (add (suc yT)(suc yT), add xT xT, add yT yT) dm hdd; (* le (Suc y+Suc y)(y+y) *)
        (* Suc y + Suc y = Suc(Suc(y+y)) : addSuc (Suc y + Suc y) = Suc(y + Suc y);
           y + Suc y = Suc(y+y) [add_Suc_right]; so = Suc(Suc(y+y)). *)
        val aS = addSuc_atG (yT, suc yT);                (* (Suc y + Suc y)=Suc(y + Suc y) *)
        val asr = beta_norm (Drule.infer_instantiate ctxtG
              [(("m",0), ctermG yT), (("n",0), ctermG yT)] (varifyG add_Suc_right)); (* (y + Suc y)=Suc(y+y) *)
        val ySy = oeq_trans OF [aS, Succong_G asr];      (* (Suc y + Suc y)=Suc(Suc(y+y)) *)
        (* rewrite le (Suc y+Suc y)(y+y) -> le (Suc(Suc(y+y)))(y+y) *)
        val zF = Free("z_dc", natT);
        val Pz = Term.lambda zF (le zF (add yT yT));
        val le_SS2 = oeq_rw_G (Pz, add (suc yT)(suc yT), suc (suc (add yT yT))) ySy le_SS_yy;
                                                         (* le (Suc(Suc(y+y)))(y+y) *)
        (* le (Suc(y+y))(Suc(Suc(y+y))) [le_suc_self] ; le_trans -> le (Suc(y+y))(y+y) = lt (y+y)(y+y) *)
        val lss = le_suc_self_atG (suc (add yT yT));     (* le (Suc(y+y))(Suc(Suc(y+y))) *)
        val lt_yy = le_trans_atG (suc (add yT yT), suc (suc (add yT yT)), add yT yT) lss le_SS2;
                                                         (* le (Suc(y+y))(y+y) = lt (y+y)(y+y) *)
        val fls = lt_irrefl_atG (add yT yT) lt_yy;       (* oFalse *)
        val res = Thm.implies_elim (oFalse_elimG_at goalC) fls;
      in Thm.implies_intr (ctermG (jT (lt yT xT))) res end;
  in disjE_atG (le xT yT, lt yT xT, goalC) tri cA cB end;

val () = out "STAGE_R2_DOUBLING_READY\n";
...
val lar_in_range =
  let
    val pF = Free("p", natT); val aF = Free("a", natT); val kF = Free("k", natT); val mF = Free("m", natT);
    val hPrimeP = jT (prime2 pF);                    val hPrime = Thm.assume (ctermG hPrimeP);
    val hNaP    = jT (neg (dvd pF aF));               val hNa    = Thm.assume (ctermG hNaP);
    val hNkP    = jT (neg (dvd pF kF));               val hNk    = Thm.assume (ctermG hNkP);
    val hOddP   = jT (oeq (sub pF oneC) (add mF mF));  val hOdd   = Thm.assume (ctermG hOddP);
    val akT = mult aF kF;
    val rT  = rmod akT pF;
    val qT  = rdiv akT pF;
    val larT = lar aF pF kF;
    val goalC = mkConj (le (suc ZeroC) larT) (le larT mF);

    val lt0p = lt0_of_prime pF hPrime;                          (* lt 0 p *)
    val divEq = div_mod_eq_atG (akT, pF) lt0p;                  (* a*k = p*q + r *)
    val divEq2 = oeq_trans OF [divEq, addcomm_atG (mult pF qT, rT)];  (* a*k = r + p*q *)
    val rltp = rmod_lt_atG (akT, pF) lt0p;                      (* lt r p *)
    val ndvd_ak = prime_not_dvd_mult_atG (pF, aF, kF) hPrime hNa hNk;  (* ~(dvd p (a*k)) *)

    (* lt 0 r : if r = 0 then a*k = r + p*q = p*q -> dvd p (a*k), contra. Use le_or_lt(0,r)?
       Simpler : r = 0 or r = Suc r0 (dichotomy). If r=0 derive dvd p (a*k) -> oFalse. *)
    val lt0r =
      let
        (* dichotomy : Disj (oeq r 0)(Ex r0. oeq r (Suc r0)) — build via le_or_lt? use le_or_lt(r,0):
           Disj (le r 0)(lt 0 r). le r 0 -> r=0 (only 0<=0... le r 0 means Ex w. 0 = r + w -> r=0).
           Cleaner: le_or_lt (Suc 0, r)? lt 0 r = le (Suc 0) r. We want exactly lt 0 r.
           Use le_or_lt (r, 0) : Disj (le r 0)(lt 0 r). RIGHT is the goal. LEFT (le r 0) -> r=0 -> contra. *)
        val tri0 = le_or_lt (rT, ZeroC);                       (* Disj (le r 0)(lt 0 r) *)
        val gC = lt ZeroC rT;
        val cA =
          let
            val hle0 = Thm.assume (ctermG (jT (le rT ZeroC)));  (* le r 0 = Ex w. 0 = r + w *)
            val Pabs = Abs("w", natT, oeq ZeroC (add rT (Bound 0)));
            fun body wF (hw:thm) =       (* hw : 0 = r + w *)
              let
                (* r = 0 : add_eq_zero_left style. 0 = r + w -> r = 0.
                   from 0 = r + w : sub both? Use: r <= r+w = 0 -> r = 0 by le_antisym? simpler:
                   0 = r + w ; le r (r+w) [le_add] ; rewrite (r+w)->0 -> le r 0 ; with le 0 r (le_add 0)
                   -> oeq r 0 (le_antisym). *)
                val le_r_rw = le_add_atG (rT, wF);              (* le r (r+w) *)
                val zF = Free("z_r0", natT);
                val Pz = Term.lambda zF (le rT zF);
                val le_r_0 = oeq_rw_G (Pz, add rT wF, ZeroC) (oeq_sym OF [hw]) le_r_rw;  (* le r 0 *)
                val le_0_r = le_add_atG (ZeroC, rT);            (* le 0 (0 + r) *)
                val a0 = add0l_atG rT;                          (* (0 + r) = r *)
                val zF2 = Free("z_r0b", natT);
                val Pz2 = Term.lambda zF2 (le ZeroC zF2);
                val le_0_r2 = oeq_rw_G (Pz2, add ZeroC rT, rT) a0 le_0_r;  (* le 0 r *)
                val req0 = le_antisym_atG (rT, ZeroC) le_r_0 le_0_r2;       (* oeq r 0 *)
                (* a*k = r + p*q ; r=0 -> a*k = 0 + p*q = p*q -> dvd p (a*k) *)
                val zF3 = Free("z_r0c", natT);
                val Pz3 = Term.lambda zF3 (oeq akT (add zF3 (mult pF qT)));
                val ak_eq = oeq_rw_G (Pz3, rT, ZeroC) req0 divEq2;  (* a*k = 0 + p*q *)
                val a0pq = add0l_atG (mult pF qT);              (* (0 + p*q) = p*q *)
                val ak_pq = oeq_trans OF [ak_eq, a0pq];         (* a*k = p*q *)
                val dvd_ak = dvd_introG (pF, akT, qT) ak_pq;    (* dvd p (a*k) *)
                val fls = mp_atG (dvd pF akT, oFalseC) ndvd_ak dvd_ak;  (* oFalse *)
              in Thm.implies_elim (oFalse_elimG_at gC) fls end;
            val r = exE_atG (Pabs, gC) hle0 "w_r0" body;
          in Thm.implies_intr (ctermG (jT (le rT ZeroC))) r end;
        val cB =
          let val h = Thm.assume (ctermG (jT (lt ZeroC rT)))
          in Thm.implies_intr (ctermG (jT (lt ZeroC rT))) h end;
      in disjE_atG (le rT ZeroC, lt ZeroC rT, gC) tri0 cA cB end;  (* lt 0 r *)

    (* pEq : oeq p (Suc(add m m)).  p = Suc w0 (from lt0p), sub p 1 = w0 = m+m (hOdd) *)
    val pEq =
      let
        (* lt0p : le (Suc 0) p = Ex w. p = add (Suc 0) w = Suc(0+w) = Suc w *)
        val Pabs = Abs("w", natT, oeq pF (add (suc ZeroC) (Bound 0)));
        fun body wF (hw:thm) =        (* hw : p = (Suc 0 + w) *)
          let
            (* (Suc 0 + w) = Suc(0 + w) = Suc w *)
            val aS = addSuc_atG (ZeroC, wF);             (* (Suc 0 + w)=Suc(0+w) *)
            val a0 = add0l_atG wF;                       (* (0 + w)=w *)
            val pSucw = oeq_trans OF [hw, oeq_trans OF [aS, Succong_G a0]];  (* p = Suc w *)
            (* sub p 1 = sub (Suc w)(Suc 0) = sub w 0 = w *)
            val zF = Free("z_pe", natT);
            val Pz = Term.lambda zF (oeq (sub pF oneC)(sub zF oneC));
            val e1 = oeq_rw_G (Pz, pF, suc wF) pSucw (oeqreflG_at (sub pF oneC)); (* sub p 1 = sub(Suc w)(Suc 0) *)
            val e2 = subSS_atG (wF, ZeroC);              (* sub (Suc w)(Suc 0) = sub w 0 *)
            val e3 = subN0_atG wF;                       (* sub w 0 = w *)
            val subp1_w = oeq_trans OF [e1, oeq_trans OF [e2, e3]];  (* sub p 1 = w *)
            (* w = m+m : sym subp1_w then hOdd *)
            val w_mm = oeq_trans OF [oeq_sym OF [subp1_w], hOdd];    (* w = m+m *)
            (* p = Suc w ; rewrite w -> m+m : p = Suc(m+m) *)
            val zF2 = Free("z_pe2", natT);
            val Pz2 = Term.lambda zF2 (oeq pF (suc zF2));
            val res = oeq_rw_G (Pz2, wF, add mF mF) w_mm pSucw;  (* p = Suc(m+m) *)
          in res end;
      in exE_atG (Pabs, oeq pF (suc (add mF mF))) lt0p "w_pe" body end;

    (* trichotomy on 2r vs p *)
    val tri = le_or_lt (add rT rT, pF);

    (* le 1 lar AND le lar m, in each half *)
    (* CASE lower : le (2r) p -> lar = r ; le 1 r (from lt0r) ; le r m (from 2r<=p=2m+1) *)
    val caseLo =
      let
        val hle = Thm.assume (ctermG (jT (le (add rT rT) pF)));
        val larEq = lar_lo_atG (aF, pF, kF) hle;            (* oeq lar r *)
        (* le 1 lar : lt0r = le (Suc 0) r ; rewrite r -> lar (sym larEq) *)
        val zF = Free("z_lo1", natT);
        val Pz = Term.lambda zF (le (suc ZeroC) zF);
        val le1lar = oeq_rw_G (Pz, rT, larT) (oeq_sym OF [larEq]) lt0r;  (* le 1 lar *)
        (* le r m : 2r <= p = Suc(m+m).  Want le r m.  le_or_lt (r,m): if le r m done;
           if lt m r = le (Suc m) r -> le_double_mono -> le (Suc m+Suc m)(r+r) ; r+r <= p? we have
           le (r+r) p [hle], and p = Suc(m+m). Suc m+Suc m = Suc(Suc(m+m)) = Suc p. so
           le (Suc p)(r+r) <= (le_trans with hle) le (Suc p) p -> contra. *)
        val le_r_m =
          let
            val tri_rm = le_or_lt (rT, mF);                 (* Disj (le r m)(lt m r) *)
            val gC = le rT mF;
            val a = let val h = Thm.assume (ctermG (jT (le rT mF)))
                    in Thm.implies_intr (ctermG (jT (le rT mF))) h end;
            val b =
              let
                val hlt = Thm.assume (ctermG (jT (lt mF rT)));   (* le (Suc m) r *)
                val dm = le_double_mono (suc mF, rT) hlt;        (* le (Suc m+Suc m)(r+r) *)
                (* Suc m + Suc m = Suc(Suc(m+m)) *)
                val aS = addSuc_atG (mF, suc mF);               (* (Suc m+Suc m)=Suc(m+Suc m) *)
                val asr = beta_norm (Drule.infer_instantiate ctxtG
                      [(("m",0), ctermG mF), (("n",0), ctermG mF)] (varifyG add_Suc_right)); (* (m+Suc m)=Suc(m+m) *)
                val SmSm = oeq_trans OF [aS, Succong_G asr];     (* (Suc m+Suc m)=Suc(Suc(m+m)) *)
                (* p = Suc(m+m) (pEq) so Suc(Suc(m+m)) = Suc p ; combine -> (Suc m+Suc m)=Suc p *)
                val SmSm_Sp = oeq_trans OF [SmSm, oeq_sym OF [Succong_G pEq]];  (* (Suc m+Suc m)=Suc p *)
                (* rewrite dm left arg -> Suc p : le (Suc p)(r+r) *)
                val zF2 = Free("z_lo2", natT);
                val Pz2 = Term.lambda zF2 (le zF2 (add rT rT));
                val le_Sp_rr = oeq_rw_G (Pz2, add (suc mF)(suc mF), suc pF) SmSm_Sp dm;  (* le (Suc p)(r+r) *)
                (* le (Suc p) p by le_trans with hle : le (r+r) p *)
                val le_Sp_p = le_trans_atG (suc pF, add rT rT, pF) le_Sp_rr hle;  (* le (Suc p) p = lt p p *)
                val fls = lt_irrefl_atG pF le_Sp_p;             (* oFalse *)
                val res = Thm.implies_elim (oFalse_elimG_at gC) fls;
              in Thm.implies_intr (ctermG (jT (lt mF rT))) res end;
          in disjE_atG (le rT mF, lt mF rT, gC) tri_rm a b end;
        (* le lar m : rewrite r -> lar in le_r_m *)
        val zF3 = Free("z_lo3", natT);
        val Pz3 = Term.lambda zF3 (le zF3 mF);
        val le_lar_m = oeq_rw_G (Pz3, rT, larT) (oeq_sym OF [larEq]) le_r_m;  (* le lar m *)
        val res = conjI_atG (le (suc ZeroC) larT, le larT mF) le1lar le_lar_m;
      in Thm.implies_intr (ctermG (jT (le (add rT rT) pF))) res end;

    (* CASE upper : lt p (2r) -> lar = p-r ; le 1 (p-r) [recover gives sub p r = Suc w] ;
       le (p-r) m [via m+1 <= r double_cancel then p-r = m-d <= m] *)
    val caseHi =
      let
        val hlt = Thm.assume (ctermG (jT (lt pF (add rT rT))));
        val larEq = lar_hi_atG (aF, pF, kF) hlt;             (* oeq lar (sub p r) *)
        ...
        fun cont wF hPeq hSub =      (* hPeq : oeq p (add r (sub p r)) ; hSub : oeq (sub p r)(Suc w) *)
          let
            (* le 1 (p-r) : hSub : sub p r = Suc w ; le 1 (Suc w) [le_suc_self-ish: le (Suc 0)(Suc w)
               from le 0 w]. le 0 w via le_add 0 w. then le_suc_mono. *)
            val le0w = let val l = le_add_atG (ZeroC, wF); val a0 = add0l_atG wF;
                           val zF = Free("z_hi0", natT); val Pz = Term.lambda zF (le ZeroC zF)
                       in oeq_rw_G (Pz, add ZeroC wF, wF) a0 l end;   (* le 0 w *)
            val le1Sw = le_suc_mono_atG (ZeroC, wF) le0w;       (* le (Suc 0)(Suc w) *)
            (* rewrite Suc w -> sub p r (sym hSub) : le 1 (sub p r) *)
            val zF = Free("z_hi1", natT);
            val Pz = Term.lambda zF (le (suc ZeroC) zF);
            val le1sr = oeq_rw_G (Pz, suc wF, srT) (oeq_sym OF [hSub]) le1Sw;  (* le 1 (sub p r) *)
            val zF2 = Free("z_hi1b", natT);
            val Pz2 = Term.lambda zF2 (le (suc ZeroC) zF2);
            val le1lar = oeq_rw_G (Pz2, srT, larT) (oeq_sym OF [larEq]) le1sr;  (* le 1 lar *)

            (* le (p-r) m :
               from lt p (2r) = le (Suc p)(r+r) ; p = Suc(m+m) (pEq) -> Suc p = Suc(Suc(m+m)).
               (Suc m + Suc m) = Suc(Suc(m+m)) = Suc p. So le (Suc m+Suc m)(r+r) [rewrite].
               le_double_cancel -> le (Suc m) r. Then r = (Suc m) + d (le def exE).
               hPeq : p = r + (p-r) = (Suc m + d) + (p-r). p = Suc(m+m).
               Suc(m+m) = (Suc m + d) + (p-r) = Suc(m + (d + (p-r))) -> m+m = m + (d + (p-r))
               -> m = d + (p-r) [add_left_cancel] -> le (p-r) m [le_add comm]. *)
            val hlt2 = hlt;                                  (* lt p (2r) = le (Suc p)(r+r) *)
            (* (Suc m + Suc m) = Suc p *)
            val aS = addSuc_atG (mF, suc mF);
            val asr = beta_norm (Drule.infer_instantiate ctxtG
                  [(("m",0), ctermG mF), (("n",0), ctermG mF)] (varifyG add_Suc_right));
            val SmSm = oeq_trans OF [aS, Succong_G asr];     (* (Suc m+Suc m)=Suc(Suc(m+m)) *)
            val SmSm_Sp = oeq_trans OF [SmSm, oeq_sym OF [Succong_G pEq]];  (* (Suc m+Suc m)=Suc p *)
            (* le (Suc p)(r+r) -> le (Suc m+Suc m)(r+r) by rewriting Suc p <- (Suc m+Suc m) *)
            val zF3 = Free("z_hiD", natT);
            val Pz3 = Term.lambda zF3 (le zF3 (add rT rT));
            val le_SmSm_rr = oeq_rw_G (Pz3, suc pF, add (suc mF)(suc mF)) (oeq_sym OF [SmSm_Sp]) hlt2;
                                                             (* le (Suc m+Suc m)(r+r) *)
            val le_Sm_r = le_double_cancel (suc mF, rT) le_SmSm_rr;  (* le (Suc m) r *)
            (* r = Suc m + d : exE on le (Suc m) r *)
            val PabsD = Abs("d", natT, oeq rT (add (suc mF) (Bound 0)));
            fun dbody dF (hd:thm) =      (* hd : r = (Suc m + d) *)
              let
                (* p = r + (p-r) [hPeq] ; r = Suc m + d ; so p = (Suc m + d)+(p-r).
                   p = Suc(m+m) [pEq].  (Suc m + d) + (p-r) = Suc(m + (d + (p-r))) [addSuc + assoc]. *)
                val s = srT;   (* p - r *)
                (* hPeq : p = r + s ; rewrite r -> (Suc m + d) : p = (Suc m + d) + s *)
                val zF4 = Free("z_hiE", natT);
                val Pz4 = Term.lambda zF4 (oeq pF (add zF4 s));
                val pEq2 = oeq_rw_G (Pz4, rT, add (suc mF) dF) hd hPeq;  (* p = (Suc m + d) + s *)
                (* (Suc m + d) + s = Suc(m + d) + s [addSuc inside? no: Suc m + d = Suc(m+d)]:
                   (Suc m + d) = Suc(m+d) [addSuc] ; so (Suc m+d)+s = (Suc(m+d))+s = Suc((m+d)+s) [addSuc]
                   = Suc(m + (d+s)) [assoc]. *)
                val e1 = addSuc_atG (mF, dF);               (* (Suc m + d)=Suc(m+d) *)
                (* rewrite ((Suc m+d)+s) left -> (Suc(m+d)) : ((Suc(m+d))+s) *)
                val zF5 = Free("z_hiF", natT);
                val Pz5 = Term.lambda zF5 (oeq (add (add (suc mF) dF) s)(add zF5 s));
                val e1b = oeq_rw_G (Pz5, add (suc mF) dF, suc (add mF dF)) e1
                            (oeqreflG_at (add (add (suc mF) dF) s));  (* ((Suc m+d)+s)=((Suc(m+d))+s) *)
                val e2 = addSuc_atG (add mF dF, s);         (* (Suc(m+d)+s)=Suc((m+d)+s) *)
                val e3 = addassoc_atG (mF, dF, s);          (* ((m+d)+s)=(m+(d+s)) *)
                val e3s = Succong_G e3;                     (* Suc((m+d)+s)=Suc(m+(d+s)) *)
                (* chain : ((Suc m+d)+s) = Suc(m+(d+s)) *)
                val rhs_eq = oeq_trans OF [e1b, oeq_trans OF [e2, e3s]];  (* ((Suc m+d)+s)=Suc(m+(d+s)) *)
                (* p = ((Suc m+d)+s) = Suc(m+(d+s)) ; also p = Suc(m+m) [pEq] *)
                val p_eq_Smds = oeq_trans OF [pEq2, rhs_eq];  (* p = Suc(m+(d+s)) *)
                val Smm_eq = oeq_trans OF [oeq_sym OF [pEq], p_eq_Smds];  (* Suc(m+m)=Suc(m+(d+s)) *)
                val mm_eq = Suc_inj_atG Smm_eq;             (* (m+m)=(m+(d+s)) *)
                val canc = add_left_cancel_atG mm_eq;       (* m = (d+s) *)
                (* le s m : s <= d+s = m. le_add (s, d) : le s (s+d) ; comm (s+d)=(d+s) ; rewrite -> le s (d+s) ;
                   rewrite (d+s) <- m (sym canc) -> le s m. *)
                val le_s_sd = le_add_atG (s, dF);           (* le s (s+d) *)
                val cmm = addcomm_atG (s, dF);              (* (s+d)=(d+s) *)
                val zF6 = Free("z_hiG", natT);
                val Pz6 = Term.lambda zF6 (le s zF6);
                val le_s_ds = oeq_rw_G (Pz6, add s dF, add dF s) cmm le_s_sd;  (* le s (d+s) *)
                val le_s_m = oeq_rw_G (Pz6, add dF s, mF) (oeq_sym OF [canc]) le_s_ds;  (* le s m *)
              in le_s_m end;
            val le_sr_m = exE_atG (PabsD, le srT mF) le_Sm_r "d_hi" dbody;  (* le (sub p r) m *)
            (* le lar m : rewrite (sub p r)->lar *)
            val zF7 = Free("z_hiH", natT);
            val Pz7 = Term.lambda zF7 (le zF7 mF);
            val le_lar_m = oeq_rw_G (Pz7, srT, larT) (oeq_sym OF [larEq]) le_sr_m;  (* le lar m *)
            val res = conjI_atG (le (suc ZeroC) larT, le larT mF) le1lar le_lar_m;
          in res end;
        val res = recover_p_r (rT, pF) rltp goalC cont;
      in Thm.implies_intr (ctermG (jT (lt pF (add rT rT)))) res end;
    ...
    val concl = disjE_atG (le (add rT rT) pF, lt pF (add rT rT), goalC) tri caseLo caseHi;
    val s1 = Thm.implies_intr (ctermG hOddP) concl;
    val s2 = Thm.implies_intr (ctermG hNkP) s1;
    val s3 = Thm.implies_intr (ctermG hNaP) s2;
    val s4 = Thm.implies_intr (ctermG hPrimeP) s3;
  in varify s4 end;

val () = out "LAR_IN_RANGE_PROVED\n";

(* ---- validate lar_in_range : 0-hyp + aconv ---- *)
val pVlr = Var (("p",0), natT); val aVlr = Var (("a",0), natT);
val kVlr = Var (("k",0), natT); val mVlr = Var (("m",0), natT);
val larVlr = lar aVlr pVlr kVlr;
val lar_in_range_intended =
  Logic.mk_implies (jT (prime2 pVlr),
    Logic.mk_implies (jT (neg (dvd pVlr aVlr)),
      Logic.mk_implies (jT (neg (dvd pVlr kVlr)),
        Logic.mk_implies (jT (oeq (sub pVlr oneC) (add mVlr mVlr)),
          jT (mkConj (le (suc ZeroC) larVlr) (le larVlr mVlr))))));
val rG_larrange = checkG ("lar_in_range", lar_in_range, lar_in_range_intended);
val probe_larrange_needs_odd =
  let val bogus = Logic.mk_implies (jT (prime2 pVlr),
        Logic.mk_implies (jT (neg (dvd pVlr aVlr)),
          Logic.mk_implies (jT (neg (dvd pVlr kVlr)),
            jT (mkConj (le (suc ZeroC) larVlr) (le larVlr mVlr)))))
  in not ((Thm.prop_of lar_in_range) aconv bogus) end;
val () = if rG_larrange andalso probe_larrange_needs_odd
         then out "LAR_IN_RANGE_OK\n" else out "LAR_IN_RANGE_FAILED\n";

val () = if rG_larcong andalso rG_larrange andalso probe_larcong_nontrivial andalso probe_larrange_needs_odd
         then out "RESIDUE_LEMMAS_OK\n" else out "RESIDUE_LEMMAS_FAILED\n";

(* ============================================================================
   GAUSS STAGE-R AXIOM AUDIT + FINAL VERDICT
   The ONLY classical assumption anywhere in thyG must remain ex_middle; every
   other axiom is a conservative DEFINITION (Peano/oeq/order/dvd recursion,
   sub/sumf/prodf recursion, the natlist datatype + lmap, the rmod/rdiv/upto
   function axioms — the existential content of the already-proved
   div_mod_exists — and the two lar half-case defining axioms).
   ============================================================================ *)
val () = out "GAUSS_STAGER_AUDIT_BEGIN\n";
val allAxG = Theory.all_axioms_of thyG;
val () = out ("gauss_thyG_axiom_count=" ^ Int.toString (length allAxG) ^ "\n");
val hasEMg = List.exists (fn (nm,_) =>
      String.isSuffix "ex_middle" nm orelse nm = "ex_middle") allAxG;
val () = out ("gauss_thyG_ex_middle_present=" ^ Bool.toString hasEMg ^ "\n");
val () = out "GAUSS_STAGER_AUDIT_END\n";

(* soundness gate : both residue lemmas 0-hyp AND no extra sort-hyps *)
val residue_clean =
     length (Thm.hyps_of lar_cong) = 0     andalso length (Thm.extra_shyps lar_cong) = 0
     andalso length (Thm.hyps_of lar_in_range) = 0 andalso length (Thm.extra_shyps lar_in_range) = 0;
val () = if residue_clean andalso rG_larcong andalso rG_larrange andalso hasEMg
         then out "GAUSS_STAGE_R_ALL_OK\n" else out "GAUSS_STAGE_R_INCOMPLETE\n";

(* ############################################################################
   ###############  GAUSS LEMMA — STAGE P : PERMUTATION-PRODUCT  ###############
   ----------------------------------------------------------------------------
   APPENDED to gauss_lemma.sml (base ends at Stage R: ctxtG/ctermG/thyG, with
   the natlist datatype + lmap + rmod/rdiv/upto + lar, plus lar_cong /
   lar_in_range / abs_inj already proved and re-varified on ctxtG).

   (P1) SPLICE the natlist strong-induction library onto a context extending
        thyG with lhd/ltl (thyGG/ctxtGG):
          extract, mem_remove_fwd, mem_remove_bwd, mem_remove_neq,
          llen_remove, nodup_remove, list_cases, list_strong_induct,
          lnodup_upto, lmem_upto_fwd, lmem_upto_bwd.
        Marker NATLIST_LIB_OK.
   (P2) lprod_perm_of_inj.   Marker LPROD_PERM_OK.
   (P3) lar_perm.            Marker LAR_PERM_OK.
   ############################################################################ *)
val () = out "STAGE_P_BEGIN\n";

(* ----------------------------------------------------------------------------
   Extend thyG with lhd / ltl (needed for list_cases inside strong induction).
   thyGG is the ONE final context for all Stage-P work.
   ---------------------------------------------------------------------------- *)
val thyGGc = Sign.add_consts
  [(Binding.name "lhd", natlistT --> natT, NoSyn),
   (Binding.name "ltl", natlistT --> natlistT, NoSyn)] thyG;
fun cnstGG nm T = Const (Sign.full_name thyGGc (Binding.name nm), T);
val lhdC = cnstGG "lhd" (natlistT --> natT);     fun lhd l = lhdC $ l;
val ltlC = cnstGG "ltl" (natlistT --> natlistT); fun ltl l = ltlC $ l;
val xGG = Free("x", natT); val tGG = Free("t", natlistT);
val ((_,lhd_cons_ax), tGGa) = Thm.add_axiom_global (Binding.name "lhd_cons",
      jT (oeq (lhd (lcons xGG tGG)) xGG)) thyGGc;
val ((_,ltl_cons_ax), thyGG) = Thm.add_axiom_global (Binding.name "ltl_cons",
      jT (leq (ltl (lcons xGG tGG)) tGG)) tGGa;

val ctxtGG  = Proof_Context.init_global thyGG;
val ctermGG = Thm.cterm_of ctxtGG;
val () = out "STAGE_P_GGCTX_READY\n";

(* ----------------------------------------------------------------------------
   transfer+varify everything we need onto thyGG (so infer_instantiate ctxtGG
   never hits "Cannot transfer: not a super theory").
   ---------------------------------------------------------------------------- *)
fun toGG th = Thm.transfer thyGG th;
fun varifyGG th = toGG (varify th);

(* core logical + arithmetic *)
val oeq_refl_GG    = varifyGG oeq_refl;
val oeq_subst_GG   = varifyGG oeq_subst;
val exI_GG         = varifyGG exI_ax;
val exE_GG         = varifyGG exE_ax;
val disjE_GG       = varifyGG disjE_ax;
val disjI1_GG      = varifyGG disjI1_ax;
val disjI2_GG      = varifyGG disjI2_ax;
val mp_GG          = varifyGG mp_ax;
val impI_GG        = varifyGG impI_ax;
val conjI_GG       = varifyGG conjI_ax;
val conjunct1_GG   = varifyGG conjunct1_ax;
val conjunct2_GG   = varifyGG conjunct2_ax;
val oFalse_elim_GG = varifyGG oFalse_elim_ax;
val ex_middle_GG   = varifyGG ex_middle_ax;
val allI_GG        = varifyGG allI_ax;
val allE_GG        = varifyGG allE_ax;
val Suc_neq_Zero_GG= varifyGG Suc_neq_Zero_ax;
val Suc_cong_GG    = varifyGG Suc_cong;
val add_0_right_GG = varifyGG add_0_right;
val add_Suc_GG     = varifyGG add_Suc;
val mult_comm_GG   = varifyGG mult_comm;
val mult_assoc_GG  = varifyGG mult_assoc;
val mult_1_left_GG = varifyGG mult_1_left;
val lt_suc_GG      = varifyGG lt_suc;
val lt_suc_cases_GG= varifyGG lt_suc_cases;
val lt_trans_GG    = varifyGG lt_trans;
val lt_irrefl_GG   = varifyGG lt_irrefl;
val le_refl_GG     = varifyGG le_refl;
val le_trans_GG    = varifyGG le_trans;
val le_add_GG      = varifyGG le_add;
val meta_nat_induct_GG = varifyGG meta_nat_induct_ax2;
val nat_induct_GG  = varifyGG nat_induct;

(* natlist + lmap + upto axioms (added to intermediate theories below thyGG) *)
val leq_refl_GG    = varifyGG leq_refl_ax;
val leq_subst_GG   = varifyGG leq_subst_ax;
val list_induct_GG = varifyGG list_induct_ax;
val lprod_nil_GG   = varifyGG lprod_nil_ax;
val lprod_cons_GG  = varifyGG lprod_cons_ax;
val lmem_nil_elim_GG = varifyGG lmem_nil_elim_ax;
val lmem_cons_fwd_GG = varifyGG lmem_cons_fwd_ax;
val lmem_cons_bwd_GG = varifyGG lmem_cons_bwd_ax;
val lremove_nil_GG = varifyGG lremove_nil_ax;
val lremove_cons_eq_GG  = varifyGG lremove_cons_eq_ax;
val lremove_cons_neq_GG = varifyGG lremove_cons_neq_ax;
val llen_nil_GG    = varifyGG llen_nil_ax;
val llen_cons_GG   = varifyGG llen_cons_ax;
val lnodup_nil_GG  = varifyGG lnodup_nil_ax;
val lnodup_cons_fwd_GG = varifyGG lnodup_cons_fwd_ax;
val lnodup_cons_bwd_GG = varifyGG lnodup_cons_bwd_ax;
val lmap_nil_GG    = varifyGG lmap_nil_ax;
val lmap_cons_GG   = varifyGG lmap_cons_ax;
val upto_zero_GG   = varifyGG upto_zero_ax;
val upto_suc_GG    = varifyGG upto_suc_ax;
val lhd_cons_GG    = varifyGG lhd_cons_ax;
val ltl_cons_GG    = varifyGG ltl_cons_ax;

(* cong helpers *)
val cong_refl_GG   = varifyGG cong_refl;
val cong_mult_GG   = varifyGG cong_mult;
val cong_trans_GG  = varifyGG cong_trans;
val cong_sym_GG    = varifyGG cong_sym;

val () = out "STAGE_P_VARIFY_DONE\n";

(* ----------------------------------------------------------------------------
   ground instantiators on ctxtGG  (suffix _GG; same shape as ctxtG layer)
   ---------------------------------------------------------------------------- *)
fun oeqRefl_GG t = beta_norm (Drule.infer_instantiate ctxtGG [(("a",0), ctermGG t)] oeq_refl_GG);
fun oeq_rw_GG (Pabs, xT, yT) hxy hPx =
  let val inst = beta_norm (Drule.infer_instantiate ctxtGG
        [(("P",0), ctermGG Pabs), (("a",0), ctermGG xT), (("b",0), ctermGG yT)] oeq_subst_GG)
  in (inst OF [hxy]) OF [hPx] end;
fun leq_rw_GG (Pabs, xT, yT) hxy hPx =
  let val inst = beta_norm (Drule.infer_instantiate ctxtGG
        [(("P",0), ctermGG Pabs), (("a",0), ctermGG xT), (("b",0), ctermGG yT)] leq_subst_GG)
  in (inst OF [hxy]) OF [hPx] end;
fun leqRefl_GG l = beta_norm (Drule.infer_instantiate ctxtGG [(("a",0), ctermGG l)] leq_refl_GG);
(* leq is symmetric : derive leq b a from leq a b via leq_subst on %z. leq z a *)
fun leq_sym_GG hab =
  let val (aT, bT) = case Thm.prop_of hab of
          _ $ (_ $ a $ b) => (a, b)
        | _ => raise Fail "leq_sym_GG: shape";
      val Pabs = Abs("z", natlistT, leq (Bound 0) aT);
      val refl = leqRefl_GG aT;
  in leq_rw_GG (Pabs, aT, bT) hab refl end;

fun exI_GG_at (Pabs, w) hbody =
  let val inst = beta_norm (Drule.infer_instantiate ctxtGG
        [(("P",0), ctermGG Pabs), (("a",0), ctermGG w)] exI_GG)
  in Thm.implies_elim inst hbody end;
fun exE_GG_at (Pabs, goalC) exThm wName bodyFn =
  let
    val inst = beta_norm (Drule.infer_instantiate ctxtGG
          [(("P",0), ctermGG Pabs), (("Q",0), ctermGG goalC)] exE_GG);
    val p1 = Thm.implies_elim inst exThm;
    val wF = Free (wName, natT);
    val hyp = Thm.assume (ctermGG (jT (Term.betapply (Pabs, wF))));
    val body = bodyFn wF hyp;
    val minor = Thm.forall_intr (ctermGG wF) (Thm.implies_intr (ctermGG (jT (Term.betapply (Pabs, wF)))) body);
  in Thm.implies_elim p1 minor end;
fun disjE_GG_at (At, Bt, Ct) dThm caseA caseB =
  let
    val inst = beta_norm (Drule.infer_instantiate ctxtGG
          [(("A",0), ctermGG At), (("B",0), ctermGG Bt), (("C",0), ctermGG Ct)] disjE_GG);
  in Thm.implies_elim (Thm.implies_elim (Thm.implies_elim inst dThm) caseA) caseB end;
fun disjI1_GG_at (At, Bt) hA =
  Thm.implies_elim (beta_norm (Drule.infer_instantiate ctxtGG
        [(("A",0), ctermGG At), (("B",0), ctermGG Bt)] disjI1_GG)) hA;
fun disjI2_GG_at (At, Bt) hB =
  Thm.implies_elim (beta_norm (Drule.infer_instantiate ctxtGG
        [(("A",0), ctermGG At), (("B",0), ctermGG Bt)] disjI2_GG)) hB;
fun mp_GG_at (At, Bt) hImp hA =
  Thm.implies_elim (Thm.implies_elim (beta_norm (Drule.infer_instantiate ctxtGG
        [(("A",0), ctermGG At), (("B",0), ctermGG Bt)] mp_GG)) hImp) hA;
fun impI_GG_at (At, Bt) hMeta =
  Thm.implies_elim (beta_norm (Drule.infer_instantiate ctxtGG
        [(("A",0), ctermGG At), (("B",0), ctermGG Bt)] impI_GG)) hMeta;
fun conjI_GG_at (At, Bt) hA hB =
  Thm.implies_elim (Thm.implies_elim (beta_norm (Drule.infer_instantiate ctxtGG
        [(("A",0), ctermGG At), (("B",0), ctermGG Bt)] conjI_GG)) hA) hB;
fun conjunct1_GG_at (At, Bt) h =
  Thm.implies_elim (beta_norm (Drule.infer_instantiate ctxtGG
        [(("A",0), ctermGG At), (("B",0), ctermGG Bt)] conjunct1_GG)) h;
fun conjunct2_GG_at (At, Bt) h =
  Thm.implies_elim (beta_norm (Drule.infer_instantiate ctxtGG
        [(("A",0), ctermGG At), (("B",0), ctermGG Bt)] conjunct2_GG)) h;
fun oFalse_elim_GG_at Rt =
  beta_norm (Drule.infer_instantiate ctxtGG [(("R",0), ctermGG Rt)] oFalse_elim_GG);
fun ex_middle_GG_at At =
  beta_norm (Drule.infer_instantiate ctxtGG [(("A",0), ctermGG At)] ex_middle_GG);
fun allI_GG_at Pabs hAll =
  Thm.implies_elim (beta_norm (Drule.infer_instantiate ctxtGG [(("P",0), ctermGG Pabs)] allI_GG)) hAll;
fun allE_GG_at Pabs at hForall =
  Thm.implies_elim (beta_norm (Drule.infer_instantiate ctxtGG
        [(("P",0), ctermGG Pabs), (("a",0), ctermGG at)] allE_GG)) hForall;

(* Suc / add helpers on ctxtGG *)
fun Succong_GG h = Suc_cong_GG OF [h];
fun add0r_GG t = beta_norm (Drule.infer_instantiate ctxtGG [(("n",0), ctermGG t)] add_0_right_GG);
fun addSuc_GG (mt,nt) = beta_norm (Drule.infer_instantiate ctxtGG
        [(("m",0), ctermGG mt),(("n",0), ctermGG nt)] add_Suc_GG);
fun Suc_neq_Zero_GG_at nt heq =
  Thm.implies_elim (beta_norm (Drule.infer_instantiate ctxtGG [(("n",0), ctermGG nt)] Suc_neq_Zero_GG)) heq;

(* mult helpers on ctxtGG *)
fun multcomm_GG (mt,nt) = beta_norm (Drule.infer_instantiate ctxtGG
        [(("m",0), ctermGG mt),(("n",0), ctermGG nt)] mult_comm_GG);
fun multassoc_GG (mt,nt,kt) = beta_norm (Drule.infer_instantiate ctxtGG
        [(("m",0), ctermGG mt),(("n",0), ctermGG nt),(("k",0), ctermGG kt)] mult_assoc_GG);
fun mult1l_GG t = beta_norm (Drule.infer_instantiate ctxtGG [(("n",0), ctermGG t)] mult_1_left_GG);
(* mult congruence on left / right argument *)
fun mult_cong_l_GG (pT,qT,kT) hpq =
  let val Pabs = Abs("z", natT, oeq (mult pT kT) (mult (Bound 0) kT))
  in oeq_rw_GG (Pabs, pT, qT) hpq (oeqRefl_GG (mult pT kT)) end;
fun mult_cong_r_GG (hT,pT,qT) hpq =
  let val Pabs = Abs("z", natT, oeq (mult hT pT) (mult hT (Bound 0)))
  in oeq_rw_GG (Pabs, pT, qT) hpq (oeqRefl_GG (mult hT pT)) end;

(* lt helpers on ctxtGG *)
fun lt_suc_GG_at nt = beta_norm (Drule.infer_instantiate ctxtGG [(("n",0), ctermGG nt)] lt_suc_GG);
fun lt_suc_cases_GG_at (mt,nt) hlt =
  Thm.implies_elim (beta_norm (Drule.infer_instantiate ctxtGG
        [(("m",0), ctermGG mt),(("n",0), ctermGG nt)] lt_suc_cases_GG)) hlt;
fun lt_trans_GG_at (at,bt,ct) h1 h2 =
  let val inst = beta_norm (Drule.infer_instantiate ctxtGG
        [(("a",0), ctermGG at),(("b",0), ctermGG bt),(("c",0), ctermGG ct)] lt_trans_GG)
  in Thm.implies_elim (Thm.implies_elim inst h1) h2 end;
fun lt_irrefl_GG_at nt hlt =
  Thm.implies_elim (beta_norm (Drule.infer_instantiate ctxtGG [(("n",0), ctermGG nt)] lt_irrefl_GG)) hlt;
fun le_refl_GG_at t = beta_norm (Drule.infer_instantiate ctxtGG [(("n",0), ctermGG t)] le_refl_GG);
fun le_trans_GG_at (mt,nt,kt) h1 h2 =
  let val inst = beta_norm (Drule.infer_instantiate ctxtGG
        [(("m",0), ctermGG mt),(("n",0), ctermGG nt),(("k",0), ctermGG kt)] le_trans_GG)
  in Thm.implies_elim (Thm.implies_elim inst h1) h2 end;
fun le_add_GG_at (mt,pt) = beta_norm (Drule.infer_instantiate ctxtGG
        [(("m",0), ctermGG mt),(("p",0), ctermGG pt)] le_add_GG);
(* le_intro on ctxtGG : from (hyp : oeq n (add m w)) build jT (le m n) *)
fun le_intro_GG (mT, nT, w) hyp =
  let val LAbs = Abs ("p", natT, oeq nT (add mT (Bound 0)))
  in exI_GG_at (LAbs, w) hyp end;

(* natlist op instantiators on ctxtGG *)
fun lprodNil_GG () = lprod_nil_GG;
fun lprodCons_GG (h,t) = beta_norm (Drule.infer_instantiate ctxtGG
      [(("x",0), ctermGG h),(("t",0), ctermGG t)] lprod_cons_GG);
fun llenNil_GG () = llen_nil_GG;
fun llenCons_GG (h,t) = beta_norm (Drule.infer_instantiate ctxtGG
      [(("x",0), ctermGG h),(("t",0), ctermGG t)] llen_cons_GG);
fun lmemNilElim_GG x = beta_norm (Drule.infer_instantiate ctxtGG [(("x",0), ctermGG x)] lmem_nil_elim_GG);
fun lmemConsFwd_GG (x,y,t) = beta_norm (Drule.infer_instantiate ctxtGG
      [(("x",0), ctermGG x),(("y",0), ctermGG y),(("t",0), ctermGG t)] lmem_cons_fwd_GG);
fun lmemConsBwd_GG (x,y,t) = beta_norm (Drule.infer_instantiate ctxtGG
      [(("x",0), ctermGG x),(("y",0), ctermGG y),(("t",0), ctermGG t)] lmem_cons_bwd_GG);
fun lremoveNil_GG x = beta_norm (Drule.infer_instantiate ctxtGG [(("x",0), ctermGG x)] lremove_nil_GG);
fun lremoveConsEq_GG (x,y,t) = beta_norm (Drule.infer_instantiate ctxtGG
      [(("x",0), ctermGG x),(("y",0), ctermGG y),(("t",0), ctermGG t)] lremove_cons_eq_GG);
fun lremoveConsNeq_GG (x,y,t) = beta_norm (Drule.infer_instantiate ctxtGG
      [(("x",0), ctermGG x),(("y",0), ctermGG y),(("t",0), ctermGG t)] lremove_cons_neq_GG);
fun lnodupConsFwd_GG (x,t) = beta_norm (Drule.infer_instantiate ctxtGG
      [(("x",0), ctermGG x),(("t",0), ctermGG t)] lnodup_cons_fwd_GG);
fun lnodupConsBwd_GG (x,t) = beta_norm (Drule.infer_instantiate ctxtGG
      [(("x",0), ctermGG x),(("t",0), ctermGG t)] lnodup_cons_bwd_GG);
val lnodupNil_GG = lnodup_nil_GG;
fun lmapNil_GG f = beta_norm (Drule.infer_instantiate ctxtGG [(("f",0), ctermGG f)] lmap_nil_GG);
fun lmapCons_GG (f,h,t) = beta_norm (Drule.infer_instantiate ctxtGG
      [(("f",0), ctermGG f),(("x",0), ctermGG h),(("t",0), ctermGG t)] lmap_cons_GG);
fun uptoZero_GG () = upto_zero_GG;
fun uptoSuc_GG nt = beta_norm (Drule.infer_instantiate ctxtGG [(("n",0), ctermGG nt)] upto_suc_GG);
fun lhdCons_GG (h,t) = beta_norm (Drule.infer_instantiate ctxtGG
      [(("x",0), ctermGG h),(("t",0), ctermGG t)] lhd_cons_GG);
fun ltlCons_GG (h,t) = beta_norm (Drule.infer_instantiate ctxtGG
      [(("x",0), ctermGG h),(("t",0), ctermGG t)] ltl_cons_GG);

(* congruence under list ops via leq_subst / oeq_subst *)
fun lprod_cong_GG (aT,bT) hab =
  let val Pabs = Abs("z", natlistT, oeq (lprod aT) (lprod (Bound 0)))
  in leq_rw_GG (Pabs, aT, bT) hab (oeqRefl_GG (lprod aT)) end;
fun llen_cong_GG (aT,bT) hab =
  let val Pabs = Abs("z", natlistT, oeq (llen aT) (llen (Bound 0)))
  in leq_rw_GG (Pabs, aT, bT) hab (oeqRefl_GG (llen aT)) end;
fun lmem_transfer_GG (yT, aT, bT) hleq hmem =
  let val Pabs = Abs("z", natlistT, lmem yT (Bound 0))
  in leq_rw_GG (Pabs, aT, bT) hleq hmem end;
fun lnodup_transfer_GG (aT, bT) hleq hnd =
  let val Pabs = Abs("z", natlistT, lnodup (Bound 0))
  in leq_rw_GG (Pabs, aT, bT) hleq hnd end;
fun lmap_transfer_GG (fT, aT, bT) hleq hp =
  (* rewrite a leq-equal list inside an lmap term : we mostly use lmap on lcons *)
  let val Pabs = Abs("z", natlistT, leq (lmap fT (Bound 0)) (lmap fT bT))
  in leq_rw_GG (Pabs, aT, bT) hleq hp end;

(* cong instantiators on ctxtGG *)
fun cong_refl_GG_at (mt, at) = beta_norm (Drule.infer_instantiate ctxtGG
      [(("m",0), ctermGG mt), (("a",0), ctermGG at)] cong_refl_GG);
fun cong_mult_GG_at (mt, at, a2t, bt, b2t) h1 h2 =
  let val inst = beta_norm (Drule.infer_instantiate ctxtGG
        [(("m",0), ctermGG mt), (("a",0), ctermGG at), (("a2",0), ctermGG a2t),
         (("b",0), ctermGG bt), (("b2",0), ctermGG b2t)] cong_mult_GG)
  in Thm.implies_elim (Thm.implies_elim inst h1) h2 end;
fun cong_trans_GG_at (mt, at, bt, ct) h1 h2 =
  let val inst = beta_norm (Drule.infer_instantiate ctxtGG
        [(("m",0), ctermGG mt), (("a",0), ctermGG at), (("b",0), ctermGG bt), (("c",0), ctermGG ct)] cong_trans_GG)
  in Thm.implies_elim (Thm.implies_elim inst h1) h2 end;
fun cong_sym_GG_at (mt, at, bt) h =
  Thm.implies_elim (beta_norm (Drule.infer_instantiate ctxtGG
        [(("m",0), ctermGG mt), (("a",0), ctermGG at), (("b",0), ctermGG bt)] cong_sym_GG)) h;
fun cong_cong_GG (mF, aF, a2F, bF, b2F) haa2 hbb2 hcong =
  let
    val zF1 = Free("zz1g", natT);
    val Pabs1 = Term.lambda zF1 (cong mF zF1 bF);
    val cong1 = oeq_rw_GG (Pabs1, aF, a2F) haa2 hcong;
    val zF2 = Free("zz2g", natT);
    val Pabs2 = Term.lambda zF2 (cong mF a2F zF2);
    val cong2 = oeq_rw_GG (Pabs2, bF, b2F) hbb2 cong1;
  in cong2 end;

val () = out "STAGE_P_HELPERS_READY\n";

(* ============================================================================
   (P1.1) extract : lmem x L ==> oeq (lprod L) (mult x (lprod (lremove x L)))
   ============================================================================ *)
val extract =
  let
    val xF = Free("x", natT);
    ...
    val ind = beta_norm (Drule.infer_instantiate ctxtGG
          [(("P",0), ctermGG Qpred), (("a",0), ctermGG LF)] list_induct_GG);
    val base =
      let
        val hmem = Thm.assume (ctermGG (jT (lmem xF lnilC)));
        val ff   = Thm.implies_elim (lmemNilElim_GG xF) hmem;
        val conc = Thm.implies_elim (oFalse_elim_GG_at (concBody lnilC)) ff;
        val dis  = Thm.implies_intr (ctermGG (jT (lmem xF lnilC))) conc;
      in impI_GG_at (lmem xF lnilC, concBody lnilC) dis end;
    val yF = Free("y", natT); val tF = Free("t", natlistT);
    val ihprop = jT (predBody tF);
    val IH = Thm.assume (ctermGG ihprop);
    val stepConcl =
      let
        val hmem = Thm.assume (ctermGG (jT (lmem xF (lcons yF tF))));
        val disjmem = Thm.implies_elim (lmemConsFwd_GG (xF, yF, tF)) hmem;
        val lpc = lprodCons_GG (yF, tF);
        val caseEq =
          let
            val heq = Thm.assume (ctermGG (jT (oeq xF yF)));
            val lrm = Thm.implies_elim (lremoveConsEq_GG (xF, yF, tF)) heq;
            val lp_lrm = lprod_cong_GG (lremove xF (lcons yF tF), tF) lrm;
            val rhs_eq = mult_cong_r_GG (xF, lprod (lremove xF (lcons yF tF)), lprod tF) lp_lrm;
            val yx = oeq_sym OF [heq];
            val my_mx = mult_cong_l_GG (yF, xF, lprod tF) yx;
            val rhs_eq_sym = oeq_sym OF [rhs_eq];
            val conc = oeq_trans OF [oeq_trans OF [lpc, my_mx], rhs_eq_sym];
          in Thm.implies_intr (ctermGG (jT (oeq xF yF))) conc end;
        val caseNeq =
          let
            val hneq = Thm.assume (ctermGG (jT (neg (oeq xF yF))));
            val memT =
              let
                val cA = let val hxy = Thm.assume (ctermGG (jT (oeq xF yF)))
                             val ff  = mp_GG_at (oeq xF yF, oFalseC) hneq hxy
                             val r   = Thm.implies_elim (oFalse_elim_GG_at (lmem xF tF)) ff
                         in Thm.implies_intr (ctermGG (jT (oeq xF yF))) r end;
                val cB = let val hm = Thm.assume (ctermGG (jT (lmem xF tF)))
                         in Thm.implies_intr (ctermGG (jT (lmem xF tF))) hm end;
              in disjE_GG_at (oeq xF yF, lmem xF tF, lmem xF tF) disjmem cA cB end;
            val ihconc = mp_GG_at (lmem xF tF, concBody tF) IH memT;
            val lrm = Thm.implies_elim (lremoveConsNeq_GG (xF, yF, tF)) hneq;
            val rmtl = lremove xF tF;
            val lp_lrm = lprod_cong_GG (lremove xF (lcons yF tF), lcons yF rmtl) lrm;
            val lp_cons = lprodCons_GG (yF, rmtl);
            val lp_lrm2 = oeq_trans OF [lp_lrm, lp_cons];
            val rhs1 = mult_cong_r_GG (xF, lprod (lremove xF (lcons yF tF)), mult yF (lprod rmtl)) lp_lrm2;
            val q = lprod rmtl;
            val mut = mult_cong_r_GG (yF, lprod tF, mult xF q) ihconc;
            val lhs1 = oeq_trans OF [lpc, mut];
            val assoc1 = multassoc_GG (yF, xF, q);
            val assoc1s = oeq_sym OF [assoc1];
            val comm = multcomm_GG (yF, xF);
            val commc = mult_cong_l_GG (mult yF xF, mult xF yF, q) comm;
            val assoc2 = multassoc_GG (xF, yF, q);
          ...
          in Thm.implies_intr (ctermGG (jT (neg (oeq xF yF)))) conc end;
        val em = ex_middle_GG_at (oeq xF yF);
        val conc = disjE_GG_at (oeq xF yF, neg (oeq xF yF), concBody (lcons yF tF)) em caseEq caseNeq;
        val dis = Thm.implies_intr (ctermGG (jT (lmem xF (lcons yF tF)))) conc;
      in impI_GG_at (lmem xF (lcons yF tF), concBody (lcons yF tF)) dis end;
    val step1 = Thm.forall_intr (ctermGG yF)
                  (Thm.forall_intr (ctermGG tF) (Thm.implies_intr (ctermGG ihprop) stepConcl));
    val r1 = Thm.implies_elim ind base;
    val r2 = Thm.implies_elim r1 step1;
    val hmemL = Thm.assume (ctermGG (jT (lmem xF LF)));
    val concL = mp_GG_at (lmem xF LF, concBody LF) r2 hmemL;
    val d1 = Thm.implies_intr (ctermGG (jT (lmem xF LF))) concL;
  in varify d1 end;
...
val r_extract = (length (Thm.hyps_of extract) = 0) andalso ((Thm.prop_of extract) aconv i_extract);
val () = if r_extract then out "OK extract\n" else out "FAIL extract\n";

(* ============================================================================
   (P1.2) mem_remove_fwd : lmem y (lremove x L) ==> lmem y L
   ============================================================================ *)
val mem_remove_fwd =
  let
    ...
    val base =
      let
        val hassm = Thm.assume (ctermGG (jT (lmem yF (lremove xF lnilC))));
        val lrm = lremoveNil_GG xF;
        val mem_lnil = lmem_transfer_GG (yF, lremove xF lnilC, lnilC) lrm hassm;
        val ff  = Thm.implies_elim (lmemNilElim_GG yF) mem_lnil;
        val conc = Thm.implies_elim (oFalse_elim_GG_at (lmem yF lnilC)) ff;
        val dis  = Thm.implies_intr (ctermGG (jT (lmem yF (lremove xF lnilC)))) conc;
      in impI_GG_at (lmem yF (lremove xF lnilC), lmem yF lnilC) dis end;
    val hF = Free("h", natT); val tF = Free("t", natlistT);
    val ihprop = jT (concBody tF);
    val IH = Thm.assume (ctermGG ihprop);
    val stepConcl =
      let
        val hassm = Thm.assume (ctermGG (jT (lmem yF (lremove xF (lcons hF tF)))));
        val caseEq =
          let
            val heq = Thm.assume (ctermGG (jT (oeq xF hF)));
            val lrm = Thm.implies_elim (lremoveConsEq_GG (xF, hF, tF)) heq;
            val mem_t = lmem_transfer_GG (yF, lremove xF (lcons hF tF), tF) lrm hassm;
            val dj  = disjI2_GG_at (oeq yF hF, lmem yF tF) mem_t;
            val res = Thm.implies_elim (lmemConsBwd_GG (yF, hF, tF)) dj;
          in Thm.implies_intr (ctermGG (jT (oeq xF hF))) res end;
        val caseNeq =
          let
            val hneq = Thm.assume (ctermGG (jT (neg (oeq xF hF))));
            val lrm = Thm.implies_elim (lremoveConsNeq_GG (xF, hF, tF)) hneq;
            val mem_cons = lmem_transfer_GG (yF, lremove xF (lcons hF tF), lcons hF (lremove xF tF)) lrm hassm;
            val dj = Thm.implies_elim (lmemConsFwd_GG (yF, hF, lremove xF tF)) mem_cons;
            val cA = let val hyh = Thm.assume (ctermGG (jT (oeq yF hF)))
                         val r = Thm.implies_elim (lmemConsBwd_GG (yF, hF, tF))
                                   (disjI1_GG_at (oeq yF hF, lmem yF tF) hyh)
                     in Thm.implies_intr (ctermGG (jT (oeq yF hF))) r end;
            val cB = let val hmr = Thm.assume (ctermGG (jT (lmem yF (lremove xF tF))))
                         val mt  = mp_GG_at (lmem yF (lremove xF tF), lmem yF tF) IH hmr
                         val r = Thm.implies_elim (lmemConsBwd_GG (yF, hF, tF))
                                   (disjI2_GG_at (oeq yF hF, lmem yF tF) mt)
                     in Thm.implies_intr (ctermGG (jT (lmem yF (lremove xF tF)))) r end;
            val res = disjE_GG_at (oeq yF hF, lmem yF (lremove xF tF), lmem yF (lcons hF tF)) dj cA cB;
          in Thm.implies_intr (ctermGG (jT (neg (oeq xF hF)))) res end;
        val em = ex_middle_GG_at (oeq xF hF);
        val conc = disjE_GG_at (oeq xF hF, neg (oeq xF hF), lmem yF (lcons hF tF)) em caseEq caseNeq;
        val dis = Thm.implies_intr (ctermGG (jT (lmem yF (lremove xF (lcons hF tF))))) conc;
      in impI_GG_at (lmem yF (lremove xF (lcons hF tF)), lmem yF (lcons hF tF)) dis end;
    val step1 = Thm.forall_intr (ctermGG hF)
                  (Thm.forall_intr (ctermGG tF) (Thm.implies_intr (ctermGG ihprop) stepConcl));
    val r1 = Thm.implies_elim ind base;
    val r2 = Thm.implies_elim r1 step1;
    val hassmL = Thm.assume (ctermGG (jT (lmem yF (lremove xF LF))));
    val concL = mp_GG_at (lmem yF (lremove xF LF), lmem yF LF) r2 hassmL;
    val d1 = Thm.implies_intr (ctermGG (jT (lmem yF (lremove xF LF)))) concL;
  in varify d1 end;
...
val r_mem_remove_fwd = (length (Thm.hyps_of mem_remove_fwd) = 0) andalso ((Thm.prop_of mem_remove_fwd) aconv i_mem_remove_fwd);
val () = if r_mem_remove_fwd then out "OK mem_remove_fwd\n" else out "FAIL mem_remove_fwd\n";

(* ============================================================================
   (P1.3) mem_remove_bwd : (Conj (lmem y L) (neg (oeq y x))) ==> lmem y (lremove x L)
   ============================================================================ *)
val mem_remove_bwd =
  let
    val xF = Free("x", natT); val yF = Free("y", natT);
    ...
    val base =
      let
        val hh = Thm.assume (ctermGG (jT (hypBody lnilC)));
        val mem = conjunct1_GG_at (lmem yF lnilC, neg (oeq yF xF)) hh;
        val ff  = Thm.implies_elim (lmemNilElim_GG yF) mem;
        val conc = Thm.implies_elim (oFalse_elim_GG_at (lmem yF (lremove xF lnilC))) ff;
        val dis = Thm.implies_intr (ctermGG (jT (hypBody lnilC))) conc;
      in impI_GG_at (hypBody lnilC, lmem yF (lremove xF lnilC)) dis end;
    val hF = Free("h", natT); val tF = Free("t", natlistT);
    val ihprop = jT (concBody tF);
    val IH = Thm.assume (ctermGG ihprop);
    val stepConcl =
      let
        val hh = Thm.assume (ctermGG (jT (hypBody (lcons hF tF))));
        val mem = conjunct1_GG_at (lmem yF (lcons hF tF), neg (oeq yF xF)) hh;
        val hneqYX = conjunct2_GG_at (lmem yF (lcons hF tF), neg (oeq yF xF)) hh;
        val djmem = Thm.implies_elim (lmemConsFwd_GG (yF, hF, tF)) mem;
        val caseEqXH =
          let
            val heq = Thm.assume (ctermGG (jT (oeq xF hF)));
            val lrm = Thm.implies_elim (lremoveConsEq_GG (xF, hF, tF)) heq;
            val mem_t =
              let
                val cA = let val hyh = Thm.assume (ctermGG (jT (oeq yF hF)))
                             ...
                             val ff = mp_GG_at (oeq yF xF, oFalseC) hneqYX yx
                             val r  = Thm.implies_elim (oFalse_elim_GG_at (lmem yF tF)) ff
                         in Thm.implies_intr (ctermGG (jT (oeq yF hF))) r end;
                val cB = let val hm = Thm.assume (ctermGG (jT (lmem yF tF)))
                         in Thm.implies_intr (ctermGG (jT (lmem yF tF))) hm end;
              in disjE_GG_at (oeq yF hF, lmem yF tF, lmem yF tF) djmem cA cB end;
            val lrm_s = leq_sym_GG lrm;
            val res = lmem_transfer_GG (yF, tF, lremove xF (lcons hF tF)) lrm_s mem_t;
          in Thm.implies_intr (ctermGG (jT (oeq xF hF))) res end;
        val caseNeqXH =
          let
            val hneqXH = Thm.assume (ctermGG (jT (neg (oeq xF hF))));
            val lrm = Thm.implies_elim (lremoveConsNeq_GG (xF, hF, tF)) hneqXH;
            val target = lcons hF (lremove xF tF);
            val mem_target =
              let
                val cA = let val hyh = Thm.assume (ctermGG (jT (oeq yF hF)))
                             val r = Thm.implies_elim (lmemConsBwd_GG (yF, hF, lremove xF tF))
                                       (disjI1_GG_at (oeq yF hF, lmem yF (lremove xF tF)) hyh)
                         in Thm.implies_intr (ctermGG (jT (oeq yF hF))) r end;
                val cB = let val hmt = Thm.assume (ctermGG (jT (lmem yF tF)))
                             val cj  = conjI_GG_at (lmem yF tF, neg (oeq yF xF)) hmt hneqYX
                             val mr  = mp_GG_at (hypBody tF, lmem yF (lremove xF tF)) IH cj
                             val r = Thm.implies_elim (lmemConsBwd_GG (yF, hF, lremove xF tF))
                                       (disjI2_GG_at (oeq yF hF, lmem yF (lremove xF tF)) mr)
                         in Thm.implies_intr (ctermGG (jT (lmem yF tF))) r end;
              in disjE_GG_at (oeq yF hF, lmem yF tF, lmem yF target) djmem cA cB end;
            val lrm_s = leq_sym_GG lrm;
            val res = lmem_transfer_GG (yF, target, lremove xF (lcons hF tF)) lrm_s mem_target;
          in Thm.implies_intr (ctermGG (jT (neg (oeq xF hF)))) res end;
        val em = ex_middle_GG_at (oeq xF hF);
        val conc = disjE_GG_at (oeq xF hF, neg (oeq xF hF), lmem yF (lremove xF (lcons hF tF))) em caseEqXH caseNeqXH;
        val dis = Thm.implies_intr (ctermGG (jT (hypBody (lcons hF tF)))) conc;
      in impI_GG_at (hypBody (lcons hF tF), lmem yF (lremove xF (lcons hF tF))) dis end;
    val step1 = Thm.forall_intr (ctermGG hF)
                  (Thm.forall_intr (ctermGG tF) (Thm.implies_intr (ctermGG ihprop) stepConcl));
    val r1 = Thm.implies_elim ind base;
    val r2 = Thm.implies_elim r1 step1;
    val hhL = Thm.assume (ctermGG (jT (hypBody LF)));
    val concL = mp_GG_at (hypBody LF, lmem yF (lremove xF LF)) r2 hhL;
    val d1 = Thm.implies_intr (ctermGG (jT (hypBody LF))) concL;
  in varify d1 end;

val i_mem_remove_bwd = Logic.mk_implies (jT (mkConj (lmem yVm LVm) (neg (oeq yVm xVm))),
      jT (lmem yVm (lremove xVm LVm)));
val r_mem_remove_bwd = (length (Thm.hyps_of mem_remove_bwd) = 0) andalso ((Thm.prop_of mem_remove_bwd) aconv i_mem_remove_bwd);
val () = if r_mem_remove_bwd then out "OK mem_remove_bwd\n" else out "FAIL mem_remove_bwd\n";

(* ============================================================================
   (P1.4) llen_remove : lmem x L ==> lt (llen (lremove x L)) (llen L)
   ============================================================================ *)
val llen_remove_eq =
  let
    val xF = Free("x", natT);
    ...
    val stepConcl =
      let
        val hmem = Thm.assume (ctermGG (jT (lmem xF (lcons hF tF))));
        val djmem = Thm.implies_elim (lmemConsFwd_GG (xF, hF, tF)) hmem;
        val llc = llenCons_GG (hF, tF);
        val caseEq =
          let
            val heq = Thm.assume (ctermGG (jT (oeq xF hF)));
            val lrm = Thm.implies_elim (lremoveConsEq_GG (xF, hF, tF)) heq;
            val lc  = llen_cong_GG (lremove xF (lcons hF tF), tF) lrm;
            val sc  = Suc_cong_GG OF [lc];
            val scs = oeq_sym OF [sc];
            val conc = oeq_trans OF [llc, scs];
          in Thm.implies_intr (ctermGG (jT (oeq xF hF))) conc end;
        val caseNeq =
          let
            val hneq = Thm.assume (ctermGG (jT (neg (oeq xF hF))));
            val memT =
              let
                val cA = let val hxh = Thm.assume (ctermGG (jT (oeq xF hF)))
                             val ff  = mp_GG_at (oeq xF hF, oFalseC) hneq hxh
                             val r   = Thm.implies_elim (oFalse_elim_GG_at (lmem xF tF)) ff
                         in Thm.implies_intr (ctermGG (jT (oeq xF hF))) r end;
                val cB = let val hm = Thm.assume (ctermGG (jT (lmem xF tF)))
                         in Thm.implies_intr (ctermGG (jT (lmem xF tF))) hm end;
              in disjE_GG_at (oeq xF hF, lmem xF tF, lmem xF tF) djmem cA cB end;
            val ihconc = mp_GG_at (lmem xF tF, concBody tF) IH memT;
            val lrm = Thm.implies_elim (lremoveConsNeq_GG (xF, hF, tF)) hneq;
            val lc  = llen_cong_GG (lremove xF (lcons hF tF), lcons hF (lremove xF tF)) lrm;
            val lcc = llenCons_GG (hF, lremove xF tF);
            val rhs_in = oeq_trans OF [lc, lcc];
            val rhs = Suc_cong_GG OF [rhs_in];
            val lhs_in = Suc_cong_GG OF [ihconc];
            val lhs = oeq_trans OF [llc, lhs_in];
            val rhs_s = oeq_sym OF [rhs];
            val conc = oeq_trans OF [lhs, rhs_s];
          in Thm.implies_intr (ctermGG (jT (neg (oeq xF hF)))) conc end;
        val em = ex_middle_GG_at (oeq xF hF);
        val conc = disjE_GG_at (oeq xF hF, neg (oeq xF hF), concBody (lcons hF tF)) em caseEq caseNeq;
        val dis = Thm.implies_intr (ctermGG (jT (lmem xF (lcons hF tF)))) conc;
      in impI_GG_at (lmem xF (lcons hF tF), concBody (lcons hF tF)) dis end;
    val step1 = Thm.forall_intr (ctermGG hF)
                  (Thm.forall_intr (ctermGG tF) (Thm.implies_intr (ctermGG ihprop) stepConcl));
    val r1 = Thm.implies_elim ind base;
    val r2 = Thm.implies_elim r1 step1;
...
val llen_remove =
  let
    ...
    val heq = Thm.assume (ctermGG (jT (lmem xF LF)));
    val eqv = toGG (varify llen_remove_eq);
    val streq = beta_norm (Drule.infer_instantiate ctxtGG
          [(("x",0), ctermGG xF), (("L",0), ctermGG LF)] eqv);
    val oeqLL = Thm.implies_elim streq heq;
    val aT = llen (lremove xF LF); val bT = llen LF;
    val a0r = add0r_GG (suc aT);
    val a0rs = oeq_sym OF [a0r];
    val body = oeq_trans OF [oeqLL, a0rs];
    val Pabs = Abs("p", natT, oeq bT (add (suc aT) (Bound 0)));
    val ltThm = exI_GG_at (Pabs, ZeroC) body;
    val d1 = Thm.implies_intr (ctermGG (jT (lmem xF LF))) ltThm;
  in varify d1 end;

val i_llen_remove = Logic.mk_implies (jT (lmem xVm LVm), jT (lt (llen (lremove xVm LVm)) (llen LVm)));
val r_llen_remove = (length (Thm.hyps_of llen_remove) = 0) andalso ((Thm.prop_of llen_remove) aconv i_llen_remove);
val () = if r_llen_remove then out "OK llen_remove\n" else out "FAIL llen_remove\n";

(* ============================================================================
   (P1.5) nodup_remove : lnodup L ==> lnodup (lremove x L)
   ============================================================================ *)
val mem_remove_fwd_GG = toGG mem_remove_fwd;
val nodup_remove =
  let
    val xF = Free("x", natT);
    ...
    val base =
      let
        val hnd = Thm.assume (ctermGG (jT (lnodup lnilC)));
        val lrm = lremoveNil_GG xF;
        val lrm_s = leq_sym_GG lrm;
        val res = lnodup_transfer_GG (lnilC, lremove xF lnilC) lrm_s hnd;
        val dis = Thm.implies_intr (ctermGG (jT (lnodup lnilC))) res;
      in impI_GG_at (lnodup lnilC, lnodup (lremove xF lnilC)) dis end;
    val hF = Free("h", natT); val tF = Free("t", natlistT);
    val ihprop = jT (concBody tF);
    val IH = Thm.assume (ctermGG ihprop);
    val stepConcl =
      let
        val hnd = Thm.assume (ctermGG (jT (lnodup (lcons hF tF))));
        val cj  = Thm.implies_elim (lnodupConsFwd_GG (hF, tF)) hnd;
        val nmem = conjunct1_GG_at (neg (lmem hF tF), lnodup tF) cj;
        val ndt  = conjunct2_GG_at (neg (lmem hF tF), lnodup tF) cj;
        val caseEq =
          let
            val heq = Thm.assume (ctermGG (jT (oeq xF hF)));
            val lrm = Thm.implies_elim (lremoveConsEq_GG (xF, hF, tF)) heq;
            val lrm_s = leq_sym_GG lrm;
            val res = lnodup_transfer_GG (tF, lremove xF (lcons hF tF)) lrm_s ndt;
          in Thm.implies_intr (ctermGG (jT (oeq xF hF))) res end;
        val caseNeq =
          let
            val hneq = Thm.assume (ctermGG (jT (neg (oeq xF hF))));
            val lrm = Thm.implies_elim (lremoveConsNeq_GG (xF, hF, tF)) hneq;
            val ndrt = mp_GG_at (lnodup tF, lnodup (lremove xF tF)) IH ndt;
            val mrf = beta_norm (Drule.infer_instantiate ctxtGG
                  [(("y",0), ctermGG hF), (("x",0), ctermGG xF), (("L",0), ctermGG tF)] mem_remove_fwd_GG);
            val nmem_rt =
              let val hassm = Thm.assume (ctermGG (jT (lmem hF (lremove xF tF))))
                  val inT   = Thm.implies_elim mrf hassm
                  val ff    = mp_GG_at (lmem hF tF, oFalseC) nmem inT
                  val dis   = Thm.implies_intr (ctermGG (jT (lmem hF (lremove xF tF)))) ff
              in impI_GG_at (lmem hF (lremove xF tF), oFalseC) dis end;
            val cj2 = conjI_GG_at (neg (lmem hF (lremove xF tF)), lnodup (lremove xF tF)) nmem_rt ndrt;
            val nd_target = Thm.implies_elim (lnodupConsBwd_GG (hF, lremove xF tF)) cj2;
            val lrm_s = leq_sym_GG lrm;
            val res = lnodup_transfer_GG (lcons hF (lremove xF tF), lremove xF (lcons hF tF)) lrm_s nd_target;
          in Thm.implies_intr (ctermGG (jT (neg (oeq xF hF)))) res end;
        val em = ex_middle_GG_at (oeq xF hF);
        val conc = disjE_GG_at (oeq xF hF, neg (oeq xF hF), lnodup (lremove xF (lcons hF tF))) em caseEq caseNeq;
        val dis = Thm.implies_intr (ctermGG (jT (lnodup (lcons hF tF)))) conc;
      in impI_GG_at (lnodup (lcons hF tF), lnodup (lremove xF (lcons hF tF))) dis end;
    val step1 = Thm.forall_intr (ctermGG hF)
                  (Thm.forall_intr (ctermGG tF) (Thm.implies_intr (ctermGG ihprop) stepConcl));
    val r1 = Thm.implies_elim ind base;
    val r2 = Thm.implies_elim r1 step1;
    val hndL = Thm.assume (ctermGG (jT (lnodup LF)));
    val concL = mp_GG_at (lnodup LF, lnodup (lremove xF LF)) r2 hndL;
    val d1 = Thm.implies_intr (ctermGG (jT (lnodup LF))) concL;
  in varify d1 end;

val i_nodup_remove = Logic.mk_implies (jT (lnodup LVm), jT (lnodup (lremove xVm LVm)));
val r_nodup_remove = (length (Thm.hyps_of nodup_remove) = 0) andalso ((Thm.prop_of nodup_remove) aconv i_nodup_remove);
val () = if r_nodup_remove then out "OK nodup_remove\n" else out "FAIL nodup_remove\n";

(* ============================================================================
   (P1.6) mem_remove_neq : lnodup L ==> lmem y (lremove x L) ==> neg (oeq y x)
   ============================================================================ *)
val mem_remove_neq =
  let
    val xF = Free("x", natT); val yF = Free("y", natT);
    ...
    val base =
      let
        val hmem0 = Thm.assume (ctermGG (jT (lmem yF (lremove xF lnilC))));
        val lrm = lremoveNil_GG xF;
        val mem_lnil = lmem_transfer_GG (yF, lremove xF lnilC, lnilC) lrm hmem0;
        val ff  = Thm.implies_elim (lmemNilElim_GG yF) mem_lnil;
        val conc = Thm.implies_elim (oFalse_elim_GG_at (neg (oeq yF xF))) ff;
        val d2 = Thm.implies_intr (ctermGG (jT (lmem yF (lremove xF lnilC)))) conc;
        val d1 = impI_GG_at (lmem yF (lremove xF lnilC), neg (oeq yF xF)) d2;
        val d0 = Thm.implies_intr (ctermGG (jT (lnodup lnilC))) d1;
      in impI_GG_at (lnodup lnilC, mkImp (lmem yF (lremove xF lnilC)) (neg (oeq yF xF))) d0 end;
    val hF = Free("h", natT); val tF = Free("t", natlistT);
    val ihprop = jT (concBody tF);
    val IH = Thm.assume (ctermGG ihprop);
    fun mkNegYX metaImpl = impI_GG_at (oeq yF xF, oFalseC) metaImpl;
    val stepConcl =
      let
        val hnd = Thm.assume (ctermGG (jT (lnodup (lcons hF tF))));
        val cj  = Thm.implies_elim (lnodupConsFwd_GG (hF, tF)) hnd;
        val nmemH = conjunct1_GG_at (neg (lmem hF tF), lnodup tF) cj;
        val ndt   = conjunct2_GG_at (neg (lmem hF tF), lnodup tF) cj;
        val hmem = Thm.assume (ctermGG (jT (lmem yF (lremove xF (lcons hF tF)))));
        val caseEq =
          let
            val heq = Thm.assume (ctermGG (jT (oeq xF hF)));
            val lrm = Thm.implies_elim (lremoveConsEq_GG (xF, hF, tF)) heq;
            val mem_t = lmem_transfer_GG (yF, lremove xF (lcons hF tF), tF) lrm hmem;
            val negYX =
              let val hyx = Thm.assume (ctermGG (jT (oeq yF xF)))
                  val yh  = oeq_trans OF [hyx, heq]
                  val Pr  = Abs("z", natT, lmem (Bound 0) tF)
                  val memH= oeq_rw_GG (Pr, yF, hF) yh mem_t
                  val ff  = mp_GG_at (lmem hF tF, oFalseC) nmemH memH
              in mkNegYX (Thm.implies_intr (ctermGG (jT (oeq yF xF))) ff) end;
          in Thm.implies_intr (ctermGG (jT (oeq xF hF))) negYX end;
        val caseNeq =
          let
            val hneq = Thm.assume (ctermGG (jT (neg (oeq xF hF))));
            val lrm = Thm.implies_elim (lremoveConsNeq_GG (xF, hF, tF)) hneq;
            val mem_cons = lmem_transfer_GG (yF, lremove xF (lcons hF tF), lcons hF (lremove xF tF)) lrm hmem;
            val dj = Thm.implies_elim (lmemConsFwd_GG (yF, hF, lremove xF tF)) mem_cons;
            val cA = let val hyh = Thm.assume (ctermGG (jT (oeq yF hF)))
                         val negYX =
                           let val hyx = Thm.assume (ctermGG (jT (oeq yF xF)))
                               val hy_s= oeq_sym OF [hyh]
                               val hx  = oeq_trans OF [hy_s, hyx]
                               val hxh = oeq_sym OF [hx]
                               val ff  = mp_GG_at (oeq xF hF, oFalseC) hneq hxh
                           in mkNegYX (Thm.implies_intr (ctermGG (jT (oeq yF xF))) ff) end;
                     in Thm.implies_intr (ctermGG (jT (oeq yF hF))) negYX end;
            val cB = let val hmt = Thm.assume (ctermGG (jT (lmem yF (lremove xF tF))))
                         val ihA = mp_GG_at (lnodup tF, mkImp (lmem yF (lremove xF tF)) (neg (oeq yF xF))) IH ndt
                         val res = mp_GG_at (lmem yF (lremove xF tF), neg (oeq yF xF)) ihA hmt
                     in Thm.implies_intr (ctermGG (jT (lmem yF (lremove xF tF)))) res end;
            val res = disjE_GG_at (oeq yF hF, lmem yF (lremove xF tF), neg (oeq yF xF)) dj cA cB;
          in Thm.implies_intr (ctermGG (jT (neg (oeq xF hF)))) res end;
        val em = ex_middle_GG_at (oeq xF hF);
        val negThm = disjE_GG_at (oeq xF hF, neg (oeq xF hF), neg (oeq yF xF)) em caseEq caseNeq;
        val d2 = Thm.implies_intr (ctermGG (jT (lmem yF (lremove xF (lcons hF tF))))) negThm;
        val d1 = impI_GG_at (lmem yF (lremove xF (lcons hF tF)), neg (oeq yF xF)) d2;
        val d0 = Thm.implies_intr (ctermGG (jT (lnodup (lcons hF tF)))) d1;
      in impI_GG_at (lnodup (lcons hF tF), mkImp (lmem yF (lremove xF (lcons hF tF))) (neg (oeq yF xF))) d0 end;
    val step1 = Thm.forall_intr (ctermGG hF)
                  (Thm.forall_intr (ctermGG tF) (Thm.implies_intr (ctermGG ihprop) stepConcl));
    val r1 = Thm.implies_elim ind base;
    val r2 = Thm.implies_elim r1 step1;
    val hndL = Thm.assume (ctermGG (jT (lnodup LF)));
    val hmemL = Thm.assume (ctermGG (jT (lmem yF (lremove xF LF))));
    val resA = mp_GG_at (lnodup LF, mkImp (lmem yF (lremove xF LF)) (neg (oeq yF xF))) r2 hndL;
    val resB = mp_GG_at (lmem yF (lremove xF LF), neg (oeq yF xF)) resA hmemL;
    val d2 = Thm.implies_intr (ctermGG (jT (lmem yF (lremove xF LF)))) resB;
    val d1 = Thm.implies_intr (ctermGG (jT (lnodup LF))) d2;
  in varify d1 end;
...
(* ============================================================================
   (P1.7) list_cases : !!L. Disj (leq L lnil)(leq L (lcons (lhd L)(ltl L)))
   ============================================================================ *)
...
val list_cases_thm =
  let
    ...
    val ind = beta_norm (Drule.infer_instantiate ctxtGG
          [(("P",0), ctermGG Qpred), (("a",0), ctermGG LF)] list_induct_GG);
    val base =
      let val nilRefl = leqRefl_GG lnilC
      in disjI1_GG_at (leq lnilC lnilC, leq lnilC (lcons (lhd lnilC)(ltl lnilC))) nilRefl end;
    val hF = Free("h_c", natT); val tF = Free("t_c", natlistT);
    val ihprop = jT (casesBody tF);
    val stepConcl =
      let
        val consL = lcons hF tF;
        val refl0 = leqRefl_GG consL;
        val hLhd = lhdCons_GG (hF, tF);
        val hLhd_s = oeq_sym OF [hLhd];
        val hLtl = ltlCons_GG (hF, tF);
        val hLtl_s = leq_sym_GG hLtl;
        val P1 = Abs("z", natT, leq consL (lcons (Bound 0) tF));
        val r1 = oeq_rw_GG (P1, hF, lhd consL) hLhd_s refl0;
        val P2 = Abs("z", natlistT, leq consL (lcons (lhd consL) (Bound 0)));
        val r2 = leq_rw_GG (P2, tF, ltl consL) hLtl_s r1;
      in disjI2_GG_at (leq consL lnilC, leq consL (lcons (lhd consL)(ltl consL))) r2 end;
    val step1 = Thm.forall_intr (ctermGG hF)
                  (Thm.forall_intr (ctermGG tF) (Thm.implies_intr (ctermGG ihprop) stepConcl));
    val r1 = Thm.implies_elim ind base;
    val r2 = Thm.implies_elim r1 step1;
...
fun list_cases (LT, goalC) caseNilFn caseConsFn =
  let
    ...
    val casesAtL = beta_norm (Drule.infer_instantiate ctxtGG
          [(("L_c",0), ctermGG LT)] (varify (Thm.forall_intr (ctermGG LFc) casesL)));
    val nilP  = leq LT lnilC;
    val consP = leq LT (lcons (lhd LT)(ltl LT));
    val cA = let val h = Thm.assume (ctermGG (jT nilP))
             in Thm.implies_intr (ctermGG (jT nilP)) (caseNilFn h) end;
    val cB = let val h = Thm.assume (ctermGG (jT consP))
             in Thm.implies_intr (ctermGG (jT consP)) (caseConsFn (lhd LT, ltl LT) h) end;
  in disjE_GG_at (nilP, consP, goalC) casesAtL cA cB end;

val r_list_cases =
  let val (LFc, casesL) = list_cases_thm;
      val th = varify (Thm.forall_intr (ctermGG LFc) casesL);
  in (length (Thm.hyps_of th) = 0) end;
val () = if r_list_cases then out "OK list_cases_thm\n" else out "FAIL list_cases_thm\n";

(* ============================================================================
   (P1.8) list_strong_induct (on llen L), parameterised by PhiBody/stepFn.
   ============================================================================ *)
(* lt_zero_elim : jT (lt m Zero) ==> jT goalC *)
fun lt_zero_elim_GG mT goalC hlt =
  let
    val Pabs = Abs("p", natT, oeq ZeroC (add (suc mT) (Bound 0)));
    fun body w (hw : thm) =
      let
        val aS    = addSuc_GG (mT, w);
        val z_S   = oeq_trans OF [hw, aS];
        val S_z   = oeq_sym OF [z_S];
        val fls   = Suc_neq_Zero_GG_at (add mT w) S_z;
      in Thm.implies_elim (oFalse_elim_GG_at goalC) fls end;
  in exE_GG_at (Pabs, goalC) hlt "w_lz" body end;
  ...
  let
    ...
    val base =
      let val LB = Free("L_b", natlistT)
          val hlt = Thm.assume (ctermGG (jT (lt (llen LB) ZeroC)))
          val res = lt_zero_elim_GG (llen LB) (PhiBody LB) hlt
      in Thm.forall_intr (ctermGG LB) (Thm.implies_intr (ctermGG (jT (lt (llen LB) ZeroC))) res) end;
    val step =
      let
        val xS = Free("x_lsi", natT)
        val auxX = auxBody xS
        val IHmeta = Thm.assume (ctermGG auxX)
        fun applyAUXx L2 hlt = Thm.implies_elim (Thm.forall_elim (ctermGG L2) IHmeta) hlt
        val LS = Free("L_s", natlistT)
        val hltS = Thm.assume (ctermGG (jT (lt (llen LS) (suc xS))))
        val dThm = lt_suc_cases_GG_at (llen LS, xS) hltS
        val goalC = PhiBody LS
        val caseA = let val hA = Thm.assume (ctermGG (jT (lt (llen LS) xS)))
                    in Thm.implies_intr (ctermGG (jT (lt (llen LS) xS))) (applyAUXx LS hA) end
        val caseB =
          let val hB = Thm.assume (ctermGG (jT (oeq (llen LS) xS)))
              fun applyIH L2 (h_lt:thm) =
                let val zpr = Free("z_pr", natT)
                    val Pr = Term.lambda zpr (lt (llen L2) zpr)
                    val h_lt_x = oeq_rw_GG (Pr, llen LS, xS) hB h_lt
                in applyAUXx L2 h_lt_x end
              val r = stepFn LS applyIH
          in Thm.implies_intr (ctermGG (jT (oeq (llen LS) xS))) r end
        val inst = beta_norm (Drule.infer_instantiate ctxtGG
              [(("A",0), ctermGG (lt (llen LS) xS)),(("B",0), ctermGG (oeq (llen LS) xS)),
               (("C",0), ctermGG goalC)] disjE_GG)
        val pm = Thm.implies_elim (Thm.implies_elim (Thm.implies_elim inst dThm) caseA) caseB
        val dischLt = Thm.implies_intr (ctermGG (jT (lt (llen LS) (suc xS)))) pm
        val auxSucx = Thm.forall_intr (ctermGG LS) dischLt
      in Thm.forall_intr (ctermGG xS) (Thm.implies_intr (ctermGG auxX) auxSucx) end;
    val LF = Free("L_fin", natlistT)
    val kFin = suc (llen LF)
    val indK = beta_norm (Drule.infer_instantiate ctxtGG
                 [(("Phi",0), ctermGG PhiMetaAbs),(("k",0), ctermGG kFin)] meta_nat_induct_GG)
    val auxK = Thm.implies_elim (Thm.implies_elim indK base) step
    val auxKL = Thm.forall_elim (ctermGG LF) auxK
    val resLF = Thm.implies_elim auxKL (lt_suc_GG_at (llen LF))
  in (LF, resLF) end;
...
val lsi_smoke =
  let
    ...
    fun stepFn LF applyIH = oeqRefl_GG (lprod LF);
    val (LF, res) = list_strong_induct PhiBody stepFn;
  in varify (Thm.forall_intr (ctermGG LF) res) end;
val () = if (length (Thm.hyps_of lsi_smoke) = 0) then out "OK list_strong_induct (smoke)\n"
         else out "FAIL list_strong_induct smoke\n";

(* ============================================================================
   (P1.9) lmem_upto_bwd / lmem_upto_fwd / lnodup_upto  (induction on n)
   ============================================================================ *)
(* nat induction wrapper on ctxtGG : nat_induct_GG_at (Pabs, k) base step *)
fun nat_induct_GG_run Pabs kF base step =
  let val inst = beta_norm (Drule.infer_instantiate ctxtGG
        [(("P",0), ctermGG Pabs), (("k",0), ctermGG kF)] nat_induct_GG)
  in beta_norm (Thm.implies_elim (Thm.implies_elim inst base) step) end;

(* small le helpers *)
fun lt_0_suc_GG nt =    (* lt 0 (Suc n) = le (Suc 0)(Suc n) ; le_add (Suc 0) n then rewrite (Suc 0 + n) -> Suc n? simpler: Suc 0 <= Suc n by le_add and add. Actually: lt 0 (Suc n) means le 1 (Suc n). 1 <= Suc n : exI witness n with oeq (Suc n)(add 1 n). add 1 n = Suc(0+n)=Suc n. *)
  let
    val aS = addSuc_GG (ZeroC, nt);                 (* (Suc 0 + n)=Suc(0+n) *)
    val a0 = beta_norm (Drule.infer_instantiate ctxtGG [(("n",0), ctermGG nt)] (varifyGG add_0)); (* (0+n)=n *)
    val sc = Succong_GG a0;                          (* Suc(0+n)=Suc n *)
    val sum = oeq_trans OF [aS, sc];                 (* (Suc 0 + n)=Suc n *)
    val sum_s = oeq_sym OF [sum];                    (* Suc n = (Suc 0 + n) *)
  in le_intro_GG (suc ZeroC, suc nt, nt) sum_s end;  (* le (Suc 0)(Suc n) = lt 0 (Suc n) *)
fun le_self_suc_GG nt =   (* le n (Suc n) : witness Suc 0, oeq (Suc n)(add n (Suc 0)) *)
  let
    val aSr = beta_norm (Drule.infer_instantiate ctxtGG
                [(("m",0), ctermGG nt), (("n",0), ctermGG ZeroC)] (varifyGG add_Suc_right)); (* (n + Suc 0)=Suc(n+0) *)
    val a0r = add0r_GG nt;                           (* (n+0)=n *)
    val sa0r = Succong_GG a0r;                       (* Suc(n+0)=Suc n *)
    val sum = oeq_trans OF [aSr, sa0r];              (* (n + Suc 0)=Suc n *)
    val sum_s = oeq_sym OF [sum];                    (* Suc n = (n + Suc 0) *)
  in le_intro_GG (nt, suc nt, suc ZeroC) sum_s end;  (* le n (Suc n) *)

(* add_cong_r on ctxtGG : oeq p q ==> oeq (add h p)(add h q) *)
fun add_cong_r_GG (hT, pT, qT) hpq =
  let val Pabs = Abs("z", natT, oeq (add hT pT) (add hT (Bound 0)))
  in oeq_rw_GG (Pabs, pT, qT) hpq (oeqRefl_GG (add hT pT)) end;
val add_eq_zero_left_GG = varifyGG add_eq_zero_left;
fun add_eq_zero_left_GG_at (at,bt) heq =
  let val inst = beta_norm (Drule.infer_instantiate ctxtGG
        [(("a",0), ctermGG at),(("b",0), ctermGG bt)] add_eq_zero_left_GG)
  in inst OF [heq] end;
val Suc_inj_GG = varifyGG Suc_inj_ax;
fun Suc_inj_GG_at (at,bt) h =
  Thm.implies_elim (beta_norm (Drule.infer_instantiate ctxtGG
        [(("a",0), ctermGG at),(("b",0), ctermGG bt)] Suc_inj_GG)) h;
val disj_zero_or_suc_GG = varifyGG disj_zero_or_suc;
fun dzos_GG t = beta_norm (Drule.infer_instantiate ctxtGG [(("p",0), ctermGG t)] disj_zero_or_suc_GG);
fun mkExSuc_GG t = mkEx (Abs ("q", natT, oeq t (suc (Bound 0))));
fun addSr_GG (mt,nt) = beta_norm (Drule.infer_instantiate ctxtGG
      [(("m",0), ctermGG mt),(("n",0), ctermGG nt)] (varifyGG add_Suc_right));
...
val le_suc_split =
  let
    ...
    val hle  = Thm.assume (ctermGG hypP);
    ...
    fun body d (hd:thm) =
      let
        val dz = dzos_GG d;
        val caseZ =
          let val hz = Thm.assume (ctermGG (jT (oeq d ZeroC)))
              val c1 = add_cong_r_GG (bF, d, ZeroC) hz
              val b0 = add0r_GG bF
              val c2 = oeq_trans OF [c1, b0]
              val Sn_b = oeq_trans OF [hd, c2]
              val b_Sn = oeq_sym OF [Sn_b]
          in Thm.implies_intr (ctermGG (jT (oeq d ZeroC)))
               (disjI1_GG_at (oeq bF (suc nF), le bF nF) b_Sn) end;
        val caseS =
          let val hsP = jT (mkExSuc_GG d)
              val hs  = Thm.assume (ctermGG hsP)
              val Pq = Abs("q", natT, oeq d (suc (Bound 0)))
              fun bodyS e (he:thm) =
                let
                  val c1 = add_cong_r_GG (bF, d, suc e) he
                  val Sn_bSe = oeq_trans OF [hd, c1]
                  val bSe = addSr_GG (bF, e)
                  val Sn_Sbe = oeq_trans OF [Sn_bSe, bSe]
                  val n_be = Suc_inj_GG_at (nF, add bF e) Sn_Sbe
                  val le_bn = le_intro_GG (bF, nF, e) n_be
                in disjI2_GG_at (oeq bF (suc nF), le bF nF) le_bn end
              val res = exE_GG_at (Pq, goalC) hs "eSS" bodyS
          in Thm.implies_intr (ctermGG (jT (mkExSuc_GG d))) res end;
      in disjE_GG_at (oeq d ZeroC, mkExSuc_GG d, goalC) dz caseZ caseS end;
    val res = exE_GG_at (Pabs, goalC) hle "dSS" body;
  in varify (Thm.implies_intr (ctermGG hypP) res) end;
val () = if length (Thm.hyps_of le_suc_split) = 0 then out "OK le_suc_split\n" else out "FAIL le_suc_split\n";
val le_suc_split_GG = toGG le_suc_split;
fun le_suc_split_at (bt,nt) hle =
  Thm.implies_elim (beta_norm (Drule.infer_instantiate ctxtGG
      [(("b",0), ctermGG bt),(("n",0), ctermGG nt)] le_suc_split_GG)) hle;
  ...
  let
    ...
    val base =
      let
        ...
        val hA   = Thm.assume (ctermGG hypA)
        val hpos = conjunct1_GG_at (lt ZeroC bF, le bF ZeroC) hA
        val hleb0= conjunct2_GG_at (lt ZeroC bF, le bF ZeroC) hA
        val Pd = Abs("d", natT, oeq ZeroC (add bF (Bound 0)))
        fun bd d (hd:thm) =
          let
            val hbd = oeq_sym OF [hd]                            (* oeq (add b d) 0 *)
            val b0  = add_eq_zero_left_GG_at (bF, d) hbd          (* oeq b 0 *)
            val zLt = Free("z_lt", natT)
            val Plt = Term.lambda zLt (lt ZeroC zLt)
            val lt00 = oeq_rw_GG (Plt, bF, ZeroC) b0 hpos        (* lt 0 0 = le 1 0 *)
            val Pe = Abs("e", natT, oeq ZeroC (add (suc ZeroC) (Bound 0)))
            fun be e (he:thm) =
              let val aS = addSuc_GG (ZeroC, e)
                  val z_S = oeq_trans OF [he, aS]
                  val S_z = oeq_sym OF [z_S]
                  val fls = Suc_neq_Zero_GG_at (add ZeroC e) S_z
              in Thm.implies_elim (oFalse_elim_GG_at (lmem bF (uptoF ZeroC))) fls end
            val res = exE_GG_at (Pe, lmem bF (uptoF ZeroC)) lt00 "e_b0" be
          in res end
        val resmem = exE_GG_at (Pd, lmem bF (uptoF ZeroC)) hleb0 "d_b0" bd
      in impI_GG_at (mkConj (lt ZeroC bF)(le bF ZeroC), lmem bF (uptoF ZeroC)) (Thm.implies_intr (ctermGG hypA) resmem) end;
    val step =
      let
        val xF = Free("x_iu", natT)
        val ihP = jT (mkImp (mkConj (lt ZeroC bF)(le bF xF)) (lmem bF (uptoF xF)))
        val hIH = Thm.assume (ctermGG ihP)
        val hypA = jT (mkConj (lt ZeroC bF)(le bF (suc xF)))
        val hA   = Thm.assume (ctermGG hypA)
        val hpos = conjunct1_GG_at (lt ZeroC bF, le bF (suc xF)) hA
        val hle  = conjunct2_GG_at (lt ZeroC bF, le bF (suc xF)) hA
        val dj = le_suc_split_at (bF, xF) hle
        val goalMem = lmem bF (uptoF (suc xF))
        val hupto = uptoSuc_GG xF
        val hupto_s = leq_sym_GG hupto
        val consL = lcons (suc xF) (uptoF xF)
        val caseEq =
          let val heq = Thm.assume (ctermGG (jT (oeq bF (suc xF))))
              val dmem = disjI1_GG_at (oeq bF (suc xF), lmem bF (uptoF xF)) heq
              val memCons = Thm.implies_elim (lmemConsBwd_GG (bF, suc xF, uptoF xF)) dmem
              val Pmem = Abs("z", natlistT, lmem bF (Bound 0))
              val res = leq_rw_GG (Pmem, consL, uptoF (suc xF)) hupto_s memCons
          in Thm.implies_intr (ctermGG (jT (oeq bF (suc xF)))) res end
        val caseLe =
          let val hlex = Thm.assume (ctermGG (jT (le bF xF)))
              val conjx = conjI_GG_at (lt ZeroC bF, le bF xF) hpos hlex
              val memUx = mp_GG_at (mkConj (lt ZeroC bF)(le bF xF), lmem bF (uptoF xF)) hIH conjx
              val dmem = disjI2_GG_at (oeq bF (suc xF), lmem bF (uptoF xF)) memUx
              val memCons = Thm.implies_elim (lmemConsBwd_GG (bF, suc xF, uptoF xF)) dmem
              val Pmem = Abs("z", natlistT, lmem bF (Bound 0))
              val res = leq_rw_GG (Pmem, consL, uptoF (suc xF)) hupto_s memCons
          in Thm.implies_intr (ctermGG (jT (le bF xF))) res end
        val memThm = disjE_GG_at (oeq bF (suc xF), le bF xF, goalMem) dj caseEq caseLe
        val impThm = impI_GG_at (mkConj (lt ZeroC bF)(le bF (suc xF)), goalMem)
                       (Thm.implies_intr (ctermGG hypA) memThm)
      in Thm.forall_intr (ctermGG xF) (Thm.implies_intr (ctermGG ihP) impThm) end;
    val kF = Free("k_iu", natT)
    val concl = nat_induct_GG_run Pabs kF base step
  in varify concl end;
val bVu = Var(("b",0),natT); val nVu = Var(("k_iu",0),natT);
val i_lmem_upto_bwd = jT (mkImp (mkConj (lt ZeroC bVu)(le bVu nVu)) (lmem bVu (uptoF nVu)));
val r_lmem_upto_bwd = (length (Thm.hyps_of lmem_upto_bwd) = 0) andalso ((Thm.prop_of lmem_upto_bwd) aconv i_lmem_upto_bwd);
val () = if r_lmem_upto_bwd then out "OK lmem_upto_bwd\n" else out "FAIL lmem_upto_bwd\n";
fun lmem_upto_bwd_at (bt,nt) hconj =
  let val inst = beta_norm (Drule.infer_instantiate ctxtGG
          [(("b",0), ctermGG bt),(("k_iu",0), ctermGG nt)] (toGG lmem_upto_bwd))
  in mp_GG_at (mkConj (lt ZeroC bt)(le bt nt), lmem bt (uptoF nt)) inst hconj end;
  ...
  let
    ...
    val base =
      let
        ...
        val hmem  = Thm.assume (ctermGG hmemP)
        val hu0   = uptoZero_GG ()
        val Pmem = Abs("z", natlistT, lmem bF (Bound 0))
        val memNil = leq_rw_GG (Pmem, uptoF ZeroC, lnilC) hu0 hmem
        val fls = Thm.implies_elim (lmemNilElim_GG bF) memNil
        val res = Thm.implies_elim (oFalse_elim_GG_at goalC) fls
      in impI_GG_at (lmem bF (uptoF ZeroC), goalC) (Thm.implies_intr (ctermGG hmemP) res) end;
    val step =
      let
        ...
        val consL = lcons (suc xF) (uptoF xF)
        val Pmem = Abs("z", natlistT, lmem bF (Bound 0))
        val memCons = leq_rw_GG (Pmem, uptoF (suc xF), consL) hupto hmem
        val dj = Thm.implies_elim (lmemConsFwd_GG (bF, suc xF, uptoF xF)) memCons
        val caseEq =
          let val heq = Thm.assume (ctermGG (jT (oeq bF (suc xF))))
              val lt0Sx = lt_0_suc_GG xF
              val heq_s = oeq_sym OF [heq]
              val zlf = Free("z_lf",natT)
              val Plt = Term.lambda zlf (lt ZeroC zlf)
              val lt0b = oeq_rw_GG (Plt, suc xF, bF) heq_s lt0Sx
              val leRefl = le_refl_GG_at (suc xF)
              val zle = Free("z_le",natT)
              val Ple = Term.lambda zle (le zle (suc xF))
              val leb = oeq_rw_GG (Ple, suc xF, bF) heq_s leRefl
              val cj = conjI_GG_at (lt ZeroC bF, le bF (suc xF)) lt0b leb
          in Thm.implies_intr (ctermGG (jT (oeq bF (suc xF)))) cj end
        val caseMem =
          let val hmemx = Thm.assume (ctermGG (jT (lmem bF (uptoF xF))))
              val cjx = mp_GG_at (lmem bF (uptoF xF), mkConj (lt ZeroC bF)(le bF xF)) hIH hmemx
              val lt0b = conjunct1_GG_at (lt ZeroC bF, le bF xF) cjx
              val lebx = conjunct2_GG_at (lt ZeroC bF, le bF xF) cjx
              val lex_Sx = le_self_suc_GG xF
              val leb_Sx = le_trans_GG_at (bF, xF, suc xF) lebx lex_Sx
              val cj = conjI_GG_at (lt ZeroC bF, le bF (suc xF)) lt0b leb_Sx
          in Thm.implies_intr (ctermGG (jT (lmem bF (uptoF xF)))) cj end
        val cjThm = disjE_GG_at (oeq bF (suc xF), lmem bF (uptoF xF), goalC) dj caseEq caseMem
        val impThm = impI_GG_at (lmem bF (uptoF (suc xF)), goalC)
                       (Thm.implies_intr (ctermGG hmemP) cjThm)
      in Thm.forall_intr (ctermGG xF) (Thm.implies_intr (ctermGG ihP) impThm) end;
    val kF = Free("k_if", natT)
    val concl = nat_induct_GG_run Pabs kF base step
  in varify concl end;
val nVf = Var(("k_if",0),natT);
val i_lmem_upto_fwd = jT (mkImp (lmem bVu (uptoF nVf)) (mkConj (lt ZeroC bVu)(le bVu nVf)));
val r_lmem_upto_fwd = (length (Thm.hyps_of lmem_upto_fwd) = 0) andalso ((Thm.prop_of lmem_upto_fwd) aconv i_lmem_upto_fwd);
val () = if r_lmem_upto_fwd then out "OK lmem_upto_fwd\n" else out "FAIL lmem_upto_fwd\n";
fun lmem_upto_fwd_at (bt,nt) hmem =
  let val inst = beta_norm (Drule.infer_instantiate ctxtGG
          [(("b",0), ctermGG bt),(("k_if",0), ctermGG nt)] (toGG lmem_upto_fwd))
  in mp_GG_at (lmem bt (uptoF nt), mkConj (lt ZeroC bt)(le bt nt)) inst hmem end;

(* ============================================================================
   lnodup_upto : lnodup (uptoF n)   (induction on n)
   ============================================================================ *)
val lnodup_upto =
  let
    ...
    val base =
      let
        val hu0 = uptoZero_GG ()
        val hu0s = leq_sym_GG hu0
        val ndNil = lnodupNil_GG
      in lnodup_transfer_GG (lnilC, uptoF ZeroC) hu0s ndNil end;
    val step =
      let
        ...
        val hIH = Thm.assume (ctermGG ihP)
        val notmem =
          let val hmem = Thm.assume (ctermGG (jT (lmem (suc xF) (uptoF xF))))
              val cj = lmem_upto_fwd_at (suc xF, xF) hmem
              val leSxx = conjunct2_GG_at (lt ZeroC (suc xF), le (suc xF) xF) cj
              val fls = lt_irrefl_GG_at xF leSxx
              val metaImp = Thm.implies_intr (ctermGG (jT (lmem (suc xF) (uptoF xF)))) fls
          in impI_GG_at (lmem (suc xF) (uptoF xF), oFalseC) metaImp end;
        val conjND = conjI_GG_at (neg (lmem (suc xF) (uptoF xF)), lnodup (uptoF xF)) notmem hIH
        val ndCons = Thm.implies_elim (lnodupConsBwd_GG (suc xF, uptoF xF)) conjND
        val hupto = uptoSuc_GG xF
        val hupto_s = leq_sym_GG hupto
        val res = lnodup_transfer_GG (lcons (suc xF) (uptoF xF), uptoF (suc xF)) hupto_s ndCons
      in Thm.forall_intr (ctermGG xF) (Thm.implies_intr (ctermGG ihP) res) end;
    val kF = Free("k_nd", natT)
    val concl = nat_induct_GG_run Pabs kF base step
  in varify concl end;
val nVn = Var(("k_nd",0),natT);
val i_lnodup_upto = jT (lnodup (uptoF nVn));
val r_lnodup_upto = (length (Thm.hyps_of lnodup_upto) = 0) andalso ((Thm.prop_of lnodup_upto) aconv i_lnodup_upto);
val () = if r_lnodup_upto then out "OK lnodup_upto\n" else out "FAIL lnodup_upto\n";

(* ============================================================================
   NATLIST LIBRARY VERDICT
   ============================================================================ *)
val natlistLibOK =
  r_extract andalso r_mem_remove_fwd andalso r_mem_remove_bwd andalso r_mem_remove_neq
  andalso r_llen_remove andalso r_nodup_remove andalso r_list_cases
  andalso r_lmem_upto_bwd andalso r_lmem_upto_fwd andalso r_lnodup_upto;
val () = if natlistLibOK then out "NATLIST_LIB_OK\n" else out "NATLIST_LIB_FAILED\n";

(* ############################################################################
   ###################  (P2)  lprod_perm_of_inj  ##############################
   ----------------------------------------------------------------------------
   THE permutation-product (2nd crux).  Two-list injection form:
     lnodup A ==> lnodup B ==> oeq (llen A)(llen B) ==>
     (forall x. lmem x A ==> lmem (g x) B) ==>
     (forall x y. lmem x A ==> lmem y A ==> oeq (g x)(g y) ==> oeq x y) ==>
     oeq (lprod (lmap g A)) (lprod B)
   Proof: strong induction on llen A, then list_cases on A.
     nil  : llen A = 0 = llen B ; B has llen 0 -> B = nil (need: llen=0 -> nil).
            both products = 1.
     a::t : ga = g a ; ga in B (into-range at a).  lmap g (a::t) = (ga)::(lmap g t).
            lprod(lmap g (a::t)) = ga * lprod(lmap g t).
            extract ga from B : lprod B = ga * lprod (lremove ga B).
            recurse with A':=t, B':=lremove ga B :
              lnodup t ; lnodup (lremove ga B) ; llen t = llen (lremove ga B) ;
              g maps t into lremove ga B (g y in B and g y != ga by inj+nodup) ;
              g inj on t (subset of A).
            IH -> lprod(lmap g t) = lprod(lremove ga B).  Multiply by ga, chain.
   ############################################################################ *)
val () = out "STAGE_P2_BEGIN\n";

(* llen_zero_nil : oeq (llen L) 0 ==> leq L lnil   (by list_cases)
   If L = a::t then llen = Suc(llen t) != 0.  So L = lnil. *)
val llen_zero_nil =
  let
    val LF = Free("L_lz", natlistT);
    val goalC = leq LF lnilC;
    val hz = Thm.assume (ctermGG (jT (oeq (llen LF) ZeroC)));
    fun caseNil hnil = hnil   (* hnil : leq L lnil *)
    fun caseCons (a, rest) hcons =
      let
        (* llen L = llen (a::rest) = Suc(llen rest) ; but llen L = 0 -> Suc(..)=0 contradiction *)
        val llc = llenCons_GG (a, rest);                       (* llen (a::rest) = Suc(llen rest) *)
        val llen_L_cons = llen_cong_GG (LF, lcons a rest) hcons; (* llen L = llen (a::rest) *)
        val llen_L_suc = oeq_trans OF [llen_L_cons, llc];      (* llen L = Suc(llen rest) *)
        val suc_eq_z = oeq_trans OF [oeq_sym OF [llen_L_suc], hz];  (* Suc(llen rest) = 0 *)
        val fls = Suc_neq_Zero_GG_at (llen rest) suc_eq_z;
      in Thm.implies_elim (oFalse_elim_GG_at goalC) fls end
    val res = list_cases (LF, goalC) caseNil caseCons;
  in varify (Thm.implies_intr (ctermGG (jT (oeq (llen LF) ZeroC))) res) end;
val () = if length (Thm.hyps_of llen_zero_nil) = 0 then out "OK llen_zero_nil\n" else out "FAIL llen_zero_nil\n";
fun llen_zero_nil_at L hz =
  Thm.implies_elim (beta_norm (Drule.infer_instantiate ctxtGG
      [(("L_lz",0), ctermGG L)] (toGG llen_zero_nil))) hz;

(* lmap leq-transfer : leq A B ==> leq (lmap g A)(lmap g B) *)
fun lmap_cong_GG g (aT,bT) hab =
  let val Pabs = Abs("z", natlistT, leq (lmap g aT) (lmap g (Bound 0)))
  in leq_rw_GG (Pabs, aT, bT) hab (leqRefl_GG (lmap g aT)) end;

(* the function g is a fixed Free of type lfnT throughout *)
val gF = Free("g_inj", lfnT);
fun g_app x = gF $ x;

(* bundle the hypotheses as object predicates over a list L *)
fun IntoBody B L = mkForall (Term.lambda (Free("xi", natT))
      (mkImp (lmem (Free("xi", natT)) L) (lmem (g_app (Free("xi", natT))) B)));
fun InjBody L = mkForall (Term.lambda (Free("xj", natT))
      (mkForall (Term.lambda (Free("yj", natT))
        (mkImp (lmem (Free("xj", natT)) L)
          (mkImp (lmem (Free("yj", natT)) L)
            (mkImp (oeq (g_app (Free("xj", natT))) (g_app (Free("yj", natT))))
                   (oeq (Free("xj", natT)) (Free("yj", natT)))))))));

(* use helpers : extract the Into / Inj facts at concrete points *)
fun useInto B L hInto z hmem =
  let val xf = Free("xi", natT)
      val Pabs = Term.lambda xf (mkImp (lmem xf L) (lmem (g_app xf) B))
      val inst = allE_GG_at Pabs z hInto
  in mp_GG_at (lmem z L, lmem (g_app z) B) inst hmem end;
fun useInj L hInj z w hmz hmw heq =
  let val xf = Free("xj", natT); val yf = Free("yj", natT)
      val Pabs = Term.lambda xf (mkForall (Term.lambda yf
            (mkImp (lmem xf L) (mkImp (lmem yf L)
              (mkImp (oeq (g_app xf)(g_app yf)) (oeq xf yf))))))
      val instX = allE_GG_at Pabs z hInj      (* forall y. lmem z L -> lmem y L -> g z = g y -> z = y *)
      val Qabs = Term.lambda yf (mkImp (lmem z L) (mkImp (lmem yf L)
                    (mkImp (oeq (g_app z)(g_app yf)) (oeq z yf))))
      val instY = allE_GG_at Qabs w instX
      val s1 = mp_GG_at (lmem z L, mkImp (lmem w L)(mkImp (oeq (g_app z)(g_app w))(oeq z w))) instY hmz
      val s2 = mp_GG_at (lmem w L, mkImp (oeq (g_app z)(g_app w))(oeq z w)) s1 hmw
      val s3 = mp_GG_at (oeq (g_app z)(g_app w), oeq z w) s2 heq
  in s3 end;
val () = out "STAGE_P2_HELPERS_READY\n";

(* instantiators for the banked natlist lemmas on ctxtGG (P2) *)
fun extract_GG (xt, Lt) hmem =
  Thm.implies_elim (beta_norm (Drule.infer_instantiate ctxtGG
      [(("x",0), ctermGG xt),(("L",0), ctermGG Lt)] (toGG extract))) hmem;
fun llen_remove_eq_GG (xt, Lt) hmem =
  Thm.implies_elim (beta_norm (Drule.infer_instantiate ctxtGG
      [(("x",0), ctermGG xt),(("L",0), ctermGG Lt)] (toGG llen_remove_eq))) hmem;
fun nodup_remove_GG (xt, Lt) hnd =
  Thm.implies_elim (beta_norm (Drule.infer_instantiate ctxtGG
      [(("x",0), ctermGG xt),(("L",0), ctermGG Lt)] (toGG nodup_remove))) hnd;
fun mem_remove_bwd_GG_at (yt, xt, Lt) hconj =
  Thm.implies_elim (beta_norm (Drule.infer_instantiate ctxtGG
      [(("y",0), ctermGG yt),(("x",0), ctermGG xt),(("L",0), ctermGG Lt)] (toGG mem_remove_bwd))) hconj;
val () = out "STAGE_P2_INST_READY\n";

(* ----------------------------------------------------------------------------
   THE PROOF (meta-B strong induction).
   PhiMeta n := !!L. jT(lt (llen L) n)
                  ==> !!B. jT(lnodup L) ==> jT(lnodup B) ==> jT(oeq(llen L)(llen B))
                        ==> jT(IntoBody B L) ==> jT(InjBody L)
                        ==> jT(oeq(lprod(lmap g L))(lprod B))
   The disjE in the step is on the BARE object equation (after the !!B and the 5
   hyps have been introduced), so object disjE is legitimate.
   ---------------------------------------------------------------------------- *)
val lprod_perm_meta =
  let
      ...
      Logic.mk_implies (jT (lnodup L),
        Logic.mk_implies (jT (lnodup B),
          Logic.mk_implies (jT (oeq (llen L)(llen B)),
            Logic.mk_implies (jT (IntoBody B L),
              Logic.mk_implies (jT (InjBody L),
                jT (oeq (lprod (lmap gF L)) (lprod B)))))));
    fun auxBody nt =
    ...
    (* discharge the 5 object hyps around a proof of the bare equation *)
    fun dischChain L B eqThm =
      let
        val d5 = Thm.implies_intr (ctermGG (jT (InjBody L))) eqThm;
        val d4 = Thm.implies_intr (ctermGG (jT (IntoBody B L))) d5;
        val d3 = Thm.implies_intr (ctermGG (jT (oeq (llen L)(llen B)))) d4;
        val d2 = Thm.implies_intr (ctermGG (jT (lnodup B))) d3;
        val d1 = Thm.implies_intr (ctermGG (jT (lnodup L))) d2;
      in d1 end;
    ...
    val base =
      let
        ...
        val hND_A = Thm.assume (ctermGG (jT (lnodup LB)));
        val hND_B = Thm.assume (ctermGG (jT (lnodup BB)));
        val hLen  = Thm.assume (ctermGG (jT (oeq (llen LB)(llen BB))));
        val hInto = Thm.assume (ctermGG (jT (IntoBody BB LB)));
        val hInj  = Thm.assume (ctermGG (jT (InjBody LB)));
        val concl0 = lt_zero_elim_GG (llen LB) (oeq (lprod (lmap gF LB)) (lprod BB)) hlt;
        val chainLB = dischChain LB BB concl0;
        val allB = Thm.forall_intr (ctermGG BB) chainLB;
        val disch = Thm.implies_intr (ctermGG (jT (lt (llen LB) ZeroC))) allB;
      in Thm.forall_intr (ctermGG LB) disch end;
    ...
    val step =
      let
        ...
        val hltS = Thm.assume (ctermGG (jT (lt (llen LS) (suc xS))));
        val dThm = lt_suc_cases_GG_at (llen LS, xS) hltS;   (* Disj (lt (llen LS) x)(oeq (llen LS) x) *)
        val BS = Free("B_ps", natlistT);
        (* introduce the 5 hyps; the GOAL becomes the bare object equation *)
        val hND_A = Thm.assume (ctermGG (jT (lnodup LS)));
        val hND_B = Thm.assume (ctermGG (jT (lnodup BS)));
        val hLen  = Thm.assume (ctermGG (jT (oeq (llen LS)(llen BS))));
        val hInto = Thm.assume (ctermGG (jT (IntoBody BS LS)));
        val hInj  = Thm.assume (ctermGG (jT (InjBody LS)));
        val goalC = oeq (lprod (lmap gF LS)) (lprod BS);

        (* CASE lt : use AUX at LS, specialize B := BS, discharge 5 hyps -> eq *)
        val caseLt =
          let
            val hA = Thm.assume (ctermGG (jT (lt (llen LS) xS)))
            val chainLS = applyAUXx LS hA                 (* !!B. chain LS B *)
            val chainLS_BS = Thm.forall_elim (ctermGG BS) chainLS
            ...
            val eqv = Thm.implies_elim e4 hInj            (* jT(eq) *)
          in Thm.implies_intr (ctermGG (jT (lt (llen LS) xS))) eqv end;

        (* CASE oeq : build the equation via list_cases on LS + IH from AUX *)
        val caseEq =
          let
            val hB = Thm.assume (ctermGG (jT (oeq (llen LS) xS)))
            fun applyIH L2 (h_lt:thm) =
              let val zpr = Free("z_prP", natT)
                  val Pr = Term.lambda zpr (lt (llen L2) zpr)
                  val h_lt_x = oeq_rw_GG (Pr, llen LS, xS) hB h_lt
              in applyAUXx L2 h_lt_x end                   (* !!B. chain L2 B *)

            fun caseNil hnil =
              let
                val lmAS = lmap_cong_GG gF (LS, lnilC) hnil;
                val lmnil = lmapNil_GG gF;
                val lp_lmAS = lprod_cong_GG (lmap gF LS, lmap gF lnilC) lmAS;
                val lp_lmnil = lprod_cong_GG (lmap gF lnilC, lnilC) lmnil;
                val lpn = lprodNil_GG ();
                val lhs1 = oeq_trans OF [oeq_trans OF [lp_lmAS, lp_lmnil], lpn];
                val llAS = llen_cong_GG (LS, lnilC) hnil;
                val lln = llenNil_GG ();
                val llAS0 = oeq_trans OF [llAS, lln];
                val llB0 = oeq_trans OF [oeq_sym OF [hLen], llAS0];
                val Bnil = llen_zero_nil_at BS llB0;
                val lpB = lprod_cong_GG (BS, lnilC) Bnil;
                val lpB1 = oeq_trans OF [lpB, lpn];
                val lpB1s = oeq_sym OF [lpB1];
              in oeq_trans OF [lhs1, lpB1s] end;

            fun caseCons (a, rest) hcons =
              let
                val consA = lcons a rest;
                val hcons_s = leq_sym_GG hcons;
                val ga = g_app a;
                val a_in_cons = Thm.implies_elim (lmemConsBwd_GG (a, a, rest))
                                  (disjI1_GG_at (oeq a a, lmem a rest) (oeqRefl_GG a));
                val memA = lmem_transfer_GG (a, consA, LS) hcons_s a_in_cons;
                val ga_in_B = useInto BS LS hInto a memA;
                val hND_consA = lnodup_transfer_GG (LS, consA) hcons hND_A;
                val cjND = Thm.implies_elim (lnodupConsFwd_GG (a, rest)) hND_consA;
                val a_notin_rest = conjunct1_GG_at (neg (lmem a rest), lnodup rest) cjND;
                val ndRest = conjunct2_GG_at (neg (lmem a rest), lnodup rest) cjND;
                val Bp = lremove ga BS;

                val lmAS_cons = lmap_cong_GG gF (LS, consA) hcons;
                val lm_cons = lmapCons_GG (gF, a, rest);
                val lp_lmAS = lprod_cong_GG (lmap gF LS, lmap gF consA) lmAS_cons;
                val lp_lmcons = lprod_cong_GG (lmap gF consA, lcons ga (lmap gF rest)) lm_cons;
                val lp_cons_eq = lprodCons_GG (ga, lmap gF rest);
                val lhsChain = oeq_trans OF [oeq_trans OF [lp_lmAS, lp_lmcons], lp_cons_eq];

                val lp_B_extract = extract_GG (ga, BS) ga_in_B;

                val llc = llenCons_GG (a, rest);
                val llAS_cons = llen_cong_GG (LS, consA) hcons;
                val llAS_suc = oeq_trans OF [llAS_cons, llc];
                val lt_self = lt_suc_GG_at (llen rest);
                val zlt = Free("z_ltP", natT);
                val Plt = Term.lambda zlt (lt (llen rest) zlt);
                val llAS_suc_s = oeq_sym OF [llAS_suc];
                val ltRest_AS = oeq_rw_GG (Plt, suc (llen rest), llen LS) llAS_suc_s lt_self;
                val ihMeta = applyIH rest ltRest_AS;        (* !!B. chain rest B *)

                val ndBp = nodup_remove_GG (ga, BS) hND_B;
                val llBv_eq = llen_remove_eq_GG (ga, BS) ga_in_B;
                val s_rest_s_Bp = oeq_trans OF [oeq_trans OF [oeq_sym OF [llAS_suc], hLen], llBv_eq];
                val llRest_Bp = Suc_inj_GG_at (llen rest, llen Bp) s_rest_s_Bp;

                val intoBpRest =
                  let
                    val yf = Free("y_into", natT);
                    val Pabs = Term.lambda yf (mkImp (lmem yf rest) (lmem (g_app yf) Bp));
                    val hyR = Thm.assume (ctermGG (jT (lmem yf rest)));
                    val y_in_cons = Thm.implies_elim (lmemConsBwd_GG (yf, a, rest))
                                      (disjI2_GG_at (oeq yf a, lmem yf rest) hyR);
                    val y_in_AS = lmem_transfer_GG (yf, consA, LS) hcons_s y_in_cons;
                    val gy_in_B = useInto BS LS hInto yf y_in_AS;
                    val gy_neq_ga =
                      let
                        val heq = Thm.assume (ctermGG (jT (oeq (g_app yf) ga)));
                        val yEqa = useInj LS hInj yf a y_in_AS memA heq;
                        val Pr = Abs("z", natT, lmem (Bound 0) rest);
                        val a_in_rest = oeq_rw_GG (Pr, yf, a) yEqa hyR;
                        val ff = mp_GG_at (lmem a rest, oFalseC) a_notin_rest a_in_rest;
                      in impI_GG_at (oeq (g_app yf) ga, oFalseC)
                           (Thm.implies_intr (ctermGG (jT (oeq (g_app yf) ga))) ff) end;
                    val conjBwd = conjI_GG_at (lmem (g_app yf) BS, neg (oeq (g_app yf) ga)) gy_in_B gy_neq_ga;
                    val gy_in_Bp = mem_remove_bwd_GG_at (g_app yf, ga, BS) conjBwd;
                    val imp = impI_GG_at (lmem yf rest, lmem (g_app yf) Bp)
                                (Thm.implies_intr (ctermGG (jT (lmem yf rest))) gy_in_Bp);
                  in allI_GG_at Pabs (Thm.forall_intr (ctermGG yf) imp) end;
                  ...
                  let
                    ...
                    val Pabs = Term.lambda xf (mkForall (Term.lambda yf
                                  (mkImp (lmem xf rest) (mkImp (lmem yf rest)
                                    (mkImp (oeq (g_app xf)(g_app yf)) (oeq xf yf))))));
                    val hxR = Thm.assume (ctermGG (jT (lmem xf rest)));
                    val hyR = Thm.assume (ctermGG (jT (lmem yf rest)));
                    val heq = Thm.assume (ctermGG (jT (oeq (g_app xf)(g_app yf))));
                    val x_in_cons = Thm.implies_elim (lmemConsBwd_GG (xf, a, rest))
                                      (disjI2_GG_at (oeq xf a, lmem xf rest) hxR);
                    val x_in_AS = lmem_transfer_GG (xf, consA, LS) hcons_s x_in_cons;
                    val y_in_cons = Thm.implies_elim (lmemConsBwd_GG (yf, a, rest))
                                      (disjI2_GG_at (oeq yf a, lmem yf rest) hyR);
                    val y_in_AS = lmem_transfer_GG (yf, consA, LS) hcons_s y_in_cons;
                    val xy = useInj LS hInj xf yf x_in_AS y_in_AS heq;
                    val impE = impI_GG_at (oeq (g_app xf)(g_app yf), oeq xf yf)
                                 (Thm.implies_intr (ctermGG (jT (oeq (g_app xf)(g_app yf)))) xy);
                    val impY = impI_GG_at (lmem yf rest, mkImp (oeq (g_app xf)(g_app yf)) (oeq xf yf))
                                 (Thm.implies_intr (ctermGG (jT (lmem yf rest))) impE);
                    val impX = impI_GG_at (lmem xf rest, mkImp (lmem yf rest)(mkImp (oeq (g_app xf)(g_app yf)) (oeq xf yf)))
                                 (Thm.implies_intr (ctermGG (jT (lmem xf rest))) impY);
                    val Qabs = Term.lambda yf (mkImp (lmem xf rest) (mkImp (lmem yf rest)
                                  (mkImp (oeq (g_app xf)(g_app yf)) (oeq xf yf))));
                    val innerAll = allI_GG_at Qabs (Thm.forall_intr (ctermGG yf) impX);
                  in allI_GG_at Pabs (Thm.forall_intr (ctermGG xf) innerAll) end;

                val chainRestBp = Thm.forall_elim (ctermGG Bp) ihMeta;
                ...
                val ga_mul = mult_cong_r_GG (ga, lprod (lmap gF rest), lprod Bp) ihEq;
                val lp_B_extract_s = oeq_sym OF [lp_B_extract];
                val rhsChain = oeq_trans OF [ga_mul, lp_B_extract_s];
              in oeq_trans OF [lhsChain, rhsChain] end;

            val eqv = list_cases (LS, goalC) caseNil caseCons;
          in Thm.implies_intr (ctermGG (jT (oeq (llen LS) xS))) eqv end;

        (* object disjE on the bare equation goal *)
        val eqGoal = disjE_GG_at (lt (llen LS) xS, oeq (llen LS) xS, goalC) dThm caseLt caseEq;
        val chainLS = dischChain LS BS eqGoal;
        val allB = Thm.forall_intr (ctermGG BS) chainLS;
        val dischLt = Thm.implies_intr (ctermGG (jT (lt (llen LS) (suc xS)))) allB;
        val auxSucx = Thm.forall_intr (ctermGG LS) dischLt;
      in Thm.forall_intr (ctermGG xS) (Thm.implies_intr (ctermGG auxX) auxSucx) end;

    (* instantiate AUX at k := Suc(llen AF), apply at AF -> chain AF B for any B *)
    val AF = Free("A_fin", natlistT);
    val kFin = suc (llen AF);
    val indK = beta_norm (Drule.infer_instantiate ctxtGG
                 [(("Phi",0), ctermGG PhiMetaAbs),(("k",0), ctermGG kFin)] meta_nat_induct_GG);
    val auxK = Thm.implies_elim (Thm.implies_elim indK base) step;   (* !!L. lt(..)(Suc(llen AF)) ==> !!B chain L B *)
    val auxKA = Thm.forall_elim (ctermGG AF) auxK;
    val selfLt = lt_suc_GG_at (llen AF);
    val chainAFall = Thm.implies_elim auxKA selfLt;     (* !!B. chain AF B *)
  in (AF, chainAFall) end;
val () = out "STAGE_P2_PHI_BUILT\n";

(* ----------------------------------------------------------------------------
   lprod_perm_of_inj : the clean meta form, then varify.
   ---------------------------------------------------------------------------- *)
val lprod_perm_of_inj =
  let
    val (AF, chainAFall) = lprod_perm_meta;
    val Bf = Free("B_ext", natlistT);
    val chainAF = Thm.forall_elim (ctermGG Bf) chainAFall;   (* chain AF Bf (meta, 5 imps) *)
  in varify chainAF end;

(* ---- validate lprod_perm_of_inj : 0-hyp + aconv ---- *)
val gVp = Var(("g_inj",0), lfnT);
fun IntoBodyV B A = mkForall (Term.lambda (Free("xi", natT))
      (mkImp (lmem (Free("xi", natT)) A) (lmem (gVp $ (Free("xi", natT))) B)));
fun InjBodyV A = mkForall (Term.lambda (Free("xj", natT))
      (mkForall (Term.lambda (Free("yj", natT))
        (mkImp (lmem (Free("xj", natT)) A)
          (mkImp (lmem (Free("yj", natT)) A)
            (mkImp (oeq (gVp $ (Free("xj", natT))) (gVp $ (Free("yj", natT))))
                   (oeq (Free("xj", natT)) (Free("yj", natT)))))))));
val AVp = Var(("A_fin",0), natlistT);  val BVp = Var(("B_ext",0), natlistT);
val i_lprod_perm =
  Logic.mk_implies (jT (lnodup AVp),
    Logic.mk_implies (jT (lnodup BVp),
      Logic.mk_implies (jT (oeq (llen AVp)(llen BVp)),
        Logic.mk_implies (jT (IntoBodyV BVp AVp),
          Logic.mk_implies (jT (InjBodyV AVp),
            jT (oeq (lprod (lmapC $ gVp $ AVp)) (lprod BVp)))))));
val r_lprod_perm = (length (Thm.hyps_of lprod_perm_of_inj) = 0)
                   andalso ((Thm.prop_of lprod_perm_of_inj) aconv i_lprod_perm);
val () = if r_lprod_perm then out "OK lprod_perm_of_inj\n" else out "FAIL lprod_perm_of_inj\n";
val probe_perm_needs_inj =
  let val bogus = Logic.mk_implies (jT (lnodup AVp),
        Logic.mk_implies (jT (lnodup BVp),
          Logic.mk_implies (jT (oeq (llen AVp)(llen BVp)),
            Logic.mk_implies (jT (IntoBodyV BVp AVp),
              jT (oeq (lprod (lmapC $ gVp $ AVp)) (lprod BVp))))))
  in not ((Thm.prop_of lprod_perm_of_inj) aconv bogus) end;
val () = if probe_perm_needs_inj then out "PROBE_OK lprod_perm_of_inj keeps injectivity\n"
         else out "PROBE_FAIL lprod_perm_of_inj\n";
val perm_clean = length (Thm.extra_shyps lprod_perm_of_inj) = 0;
val () = if r_lprod_perm andalso probe_perm_needs_inj andalso perm_clean
         then out "LPROD_PERM_OK\n" else out "LPROD_PERM_FAILED\n";

(* ############################################################################
   ###########################  (P3)  lar_perm  ##############################
   ----------------------------------------------------------------------------
   Instantiate lprod_perm_of_inj with g = (%k. lar a p k), A = B = uptoF m :
     prime2 p ==> ~(dvd p a) ==> oeq (sub p 1)(add m m) ==>
       oeq (lprod (lmap (%k. lar a p k) (uptoF m))) (lprod (uptoF m))
   discharging:
     Into  : k in [1,m] ==> lar a p k in [1,m]  (lar_in_range, after ~(p|k))
     Inj   : lar a p k = lar a p k2 ==> k = k2  (lar_cong -> dichotomy -> abs_inj)
   ############################################################################ *)
val () = out "STAGE_P3_BEGIN\n";

(* ---- re-varify the residue/cong/order base lemmas onto ctxtGG ---- *)
val lar_cong_GG      = varifyGG lar_cong;
val lar_in_range_GG  = varifyGG lar_in_range;
val abs_inj_GG       = varifyGG abs_inj;
val prime_not_dvd_pos_lt_GG = varifyGG prime_not_dvd_pos_lt;
val cong_add_GG      = varifyGG cong_add;
val le_suc_mono_GG   = varifyGG le_suc_mono;
val sub_Suc_add_l_GG = varifyGG sub_Suc_add_l;
val add_comm_GG      = varifyGG add_comm;
val add_assoc_GG     = varifyGG add_assoc;

fun lar_cong_at (pt, at, kt) hPrime hNa hNk =
  let val inst = beta_norm (Drule.infer_instantiate ctxtGG
        [(("p",0), ctermGG pt),(("a",0), ctermGG at),(("k",0), ctermGG kt)] lar_cong_GG)
  in Thm.implies_elim (Thm.implies_elim (Thm.implies_elim inst hPrime) hNa) hNk end;
fun lar_in_range_at (pt, at, kt, mt) hPrime hNa hNk hOdd =
  let val inst = beta_norm (Drule.infer_instantiate ctxtGG
        [(("p",0), ctermGG pt),(("a",0), ctermGG at),(("k",0), ctermGG kt),(("m",0), ctermGG mt)] lar_in_range_GG)
  in Thm.implies_elim (Thm.implies_elim (Thm.implies_elim (Thm.implies_elim inst hPrime) hNa) hNk) hOdd end;
fun abs_inj_at (pt, at, kt, k2t, mt) hPrime hNa hOdd h1 h2 h3 h4 hDisj =
  let val inst = beta_norm (Drule.infer_instantiate ctxtGG
        [(("p",0), ctermGG pt),(("a",0), ctermGG at),(("k",0), ctermGG kt),
         (("k2",0), ctermGG k2t),(("m",0), ctermGG mt)] abs_inj_GG)
  in Thm.implies_elim (Thm.implies_elim (Thm.implies_elim (Thm.implies_elim (Thm.implies_elim
       (Thm.implies_elim (Thm.implies_elim (Thm.implies_elim inst hPrime) hNa) hOdd) h1) h2) h3) h4) hDisj end;
fun prime_not_dvd_pos_lt_at (pt, rt) hdvd hpos hlt =
  let val inst = beta_norm (Drule.infer_instantiate ctxtGG
        [(("p",0), ctermGG pt),(("r",0), ctermGG rt)] prime_not_dvd_pos_lt_GG)
  in Thm.implies_elim (Thm.implies_elim (Thm.implies_elim inst hdvd) hpos) hlt end;
fun cong_add_at (mt, at, a2t, bt, b2t) h1 h2 =
  let val inst = beta_norm (Drule.infer_instantiate ctxtGG
        [(("m",0), ctermGG mt),(("a",0), ctermGG at),(("a2",0), ctermGG a2t),
         (("b",0), ctermGG bt),(("b2",0), ctermGG b2t)] cong_add_GG)
  in Thm.implies_elim (Thm.implies_elim inst h1) h2 end;
fun le_suc_mono_at (mt, nt) h =
  Thm.implies_elim (beta_norm (Drule.infer_instantiate ctxtGG
        [(("m",0), ctermGG mt),(("n",0), ctermGG nt)] le_suc_mono_GG)) h;
fun addcomm_GG (mt,nt) = beta_norm (Drule.infer_instantiate ctxtGG
        [(("m",0), ctermGG mt),(("n",0), ctermGG nt)] add_comm_GG);
fun addassoc_GG (mt,nt,kt) = beta_norm (Drule.infer_instantiate ctxtGG
        [(("m",0), ctermGG mt),(("n",0), ctermGG nt),(("k",0), ctermGG kt)] add_assoc_GG);
(* lt0_of_prime / le_or_lt on ctxtGG : re-derive via varified prime2_gt1 + le_total *)
val conjunct1_GG2 = conjunct1_GG;   (* alias *)
val le_total_GG = varifyGG le_total;
fun le_total_GG_at (mt,nt) = beta_norm (Drule.infer_instantiate ctxtGG
        [(("m",0), ctermGG mt),(("n",0), ctermGG nt)] le_total_GG);
fun le_or_lt_GG (xT, yT) =
  let
    val goalC = mkDisj (le xT yT) (lt yT xT);
    val tot = le_total_GG_at (xT, yT);     (* Disj (le x y)(le y x) *)
    val cA = let val hle = Thm.assume (ctermGG (jT (le xT yT)))
             in Thm.implies_intr (ctermGG (jT (le xT yT)))
                  (disjI1_GG_at (le xT yT, lt yT xT) hle) end;
    (* case le y x : need Disj... but lt y x = le (Suc y) x is stronger than le y x.
       We only need le_or_lt for the trichotomy in some places; here build a weaker
       Disj (le x y)(le y x) is what le_total already gives. For the strict version we
       must be careful. We will instead use le_total directly where possible. *)
  in tot end;   (* returns Disj (le x y)(le y x) ; callers adapt *)
val () = out "STAGE_P3_VARIFY_DONE\n";

(* prime2_gt1 / lt0_of_prime on ctxtGG *)
fun prime2_gt1_GG p hPrime =
  conjunct1_GG_at (lt (suc ZeroC) p, mkForall (ppAbs p)) hPrime;   (* lt 1 p = le 2 p *)
fun lt0_of_prime_GG p hPrime =
  let
    val gt1 = prime2_gt1_GG p hPrime;                   (* le (Suc(Suc 0)) p *)
    val le12a = le_add_GG_at (suc ZeroC, suc ZeroC);    (* le 1 (1 + 1) *)
    val addS = addSuc_GG (ZeroC, suc ZeroC);            (* (Suc 0 + Suc 0)=Suc(0 + Suc 0) *)
    val a0 = beta_norm (Drule.infer_instantiate ctxtGG [(("n",0), ctermGG (suc ZeroC))] (varifyGG add_0)); (* (0 + Suc 0)=Suc 0 *)
    val sucCong = Succong_GG a0;                         (* Suc(0+Suc 0)=Suc(Suc 0) *)
    val addeq = oeq_trans OF [addS, sucCong];
    val zF = Free("z_lt0g", natT);
    val Pz = Term.lambda zF (le (suc ZeroC) zF);
    val le12 = oeq_rw_GG (Pz, add (suc ZeroC)(suc ZeroC), suc (suc ZeroC)) addeq le12a;
  in le_trans_GG_at (suc ZeroC, suc (suc ZeroC), p) le12 gt1 end;   (* lt 0 p *)

(* ---- pEq : prime2 p ==> oeq(sub p 1)(add m m) ==> oeq p (Suc(add m m)) ---- *)
(* (replicates lar_in_range's pEq derivation on ctxtGG) *)
val Suc_neq_Zero_GGv = Suc_neq_Zero_GG;
fun pEq_of p m hPrime hOdd =
  let
    val lt0p = lt0_of_prime_GG p hPrime;        (* lt 0 p = le (Suc 0) p = Ex w. p = Suc 0 + w *)
    val Pabs = Abs("w", natT, oeq p (add (suc ZeroC) (Bound 0)));
    fun body wF (hw:thm) =      (* hw : p = add (Suc 0) w = Suc(0+w) *)
      let
        val aS = addSuc_GG (ZeroC, wF);           (* (Suc 0 + w)=Suc(0+w) *)
        val pSuc0w = oeq_trans OF [hw, aS];       (* p = Suc(0+w) *)
        val a0 = beta_norm (Drule.infer_instantiate ctxtGG [(("n",0), ctermGG wF)] (varifyGG add_0)); (* (0+w)=w *)
        val pSucw = oeq_trans OF [pSuc0w, Succong_GG a0];  (* p = Suc w *)
        (* sub p 1 = w : from p = Suc w, sub (Suc w) (Suc 0) = sub w 0 = w. but we have hOdd: sub p 1 = m+m.
           Want oeq p (Suc(m+m)).  We have p = Suc w. Need w = m+m.
           sub p 1 = sub (Suc w) 1 ; sub (Suc w)(Suc 0) = sub w 0 = w [sub_Suc_Suc + sub_n_0].
           So sub p 1 = w. With hOdd: w = m+m. *)
        val zsp = Free("z_sp", natT);
        val Psp = Term.lambda zsp (oeq (sub zsp oneC) (add m m));
        val hOdd_w = oeq_rw_GG (Psp, p, suc wF) pSucw hOdd;   (* sub (Suc w) 1 = m+m *)
        val ss = subSS_atG (wF, ZeroC);    (* sub (Suc w)(Suc 0) = sub w 0 *)
        (* NOTE: subSS_atG is on ctxtG; reuse via toGG? Build GG version. *)
        val sn0 = subN0_atG wF;            (* sub w 0 = w  (ctxtG) *)
        val sub_eq_w = oeq_trans OF [ss, sn0];  (* sub (Suc w) 1 = w *)
        val w_mm = oeq_trans OF [oeq_sym OF [sub_eq_w], hOdd_w];  (* w = m+m *)
        val res = oeq_rw_GG (Abs("z", natT, oeq p (suc (Bound 0))), wF, add m m) w_mm pSucw;  (* p = Suc(m+m) *)
      in res end;
  in exE_GG_at (Pabs, oeq p (suc (add m m))) lt0p "w_pe" body end;
val () = out "STAGE_P3_PEQ_NOTE\n";

(* ---- lt_m_p : prime2 p ==> oeq(sub p 1)(add m m) ==> lt m p ---- *)
fun lt_m_p p m hPrime hOdd =
  let
    val pEq = pEq_of p m hPrime hOdd;            (* p = Suc(m+m) *)
    val aS = addSuc_GG (m, m);                    (* (Suc m + m)=Suc(m+m) *)
    val aS_s = oeq_sym OF [aS];                   (* Suc(m+m)=(Suc m + m) *)
    val pAdd = oeq_trans OF [pEq, aS_s];          (* p = (Suc m + m) *)
  in le_intro_GG (suc m, p, m) pAdd end;          (* le (Suc m) p = lt m p *)

(* ---- not_dvd_in_range : prime2 p ==> oeq(sub p 1)(add m m)
                            ==> le 1 k ==> le k m ==> ~(dvd p k) ---- *)
fun not_dvd_in_range p m k hPrime hOdd h1k hkm =
  let
    val lt0k = h1k;                               (* le (Suc 0) k = lt 0 k *)
    val ltmp = lt_m_p p m hPrime hOdd;            (* lt m p = le (Suc m) p *)
    val leSkSm = le_suc_mono_at (k, m) hkm;       (* le (Suc k)(Suc m) *)
    val ltkp = le_trans_GG_at (suc k, suc m, p) leSkSm ltmp;  (* le (Suc k) p = lt k p *)
    val hdvd = Thm.assume (ctermGG (jT (dvd p k)));
    val ff = prime_not_dvd_pos_lt_at (p, k) hdvd lt0k ltkp;   (* oFalse *)
    val metaImp = Thm.implies_intr (ctermGG (jT (dvd p k))) ff;
  in impI_GG_at (dvd p k, oFalseC) metaImp end;    (* ~(dvd p k) *)
val () = out "STAGE_P3_NOTDVD_READY\n";

(* ============================================================================
   INJECTIVITY BRIDGE : lar_inj_dichotomy
     prime2 p ==> ~(dvd p a) ==> ~(dvd p k) ==> ~(dvd p k2)
       ==> oeq (lar a p k)(lar a p k2)
       ==> Disj (cong p (a*k)(a*k2)) (cong p (a*k + a*k2) 0)
   via lar_cong (two instances) + cong arithmetic.
   ============================================================================ *)
(* cong helpers on ctxtGG *)
fun cong_refl_at (mt, at) = beta_norm (Drule.infer_instantiate ctxtGG
      [(("m",0), ctermGG mt), (("a",0), ctermGG at)] cong_refl_GG);
...
fun cong_trans_at (mt, at, bt, ct) h1 h2 =
  let val inst = beta_norm (Drule.infer_instantiate ctxtGG
        [(("m",0), ctermGG mt), (("a",0), ctermGG at), (("b",0), ctermGG bt), (("c",0), ctermGG ct)] cong_trans_GG)
  in Thm.implies_elim (Thm.implies_elim inst h1) h2 end;
(* cong_cong : oeq a a2 -> oeq b b2 -> cong m a b -> cong m a2 b2  (already cong_cong_GG) *)

(* cong intro helpers on ctxtGG *)
fun cong_introL_GG (m, a, b, w) hyp =   (* hyp : oeq b (add a (mult m w)) -> cong m a b (LEFT) *)
  let val LAbs = Abs ("k", natT, oeq b (add a (mult m (Bound 0))))
  in disjI1_GG_at (congL m a b, congR m a b) (exI_GG_at (LAbs, w) hyp) end;
fun cong_introR_GG (m, a, b, w) hyp =   (* hyp : oeq a (add b (mult m w)) -> cong m a b (RIGHT) *)
  let val RAbs = Abs ("k", natT, oeq a (add b (mult m (Bound 0))))
  in disjI2_GG_at (congL m a b, congR m a b) (exI_GG_at (RAbs, w) hyp) end;

(* ---- cong_add_cancel_r : cong p (add x r)(add y r) ==> cong p x y ---- *)
fun cong_add_cancel_r p x y r hcong =
  let
    val goalC = cong p x y;
    val xr = add x r; val yr = add y r;
    (* LEFT case : congL = Ex k. (y+r) = (x+r) + p*k *)
    val caseL =
      let
        val LAbs = Abs("k", natT, oeq yr (add xr (mult p (Bound 0))));  (* body of congL p xr yr *)
        fun body w (hw:thm) =   (* hw : (y+r) = (x+r) + p*w *)
          let
            (* (x+r) + p*w = (x + p*w) + r  : assoc/comm rearrangement.
               (x+r)+pw = x + (r + pw) [assoc] = x + (pw + r) [comm] = (x + pw) + r [assoc^-1] *)
            val a1 = addassoc_GG (x, r, mult p w);                 (* (x+r)+pw = x+(r+pw) *)
            val c1 = addcomm_GG (r, mult p w);                     (* (r+pw)=(pw+r) *)
            val c1c = add_cong_r_GG (x, add r (mult p w), add (mult p w) r) c1;  (* x+(r+pw)=x+(pw+r) *)
            val a2 = addassoc_GG (x, mult p w, r);                 (* (x+pw)+r = x+(pw+r) *)
            val a2s = oeq_sym OF [a2];                             (* x+(pw+r)=(x+pw)+r *)
            val rearr = oeq_trans OF [oeq_trans OF [a1, c1c], a2s]; (* (x+r)+pw = (x+pw)+r *)
            val yr_eq = oeq_trans OF [hw, rearr];                  (* y+r = (x+pw)+r *)
            (* y + r = (x+pw) + r ; comm both -> r+y = r+(x+pw) ; add_left_cancel -> y = x+pw *)
            val cy = addcomm_GG (y, r);                            (* y+r = r+y *)
            val crhs = addcomm_GG (add x (mult p w), r);           (* (x+pw)+r = r+(x+pw) *)
            val ry_eq = oeq_trans OF [oeq_trans OF [oeq_sym OF [cy], yr_eq], crhs];  (* r+y = r+(x+pw) *)
            val y_eq = add_left_cancel_atG ry_eq;                  (* y = x + pw *)
          in cong_introL_GG (p, x, y, w) y_eq end;
      in body end;
    (* RIGHT case : congR = Ex k. (x+r) = (y+r) + p*k *)
    val caseR =
      let
        fun body w (hw:thm) =   (* hw : (x+r) = (y+r) + p*w *)
          let
            val a1 = addassoc_GG (y, r, mult p w);
            val c1 = addcomm_GG (r, mult p w);
            val c1c = add_cong_r_GG (y, add r (mult p w), add (mult p w) r) c1;
            val a2 = addassoc_GG (y, mult p w, r);
            val a2s = oeq_sym OF [a2];
            val rearr = oeq_trans OF [oeq_trans OF [a1, c1c], a2s];  (* (y+r)+pw = (y+pw)+r *)
            val xr_eq = oeq_trans OF [hw, rearr];                    (* x+r = (y+pw)+r *)
            val cx = addcomm_GG (x, r);
            val crhs = addcomm_GG (add y (mult p w), r);
            val rx_eq = oeq_trans OF [oeq_trans OF [oeq_sym OF [cx], xr_eq], crhs];  (* r+x = r+(y+pw) *)
            val x_eq = add_left_cancel_atG rx_eq;                    (* x = y + pw *)
          in cong_introR_GG (p, x, y, w) x_eq end;
      in body end;
    val cA =
      let val hL = Thm.assume (ctermGG (jT (congL p xr yr)))
          val LAbs = Abs("k", natT, oeq yr (add xr (mult p (Bound 0))))
          val res = exE_GG_at (LAbs, goalC) hL "kL_cac" (caseL)
      in Thm.implies_intr (ctermGG (jT (congL p xr yr))) res end;
    val cB =
      let val hR = Thm.assume (ctermGG (jT (congR p xr yr)))
          val RAbs = Abs("k", natT, oeq xr (add yr (mult p (Bound 0))))
          val res = exE_GG_at (RAbs, goalC) hR "kR_cac" (caseR)
      in Thm.implies_intr (ctermGG (jT (congR p xr yr))) res end;
  in disjE_GG_at (congL p xr yr, congR p xr yr, goalC) hcong cA cB end;
val () = out "STAGE_P3_CANCEL_READY\n";
...
(* ============================================================================
   lar_inj_dichotomy :
     prime2 p ==> ~(dvd p a) ==> ~(dvd p k) ==> ~(dvd p k2)
       ...
       ==> Disj (cong p (a*k)(a*k2)) (cong p (a*k + a*k2) 0)
   ============================================================================ *)
fun lar_inj_dichotomy p a k k2 hPrime hNa hNk hNk2 hLarEq =
  let
    val r  = lar a p k;
    val r2 = lar a p k2;
    val ak = mult a k; val ak2 = mult a k2;
    val djk  = lar_cong_at (p, a, k)  hPrime hNa hNk;   (* Disj (cong p ak r)(cong p (ak+r) 0) *)
    val djk2 = lar_cong_at (p, a, k2) hPrime hNa hNk2;  (* Disj (cong p ak2 r2)(cong p (ak2+r2) 0) *)
    val goalC = mkDisj (cong p ak ak2) (cong p (add ak ak2) ZeroC);
    val hEq_s = oeq_sym OF [hLarEq];   (* oeq r2 r *)
    (* rewrite r2 -> r in a "cong p ak2 r2" thm *)
    fun rw_r2_r_in_2nd h =     (* h : cong p ak2 r2 -> cong p ak2 r *)
      let val zc = Free("z_rr", natT)
          val Pc = Term.lambda zc (cong p ak2 zc)
      in oeq_rw_GG (Pc, r2, r) hEq_s h end;
    fun rw_r2_r_in_sum h =     (* h : cong p (ak2 + r2) 0 -> cong p (ak2 + r) 0 *)
      let val zc = Free("z_rs", natT)
          val Pc = Term.lambda zc (cong p (add ak2 zc) ZeroC)
      in oeq_rw_GG (Pc, r2, r) hEq_s h end;

    fun caseKlo hKlo =   (* hKlo : cong p ak r *)
      let
        val caseK2lo =
          let
            val hK2lo = Thm.assume (ctermGG (jT (cong p ak2 r2)))
            val hK2lo_r = rw_r2_r_in_2nd hK2lo                 (* cong p ak2 r *)
            val r_ak2 = cong_sym_at (p, ak2, r) hK2lo_r        (* cong p r ak2 *)
            val ak_ak2 = cong_trans_at (p, ak, r, ak2) hKlo r_ak2  (* cong p ak ak2 *)
          in Thm.implies_intr (ctermGG (jT (cong p ak2 r2)))
               (disjI1_GG_at (cong p ak ak2, cong p (add ak ak2) ZeroC) ak_ak2) end;
        val caseK2hi =
          let
            val hK2hi0 = Thm.assume (ctermGG (jT (cong p (add ak2 r2) ZeroC)))
            val hK2hi  = rw_r2_r_in_sum hK2hi0                 (* cong p (ak2 + r) 0 *)
            (* cong_add (cong p ak r)(cong_refl ak2) : cong p (ak + ak2)(r + ak2) *)
            val refl_ak2 = cong_refl_at (p, ak2)
            val add1 = cong_add_at (p, ak, r, ak2, ak2) hKlo refl_ak2  (* cong p (ak+ak2)(r+ak2) *)
            (* r + ak2 = ak2 + r  ; rewrite RHS of cong *)
            val comm = addcomm_GG (r, ak2)                     (* (r+ak2)=(ak2+r) *)
            val zc = Free("z_ck", natT)
            val Pc = Term.lambda zc (cong p (add ak ak2) zc)
            val add1' = oeq_rw_GG (Pc, add r ak2, add ak2 r) comm add1  (* cong p (ak+ak2)(ak2+r) *)
            (* cong p (ak2 + r) 0 ; trans *)
            val res = cong_trans_at (p, add ak ak2, add ak2 r, ZeroC) add1' hK2hi  (* cong p (ak+ak2) 0 *)
          in Thm.implies_intr (ctermGG (jT (cong p (add ak2 r2) ZeroC)))
               (disjI2_GG_at (cong p ak ak2, cong p (add ak ak2) ZeroC) res) end;
      in disjE_GG_at (cong p ak2 r2, cong p (add ak2 r2) ZeroC, goalC) djk2 caseK2lo caseK2hi end;

    fun caseKhi hKhi =   (* hKhi : cong p (ak + r) 0 *)
      let
        val caseK2lo =
          let
            val hK2lo = Thm.assume (ctermGG (jT (cong p ak2 r2)))
            val hK2lo_r = rw_r2_r_in_2nd hK2lo                 (* cong p ak2 r *)
            (* cong_add (cong p ak2 r)(cong_refl ak) : cong p (ak2 + ak)(r + ak) *)
            val refl_ak = cong_refl_at (p, ak)
            val add1 = cong_add_at (p, ak2, r, ak, ak) hK2lo_r refl_ak  (* cong p (ak2+ak)(r+ak) *)
            val comm = addcomm_GG (r, ak)                      (* (r+ak)=(ak+r) *)
            val zc = Free("z_ch", natT)
            val Pc = Term.lambda zc (cong p (add ak2 ak) zc)
            val add1' = oeq_rw_GG (Pc, add r ak, add ak r) comm add1  (* cong p (ak2+ak)(ak+r) *)
            val res0 = cong_trans_at (p, add ak2 ak, add ak r, ZeroC) add1' hKhi  (* cong p (ak2+ak) 0 *)
            (* rewrite (ak2+ak) -> (ak+ak2) on the LEFT of cong *)
            val comm2 = addcomm_GG (ak2, ak)                   (* (ak2+ak)=(ak+ak2) *)
            val zc2 = Free("z_ch2", natT)
            val Pc2 = Term.lambda zc2 (cong p zc2 ZeroC)
            val res = oeq_rw_GG (Pc2, add ak2 ak, add ak ak2) comm2 res0  (* cong p (ak+ak2) 0 *)
          in Thm.implies_intr (ctermGG (jT (cong p ak2 r2)))
               (disjI2_GG_at (cong p ak ak2, cong p (add ak ak2) ZeroC) res) end;
        val caseK2hi =
          let
            val hK2hi0 = Thm.assume (ctermGG (jT (cong p (add ak2 r2) ZeroC)))
            val hK2hi  = rw_r2_r_in_sum hK2hi0                 (* cong p (ak2 + r) 0 *)
            (* both: cong p (ak+r) 0 and cong p (ak2+r) 0 -> cong p (ak+r)(ak2+r) (sym+trans)
               -> cancel r -> cong p ak ak2.  LEFT. *)
            val sym2 = cong_sym_at (p, add ak2 r, ZeroC) hK2hi        (* cong p 0 (ak2+r) *)
            val both = cong_trans_at (p, add ak r, ZeroC, add ak2 r) hKhi sym2  (* cong p (ak+r)(ak2+r) *)
            val cancel = cong_add_cancel_r p ak ak2 r both           (* cong p ak ak2 *)
          in Thm.implies_intr (ctermGG (jT (cong p (add ak2 r2) ZeroC)))
               (disjI1_GG_at (cong p ak ak2, cong p (add ak ak2) ZeroC) cancel) end;
      in disjE_GG_at (cong p ak2 r2, cong p (add ak2 r2) ZeroC, goalC) djk2 caseK2lo caseK2hi end;

    val cLo = let val h = Thm.assume (ctermGG (jT (cong p ak r)))
              in Thm.implies_intr (ctermGG (jT (cong p ak r))) (caseKlo h) end;
    val cHi = let val h = Thm.assume (ctermGG (jT (cong p (add ak r) ZeroC)))
              in Thm.implies_intr (ctermGG (jT (cong p (add ak r) ZeroC))) (caseKhi h) end;
  in disjE_GG_at (cong p ak r, cong p (add ak r) ZeroC, goalC) djk cLo cHi end;
val () = out "STAGE_P3_BRIDGE_READY\n";

(* ============================================================================
   LAR_PERM  (the P3 deliverable)
     prime2 p ==> ~(dvd p a) ==> oeq (sub p 1)(add m m) ==>
       oeq (lprod (lmap (%k. lar a p k) (uptoF m))) (lprod (uptoF m))
   ============================================================================ *)
val lar_perm =
  let
    ...
    fun gOf k = lar aF pF k;     (* beta of gLam at k *)

    (* instantiate lprod_perm_of_inj at g := gLam, A := U, B := U ; beta_norm *)
    val permI = beta_norm (Drule.infer_instantiate ctxtGG
          [(("g_inj",0), ctermGG gLam), (("A_fin",0), ctermGG U), (("B_ext",0), ctermGG U)]
          (toGG lprod_perm_of_inj));
    (* permI : jT(lnodup U) ==> jT(lnodup U) ==> jT(oeq(llen U)(llen U))
                 ==> jT(Into U U) ==> jT(Inj U) ==> jT(oeq(lprod(lmap gLam U))(lprod U)) *)

    (* (1) lnodup U *)
    val hND = beta_norm (Drule.infer_instantiate ctxtGG [(("k_nd",0), ctermGG mF)] (toGG lnodup_upto));
                  (* jT (lnodup (uptoF m)) *)
    (* (2) llen U = llen U : refl *)
    val hLen = oeqRefl_GG (llen U);
    (* (3) Into U U : forall xi. lmem xi U ==> lmem (lar a p xi) U *)
    val intoThm =
      let
        val xi = Free("xi", natT);
        val Pabs = Term.lambda xi (mkImp (lmem xi U) (lmem (gOf xi) U));
        val hmem = Thm.assume (ctermGG (jT (lmem xi U)));
        val cj = lmem_upto_fwd_at (xi, mF) hmem;          (* Conj (lt 0 xi)(le xi m) *)
        val h1 = conjunct1_GG_at (lt ZeroC xi, le xi mF) cj;   (* lt 0 xi = le 1 xi *)
        val h2 = conjunct2_GG_at (lt ZeroC xi, le xi mF) cj;   (* le xi m *)
        val nNk = not_dvd_in_range pF mF xi hPrime hOdd h1 h2;  (* ~(dvd p xi) *)
        val rng = lar_in_range_at (pF, aF, xi, mF) hPrime hNa nNk hOdd;  (* Conj (le 1 (lar a p xi))(le (lar a p xi) m) *)
        val r1 = conjunct1_GG_at (le (suc ZeroC) (gOf xi), le (gOf xi) mF) rng;  (* le 1 (lar..) = lt 0 (lar..) *)
        val r2 = conjunct2_GG_at (le (suc ZeroC) (gOf xi), le (gOf xi) mF) rng;  (* le (lar..) m *)
        val conj = conjI_GG_at (lt ZeroC (gOf xi), le (gOf xi) mF) r1 r2;
        val memU = lmem_upto_bwd_at (gOf xi, mF) conj;     (* lmem (lar a p xi) U *)
    ...
    (* (4) Inj U : forall xj yj. lmem xj U ==> lmem yj U ==> oeq (lar a p xj)(lar a p yj) ==> oeq xj yj *)
    val injThm =
      let
        ...
        (* dichotomy from the bridge *)
        val dicho = lar_inj_dichotomy pF aF xj yj hPrime hNa nNkx nNky hEq;
            (* Disj (cong p (a*xj)(a*yj))(cong p (a*xj + a*yj) 0) *)
        (* abs_inj needs : le 1 xj, le xj m, le 1 yj, le yj m, dichotomy *)
        val xjEq = abs_inj_at (pF, aF, xj, yj, mF) hPrime hNa hOdd x1 x2 y1 y2 dicho;  (* oeq xj yj *)
    ...
    val eqv = Thm.implies_elim e4 injThm;
        (* jT (oeq (lprod (lmap gLam U)) (lprod U)) *)
    val d3 = Thm.implies_intr (ctermGG (jT (oeq (sub pF oneC) (add mF mF)))) eqv;
    val d2 = Thm.implies_intr (ctermGG (jT (neg (dvd pF aF)))) d3;
    val d1 = Thm.implies_intr (ctermGG (jT (prime2 pF))) d2;
  in varify d1 end;

(* ---- validate lar_perm : 0-hyp + aconv ---- *)
val pVlp = Var(("p",0), natT); val aVlp = Var(("a",0), natT); val mVlp = Var(("m",0), natT);
val gPartV = larC $ aVlp $ pVlp;                       (* eta-contracted : lar a p *)
val gLamV  = Abs("k_g", natT, lar aVlp pVlp (Bound 0)); (* full lambda form *)
fun mk_i_lar_perm gform =
  Logic.mk_implies (jT (prime2 pVlp),
    Logic.mk_implies (jT (neg (dvd pVlp aVlp)),
      Logic.mk_implies (jT (oeq (sub pVlp oneC) (add mVlp mVlp)),
        jT (oeq (lprod (lmapC $ gform $ (uptoF mVlp))) (lprod (uptoF mVlp))))));
val i_lar_perm     = mk_i_lar_perm gPartV;
val i_lar_perm_lam = mk_i_lar_perm gLamV;
val r_lar_perm = (length (Thm.hyps_of lar_perm) = 0)
                 andalso (((Thm.prop_of lar_perm) aconv i_lar_perm)
                          orelse ((Thm.prop_of lar_perm) aconv i_lar_perm_lam))
                 andalso (length (Thm.extra_shyps lar_perm) = 0);
val () = if r_lar_perm then out "OK lar_perm\n" else out "FAIL lar_perm\n";
(* soundness probe : dropping ~(dvd p a) must NOT be provable *)
val probe_lar_perm_needs_ndvd =
  let val bogus = Logic.mk_implies (jT (prime2 pVlp),
        Logic.mk_implies (jT (oeq (sub pVlp oneC) (add mVlp mVlp)),
          jT (oeq (lprod (lmapC $ gLamV $ (uptoF mVlp))) (lprod (uptoF mVlp)))))
  in not ((Thm.prop_of lar_perm) aconv bogus) end;
val () = if probe_lar_perm_needs_ndvd then out "PROBE_OK lar_perm keeps ~(dvd p a)\n"
         else out "PROBE_FAIL lar_perm\n";
val () = if r_lar_perm andalso probe_lar_perm_needs_ndvd then out "LAR_PERM_OK\n" else out "LAR_PERM_FAILED\n";

(* ============================================================================
   STAGE P : AXIOM AUDIT + FINAL VERDICT
   The only classical axiom in thyGG must remain ex_middle; all else are the
   conservative Peano/oeq/order/dvd/sub/sumf/prodf recursion axioms, the
   natlist datatype axioms, lmap/rmod/rdiv/upto/lar defining axioms, and the
   two NEW conservative defs added in Stage P : lhd_cons / ltl_cons (list
   destructors).  NO Gauss / permutation result is asserted as an axiom.
   ============================================================================ *)
val () = out "STAGE_P_AUDIT_BEGIN\n";
val allAxGG = Theory.all_axioms_of thyGG;
val () = out ("stageP_thyGG_axiom_count=" ^ Int.toString (length allAxGG) ^ "\n");
val hasEMgg = List.exists (fn (nm,_) =>
      String.isSuffix "ex_middle" nm orelse nm = "ex_middle") allAxGG;
val () = out ("stageP_thyGG_ex_middle_present=" ^ Bool.toString hasEMgg ^ "\n");
(* confirm lhd_cons / ltl_cons are the only NEW axioms over thyG *)
val newAxNames = List.filter (fn nm => String.isSuffix "lhd_cons" nm orelse String.isSuffix "ltl_cons" nm)
                   (map fst allAxGG);
val () = out ("stageP_new_axioms=" ^ String.concatWith "," newAxNames ^ "\n");
val () = out "STAGE_P_AUDIT_END\n";

(* soundness gate : all banked Stage-P lemmas 0-hyp + no extra sort-hyps *)
fun clean th = (length (Thm.hyps_of th) = 0) andalso (length (Thm.extra_shyps th) = 0);
val stageP_clean =
     clean extract andalso clean mem_remove_fwd andalso clean mem_remove_bwd
     andalso clean mem_remove_neq andalso clean llen_remove andalso clean nodup_remove
     andalso clean lnodup_upto andalso clean lmem_upto_fwd andalso clean lmem_upto_bwd
     andalso clean lprod_perm_of_inj andalso clean lar_perm;
val () = if natlistLibOK andalso r_lprod_perm andalso probe_perm_needs_inj
            andalso r_lar_perm andalso probe_lar_perm_needs_ndvd
            andalso stageP_clean andalso hasEMgg
         then out "GAUSS_STAGE_P_ALL_OK\n" else out "GAUSS_STAGE_P_INCOMPLETE\n";

(* ############################################################################
   ##################  GAUSS'S LEMMA — FINAL ASSEMBLY  ########################
   Appended to gauss_stageP.sml (base ends at lar_perm on ctxtGG/thyGG).
   Stages:
     (S1) prod_axk_eq_pow   : oeq (lprod (lmap axk U)) (mult (pow a m) (lprod U))
          marker PROD_AXK_OK.   [axk = %k. a*k]
     (HELP) prod_uptoF_not_dvd : prime2 p ==> oeq(sub p 1)(m+m) ==> ~(p | lprod U)
     (S2) prod_split_sign   : EXISTS S. cong p (lprod(lmap axk U)) (mult S (lprod(lmap (lar a p) U)))
                              AND (cong p S 1 OR cong p S (sub p 1))   [S = running flip-product]
          marker PROD_SPLIT_OK.
     (G)  gauss_lemma       : EXISTS S. cong p (pow a m) S AND (cong p S 1 OR cong p S (sub p 1))
          marker GAUSS_LEMMA_OK.
   ############################################################################ *)
val () = out "GAUSS_FINAL_BEGIN\n";
val () = PolyML.print_depth 100;

(* ---- pow at GG ---- *)
val pow_Zero_GG = varifyGG pow_Zero_ax;   (* oeq (pow a 0)(Suc 0) *)
val pow_Suc_GG  = varifyGG pow_Suc_ax;    (* oeq (pow a (Suc n))(mult a (pow a n)) *)
fun powZero_GG t = beta_norm (Drule.infer_instantiate ctxtGG [(("a",0), ctermGG t)] pow_Zero_GG);
fun powSuc_GG (at,nt) = beta_norm (Drule.infer_instantiate ctxtGG
      [(("a",0), ctermGG at),(("n",0), ctermGG nt)] pow_Suc_GG);

(* ---- mult units at GG ---- *)
val mult_1_right_GG = varifyGG mult_1_right;  (* oeq (mult n (Suc 0)) n *)
val mult_0_right_GG = varifyGG mult_0_right;  (* oeq (mult n 0) 0 *)
fun mult1r_GG t = beta_norm (Drule.infer_instantiate ctxtGG [(("n",0), ctermGG t)] mult_1_right_GG);
fun mult0r_GG t = beta_norm (Drule.infer_instantiate ctxtGG [(("n",0), ctermGG t)] mult_0_right_GG);

(* ---- prime_not_dvd_mult at GG (re-varify the ctxtG-proven prime_not_dvd_mult) ---- *)
val prime_not_dvd_mult_GG = varifyGG prime_not_dvd_mult;
fun prime_not_dvd_mult_at (pt, at, kt) hPrime hNa hNk =
  let val inst = beta_norm (Drule.infer_instantiate ctxtGG
        [(("p",0), ctermGG pt),(("a",0), ctermGG at),(("k",0), ctermGG kt)] prime_not_dvd_mult_GG)
  in Thm.implies_elim (Thm.implies_elim (Thm.implies_elim inst hPrime) hNa) hNk end;

(* ---- nat induction wrapper on ctxtGG (uses meta_nat_induct_GG : P 0 ==> (!n. P n ==> P (Suc n)) ==> !n. P n
        actually we use nat_induct_GG which is the same shape) ---- *)
(* shape probe of nat_induct_GG : we mirror lmem_upto_bwd's use of list_induct; but here we want
   induction over the NAT argument n of uptoF n. Use meta_nat_induct_GG. *)
val () = out "GAUSS_FINAL_HELPERS_READY\n";

(* the two map-functions ; use ETA-CONTRACTED forms so they match the shapes
   that infer_instantiate+beta_norm produce inside nat_induct (which eta-contracts). *)
fun mkAxk a = multC $ a;          (* mult a : nat->nat   (= %k. a*k eta-contracted) *)
fun mkLar a p = larC $ a $ p;     (* lar a p : nat->nat  (= %k. lar a p k eta-contracted) *)

(* ============================================================================
   (S1) prod_axk_eq_pow :
     oeq (lprod (lmap (%k. a*k) (uptoF m))) (mult (pow a m) (lprod (uptoF m)))
   by induction on m over uptoF.  No prime/odd hyps needed.
   ============================================================================ *)
local
    val aF = Free("a", natT);
    val gAxk = mkAxk aF;
    fun lhs n = lprod (lmap gAxk (uptoF n));
    fun rhs n = mult (pow aF n) (lprod (uptoF n));
    val nVar = Free("n_s1", natT);
    val Pabs = Term.lambda nVar (oeq (lhs nVar) (rhs nVar));
    val mF2  = Free("m", natT);
    (* base : n = 0 :  lprod(lmap g (uptoF 0)) = lprod(lmap g lnil) = lprod lnil = 1
              rhs = pow a 0 * lprod(uptoF 0) = 1 * lprod lnil = 1 * 1 = 1 *)
    val base =
      let
        val u0 = uptoZero_GG ();                        (* leq (uptoF 0) lnil *)
        (* lmap g (uptoF 0) : rewrite (uptoF 0) -> lnil under lmap g via leq_subst on %z. lprod(lmap g z) *)
        val zL = Free("z_b1", natlistT);
        val Plm = Term.lambda zL (oeq (lprod (lmap gAxk (uptoF ZeroC))) (lprod (lmap gAxk zL)));
        val lm_refl = oeqRefl_GG (lprod (lmap gAxk (uptoF ZeroC)));
        val lhs_to_nil = leq_rw_GG (Plm, uptoF ZeroC, lnilC) u0 lm_refl;  (* lprod(lmap g (uptoF 0)) = lprod(lmap g lnil) *)
        val lmnil = lmapNil_GG gAxk;                    (* leq (lmap g lnil) lnil *)
        val lp_lmnil = lprod_cong_GG (lmap gAxk lnilC, lnilC) lmnil;  (* lprod(lmap g lnil) = lprod lnil *)
        val lp_nil = lprodNil_GG ();                    (* lprod lnil = 1 *)
        val lhs_eq_1 = oeq_trans OF [oeq_trans OF [lhs_to_nil, lp_lmnil], lp_nil];  (* lhs 0 = 1 *)
        (* rhs : pow a 0 * lprod(uptoF 0) = 1 * 1 = 1 *)
        val pz = powZero_GG aF;                          (* pow a 0 = 1 *)
        val lpu_to_nil = lprod_cong_GG (uptoF ZeroC, lnilC) u0;  (* lprod(uptoF 0) = lprod lnil *)
        val lpu_eq_1 = oeq_trans OF [lpu_to_nil, lp_nil];        (* lprod(uptoF 0) = 1 *)
        (* mult (pow a 0)(lprod(uptoF 0)) : rewrite both args to 1 *)
        val r1 = mult_cong_l_GG (pow aF ZeroC, suc ZeroC, lprod (uptoF ZeroC)) pz;  (* (pow a 0)*lpu = 1*lpu *)
        val r2 = mult_cong_r_GG (suc ZeroC, lprod (uptoF ZeroC), suc ZeroC) lpu_eq_1; (* 1*lpu = 1*1 *)
        val m11 = mult1l_GG (suc ZeroC);                 (* 1*1 = 1 *)
        val rhs_eq_1 = oeq_trans OF [oeq_trans OF [r1, r2], m11];  (* rhs 0 = 1 *)
        val res = oeq_trans OF [lhs_eq_1, oeq_sym OF [rhs_eq_1]];  (* lhs 0 = rhs 0 *)
      in res end;
    (* step : assume P n ; prove P (Suc n).
         uptoF (Suc n) = lcons (Suc n) (uptoF n)
         lmap g (uptoF(Suc n)) = lcons (g (Suc n)) (lmap g (uptoF n))   [lmap_cons after rewriting upto]
         lprod (...) = g(Suc n) * lprod(lmap g (uptoF n))               [lprod_cons]
                     = (a*(Suc n)) * (pow a n * lprod(uptoF n))         [IH]
         rhs (Suc n) = pow a (Suc n) * lprod(uptoF (Suc n))
                     = (a * pow a n) * ((Suc n) * lprod(uptoF n))       [pow_Suc, lprod_cons after upto]
         equal by assoc/comm. *)
    val stepF =
      let
        val xn = Free("x_s1", natT);
        val IH = Thm.assume (ctermGG (jT (oeq (lhs xn) (rhs xn))));
        val Sn = suc xn;
        val gSn = mult aF Sn;            (* g (Suc n) = a*(Suc n) *)
        (* ---- LHS ---- *)
        val us = uptoSuc_GG xn;          (* leq (uptoF(Suc n)) (lcons (Suc n)(uptoF n)) *)
        (* lprod(lmap g (uptoF(Suc n))) = lprod(lmap g (lcons (Suc n)(uptoF n))) *)
        val zL1 = Free("z_s1a", natlistT);
        val P1 = Term.lambda zL1 (oeq (lprod (lmap gAxk (uptoF Sn))) (lprod (lmap gAxk zL1)));
        val lhs0 = leq_rw_GG (P1, uptoF Sn, lcons Sn (uptoF xn)) us (oeqRefl_GG (lprod (lmap gAxk (uptoF Sn))));
        (* lmap g (lcons (Suc n)(uptoF n)) = lcons (g(Suc n))(lmap g (uptoF n)) [lmap_cons] *)
        val lmc = lmapCons_GG (gAxk, Sn, uptoF xn);  (* leq (lmap g (lcons (Suc n) U)) (lcons (g(Suc n)) (lmap g U)) *)
        (* beta : g (Suc n) appears as (gAxk $ Sn); beta_norm gives mult a (Suc n) = gSn *)
        val lmc_b = beta_norm lmc;
        val lp_lmc = lprod_cong_GG (lmap gAxk (lcons Sn (uptoF xn)), lcons gSn (lmap gAxk (uptoF xn))) lmc_b;
        val lhs1 = oeq_trans OF [lhs0, lp_lmc];   (* lprod(lmap g(uptoF(Suc n))) = lprod(lcons gSn (lmap g U)) *)
        val lpc = lprodCons_GG (gSn, lmap gAxk (uptoF xn));  (* = gSn * lprod(lmap g U) *)
        val lhs2 = oeq_trans OF [lhs1, lpc];      (* = gSn * lprod(lmap g (uptoF n)) *)
        (* apply IH on the 2nd factor : lprod(lmap g (uptoF n)) = pow a n * lprod(uptoF n) *)
        val ih2 = mult_cong_r_GG (gSn, lprod (lmap gAxk (uptoF xn)), rhs xn) IH;
        val lhs3 = oeq_trans OF [lhs2, ih2];      (* = gSn * (pow a n * lprod(uptoF n)) *)
        (* ---- RHS ---- *)
        (* rhs(Suc n) = pow a (Suc n) * lprod(uptoF(Suc n)) *)
        val psuc = powSuc_GG (aF, xn);            (* pow a (Suc n) = a * pow a n *)
        (* lprod(uptoF(Suc n)) = lprod(lcons (Suc n)(uptoF n)) = (Suc n) * lprod(uptoF n) *)
        val zL2 = Free("z_s1b", natlistT);
        val P2 = Term.lambda zL2 (oeq (lprod (uptoF Sn)) (lprod zL2));
        val lpu0 = leq_rw_GG (P2, uptoF Sn, lcons Sn (uptoF xn)) us (oeqRefl_GG (lprod (uptoF Sn)));
        val lpuc = lprodCons_GG (Sn, uptoF xn);   (* lprod(lcons (Suc n) U) = (Suc n)*lprod U *)
        val lpu_eq = oeq_trans OF [lpu0, lpuc];   (* lprod(uptoF(Suc n)) = (Suc n)*lprod(uptoF n) *)
        (* rhs(Suc n) = pow a(Suc n) * lprod(uptoF(Suc n)) ; rewrite first arg via psuc, 2nd via lpu_eq *)
        val rstep1 = mult_cong_l_GG (pow aF Sn, mult aF (pow aF xn), lprod (uptoF Sn)) psuc;
                       (* pow a(Suc n)*lpu(Suc n) = (a*pow a n)*lpu(Suc n) *)
        val rstep2 = mult_cong_r_GG (mult aF (pow aF xn), lprod (uptoF Sn), mult Sn (lprod (uptoF xn))) lpu_eq;
                       (* (a*pow a n)*lpu(Suc n) = (a*pow a n)*((Suc n)*lpu n) *)
        val rhs_form = oeq_trans OF [rstep1, rstep2];
                       (* rhs(Suc n) = (a*pow a n)*((Suc n)*lprod(uptoF n)) *)
        (* ---- bridge LHS form  gSn*(pow a n * lpu)  =  (a*pow a n)*((Suc n)*lpu) ---- *)
        (* gSn = a*(Suc n).  Let P = pow a n, L = lprod(uptoF n).
           LHS form : (a*(Suc n)) * (P * L)
           RHS form : (a*P) * ((Suc n) * L)
           Both equal a * (Suc n) * P * L in any association/order. *)
        val P = pow aF xn; val L = lprod (uptoF xn);
        (* LHS: (a*Sn)*(P*L) = a*(Sn*(P*L))            [assoc^-1 : (a*Sn)*(P*L) = a*(Sn*(P*L))] *)
        val l_a1 = multassoc_GG (aF, Sn, mult P L);    (* (a*Sn)*(P*L) = a*(Sn*(P*L)) *)
        (* Sn*(P*L) = Sn*(P*L) ; reorder inside: P*L want to become P*(Sn ... no. Let me push to canonical a*(P*(Sn*L)) *)
        (* Sn*(P*L) = (Sn*P)*L [assoc] = (P*Sn)*L [comm] = P*(Sn*L) [assoc^-1] *)
        val s1 = oeq_sym OF [multassoc_GG (Sn, P, L)];  (* Sn*(P*L) = (Sn*P)*L  [sym of (Sn*P)*L = Sn*(P*L)] *)
        val s2 = mult_cong_l_GG (mult Sn P, mult P Sn, L) (multcomm_GG (Sn, P));  (* (Sn*P)*L = (P*Sn)*L *)
        val s3 = multassoc_GG (P, Sn, L);              (* (P*Sn)*L = P*(Sn*L) *)
        val sn_chain = oeq_trans OF [oeq_trans OF [s1, s2], s3];  (* Sn*(P*L) = P*(Sn*L) *)
        val l_inner = mult_cong_r_GG (aF, mult Sn (mult P L), mult P (mult Sn L)) sn_chain;
                       (* a*(Sn*(P*L)) = a*(P*(Sn*L)) *)
        val l_canon = oeq_trans OF [l_a1, l_inner];    (* (a*Sn)*(P*L) = a*(P*(Sn*L)) *)
        (* RHS: (a*P)*((Sn)*L) = a*(P*(Sn*L))          [assoc^-1] *)
        val r_a1 = multassoc_GG (aF, P, mult Sn L);    (* (a*P)*(Sn*L) = a*(P*(Sn*L)) *)
        (* now LHS form = (a*Sn)*(P*L) = a*(P*(Sn*L)) = (a*P)*(Sn*L) = rhs_form chain *)
        val canon_eq = oeq_trans OF [l_canon, oeq_sym OF [r_a1]];  (* (a*Sn)*(P*L) = (a*P)*(Sn*L) *)
        (* lhs3 : lhs(Suc n) = gSn*(P*L) = (a*Sn)*(P*L).  rhs_form : rhs(Suc n) = (a*P)*(Sn*L). *)
        val res = oeq_trans OF [lhs3, oeq_trans OF [canon_eq, oeq_sym OF [rhs_form]]];
                       (* lhs(Suc n) = rhs(Suc n) *)
        (* discharge IH -> meta implication *)
        val metaStep = Thm.implies_intr (ctermGG (jT (oeq (lhs xn) (rhs xn)))) res;
      in Thm.forall_intr (ctermGG xn) metaStep end;
    val atm = nat_induct_GG_run Pabs mF2 base stepF;   (* jT (oeq (lhs m)(rhs m)) *)
in
  val prod_axk_eq_pow = varify atm;
end;

(* ---- validate S1 ---- *)
val pV1 = Var(("p",0),natT); val aV1 = Var(("a",0),natT); val mV1 = Var(("m",0),natT);
val gAxkVeta = multC $ aV1;                          (* eta-contracted *)
val gAxkVlam = Abs("k_axk", natT, mult aV1 (Bound 0));
fun mk_i_prod_axk g = jT (oeq (lprod (lmapC $ g $ (uptoF mV1)))
                              (mult (pow aV1 mV1) (lprod (uptoF mV1))));
val i_prod_axk     = mk_i_prod_axk gAxkVeta;
val i_prod_axk_lam = mk_i_prod_axk gAxkVlam;
val r_prod_axk = (length (Thm.hyps_of prod_axk_eq_pow) = 0)
                 andalso (((Thm.prop_of prod_axk_eq_pow) aconv i_prod_axk)
                          orelse ((Thm.prop_of prod_axk_eq_pow) aconv i_prod_axk_lam))
                 andalso (length (Thm.extra_shyps prod_axk_eq_pow) = 0);
val () = if r_prod_axk then out "OK prod_axk_eq_pow\n" else
  (out "FAIL prod_axk_eq_pow\n";
   out ("  got = " ^ Syntax.string_of_term ctxtGG (Thm.prop_of prod_axk_eq_pow) ^ "\n");
   out ("  int = " ^ Syntax.string_of_term ctxtGG i_prod_axk ^ "\n"));
val () = if r_prod_axk then out "PROD_AXK_OK\n" else out "PROD_AXK_FAILED\n";

(* ############################################################################
   ##############  SIGN MACHINERY  (the genuine (-1)^mu bookkeeping)  ##########
   The per-element flip sign is s_k in {1, p-1}.  p-1 = (sub p 1) is the kernel
   encoding of -1 (since p-1 == -1 mod p).  We prove the {1, p-1} set is closed
   under multiplication mod p, which is exactly the (-1)*(-1)=+1 / parity law.
   NONE of this uses the euler dichotomy -- it is pure modular sign arithmetic.
   ############################################################################ *)
val () = out "SIGN_MACH_BEGIN\n";

(* re-varify the needed arithmetic onto GG *)
val right_distrib_GG = varifyGG right_distrib;   (* (m+n)*k = m*k + n*k *)
fun rdist_GG (mt,nt,kt) = beta_norm (Drule.infer_instantiate ctxtGG
      [(("m",0), ctermGG mt),(("n",0), ctermGG nt),(("k",0), ctermGG kt)] right_distrib_GG);
val sub_n_0_GG     = varifyGG sub_n_0_ax;
val sub_Suc_Suc_GG = varifyGG sub_Suc_Suc_ax;
fun subN0_GG t = beta_norm (Drule.infer_instantiate ctxtGG [(("n",0), ctermGG t)] sub_n_0_GG);
fun subSS_GG (nt,kt) = beta_norm (Drule.infer_instantiate ctxtGG
      [(("n",0), ctermGG nt),(("k",0), ctermGG kt)] sub_Suc_Suc_GG);
(* add_cong_l on ctxtGG : oeq p q ==> oeq (add p k)(add q k) *)
fun add_cong_l_GG (pT,qT,kT) hpq =
  let val Pabs = Abs("z", natT, oeq (add pT kT) (add (Bound 0) kT))
  in oeq_rw_GG (Pabs, pT, qT) hpq (oeqRefl_GG (add pT kT)) end;

(* ---- one_plus_pm1 : prime2 p ==> oeq (add (Suc 0) (sub p 1)) p     [1 + (p-1) = p] ---- *)
fun one_plus_pm1 p hPrime =
  let
    val lt0p = lt0_of_prime_GG p hPrime;       (* lt 0 p = le (Suc 0) p = Ex w. p = (Suc 0) + w *)
    val Pabs = Abs("w", natT, oeq p (add (suc ZeroC) (Bound 0)));
    fun body wF (hw:thm) =     (* hw : p = (Suc 0) + w = Suc(0 + w) *)
      let
        val aS = addSuc_GG (ZeroC, wF);          (* (Suc 0 + w) = Suc(0 + w) *)
        val a0 = beta_norm (Drule.infer_instantiate ctxtGG [(("n",0), ctermGG wF)] (varifyGG add_0)); (* (0+w)=w *)
        val pSucw = oeq_trans OF [oeq_trans OF [hw, aS], Succong_GG a0];  (* p = Suc w *)
        (* sub p 1 = w *)
        val zsp = Free("z_opp", natT);
        val Psp = Term.lambda zsp (oeq (sub zsp oneC) wF);
        val ss = subSS_GG (wF, ZeroC);           (* sub (Suc w)(Suc 0) = sub w 0 *)
        val sn0 = subN0_GG wF;                   (* sub w 0 = w *)
        val sub_Sw_eq_w = oeq_trans OF [ss, sn0];  (* sub (Suc w) 1 = w *)
        (* sub p 1 = w : rewrite (Suc w) -> p backwards *)
        val sub_p1_w = oeq_rw_GG (Term.lambda zsp (oeq (sub zsp oneC) wF), suc wF, p) (oeq_sym OF [pSucw]) sub_Sw_eq_w;
                       (* sub p 1 = w *)
        (* 1 + (p-1) = 1 + w = Suc(0+w) = Suc w = p *)
        val r1 = add_cong_r_GG (suc ZeroC, sub p oneC, wF) sub_p1_w;  (* (1 + (p-1)) = (1 + w) *)
        val r2 = addSuc_GG (ZeroC, wF);          (* (Suc 0 + w) = Suc(0+w) *)
        val r3 = Succong_GG a0;                  (* Suc(0+w) = Suc w *)
        val one_w_eq_p = oeq_trans OF [oeq_trans OF [oeq_trans OF [r1, r2], r3], oeq_sym OF [pSucw]];
                       (* (1 + (p-1)) = p *)
      in one_w_eq_p end;
  in exE_GG_at (Pabs, oeq (add (suc ZeroC)(sub p oneC)) p) lt0p "w_opp" body end;

(* ---- p_mult_zero : cong p (mult p x) 0   [p*x == 0 mod p] ---- *)
(* cong p (p*x) 0 : congL = Ex k. 0 = (p*x) + p*k ... easier congR : Ex k. (p*x) = 0 + p*k.
   0 + p*x = p*x [add_0], so (p*x) = 0 + p*x via sym ; witness k = x. *)
fun p_mult_zero p x =
  let
    val a0 = beta_norm (Drule.infer_instantiate ctxtGG [(("n",0), ctermGG (mult p x))] (varifyGG add_0));
                 (* (0 + p*x) = p*x *)
    val body = oeq_sym OF [a0];   (* (p*x) = (0 + p*x) *)
  in cong_introR_GG (p, mult p x, ZeroC, x) body end;   (* cong p (p*x) 0 *)

(* ---- neg_one_mult : prime2 p ==> cong p (add x (mult (sub p 1) x)) 0
        [ x + (p-1)*x == 0 mod p,  because (1 + (p-1))*x = p*x ] ---- *)
fun neg_one_mult p x hPrime =
  let
    val opp = one_plus_pm1 p hPrime;             (* (1 + (p-1)) = p *)
    (* (1 + (p-1))*x = p*x : mult_cong_l on opp *)
    val lcong = mult_cong_l_GG (add (suc ZeroC)(sub p oneC), p, x) opp;  (* ((1+(p-1))*x) = (p*x) *)
    (* (1 + (p-1))*x = 1*x + (p-1)*x  [right_distrib] *)
    val rd = rdist_GG (suc ZeroC, sub p oneC, x);  (* ((1+(p-1))*x) = (1*x + (p-1)*x) *)
    (* 1*x = x [mult1l] *)
    val m1 = mult1l_GG x;                          (* (1*x) = x *)
    val rd_x = add_cong_l_GG (mult (suc ZeroC) x, x, mult (sub p oneC) x) m1;
                  (* (1*x + (p-1)*x) = (x + (p-1)*x) *)
    val sum_eq_px = oeq_trans OF [oeq_sym OF [oeq_trans OF [rd, rd_x]], lcong];
                  (* (x + (p-1)*x) = (p*x) *)
    val pmz = p_mult_zero p x;                     (* cong p (p*x) 0 *)
    (* rewrite (p*x) -> (x + (p-1)*x) inside cong p _ 0 *)
    val zc = Free("z_nom", natT);
    val Pc = Term.lambda zc (cong p zc ZeroC);
    val res = oeq_rw_GG (Pc, mult p x, add x (mult (sub p oneC) x)) (oeq_sym OF [sum_eq_px]) pmz;
                  (* cong p (x + (p-1)*x) 0 *)
  in res end;

(* ---- pm1_sq : prime2 p ==> cong p (mult (sub p 1)(sub p 1)) (Suc 0)
        [ (p-1)^2 == 1 mod p ].
   Proof:  neg_one_mult at x=(p-1) gives cong p ((p-1) + (p-1)*(p-1)) 0.
           p_mult_zero / direct: cong p (1 + (p-1)) ... actually use:
             cong p ((p-1)*(p-1) + (p-1)) 0     (comm)
             cong p (1 + (p-1)) 0               (since 1+(p-1)=p, p==0)
           hence cong p ((p-1)*(p-1) + (p-1)) (1 + (p-1)), cancel (p-1) -> cong p ((p-1)^2) 1.
   ---- *)
fun pm1_sq p hPrime =
  let
    val pm1 = sub p oneC;
    val nom = neg_one_mult p pm1 hPrime;         (* cong p ((p-1) + (p-1)*(p-1)) 0 *)
    (* reorder LHS: (p-1) + (p-1)*(p-1) = (p-1)*(p-1) + (p-1)  [comm] *)
    val comm = addcomm_GG (pm1, mult pm1 pm1);    (* ((p-1)+(p-1)*(p-1)) = ((p-1)*(p-1)+(p-1)) *)
    val zc = Free("z_pmsq", natT);
    val Pc = Term.lambda zc (cong p zc ZeroC);
    val nom2 = oeq_rw_GG (Pc, add pm1 (mult pm1 pm1), add (mult pm1 pm1) pm1) comm nom;
                 (* cong p ((p-1)*(p-1) + (p-1)) 0 *)
    (* (1 + (p-1)) = p, and cong p p 0 : p_mult_zero with x=1 then mult1r? simpler: cong p (1+(p-1)) 0. *)
    val opp = one_plus_pm1 p hPrime;             (* (1 + (p-1)) = p *)
    (* cong p p 0 : p = p*1 [mult1r sym], cong p (p*1) 0 [p_mult_zero], rewrite *)
    val pm1r = mult1r_GG p;                       (* (p*1) = p *)
    val pmz1 = p_mult_zero p (suc ZeroC);         (* cong p (p*1) 0 *)
    val congp0 = oeq_rw_GG (Term.lambda zc (cong p zc ZeroC), mult p (suc ZeroC), p) pm1r pmz1;
                 (* cong p p 0 *)
    val cong_1pm1_0 = oeq_rw_GG (Term.lambda zc (cong p zc ZeroC), p, add (suc ZeroC) pm1)
                        (oeq_sym OF [opp]) congp0;   (* cong p (1 + (p-1)) 0 *)
    (* now cong p ((p-1)^2 + (p-1)) 0 and cong p (1 + (p-1)) 0 -> cong p ((p-1)^2 + (p-1))(1 + (p-1)) *)
    val sym01 = cong_sym_at (p, add (suc ZeroC) pm1, ZeroC) cong_1pm1_0;  (* cong p 0 (1+(p-1)) *)
    val both = cong_trans_at (p, add (mult pm1 pm1) pm1, ZeroC, add (suc ZeroC) pm1) nom2 sym01;
                 (* cong p ((p-1)^2 + (p-1)) (1 + (p-1)) *)
    val cancel = cong_add_cancel_r p (mult pm1 pm1) (suc ZeroC) pm1 both;  (* cong p ((p-1)^2) 1 *)
  in cancel end;

val () = out "SIGN_MACH_PM1SQ_READY\n";

(* ---- sign predicate: isSign p s  ==  (cong p s 1) OR (cong p s (sub p 1)) ---- *)
fun isSign p s = mkDisj (cong p s oneC) (cong p s (sub p oneC));

(* sign_one  : isSign p 1   (left, cong_refl) *)
fun sign_one p = disjI1_GG_at (cong p oneC oneC, cong p oneC (sub p oneC)) (cong_refl_at (p, oneC));
(* sign_pm1  : isSign p (p-1)  (right, cong_refl) *)
fun sign_pm1 p = disjI2_GG_at (cong p (sub p oneC) oneC, cong p (sub p oneC)(sub p oneC))
                   (cong_refl_at (p, sub p oneC));

(* sign_close p s1 s2 hPrime hS1 hS2 :
     isSign p s1 ==> isSign p s2 ==> isSign p (mult s1 s2)
   The {1, p-1}-set is closed under * mod p:  the GENUINE (-1)^mu parity law.
   4 cases via two disjE; the (p-1)*(p-1)==1 case uses pm1_sq. ---- *)
fun sign_close p s1 s2 hPrime hS1 hS2 =
  let
    val pm1 = sub p oneC;
    val goalC = isSign p (mult s1 s2);
    fun mul_cong (c1, c2) h1 h2 = cong_mult_GG_at (p, s1, c1, s2, c2) h1 h2;  (* cong p (s1*s2)(c1*c2) *)
    fun rwR (xT, yT) hxy hcong =
      let val zc = Free("z_sc", natT) in oeq_rw_GG (Term.lambda zc (cong p (mult s1 s2) zc), xT, yT) hxy hcong end;
    fun caseS1_one h1 =   (* h1 : cong p s1 1 *)
      let
        fun caseS2_one h2 =
          let val mc = mul_cong (oneC, oneC) h1 h2
              val m11 = mult1l_GG oneC
              val c1 = rwR (mult oneC oneC, oneC) m11 mc
          in disjI1_GG_at (cong p (mult s1 s2) oneC, cong p (mult s1 s2) pm1) c1 end
        fun caseS2_pm1 h2 =
          let val mc = mul_cong (oneC, pm1) h1 h2
              val m1 = mult1l_GG pm1
              val c1 = rwR (mult oneC pm1, pm1) m1 mc
          in disjI2_GG_at (cong p (mult s1 s2) oneC, cong p (mult s1 s2) pm1) c1 end
        val cA = Thm.implies_intr (ctermGG (jT (cong p s2 oneC))) (caseS2_one (Thm.assume (ctermGG (jT (cong p s2 oneC)))))
        val cB = Thm.implies_intr (ctermGG (jT (cong p s2 pm1))) (caseS2_pm1 (Thm.assume (ctermGG (jT (cong p s2 pm1)))))
      in disjE_GG_at (cong p s2 oneC, cong p s2 pm1, goalC) hS2 cA cB end
    fun caseS1_pm1 h1 =   (* h1 : cong p s1 (p-1) *)
      let
        fun caseS2_one h2 =
          let val mc = mul_cong (pm1, oneC) h1 h2
              val m1r = mult1r_GG pm1
              val c1 = rwR (mult pm1 oneC, pm1) m1r mc
          in disjI2_GG_at (cong p (mult s1 s2) oneC, cong p (mult s1 s2) pm1) c1 end
        fun caseS2_pm1 h2 =
          let val mc = mul_cong (pm1, pm1) h1 h2
              val sq = pm1_sq p hPrime
              val c1 = cong_trans_at (p, mult s1 s2, mult pm1 pm1, oneC) mc sq
          in disjI1_GG_at (cong p (mult s1 s2) oneC, cong p (mult s1 s2) pm1) c1 end
        val cA = Thm.implies_intr (ctermGG (jT (cong p s2 oneC))) (caseS2_one (Thm.assume (ctermGG (jT (cong p s2 oneC)))))
        val cB = Thm.implies_intr (ctermGG (jT (cong p s2 pm1))) (caseS2_pm1 (Thm.assume (ctermGG (jT (cong p s2 pm1)))))
      in disjE_GG_at (cong p s2 oneC, cong p s2 pm1, goalC) hS2 cA cB end
    val cAA = Thm.implies_intr (ctermGG (jT (cong p s1 oneC))) (caseS1_one (Thm.assume (ctermGG (jT (cong p s1 oneC)))))
    val cBB = Thm.implies_intr (ctermGG (jT (cong p s1 pm1))) (caseS1_pm1 (Thm.assume (ctermGG (jT (cong p s1 pm1)))))
  in disjE_GG_at (cong p s1 oneC, cong p s1 pm1, goalC) hS1 cAA cBB end;
val () = out "SIGN_MACH_CLOSE_READY\n";

(* ---- smoke test of sign machinery on a Free p ---- *)
val () =
  let
    val pSm = Free("p", natT);
    val hPr = Thm.assume (ctermGG (jT (prime2 pSm)));
    val sq = pm1_sq pSm hPr;     (* cong p ((p-1)*(p-1)) 1 *)
    val s1 = Free("sg1", natT); val s2 = Free("sg2", natT);
    val hS1 = Thm.assume (ctermGG (jT (isSign pSm s1)));
    val hS2 = Thm.assume (ctermGG (jT (isSign pSm s2)));
    val sc = sign_close pSm s1 s2 hPr hS1 hS2;   (* isSign p (s1*s2) *)
    val okSq = ((Thm.prop_of sq) aconv (jT (cong pSm (mult (sub pSm oneC)(sub pSm oneC)) oneC)));
    val okSc = ((Thm.prop_of sc) aconv (jT (isSign pSm (mult s1 s2))));
  in out ("SIGN_SMOKE okSq=" ^ Bool.toString okSq ^ " okSc=" ^ Bool.toString okSc ^ "\n") end;

(* ############################################################################
   ####  HELP : prod_uptoF_not_dvd  --  ~(p | lprod (uptoF m))   ###############
   prime2 p ==> oeq(sub p 1)(add m m) ==> ~(dvd p (lprod (uptoF m))).
   Generalised lemma Q(n) := le n m ==> ~(dvd p (lprod (uptoF n))), nat induction
   on n, then instantiate n := m with le_refl.  Each head (Suc x) in [1,m] is
   coprime to p (not_dvd_in_range), lifted over the product (prime_not_dvd_mult).
   ############################################################################ *)
val () = out "HELP_NOTDVD_BEGIN\n";

(* ~(dvd p 1) from prime2 p : dvd p 1 with 0<1<p contradicts prime_not_dvd_pos_lt *)
fun not_dvd_one p hPrime =
  let
    val lt01 = lt_0_suc_GG ZeroC;                 (* lt 0 (Suc 0) = le 1 1 *)
    val lt1p = prime2_gt1_GG p hPrime;            (* lt 1 p = le 2 p *)
    val hdvd = Thm.assume (ctermGG (jT (dvd p oneC)))
    val ff = prime_not_dvd_pos_lt_at (p, oneC) hdvd lt01 lt1p;  (* oFalse *)
  in impI_GG_at (dvd p oneC, oFalseC) (Thm.implies_intr (ctermGG (jT (dvd p oneC))) ff) end;

val prod_uptoF_not_dvd =
  let
    val pF = Free("p", natT); val mF = Free("m", natT);
    val hPrime = Thm.assume (ctermGG (jT (prime2 pF)));
    val hOdd   = Thm.assume (ctermGG (jT (oeq (sub pF oneC) (add mF mF))));
    (* Q(n) := le n m ==> ~(dvd p (lprod (uptoF n))) *)
    val nVar = Free("n_nd", natT);
    fun Qbody n = mkImp (le n mF) (neg (dvd pF (lprod (uptoF n))));
    val Pabs = Term.lambda nVar (Qbody nVar);
    val base =
      let
        (* uptoF 0 = lnil, lprod = 1, ~(p|1) *)
        val hle = Thm.assume (ctermGG (jT (le ZeroC mF)));
        val u0 = uptoZero_GG ();                     (* leq (uptoF 0) lnil *)
        val lpu_nil = lprod_cong_GG (uptoF ZeroC, lnilC) u0;   (* lprod(uptoF 0) = lprod lnil *)
        val lp_nil = lprodNil_GG ();                 (* lprod lnil = 1 *)
        val lpu_eq1 = oeq_trans OF [lpu_nil, lp_nil]; (* lprod(uptoF 0) = 1 *)
        val nd1 = not_dvd_one pF hPrime;             (* ~(dvd p 1) *)
        (* rewrite 1 -> lprod(uptoF 0) inside ~(dvd p _) *)
        val zc = Free("z_nd0", natT);
        val Pc = Term.lambda zc (neg (dvd pF zc));
        val nd0 = oeq_rw_GG (Pc, oneC, lprod (uptoF ZeroC)) (oeq_sym OF [lpu_eq1]) nd1;  (* ~(dvd p (lprod(uptoF 0))) *)
      in impI_GG_at (le ZeroC mF, neg (dvd pF (lprod (uptoF ZeroC))))
           (Thm.implies_intr (ctermGG (jT (le ZeroC mF))) nd0) end;
    val stepF =
      let
        val xn = Free("x_nd", natT);
        val IH = Thm.assume (ctermGG (jT (Qbody xn)));     (* le x m ==> ~(dvd p (lprod(uptoF x))) *)
        val Sn = suc xn;
        val hleSn = Thm.assume (ctermGG (jT (le Sn mF)));  (* le (Suc x) m *)
        (* le x m from le x (Suc x) + le (Suc x) m *)
        val lex_sx = le_self_suc_GG xn;                    (* le x (Suc x) *)
        val lexm = le_trans_GG_at (xn, Sn, mF) lex_sx hleSn;  (* le x m *)
        val ndtail = mp_GG_at (le xn mF, neg (dvd pF (lprod (uptoF xn)))) IH lexm;  (* ~(dvd p (lprod(uptoF x))) *)
        (* ~(dvd p (Suc x)) : not_dvd_in_range with le 1 (Suc x) and le (Suc x) m *)
        val le1Sn = lt_0_suc_GG xn;                        (* le 1 (Suc x) = lt 0 (Suc x) *)
        val ndhead = not_dvd_in_range pF mF Sn hPrime hOdd le1Sn hleSn;  (* ~(dvd p (Suc x)) *)
        (* lprod(uptoF(Suc x)) = (Suc x) * lprod(uptoF x) *)
        val us = uptoSuc_GG xn;                            (* leq (uptoF(Suc x))(lcons (Suc x)(uptoF x)) *)
        val lpu0 = leq_rw_GG (Term.lambda (Free("z_nds", natlistT)) (oeq (lprod (uptoF Sn)) (lprod (Bound 0))),
                              uptoF Sn, lcons Sn (uptoF xn)) us (oeqRefl_GG (lprod (uptoF Sn)));
        val lpuc = lprodCons_GG (Sn, uptoF xn);            (* lprod(lcons(Suc x) U) = (Suc x)*lprod U *)
        val lpu_eq = oeq_trans OF [lpu0, lpuc];            (* lprod(uptoF(Suc x)) = (Suc x)*lprod(uptoF x) *)
        (* ~(p | (Suc x)*lprod(uptoF x)) via prime_not_dvd_mult *)
        val ndprod = prime_not_dvd_mult_at (pF, Sn, lprod (uptoF xn)) hPrime ndhead ndtail;
                       (* ~(dvd p ((Suc x)*lprod(uptoF x))) *)
        (* rewrite ((Suc x)*lprod(uptoF x)) -> lprod(uptoF(Suc x)) inside ~(dvd p _) *)
        val zc = Free("z_ndp", natT);
        val Pc = Term.lambda zc (neg (dvd pF zc));
        val ndSn = oeq_rw_GG (Pc, mult Sn (lprod (uptoF xn)), lprod (uptoF Sn)) (oeq_sym OF [lpu_eq]) ndprod;
                       (* ~(dvd p (lprod(uptoF(Suc x)))) *)
        val imp = impI_GG_at (le Sn mF, neg (dvd pF (lprod (uptoF Sn))))
                    (Thm.implies_intr (ctermGG (jT (le Sn mF))) ndSn)
        val metaStep = Thm.implies_intr (ctermGG (jT (Qbody xn))) imp;
      in Thm.forall_intr (ctermGG xn) metaStep end;
    val Qm = nat_induct_GG_run Pabs mF base stepF;     (* le m m ==> ~(dvd p (lprod(uptoF m))) *)
    val lemm = le_refl_GG_at mF;                        (* le m m *)
    val res = mp_GG_at (le mF mF, neg (dvd pF (lprod (uptoF mF)))) Qm lemm;  (* ~(dvd p (lprod(uptoF m))) *)
    val d2 = Thm.implies_intr (ctermGG (jT (oeq (sub pF oneC)(add mF mF)))) res;
    val d1 = Thm.implies_intr (ctermGG (jT (prime2 pF))) d2;
  in varify d1 end;

(* validate HELP *)
val pVnd = Var(("p",0),natT); val mVnd = Var(("m",0),natT);
val i_notdvd = Logic.mk_implies (jT (prime2 pVnd),
      Logic.mk_implies (jT (oeq (sub pVnd oneC)(add mVnd mVnd)),
        jT (neg (dvd pVnd (lprod (uptoF mVnd))))));
val r_notdvd = (length (Thm.hyps_of prod_uptoF_not_dvd) = 0)
               andalso ((Thm.prop_of prod_uptoF_not_dvd) aconv i_notdvd)
               andalso (length (Thm.extra_shyps prod_uptoF_not_dvd) = 0);
val () = if r_notdvd then out "OK prod_uptoF_not_dvd\n" else
  (out "FAIL prod_uptoF_not_dvd\n";
   out ("  got = " ^ Syntax.string_of_term ctxtGG (Thm.prop_of prod_uptoF_not_dvd) ^ "\n");
   out ("  int = " ^ Syntax.string_of_term ctxtGG i_notdvd ^ "\n"));
val () = if r_notdvd then out "HELP_NOTDVD_OK\n" else out "HELP_NOTDVD_FAILED\n";

(* ############################################################################
   ####  S2 : prod_split_sign  --  THE FLIP-SIGN PRODUCT  ######################
   prime2 p ==> ~(p|a) ==> oeq(sub p 1)(add m m) ==>
     EXISTS S. cong p (lprod(lmap (mult a)(uptoF m))) (mult S (lprod(lmap (lar a p)(uptoF m))))
               AND isSign p S
   S is the RUNNING PRODUCT of the per-element flip signs s_k in {1, p-1}; this is
   exactly (-1)^mu.  Proven by nat induction on n with Q(n) := le n m ==> EXISTS S....
   ############################################################################ *)
val () = out "S2_BEGIN\n";

(* middle-four interchange : (A*B)*(C*D) = (A*C)*(B*D) *)
fun mult4_swap (A,B,C,D) =
  let
    val e1 = multassoc_GG (A, B, mult C D);            (* (A*B)*(C*D) = A*(B*(C*D)) *)
    val i1 = oeq_sym OF [multassoc_GG (B, C, D)];      (* B*(C*D) = (B*C)*D *)
    val e2 = mult_cong_r_GG (A, mult B (mult C D), mult (mult B C) D) i1;  (* A*(B*(C*D)) = A*((B*C)*D) *)
    val cm = multcomm_GG (B, C);                       (* (B*C) = (C*B) *)
    val cmL = mult_cong_l_GG (mult B C, mult C B, D) cm;  (* (B*C)*D = (C*B)*D *)
    val e3 = mult_cong_r_GG (A, mult (mult B C) D, mult (mult C B) D) cmL;  (* A*((B*C)*D) = A*((C*B)*D) *)
    val i2 = multassoc_GG (C, B, D);                   (* (C*B)*D = C*(B*D) *)
    val e4 = mult_cong_r_GG (A, mult (mult C B) D, mult C (mult B D)) i2;  (* A*((C*B)*D) = A*(C*(B*D)) *)
    val e5 = oeq_sym OF [multassoc_GG (A, C, mult B D)]; (* A*(C*(B*D)) = (A*C)*(B*D) *)
  in oeq_trans OF [e1, oeq_trans OF [e2, oeq_trans OF [e3, oeq_trans OF [e4, e5]]]] end;
...
val prod_split_sign =
  let
    ...
    val hNa    = Thm.assume (ctermGG (jT (neg (dvd pF aF))));
    val hOdd   = Thm.assume (ctermGG (jT (oeq (sub pF oneC) (add mF mF))));
    val Laxk = mkAxk aF;          (* mult a   : nat->nat *)
    val Llar = mkLar aF pF;       (* lar a p  : nat->nat *)
    fun LHSp n = lprod (lmap Laxk (uptoF n));
    fun RHSp n = lprod (lmap Llar (uptoF n));
    val pm1 = sub pF oneC;
    fun ExBody n S = mkConj (cong pF (LHSp n) (mult S (RHSp n))) (isSign pF S);
    (* abstract a FRESH Free (cong injects inner Ex/Abs binders => literal Bound 0
       would be captured -- see CLAUDE.md). *)
    val sFresh = Free("S_s2fresh", natT);
    fun ExAbs n = Term.lambda sFresh (ExBody n sFresh);
    fun goalEx n = mkEx (ExAbs n);
    fun Qbody n = mkImp (le n mF) (goalEx n);
    val nVar = Free("n_s2", natT);
    val Pabs = Term.lambda nVar (Qbody nVar);
    ...
    val base =
      let
        ...
        fun lp_uptoF0_eq1 () =
          let
            val lm0 = leq_rw_GG (Term.lambda (Free("z_b0a", natlistT))
                          (oeq (lprod (lmap Laxk (uptoF ZeroC))) (lprod (lmap Laxk (Bound 0)))),
                          uptoF ZeroC, lnilC) u0 (oeqRefl_GG (lprod (lmap Laxk (uptoF ZeroC))));
            val lmnil = lmapNil_GG Laxk;
            val lpc = lprod_cong_GG (lmap Laxk lnilC, lnilC) lmnil;
            val lpnil = lprodNil_GG ();
          in oeq_trans OF [oeq_trans OF [lm0, lpc], lpnil] end;  (* LHSp 0 = 1 *)
        fun rp_uptoF0_eq1 () =
          let
            val lm0 = leq_rw_GG (Term.lambda (Free("z_b0b", natlistT))
                          (oeq (lprod (lmap Llar (uptoF ZeroC))) (lprod (lmap Llar (Bound 0)))),
                          uptoF ZeroC, lnilC) u0 (oeqRefl_GG (lprod (lmap Llar (uptoF ZeroC))));
            val lmnil = lmapNil_GG Llar;
            val lpc = lprod_cong_GG (lmap Llar lnilC, lnilC) lmnil;
            val lpnil = lprodNil_GG ();
          in oeq_trans OF [oeq_trans OF [lm0, lpc], lpnil] end;  (* RHSp 0 = 1 *)
        val lhs0_1 = lp_uptoF0_eq1 ();    (* LHSp 0 = 1 *)
        val rhs0_1 = rp_uptoF0_eq1 ();    (* RHSp 0 = 1 *)
        (* cong p (LHSp 0)(mult 1 (RHSp 0)) : build cong p 1 1 then rewrite. *)
        val cr = cong_refl_at (pF, oneC);  (* cong p 1 1 *)
        (* rewrite RHS 1 -> mult 1 (RHSp 0) : 1 = mult 1 (RHSp 0)?  mult 1 (RHSp 0) = RHSp 0 = 1.
           so oeq (mult 1 (RHSp 0)) 1, use sym. *)
        val m1r = mult1l_GG (RHSp ZeroC);          (* (1 * RHSp 0) = RHSp 0 *)
        val m1r_1 = oeq_trans OF [m1r, rhs0_1];     (* (1 * RHSp 0) = 1 *)
        val zc = Free("z_b0c", natT);
        (* cong p 1 1 -> cong p 1 (mult 1 (RHSp 0)) by rewriting 2nd arg 1 -> (mult 1 (RHSp 0)) *)
        val cr2 = oeq_rw_GG (Term.lambda zc (cong pF oneC zc), oneC, mult oneC (RHSp ZeroC)) (oeq_sym OF [m1r_1]) cr;
                   (* cong p 1 (mult 1 (RHSp 0)) *)
        (* rewrite 1st arg 1 -> LHSp 0 *)
        val cr3 = oeq_rw_GG (Term.lambda zc (cong pF zc (mult oneC (RHSp ZeroC))), oneC, LHSp ZeroC) (oeq_sym OF [lhs0_1]) cr2;
                   (* cong p (LHSp 0)(mult 1 (RHSp 0)) *)
        val sgn1 = sign_one pF;                     (* isSign p 1 *)
        val conj = conjI_GG_at (cong pF (LHSp ZeroC)(mult oneC (RHSp ZeroC)), isSign pF oneC) cr3 sgn1;
                   (* ExBody 0 1 *)
        val exq = exI_GG_at (ExAbs ZeroC, oneC) conj;  (* goalEx 0 *)
      in impI_GG_at (le ZeroC mF, goalEx ZeroC) (Thm.implies_intr (ctermGG (jT (le ZeroC mF))) exq) end;

    (* ---- step : assume Q(x), show Q(Suc x) ---- *)
    val stepF =
      let
        val xn = Free("x_s2", natT);
        val IH = Thm.assume (ctermGG (jT (Qbody xn)));
        val Sn = suc xn;
        val k = Sn;
        val ak = mult aF k;
        val lark = lar aF pF k;           (* = Llar $ k *)
        val hleSn = Thm.assume (ctermGG (jT (le Sn mF)));
        val lex_sx = le_self_suc_GG xn;
        val lexm = le_trans_GG_at (xn, Sn, mF) lex_sx hleSn;   (* le x m *)
        val exTail = mp_GG_at (le xn mF, goalEx xn) IH lexm;   (* EXISTS S. ExBody x S *)
        (* head facts *)
        val le1Sn = lt_0_suc_GG xn;                            (* le 1 (Suc x) *)
        val ndk = not_dvd_in_range pF mF k hPrime hOdd le1Sn hleSn;  (* ~(dvd p k) *)
        val djk = lar_cong_at (pF, aF, k) hPrime hNa ndk;     (* Disj (cong p (a*k) lark)(cong p (a*k + lark) 0) *)
        (* LHSp(Suc x) = (a*k) * LHSp x , RHSp(Suc x) = lark * RHSp x *)
        fun consEq (fF, fk) =     (* generic : lprod(lmap f (uptoF(Suc x))) = (f k) * lprod(lmap f (uptoF x)) *)
          let
            val us = uptoSuc_GG xn;
            val l0 = leq_rw_GG (Term.lambda (Free("z_ce", natlistT))
                        (oeq (lprod (lmap fF (uptoF Sn))) (lprod (lmap fF (Bound 0)))),
                        uptoF Sn, lcons Sn (uptoF xn)) us (oeqRefl_GG (lprod (lmap fF (uptoF Sn))));
            val lmc = lmapCons_GG (fF, Sn, uptoF xn);   (* leq (lmap f (lcons (Suc x) U))(lcons (f$Sn)(lmap f U)) *)
            val lmc_b = beta_norm lmc;                  (* f$Sn reduces to fk *)
            val lpc = lprod_cong_GG (lmap fF (lcons Sn (uptoF xn)), lcons fk (lmap fF (uptoF xn))) lmc_b;
            val l1 = oeq_trans OF [l0, lpc];
            val lpcons = lprodCons_GG (fk, lmap fF (uptoF xn));  (* lprod(lcons fk (lmap f U)) = fk * lprod(lmap f U) *)
          in oeq_trans OF [l1, lpcons] end;            (* lprod(lmap f (uptoF(Suc x))) = fk * lprod(lmap f (uptoF x)) *)
        val lhsSucEq = consEq (Laxk, ak);              (* LHSp(Suc x) = (a*k) * LHSp x *)
        val rhsSucEq = consEq (Llar, lark);            (* RHSp(Suc x) = lark * RHSp x *)

        (* core builder once we have tail witness St + tail cong + tail sign + per-element (sk, perElemCong, sgnSk) *)
        fun assemble (St, hCongT, hSignT) (sk, perElemCong, sgnSk) =
          (* perElemCong : cong p (a*k)(mult sk lark) ; sgnSk : isSign p sk
             hCongT : cong p (LHSp x)(mult St (RHSp x)) ; hSignT : isSign p St *)
          let
            (* cong_mult : cong p ((a*k)*(LHSp x))((sk*lark)*(St*(RHSp x))) *)
            val cmul = cong_mult_GG_at (pF, ak, mult sk lark, LHSp xn, mult St (RHSp xn)) perElemCong hCongT;
                       (* cong p (mult ak (LHSp x))(mult (mult sk lark)(mult St (RHSp x))) *)
            (* rearrange RHS : (sk*lark)*(St*(RHSp x)) = (sk*St)*(lark*(RHSp x))  [mult4_swap] *)
            val swap = mult4_swap (sk, lark, St, RHSp xn);
                       (* oeq ((sk*lark)*(St*RHSp x))((sk*St)*(lark*RHSp x)) *)
            val zc = Free("z_asm", natT);
            val cmul2 = oeq_rw_GG (Term.lambda zc (cong pF (mult ak (LHSp xn)) zc),
                          mult (mult sk lark)(mult St (RHSp xn)), mult (mult sk St)(mult lark (RHSp xn))) swap cmul;
                       (* cong p (ak * LHSp x)((sk*St)*(lark*RHSp x)) *)
            (* rewrite LHS (ak * LHSp x) -> LHSp(Suc x) and inner (lark * RHSp x) -> RHSp(Suc x) *)
            val cmul3 = oeq_rw_GG (Term.lambda zc (cong pF zc (mult (mult sk St)(mult lark (RHSp xn)))),
                          mult ak (LHSp xn), LHSp Sn) (oeq_sym OF [lhsSucEq]) cmul2;
                       (* cong p (LHSp(Suc x))((sk*St)*(lark*RHSp x)) *)
            val cmul4 = oeq_rw_GG (Term.lambda zc (cong pF (LHSp Sn) (mult (mult sk St) zc)),
                          mult lark (RHSp xn), RHSp Sn) (oeq_sym OF [rhsSucEq]) cmul3;
                       (* cong p (LHSp(Suc x))((sk*St)*(RHSp(Suc x))) *)
            val Snew = mult sk St;
            val sgnNew = sign_close pF sk St hPrime sgnSk hSignT;  (* isSign p (sk*St) *)
            val conj = conjI_GG_at (cong pF (LHSp Sn)(mult Snew (RHSp Sn)), isSign pF Snew) cmul4 sgnNew;
                       (* ExBody (Suc x) Snew *)
          in exI_GG_at (ExAbs Sn, Snew) conj end;   (* goalEx (Suc x) *)

        (* per-element cong & sign for the two lar_cong branches *)
        fun lowerBranch (St, hCongT, hSignT) hLo =   (* hLo : cong p (a*k) lark *)
          let
            (* cong p (a*k)(mult 1 lark) : mult 1 lark = lark, rewrite *)
            val m1 = mult1l_GG lark;                   (* (1*lark) = lark *)
            val zc = Free("z_lb", natT);
            val perElem = oeq_rw_GG (Term.lambda zc (cong pF ak zc), lark, mult oneC lark) (oeq_sym OF [m1]) hLo;
                          (* cong p (a*k)(mult 1 lark) *)
            val sgn1 = sign_one pF;                    (* isSign p 1 *)
          in assemble (St, hCongT, hSignT) (oneC, perElem, sgn1) end;
        fun upperBranch (St, hCongT, hSignT) hHi =   (* hHi : cong p (a*k + lark) 0 *)
          let
            (* neg_one_mult p lark : cong p (lark + (p-1)*lark) 0 *)
            val nom = neg_one_mult pF lark hPrime;     (* cong p (lark + pm1*lark) 0 *)
            (* reorder lark + pm1*lark -> pm1*lark + lark [comm] *)
            val comm = addcomm_GG (lark, mult pm1 lark);  (* (lark + pm1*lark) = (pm1*lark + lark) *)
            val zc = Free("z_ub", natT);
            val nom2 = oeq_rw_GG (Term.lambda zc (cong pF zc ZeroC), add lark (mult pm1 lark), add (mult pm1 lark) lark) comm nom;
                       (* cong p (pm1*lark + lark) 0 *)
            (* both cong p (a*k + lark) 0 and cong p (pm1*lark + lark) 0 ->
               cong p (a*k + lark)(pm1*lark + lark) [sym+trans] -> cancel lark -> cong p (a*k)(pm1*lark) *)
            val sym2 = cong_sym_at (pF, add (mult pm1 lark) lark, ZeroC) nom2;  (* cong p 0 (pm1*lark + lark) *)
            val both = cong_trans_at (pF, add ak lark, ZeroC, add (mult pm1 lark) lark) hHi sym2;
                       (* cong p (a*k + lark)(pm1*lark + lark) *)
            val perElem = cong_add_cancel_r pF ak (mult pm1 lark) lark both;  (* cong p (a*k)(pm1*lark) *)
            val sgnpm1 = sign_pm1 pF;                  (* isSign p (p-1) *)
          in assemble (St, hCongT, hSignT) (pm1, perElem, sgnpm1) end;

        (* exE over the tail existential, then disjE over lar_cong *)
        fun tailBody St hConjT =   (* hConjT : ExBody x St *)
          let
            val hCongT = conjunct1_GG_at (cong pF (LHSp xn)(mult St (RHSp xn)), isSign pF St) hConjT;
            val hSignT = conjunct2_GG_at (cong pF (LHSp xn)(mult St (RHSp xn)), isSign pF St) hConjT;
            val cLo = let val h = Thm.assume (ctermGG (jT (cong pF ak lark)))
                      in Thm.implies_intr (ctermGG (jT (cong pF ak lark))) (lowerBranch (St, hCongT, hSignT) h) end;
            val cHi = let val h = Thm.assume (ctermGG (jT (cong pF (add ak lark) ZeroC)))
                      in Thm.implies_intr (ctermGG (jT (cong pF (add ak lark) ZeroC))) (upperBranch (St, hCongT, hSignT) h) end;
          in disjE_GG_at (cong pF ak lark, cong pF (add ak lark) ZeroC, goalEx Sn) djk cLo cHi end;
        val exStep = exE_GG_at (ExAbs xn, goalEx Sn) exTail "St_s2" tailBody;  (* goalEx (Suc x) *)
        val imp = impI_GG_at (le Sn mF, goalEx Sn) (Thm.implies_intr (ctermGG (jT (le Sn mF))) exStep);
        val metaStep = Thm.implies_intr (ctermGG (jT (Qbody xn))) imp;
      in Thm.forall_intr (ctermGG xn) metaStep end;

    val Qm = nat_induct_GG_run Pabs mF base stepF;       (* le m m ==> goalEx m *)
    val lemm = le_refl_GG_at mF;                          (* le m m *)
    val res = mp_GG_at (le mF mF, goalEx mF) Qm lemm;     (* goalEx m *)
    val d3 = Thm.implies_intr (ctermGG (jT (oeq (sub pF oneC)(add mF mF)))) res;
    val d2 = Thm.implies_intr (ctermGG (jT (neg (dvd pF aF)))) d3;
    val d1 = Thm.implies_intr (ctermGG (jT (prime2 pF))) d2;
  in varify d1 end;
val () = out "S2_PROVED_RAW\n";

(* validate S2 *)
val pV2 = Var(("p",0),natT); val aV2 = Var(("a",0),natT); val mV2 = Var(("m",0),natT);
val LaxkV = multC $ aV2;
val LlarV = larC $ aV2 $ pV2;
val sFreshV = Free("S_chk", natT);
fun mk_i_s2 () =
  let
    fun LHSpV n = lprod (lmapC $ LaxkV $ (uptoF n));
    fun RHSpV n = lprod (lmapC $ LlarV $ (uptoF n));
    (* abstract a FRESH Free so the de-Bruijn shape matches the proved term *)
    val exBody = Term.lambda sFreshV
                   (mkConj (cong pV2 (LHSpV mV2)(mult sFreshV (RHSpV mV2)))
                           (mkDisj (cong pV2 sFreshV oneC)(cong pV2 sFreshV (sub pV2 oneC))));
  in Logic.mk_implies (jT (prime2 pV2),
       Logic.mk_implies (jT (neg (dvd pV2 aV2)),
         Logic.mk_implies (jT (oeq (sub pV2 oneC)(add mV2 mV2)),
           jT (mkEx exBody)))) end;
val i_s2 = mk_i_s2 ();
val r_s2 = (length (Thm.hyps_of prod_split_sign) = 0)
           andalso ((Thm.prop_of prod_split_sign) aconv i_s2)
           andalso (length (Thm.extra_shyps prod_split_sign) = 0);
val () = if r_s2 then out "OK prod_split_sign\n" else
  (out "FAIL prod_split_sign\n";
   out ("  got = " ^ Syntax.string_of_term ctxtGG (Thm.prop_of prod_split_sign) ^ "\n");
   out ("  int = " ^ Syntax.string_of_term ctxtGG i_s2 ^ "\n"));
(* soundness probe : dropping ~(p|a) must NOT be provable (the result genuinely needs it) *)
val probe_s2_needs_ndvd =
  let val bogus = Logic.mk_implies (jT (prime2 pV2),
        Logic.mk_implies (jT (oeq (sub pV2 oneC)(add mV2 mV2)),
          jT (mkEx (Term.lambda sFreshV
                (mkConj (cong pV2 (lprod (lmapC $ LaxkV $ (uptoF mV2)))(mult sFreshV (lprod (lmapC $ LlarV $ (uptoF mV2)))))
                        (mkDisj (cong pV2 sFreshV oneC)(cong pV2 sFreshV (sub pV2 oneC))))))))
  in not ((Thm.prop_of prod_split_sign) aconv bogus) end;
val () = if r_s2 andalso probe_s2_needs_ndvd then out "PROD_SPLIT_OK\n" else out "PROD_SPLIT_FAILED\n";

(* ############################################################################
   ####  G : GAUSS'S LEMMA  ####################################################
   prime2 p ==> ~(p|a) ==> oeq(sub p 1)(add m m) ==>
     EXISTS S. cong p (pow a m) S  AND  (cong p S 1 OR cong p S (sub p 1))
   where S is the explicit FLIP-SIGN PRODUCT delivered by S2 (= (-1)^mu).
   Chain : S1 (a^m * m! == prod a*k) + S2 (prod a*k == S * prod lar)
           + lar_perm (prod lar == m!) ==> a^m * m! == S * m! ; cancel m!
           (HELP: ~(p|m!)) via mod_cancel ==> cong p (a^m) S, with isSign p S.
   The euler dichotomy is NOT used -- the +-1 comes from S's flip construction.
   ############################################################################ *)
val () = out "GAUSS_G_BEGIN\n";

(* mod_cancel at GG *)
val mod_cancel_GG = varifyGG mod_cancel;
fun mod_cancel_at (pt, at, bt, ct) hPrime hNdvd hCong =
  let val inst = beta_norm (Drule.infer_instantiate ctxtGG
        [(("p",0), ctermGG pt),(("a",0), ctermGG at),(("b",0), ctermGG bt),(("c",0), ctermGG ct)] mod_cancel_GG)
  in Thm.implies_elim (Thm.implies_elim (Thm.implies_elim inst hPrime) hNdvd) hCong end;

(* cong_of_oeq : oeq a b ==> cong p a b  (cong_refl + rewrite 2nd arg) *)
fun cong_of_oeq p (aT, bT) hoeq =
  let
    val cr = cong_refl_at (p, aT);     (* cong p a a *)
    val zc = Free("z_coe", natT);
  in oeq_rw_GG (Term.lambda zc (cong p aT zc), aT, bT) hoeq cr end;  (* cong p a b *)
...
val gauss_lemma =
  let
    ...
    val hNa    = Thm.assume (ctermGG (jT (neg (dvd pF aF))));
    val hOdd   = Thm.assume (ctermGG (jT (oeq (sub pF oneC) (add mF mF))));
    val Laxk = mkAxk aF; val Llar = mkLar aF pF;
    val LprodAxk = lprod (lmap Laxk (uptoF mF));     (* prod (a*k) over [1..m] *)
    val LprodLar = lprod (lmap Llar (uptoF mF));     (* prod lar over [1..m] *)
    val Mfac     = lprod (uptoF mF);                 (* m! *)
    val powam    = pow aF mF;
    val pm1      = sub pF oneC;

    (* S1 instance : oeq LprodAxk (mult powam Mfac) *)
    val s1 = beta_norm (Drule.infer_instantiate ctxtGG
               [(("a",0), ctermGG aF), (("m",0), ctermGG mF)] (toGG prod_axk_eq_pow));
    (* HELP instance : ~(dvd p Mfac) *)
    val help0 = beta_norm (Drule.infer_instantiate ctxtGG
                  [(("p",0), ctermGG pF), (("m",0), ctermGG mF)] (toGG prod_uptoF_not_dvd));
    val hNdvdMfac = Thm.implies_elim (Thm.implies_elim help0 hPrime) hOdd;  (* ~(dvd p Mfac) *)
    (* lar_perm instance : oeq LprodLar Mfac *)
    val lp0 = beta_norm (Drule.infer_instantiate ctxtGG
                [(("p",0), ctermGG pF), (("a",0), ctermGG aF), (("m",0), ctermGG mF)] (toGG lar_perm));
    val larPermEq = Thm.implies_elim (Thm.implies_elim (Thm.implies_elim lp0 hPrime) hNa) hOdd;
                    (* oeq LprodLar Mfac *)
    (* S2 instance : EXISTS S. cong p LprodAxk (mult S LprodLar) AND isSign p S *)
    val s2_0 = beta_norm (Drule.infer_instantiate ctxtGG
                 [(("p",0), ctermGG pF), (("a",0), ctermGG aF), (("m",0), ctermGG mF)] (toGG prod_split_sign));
    val s2ex = Thm.implies_elim (Thm.implies_elim (Thm.implies_elim s2_0 hPrime) hNa) hOdd;
               (* EXISTS S. cong p LprodAxk (mult S LprodLar) AND isSign p S *)

    val goalG = mkEx (Term.lambda (Free("S_g", natT))
                  (mkConj (cong pF powam (Free("S_g", natT)))
                          (isSign pF (Free("S_g", natT)))));
    (* the Ex abstraction for S2's body (rebuild with fresh Free, matching S2) *)
    val sF2 = Free("S_s2fresh", natT);
    val s2Abs = Term.lambda sF2 (mkConj (cong pF LprodAxk (mult sF2 LprodLar)) (isSign pF sF2));

    fun bodyG St hConjT =   (* hConjT : cong p LprodAxk (mult St LprodLar) AND isSign p St *)
      let
        val hCong = conjunct1_GG_at (cong pF LprodAxk (mult St LprodLar), isSign pF St) hConjT;
        val hSign = conjunct2_GG_at (cong pF LprodAxk (mult St LprodLar), isSign pF St) hConjT;
        (* rewrite LprodLar -> Mfac inside (mult St LprodLar) on RHS of hCong via larPermEq *)
        val mcr = mult_cong_r_GG (St, LprodLar, Mfac) larPermEq;  (* (St*LprodLar) = (St*Mfac) *)
        val zc = Free("z_g1", natT);
        val hCong1 = oeq_rw_GG (Term.lambda zc (cong pF LprodAxk zc), mult St LprodLar, mult St Mfac) mcr hCong;
                     (* cong p LprodAxk (mult St Mfac) *)
        (* rewrite LprodAxk -> (powam * Mfac) on LHS via s1 *)
        val hCong2 = oeq_rw_GG (Term.lambda zc (cong pF zc (mult St Mfac)), LprodAxk, mult powam Mfac) s1 hCong1;
                     (* cong p (powam*Mfac)(St*Mfac) *)
        (* commute both products : (powam*Mfac) -> (Mfac*powam), (St*Mfac) -> (Mfac*St) *)
        val cL = multcomm_GG (powam, Mfac);       (* (powam*Mfac) = (Mfac*powam) *)
        val cR = multcomm_GG (St, Mfac);          (* (St*Mfac) = (Mfac*St) *)
        val hCong3 = oeq_rw_GG (Term.lambda zc (cong pF zc (mult St Mfac)), mult powam Mfac, mult Mfac powam) cL hCong2;
                     (* cong p (Mfac*powam)(St*Mfac) *)
        val hCong4 = oeq_rw_GG (Term.lambda zc (cong pF (mult Mfac powam) zc), mult St Mfac, mult Mfac St) cR hCong3;
                     (* cong p (Mfac*powam)(Mfac*St) *)
        (* mod_cancel with a:=Mfac, b:=powam, c:=St ; needs ~(dvd p Mfac) *)
        val cancelled = mod_cancel_at (pF, Mfac, powam, St) hPrime hNdvdMfac hCong4;  (* cong p powam St *)
        val conj = conjI_GG_at (cong pF powam St, isSign pF St) cancelled hSign;  (* cong p (a^m) S AND isSign p S *)
        (* exI witness St for goalG *)
        val gAbs = Term.lambda (Free("S_g", natT))
                     (mkConj (cong pF powam (Free("S_g", natT))) (isSign pF (Free("S_g", natT))));
      in exI_GG_at (gAbs, St) conj end;

    val exG = exE_GG_at (s2Abs, goalG) s2ex "S_gw" bodyG;   (* goalG *)
    val d3 = Thm.implies_intr (ctermGG (jT (oeq (sub pF oneC)(add mF mF)))) exG;
    val d2 = Thm.implies_intr (ctermGG (jT (neg (dvd pF aF)))) d3;
    val d1 = Thm.implies_intr (ctermGG (jT (prime2 pF))) d2;
  in varify d1 end;
val () = out "GAUSS_G_PROVED_RAW\n";

(* validate G *)
val pVg2 = Var(("p",0),natT); val aVg2 = Var(("a",0),natT); val mVg2 = Var(("m",0),natT);
val sFg = Free("S_gchk", natT);
val i_gauss = Logic.mk_implies (jT (prime2 pVg2),
    Logic.mk_implies (jT (neg (dvd pVg2 aVg2)),
      Logic.mk_implies (jT (oeq (sub pVg2 oneC)(add mVg2 mVg2)),
        jT (mkEx (Term.lambda sFg
              (mkConj (cong pVg2 (pow aVg2 mVg2) sFg)
                      (mkDisj (cong pVg2 sFg oneC)(cong pVg2 sFg (sub pVg2 oneC)))))))));
val r_gauss = (length (Thm.hyps_of gauss_lemma) = 0)
              andalso ((Thm.prop_of gauss_lemma) aconv i_gauss)
              andalso (length (Thm.extra_shyps gauss_lemma) = 0);
val () = if r_gauss then out "OK gauss_lemma\n" else
  (out "FAIL gauss_lemma\n";
   out ("  got = " ^ Syntax.string_of_term ctxtGG (Thm.prop_of gauss_lemma) ^ "\n");
   out ("  int = " ^ Syntax.string_of_term ctxtGG i_gauss ^ "\n"));
(* soundness probes : dropping ~(p|a) or oddness must NOT be provable *)
val probe_g_needs_ndvd =
  let val bogus = Logic.mk_implies (jT (prime2 pVg2),
        Logic.mk_implies (jT (oeq (sub pVg2 oneC)(add mVg2 mVg2)),
          jT (mkEx (Term.lambda sFg
                (mkConj (cong pVg2 (pow aVg2 mVg2) sFg)
                        (mkDisj (cong pVg2 sFg oneC)(cong pVg2 sFg (sub pVg2 oneC))))))))
  in not ((Thm.prop_of gauss_lemma) aconv bogus) end;
val probe_g_needs_odd =
  let val bogus = Logic.mk_implies (jT (prime2 pVg2),
        Logic.mk_implies (jT (neg (dvd pVg2 aVg2)),
          jT (mkEx (Term.lambda sFg
                (mkConj (cong pVg2 (pow aVg2 mVg2) sFg)
                        (mkDisj (cong pVg2 sFg oneC)(cong pVg2 sFg (sub pVg2 oneC))))))))
  in not ((Thm.prop_of gauss_lemma) aconv bogus) end;
val () = if r_gauss then (if probe_g_needs_ndvd then out "PROBE_OK gauss keeps ~(p|a)\n" else out "PROBE_FAIL gauss ndvd\n";
                          if probe_g_needs_odd then out "PROBE_OK gauss keeps oddness\n" else out "PROBE_FAIL gauss odd\n")
         else ();
val () = if r_gauss andalso probe_g_needs_ndvd andalso probe_g_needs_odd
         then out "GAUSS_LEMMA_OK\n" else out "GAUSS_LEMMA_FAILED\n";

(* ---- axiom audit : still only ex_middle classical, no Gauss axiom ---- *)
val () = out "GAUSS_AXIOM_AUDIT_BEGIN\n";
val allAxG2 = Theory.all_axioms_of thyGG;
val () = out ("gauss_final_axiom_count=" ^ Int.toString (length allAxG2) ^ "\n");
val hasEMg2 = List.exists (fn (nm,_) => String.isSuffix "ex_middle" nm orelse nm = "ex_middle") allAxG2;
val () = out ("gauss_final_ex_middle_present=" ^ Bool.toString hasEMg2 ^ "\n");
val gaussNamed = List.filter (fn nm => String.isSubstring "gauss" (String.map Char.toLower nm)
                                       orelse String.isSubstring "split" (String.map Char.toLower nm)) (map fst allAxG2);
val () = out ("gauss_axiom_names_present=[" ^ String.concatWith "," gaussNamed ^ "]\n");
val () = out "GAUSS_AXIOM_AUDIT_END\n";

(* clean gate for all final-stage lemmas *)
fun cleanG th = (length (Thm.hyps_of th) = 0) andalso (length (Thm.extra_shyps th) = 0);
val gaussCleanAll = cleanG prod_axk_eq_pow andalso cleanG prod_uptoF_not_dvd
                    andalso cleanG prod_split_sign andalso cleanG gauss_lemma;
val () = if r_prod_axk andalso r_notdvd andalso r_s2 andalso r_gauss
            andalso gaussCleanAll andalso hasEMg2 andalso (gaussNamed = [])
         then out "GAUSS_FINAL_ALL_OK\n" else out "GAUSS_FINAL_INCOMPLETE\n";

(* ############################################################################
   ####  EISENSTEIN TOOLBOX  (fleet F1) ########################################
   APPENDED to gauss_final.sml (ends at gauss_lemma on ctxtGG/ctermGG/thyGG).
   Extends thyGG with TWO new conservative-recursion consts:
     parity : nat=>nat        (pinned to {0,1}; additive mod 2)
     cnt    : (nat=>o)=>nat=>nat   (count of indices 1..n satisfying a predicate)
   on the ONE final context ctxtT (=ctxtGG + parity + cnt).  Re-varifies the
   reused base lemmas (sumf laws, div/mod, dvd, core logic) onto ctxtT and
   proves the toolbox lemmas T1-T4.
   ############################################################################ *)
val () = out "EISENSTEIN_TOOLBOX_BEGIN\n";

(* ----------------------------------------------------------------------------
   Extend thyGG with parity + cnt.  thyT is the ONE final toolbox context.
   ---------------------------------------------------------------------------- *)
val thyTc = Sign.add_consts
  [(Binding.name "parity", natT --> natT, NoSyn),
   (Binding.name "cnt",    predNatT --> natT --> natT, NoSyn)] thyGG;
val parityC = Const (Sign.full_name thyTc (Binding.name "parity"), natT --> natT);
fun parity n = parityC $ n;
val cntC = Const (Sign.full_name thyTc (Binding.name "cnt"), predNatT --> natT --> natT);
fun cnt P n = cntC $ P $ n;

val nFt = Free("n", natT);
(* parity axioms : conservative recursion, parity 0 = 0, parity(Suc n)=1-parity n *)
val ((_,parity_0_ax),   thyT1) = Thm.add_axiom_global (Binding.name "parity_0",
      jT (oeq (parity ZeroC) ZeroC)) thyTc;
val ((_,parity_Suc_ax), thyT2) = Thm.add_axiom_global (Binding.name "parity_Suc",
      jT (oeq (parity (suc nFt)) (sub (suc ZeroC) (parity nFt)))) thyT1;
(* cnt axioms : cnt P 0 = 0 ; guarded recursion (exhaustive via ex_middle) *)
val Pcnt = Free("P", predNatT);
val ((_,cnt_0_ax),   thyT3) = Thm.add_axiom_global (Binding.name "cnt_0",
      jT (oeq (cnt Pcnt ZeroC) ZeroC)) thyT2;
val ((_,cnt_Suc_t_ax), thyT4) = Thm.add_axiom_global (Binding.name "cnt_Suc_t",
      Logic.mk_implies (jT (Pcnt $ (suc nFt)),
        jT (oeq (cnt Pcnt (suc nFt)) (suc (cnt Pcnt nFt))))) thyT3;
val ((_,cnt_Suc_f_ax), thyT)   = Thm.add_axiom_global (Binding.name "cnt_Suc_f",
      Logic.mk_implies (jT (neg (Pcnt $ (suc nFt))),
        jT (oeq (cnt Pcnt (suc nFt)) (cnt Pcnt nFt)))) thyT4;

val ctxtT  = Proof_Context.init_global thyT;
val ctermT = Thm.cterm_of ctxtT;
val () = out "TOOLBOX_CTX_READY\n";

(* ----------------------------------------------------------------------------
   transfer + varify the reused base lemmas onto thyT, and bind T-instantiators.
   ---------------------------------------------------------------------------- *)
fun toT th = Thm.transfer thyT th;
fun varifyT th = toT (varify th);

(* core logic / arithmetic (re-varified onto ctxtT) *)
val oeq_refl_T    = varifyT oeq_refl;
val oeq_subst_T   = varifyT oeq_subst;
val exI_T         = varifyT exI_ax;
val exE_T         = varifyT exE_ax;
val disjE_T       = varifyT disjE_ax;
val disjI1_T      = varifyT disjI1_ax;
val disjI2_T      = varifyT disjI2_ax;
val mp_T          = varifyT mp_ax;
val impI_T        = varifyT impI_ax;
val conjI_T       = varifyT conjI_ax;
val conjunct1_T   = varifyT conjunct1_ax;
val conjunct2_T   = varifyT conjunct2_ax;
val oFalse_elim_T = varifyT oFalse_elim_ax;
val ex_middle_T   = varifyT ex_middle_ax;
val nat_induct_T  = varifyT nat_induct;
val Suc_cong_T    = varifyT Suc_cong;
val add_0_right_T = varifyT add_0_right;
val add_Suc_T     = varifyT add_Suc;
val mult_comm_T   = varifyT mult_comm;
val mult_1_left_T = varifyT mult_1_left;
val mult_0_right_T= varifyT mult_0_right;
val le_refl_T     = varifyT le_refl;
val le_add_T      = varifyT le_add;
val le_trans_T    = varifyT le_trans;

(* the new toolbox axioms, varified onto ctxtT *)
val parity_0_T     = varifyT parity_0_ax;
val parity_Suc_T   = varifyT parity_Suc_ax;
val cnt_0_T        = varifyT cnt_0_ax;
val cnt_Suc_t_T    = varifyT cnt_Suc_t_ax;
val cnt_Suc_f_T    = varifyT cnt_Suc_f_ax;

(* sub axioms + sumf laws + div/mod, lifted onto ctxtT *)
val sub_n_0_T      = varifyT sub_n_0_ax;
val sub_0_Suc_T    = varifyT sub_0_Suc_ax;
val sub_Suc_Suc_T  = varifyT sub_Suc_Suc_ax;
val sumf_0_T       = varifyT sumf_0_ax;
val sumf_Suc_T     = varifyT sumf_Suc_ax;
val sum_cong_T     = varifyT sum_cong;
val sum_mult_l_T   = varifyT sum_mult_l;
val sum_add_T      = varifyT sum_add;
val sum_peel_first_T = varifyT sum_peel_first;
val div_mod_eq_T   = varifyT div_mod_eq_ax;
val rmod_lt_T      = varifyT rmod_lt_ax;
val div_mod_unique_T = varifyT div_mod_unique;
val () = out "TOOLBOX_VARIFY_DONE\n";

(* ----------------------------------------------------------------------------
   T-context instantiators (mirror the _GG layer, bound to ctxtT/ctermT)
   ---------------------------------------------------------------------------- *)
fun oeqRefl_T t = beta_norm (Drule.infer_instantiate ctxtT [(("a",0), ctermT t)] oeq_refl_T);
fun oeq_rw_T (Pabs, xT, yT) hxy hPx =
  let val inst = beta_norm (Drule.infer_instantiate ctxtT
        [(("P",0), ctermT Pabs), (("a",0), ctermT xT), (("b",0), ctermT yT)] oeq_subst_T)
  in (inst OF [hxy]) OF [hPx] end;
fun Succong_T h = Suc_cong_T OF [h];
fun add0r_T t = beta_norm (Drule.infer_instantiate ctxtT [(("n",0), ctermT t)] add_0_right_T);
fun addSuc_T (mt,nt) = beta_norm (Drule.infer_instantiate ctxtT
        [(("m",0), ctermT mt),(("n",0), ctermT nt)] add_Suc_T);
fun mult0r_T t = beta_norm (Drule.infer_instantiate ctxtT [(("n",0), ctermT t)] mult_0_right_T);
fun mult1l_T t = beta_norm (Drule.infer_instantiate ctxtT [(("n",0), ctermT t)] mult_1_left_T);
fun multcomm_T (mt,nt) = beta_norm (Drule.infer_instantiate ctxtT
        [(("m",0), ctermT mt),(("n",0), ctermT nt)] mult_comm_T);
fun mp_T_at (At, Bt) hImp hA =
  Thm.implies_elim (Thm.implies_elim (beta_norm (Drule.infer_instantiate ctxtT
        [(("A",0), ctermT At), (("B",0), ctermT Bt)] mp_T)) hImp) hA;
fun impI_T_at (At, Bt) hMeta =
  Thm.implies_elim (beta_norm (Drule.infer_instantiate ctxtT
        [(("A",0), ctermT At), (("B",0), ctermT Bt)] impI_T)) hMeta;
fun conjI_T_at (At, Bt) hA hB =
  Thm.implies_elim (Thm.implies_elim (beta_norm (Drule.infer_instantiate ctxtT
        [(("A",0), ctermT At), (("B",0), ctermT Bt)] conjI_T)) hA) hB;
fun disjE_T_at (At, Bt, Ct) dThm caseA caseB =
  let val inst = beta_norm (Drule.infer_instantiate ctxtT
        [(("A",0), ctermT At), (("B",0), ctermT Bt), (("C",0), ctermT Ct)] disjE_T)
  in Thm.implies_elim (Thm.implies_elim (Thm.implies_elim inst dThm) caseA) caseB end;
fun ex_middle_T_at At =
  beta_norm (Drule.infer_instantiate ctxtT [(("A",0), ctermT At)] ex_middle_T);
fun oFalse_elim_T_at Rt =
  beta_norm (Drule.infer_instantiate ctxtT [(("R",0), ctermT Rt)] oFalse_elim_T);
fun disjI1_at_T (At, Bt) hA =
  Thm.implies_elim (beta_norm (Drule.infer_instantiate ctxtT
        [(("A",0), ctermT At), (("B",0), ctermT Bt)] disjI1_T)) hA;
fun disjI2_at_T (At, Bt) hB =
  Thm.implies_elim (beta_norm (Drule.infer_instantiate ctxtT
        [(("A",0), ctermT At), (("B",0), ctermT Bt)] disjI2_T)) hB;

(* oeq_trans / oeq_sym are 0-hyp schematic; OF auto-merges theories *)
val oeq_sym_T   = oeq_sym;
val oeq_trans_T = oeq_trans;

(* nat induction runner on ctxtT *)
fun nat_induct_T_run Pabs kF base step =
  let val inst = beta_norm (Drule.infer_instantiate ctxtT
        [(("P",0), ctermT Pabs), (("k",0), ctermT kF)] nat_induct_T)
  in beta_norm (Thm.implies_elim (Thm.implies_elim inst base) step) end;

(* sumf instantiators *)
fun sumf0_T fT       = beta_norm (Drule.infer_instantiate ctxtT [(("f",0), ctermT fT)] sumf_0_T);
fun sumfSuc_T (fT,nt)= beta_norm (Drule.infer_instantiate ctxtT
                          [(("f",0), ctermT fT),(("n",0), ctermT nt)] sumf_Suc_T);
(* add congruences on ctxtT *)
fun add_cong_l_T (pT, qT, kT) hpq =
  let val Pabs = Abs("z", natT, oeq (add (Bound 0) kT) (add qT kT))
  in oeq_rw_T (Pabs, qT, pT) (oeq_sym_T OF [hpq]) (oeqRefl_T (add qT kT)) end;
...
val () = out "TOOLBOX_INSTANTIATORS_READY\n";

(* clean-gate for toolbox lemmas *)
fun cleanT th = (length (Thm.hyps_of th) = 0) andalso (length (Thm.extra_shyps th) = 0);
fun checkT (nm, th, intended) =
  let val ok = cleanT th andalso ((Thm.prop_of th) aconv intended)
  in (if ok then out ("OK " ^ nm ^ "\n")
      else (out ("FAIL " ^ nm ^ "\n");
            out ("  got = " ^ Syntax.string_of_term ctxtT (Thm.prop_of th) ^ "\n");
...
(* ============================================================================
   (T1) PARITY INFRA
   ============================================================================ *)
val () = out "T1_PARITY_BEGIN\n";

(* small sub facts : sub 1 0 = 1, sub 1 1 = 0, sub 0 0 = 0 *)
val oneC = suc ZeroC;
(* sub 1 0 = 1   via sub_n_0 (sub n 0 = n) at n=1 *)
val sub1_0 = beta_norm (Drule.infer_instantiate ctxtT [(("n",0), ctermT oneC)] sub_n_0_T);
(* sub 0 0 = 0   via sub_n_0 at n=0 *)
val sub0_0 = beta_norm (Drule.infer_instantiate ctxtT [(("n",0), ctermT ZeroC)] sub_n_0_T);
(* sub 1 1 = sub (Suc 0)(Suc 0) = sub 0 0 = 0  via sub_Suc_Suc then sub0_0 *)
val subSS_11 = beta_norm (Drule.infer_instantiate ctxtT
      [(("n",0), ctermT ZeroC),(("k",0), ctermT ZeroC)] sub_Suc_Suc_T);  (* sub(Suc 0)(Suc 0)=sub 0 0 *)
val sub1_1 = oeq_trans_T OF [subSS_11, sub0_0];                          (* sub 1 1 = 0 *)

(* parity 1 = 1 : parity(Suc 0) = sub 1 (parity 0) = sub 1 0 = 1 *)
val parity1_eq =
  let
    val pS = beta_norm (Drule.infer_instantiate ctxtT [(("n",0), ctermT ZeroC)] parity_Suc_T);
            (* parity(Suc 0) = sub 1 (parity 0) *)
    (* rewrite (parity 0) -> 0 inside sub 1 (parity 0) *)
    val subAbs = Abs("z", natT, oeq (sub oneC (parity ZeroC)) (sub oneC (Bound 0)));
    val congP = oeq_rw_T (subAbs, parity ZeroC, ZeroC) parity_0_T (oeqRefl_T (sub oneC (parity ZeroC)));
            (* sub 1 (parity 0) = sub 1 0 *)
    val step = oeq_trans_T OF [pS, congP];     (* parity(Suc 0) = sub 1 0 *)
  in oeq_trans_T OF [step, sub1_0] end;        (* parity(Suc 0) = 1 *)

(* --- parity_bounded : Disj (oeq (parity n) 0)(oeq (parity n) 1) --- *)
val parity_bounded =
  let
    val kF = Free("n", natT);
    val Pabs = Abs("z", natT, mkDisj (oeq (parity (Bound 0)) ZeroC) (oeq (parity (Bound 0)) oneC));
    val baseThm = disjI1_at_T (oeq (parity ZeroC) ZeroC, oeq (parity ZeroC) oneC) parity_0_T
    val xF = Free("x", natT);
    val ihP = jT (mkDisj (oeq (parity xF) ZeroC) (oeq (parity xF) oneC));
    val IH  = Thm.assume (ctermT ihP);
    val pS  = beta_norm (Drule.infer_instantiate ctxtT [(("n",0), ctermT xF)] parity_Suc_T);
              (* parity(Suc x) = sub 1 (parity x) *)
    val goalS = mkDisj (oeq (parity (suc xF)) ZeroC) (oeq (parity (suc xF)) oneC);
    (* case parity x = 0 : parity(Suc x) = sub 1 0 = 1 -> right *)
    val cA =
      let
        val hz = Thm.assume (ctermT (jT (oeq (parity xF) ZeroC)))
        val subAbs = Abs("z", natT, oeq (sub oneC (parity xF)) (sub oneC (Bound 0)));
        val cg = oeq_rw_T (subAbs, parity xF, ZeroC) hz (oeqRefl_T (sub oneC (parity xF)));
                 (* sub 1 (parity x) = sub 1 0 *)
        val eq1 = oeq_trans_T OF [oeq_trans_T OF [pS, cg], sub1_0]; (* parity(Suc x) = 1 *)
        val r = disjI2_at_T (oeq (parity (suc xF)) ZeroC, oeq (parity (suc xF)) oneC) eq1
      in Thm.implies_intr (ctermT (jT (oeq (parity xF) ZeroC))) r end;
    (* case parity x = 1 : parity(Suc x) = sub 1 1 = 0 -> left *)
    val cB =
      let
        val ho = Thm.assume (ctermT (jT (oeq (parity xF) oneC)))
        val subAbs = Abs("z", natT, oeq (sub oneC (parity xF)) (sub oneC (Bound 0)));
        val cg = oeq_rw_T (subAbs, parity xF, oneC) ho (oeqRefl_T (sub oneC (parity xF)));
                 (* sub 1 (parity x) = sub 1 1 *)
        val eq0 = oeq_trans_T OF [oeq_trans_T OF [pS, cg], sub1_1]; (* parity(Suc x) = 0 *)
        val l = disjI1_at_T (oeq (parity (suc xF)) ZeroC, oeq (parity (suc xF)) oneC) eq0
      in Thm.implies_intr (ctermT (jT (oeq (parity xF) oneC))) l end;
    val stepConcl = disjE_T_at (oeq (parity xF) ZeroC, oeq (parity xF) oneC, goalS) IH cA cB;
    val stepF = Thm.forall_intr (ctermT xF)
                  (Thm.implies_intr (ctermT ihP) stepConcl);
    val run = nat_induct_T_run Pabs kF baseThm stepF;
  in varify run end;

(* validate parity_bounded *)
val nV_pb = Var(("n",0), natT);
val i_parity_bounded = jT (mkDisj (oeq (parity nV_pb) ZeroC) (oeq (parity nV_pb) oneC));
val r_parity_bounded = checkT ("parity_bounded", parity_bounded, i_parity_bounded);
(* ground instantiator for parity_bounded *)
fun parity_bounded_at t = beta_norm (Drule.infer_instantiate ctxtT [(("n",0), ctermT t)] parity_bounded);

(* parity_idem : oeq (parity (parity n)) (parity n)  via case on parity_bounded *)
val parity_idem =
  let
    val nF = Free("n", natT);
    val pb = parity_bounded_at nF;   (* Disj (parity n = 0)(parity n = 1) *)
    val goal = oeq (parity (parity nF)) (parity nF);
    (* case parity n = 0 : parity(parity n) = parity 0 = 0 = parity n *)
    val cA =
      let
        val hz = Thm.assume (ctermT (jT (oeq (parity nF) ZeroC)))
        (* parity(parity n) = parity 0   via rewriting argument *)
        val argAbs = Abs("z", natT, oeq (parity (parity nF)) (parity (Bound 0)));
        val cg = oeq_rw_T (argAbs, parity nF, ZeroC) hz (oeqRefl_T (parity (parity nF)));
                 (* parity(parity n) = parity 0 *)
        val toZero = oeq_trans_T OF [cg, parity_0_T];  (* parity(parity n) = 0 *)
        val res = oeq_trans_T OF [toZero, oeq_sym_T OF [hz]];  (* parity(parity n) = parity n *)
      in Thm.implies_intr (ctermT (jT (oeq (parity nF) ZeroC))) res end;
    val cB =
      let
        val ho = Thm.assume (ctermT (jT (oeq (parity nF) oneC)))
        val argAbs = Abs("z", natT, oeq (parity (parity nF)) (parity (Bound 0)));
        val cg = oeq_rw_T (argAbs, parity nF, oneC) ho (oeqRefl_T (parity (parity nF)));
                 (* parity(parity n) = parity 1 *)
        val toOne = oeq_trans_T OF [cg, parity1_eq];  (* parity(parity n) = 1 *)
        val res = oeq_trans_T OF [toOne, oeq_sym_T OF [ho]];  (* parity(parity n) = parity n *)
      in Thm.implies_intr (ctermT (jT (oeq (parity nF) oneC))) res end;
    val concl = disjE_T_at (oeq (parity nF) ZeroC, oeq (parity nF) oneC, goal) pb cA cB;
  in varify concl end;
val i_parity_idem = jT (oeq (parity (parity nV_pb)) (parity nV_pb));
val r_parity_idem = checkT ("parity_idem", parity_idem, i_parity_idem);

(* Lemma A : parity(add (Suc a) b) = sub 1 (parity(add a b))   (no induction) *)
fun parityA_at (at, bt) =
  let
    val addS = addSuc_T (at, bt);                 (* add (Suc a) b = Suc(add a b) *)
    val pcong = let val pAbs = Abs("z", natT, oeq (parity (add (suc at) bt)) (parity (Bound 0)))
                in oeq_rw_T (pAbs, add (suc at) bt, suc (add at bt)) addS (oeqRefl_T (parity (add (suc at) bt))) end;
                (* parity(add(Suc a)b) = parity(Suc(add a b)) *)
    val pS = beta_norm (Drule.infer_instantiate ctxtT [(("n",0), ctermT (add at bt))] parity_Suc_T);
             (* parity(Suc(add a b)) = sub 1 (parity(add a b)) *)
  in oeq_trans_T OF [pcong, pS] end;              (* parity(add(Suc a)b)=sub 1(parity(add a b)) *)

(* sub1_invol_bounded : Disj(x=0)(x=1) ==> sub 1 (sub 1 x) = x *)
fun sub1_invol_bounded_at (xt, pbThm) =
  let
    val goal = oeq (sub oneC (sub oneC xt)) xt;
    val cA =
      let
        val hz = Thm.assume (ctermT (jT (oeq xt ZeroC)))
        (* sub 1(sub 1 x) = sub 1(sub 1 0) = sub 1 1 = 0 = x *)
        val inAbs = Abs("z", natT, oeq (sub oneC (sub oneC xt)) (sub oneC (sub oneC (Bound 0))));
        val cg = oeq_rw_T (inAbs, xt, ZeroC) hz (oeqRefl_T (sub oneC (sub oneC xt)));
                 (* sub 1(sub 1 x) = sub 1(sub 1 0) *)
        (* sub 1 0 = 1, so sub 1(sub 1 0) = sub 1 1 *)
        val mid1Abs = Abs("z", natT, oeq (sub oneC (sub oneC ZeroC)) (sub oneC (Bound 0)));
        val cg2 = oeq_rw_T (mid1Abs, sub oneC ZeroC, oneC) sub1_0 (oeqRefl_T (sub oneC (sub oneC ZeroC)));
                  (* sub 1(sub 1 0) = sub 1 1 *)
        val toZero = oeq_trans_T OF [oeq_trans_T OF [cg, cg2], sub1_1];  (* = 0 *)
        val res = oeq_trans_T OF [toZero, oeq_sym_T OF [hz]];  (* = x *)
      in Thm.implies_intr (ctermT (jT (oeq xt ZeroC))) res end;
    val cB =
      let
        val ho = Thm.assume (ctermT (jT (oeq xt oneC)))
        val inAbs = Abs("z", natT, oeq (sub oneC (sub oneC xt)) (sub oneC (sub oneC (Bound 0))));
        val cg = oeq_rw_T (inAbs, xt, oneC) ho (oeqRefl_T (sub oneC (sub oneC xt)));
                 (* sub 1(sub 1 x) = sub 1(sub 1 1) *)
        val mid0Abs = Abs("z", natT, oeq (sub oneC (sub oneC oneC)) (sub oneC (Bound 0)));
        val cg2 = oeq_rw_T (mid0Abs, sub oneC oneC, ZeroC) sub1_1 (oeqRefl_T (sub oneC (sub oneC oneC)));
                  (* sub 1(sub 1 1) = sub 1 0 *)
        val toOne = oeq_trans_T OF [oeq_trans_T OF [cg, cg2], sub1_0];  (* = 1 *)
        val res = oeq_trans_T OF [toOne, oeq_sym_T OF [ho]];  (* = x *)
      in Thm.implies_intr (ctermT (jT (oeq xt oneC))) res end;
  in disjE_T_at (oeq xt ZeroC, oeq xt oneC, goal) pbThm cA cB end;

(* ----------------------------------------------------------------------------
   parity_norm_l : parity(add a b) = parity(add (parity a) b)   BY INDUCTION on a
   ---------------------------------------------------------------------------- *)
val parity_norm_l =
  let
    val bF = Free("b", natT);
    val kF = Free("a", natT);
    val Pabs = Abs("z", natT, oeq (parity (add (Bound 0) bF)) (parity (add (parity (Bound 0)) bF)));
    (* BASE a=0 : parity(0+b) = parity(parity 0 + b) *)
    val baseThm =
      let
        (* RHS : parity(parity 0 + b) ; rewrite parity 0 -> 0 : parity(0+b) *)
        val inAbs = Abs("z", natT, oeq (parity (add (parity ZeroC) bF)) (parity (add (Bound 0) bF)));
        val cg = oeq_rw_T (inAbs, parity ZeroC, ZeroC) parity_0_T (oeqRefl_T (parity (add (parity ZeroC) bF)));
                 (* parity(parity 0 + b) = parity(0 + b) *)
      in oeq_sym_T OF [cg] end;   (* parity(0+b) = parity(parity 0 + b) *)
    (* STEP a -> Suc a *)
    val xF = Free("x", natT);
    val ihP = jT (oeq (parity (add xF bF)) (parity (add (parity xF) bF)));
    val IH  = Thm.assume (ctermT ihP);
    val stepConcl =
      let
        (* LHS : parity(add (Suc x) b) = sub 1 (parity(add x b))  [Lemma A] *)
        val la = parityA_at (xF, bF);                 (* parity(add(Suc x)b)=sub 1(parity(add x b)) *)
        (* rewrite parity(add x b) -> parity(parity x + b) via IH inside sub 1 (.) *)
        val sAbs = Abs("z", natT, oeq (sub oneC (parity (add xF bF))) (sub oneC (Bound 0)));
        val laIH = oeq_rw_T (sAbs, parity (add xF bF), parity (add (parity xF) bF)) IH
                            (oeqRefl_T (sub oneC (parity (add xF bF))));
                   (* sub 1(parity(add x b)) = sub 1(parity(parity x + b)) *)
        val lhsEq = oeq_trans_T OF [la, laIH];   (* parity(add(Suc x)b) = sub 1(parity(parity x + b)) *)
        (* RHS : parity(add (parity(Suc x)) b) = parity(add (sub 1 (parity x)) b) *)
        val pSx = beta_norm (Drule.infer_instantiate ctxtT [(("n",0), ctermT xF)] parity_Suc_T);
                  (* parity(Suc x) = sub 1 (parity x) *)
        val rAbs = Abs("z", natT, oeq (parity (add (parity (suc xF)) bF)) (parity (add (Bound 0) bF)));
        val rhsCg = oeq_rw_T (rAbs, parity (suc xF), sub oneC (parity xF)) pSx
                             (oeqRefl_T (parity (add (parity (suc xF)) bF)));
                    (* parity(add(parity(Suc x))b) = parity(add(sub 1(parity x))b) *)
        (* Now need : sub 1(parity(parity x + b)) = parity(add(sub 1(parity x))b)
           case on parity x via parity_bounded *)
        val pbx = parity_bounded_at xF;   (* Disj(parity x=0)(parity x=1) *)
        val midGoal = oeq (sub oneC (parity (add (parity xF) bF))) (parity (add (sub oneC (parity xF)) bF));
        val cA =
          let
            val hz = Thm.assume (ctermT (jT (oeq (parity xF) ZeroC)))
            (* LHS : sub 1(parity(parity x + b)) -> rewrite parity x -> 0 -> sub 1(parity(0+b)) *)
            val l1Abs = Abs("z", natT, oeq (sub oneC (parity (add (parity xF) bF))) (sub oneC (parity (add (Bound 0) bF))));
            val l1 = oeq_rw_T (l1Abs, parity xF, ZeroC) hz (oeqRefl_T (sub oneC (parity (add (parity xF) bF))));
                     (* sub 1(parity(parity x+b)) = sub 1(parity(0+b)) *)
            (* RHS : parity(add(sub 1(parity x))b) -> rewrite parity x->0 -> parity(add(sub 1 0)b)
                     -> sub 1 0 = 1 -> parity(add 1 b) = parity(add(Suc 0)b) = sub 1(parity(0+b)) [Lemma A] *)
            val r1Abs = Abs("z", natT, oeq (parity (add (sub oneC (parity xF)) bF)) (parity (add (sub oneC (Bound 0)) bF)));
            val r1 = oeq_rw_T (r1Abs, parity xF, ZeroC) hz (oeqRefl_T (parity (add (sub oneC (parity xF)) bF)));
                     (* parity(add(sub 1(parity x))b) = parity(add(sub 1 0)b) *)
            val r2Abs = Abs("z", natT, oeq (parity (add (sub oneC ZeroC) bF)) (parity (add (Bound 0) bF)));
            val r2 = oeq_rw_T (r2Abs, sub oneC ZeroC, oneC) sub1_0 (oeqRefl_T (parity (add (sub oneC ZeroC) bF)));
                     (* parity(add(sub 1 0)b) = parity(add 1 b) ; 1 = Suc 0 *)
            val la0 = parityA_at (ZeroC, bF);  (* parity(add(Suc 0)b)=sub 1(parity(add 0 b)) *)
            (* chain RHS: r1 then r2 then la0 -> sub 1(parity(0+b)) *)
            val rhsChain = oeq_trans_T OF [oeq_trans_T OF [r1, r2], la0];
                           (* parity(add(sub 1(parity x))b) = sub 1(parity(add 0 b)) *)
            (* both equal sub 1(parity(0+b)) ; combine *)
            val res = oeq_trans_T OF [l1, oeq_sym_T OF [rhsChain]];
                      (* sub 1(parity(parity x+b)) = parity(add(sub 1(parity x))b) *)
          in Thm.implies_intr (ctermT (jT (oeq (parity xF) ZeroC))) res end;
        val cB =
          let
            val ho = Thm.assume (ctermT (jT (oeq (parity xF) oneC)))
            (* LHS : sub 1(parity(parity x+b)) -> parity x->1 -> sub 1(parity(1+b))
                     1+b = Suc 0 + b ; Lemma A : parity(add(Suc 0)b)=sub 1(parity(add 0 b)) *)
            val l1Abs = Abs("z", natT, oeq (sub oneC (parity (add (parity xF) bF))) (sub oneC (parity (add (Bound 0) bF))));
            val l1 = oeq_rw_T (l1Abs, parity xF, oneC) ho (oeqRefl_T (sub oneC (parity (add (parity xF) bF))));
                     (* = sub 1(parity(1+b)) ; 1 = Suc 0 *)
            val la1 = parityA_at (ZeroC, bF);   (* parity(add(Suc 0)b)=sub 1(parity(add 0 b)) *)
            (* rewrite parity(1+b) using la1 inside sub 1(.) *)
            val l2Abs = Abs("z", natT, oeq (sub oneC (parity (add (suc ZeroC) bF))) (sub oneC (Bound 0)));
            val l2 = oeq_rw_T (l2Abs, parity (add (suc ZeroC) bF), sub oneC (parity (add ZeroC bF))) la1
                              (oeqRefl_T (sub oneC (parity (add (suc ZeroC) bF))));
                     (* sub 1(parity(1+b)) = sub 1(sub 1(parity(0+b))) *)
            val lhsChain = oeq_trans_T OF [l1, l2];  (* sub 1(parity(parity x+b)) = sub 1(sub 1(parity(0+b))) *)
            (* RHS : parity(add(sub 1(parity x))b) -> parity x->1 -> parity(add(sub 1 1)b)
                     -> sub 1 1 = 0 -> parity(add 0 b) *)
            val r1Abs = Abs("z", natT, oeq (parity (add (sub oneC (parity xF)) bF)) (parity (add (sub oneC (Bound 0)) bF)));
            val r1 = oeq_rw_T (r1Abs, parity xF, oneC) ho (oeqRefl_T (parity (add (sub oneC (parity xF)) bF)));
                     (* parity(add(sub 1(parity x))b) = parity(add(sub 1 1)b) *)
            val r2Abs = Abs("z", natT, oeq (parity (add (sub oneC oneC) bF)) (parity (add (Bound 0) bF)));
            val r2 = oeq_rw_T (r2Abs, sub oneC oneC, ZeroC) sub1_1 (oeqRefl_T (parity (add (sub oneC oneC) bF)));
                     (* parity(add(sub 1 1)b) = parity(add 0 b) *)
            val rhsChain = oeq_trans_T OF [r1, r2];  (* parity(add(sub 1(parity x))b) = parity(add 0 b) *)
            (* Now sub 1(sub 1(parity(0+b))) = parity(0+b)  via sub1_invol on bounded parity(0+b) *)
            val pb0b = parity_bounded_at (add ZeroC bF);
            val invol = sub1_invol_bounded_at (parity (add ZeroC bF), pb0b);
                        (* sub 1(sub 1(parity(0+b))) = parity(0+b) *)
            (* LHS = sub 1(sub 1(parity(0+b))) = parity(0+b) = RHS *)
            val res = oeq_trans_T OF [oeq_trans_T OF [lhsChain, invol], oeq_sym_T OF [rhsChain]];
                      (* sub 1(parity(parity x+b)) = parity(add(sub 1(parity x))b) *)
        ...
        val midEq = disjE_T_at (oeq (parity xF) ZeroC, oeq (parity xF) oneC, midGoal) pbx cA cB;
                    (* sub 1(parity(parity x+b)) = parity(add(sub 1(parity x))b) *)
        (* LHS chain : parity(add(Suc x)b) = sub 1(parity(parity x+b)) = parity(add(sub 1(parity x))b) *)
        val lhsToMid = oeq_trans_T OF [lhsEq, midEq];
                       (* parity(add(Suc x)b) = parity(add(sub 1(parity x))b) *)
        (* combine with rhsCg (reversed) : parity(add(parity(Suc x))b) = parity(add(sub 1(parity x))b) *)
        val res = oeq_trans_T OF [lhsToMid, oeq_sym_T OF [rhsCg]];
                  (* parity(add(Suc x)b) = parity(add(parity(Suc x))b) *)
      in res end;
    val stepF = Thm.forall_intr (ctermT xF)
                  (Thm.implies_intr (ctermT ihP) stepConcl);
    val run = nat_induct_T_run Pabs kF baseThm stepF;
  in varify run end;
val aV_pn = Var(("a",0), natT); val bV_pn = Var(("b",0), natT);
val i_parity_norm_l = jT (oeq (parity (add aV_pn bV_pn)) (parity (add (parity aV_pn) bV_pn)));
val r_parity_norm_l = checkT ("parity_norm_l", parity_norm_l, i_parity_norm_l);
fun parity_norm_l_at (at, bt) = beta_norm (Drule.infer_instantiate ctxtT
      [(("a",0), ctermT at),(("b",0), ctermT bt)] parity_norm_l);

(* add_comm on ctxtT *)
val add_comm_T = varifyT add_comm;
fun add_comm_T_at (mt, nt) = beta_norm (Drule.infer_instantiate ctxtT
      [(("m",0), ctermT mt),(("n",0), ctermT nt)] add_comm_T);

(* parity_add : parity(add a b) = parity(add (parity a)(parity b)) *)
val parity_add =
  let
    val aF = Free("a", natT); val bF = Free("b", natT);
    (* step1 : parity(add a b) = parity(add (parity a) b)   [norm_l] *)
    val s1 = parity_norm_l_at (aF, bF);
    (* step2 : add (parity a) b = add b (parity a)  [comm] -> rewrite inside parity(.) *)
    val comm1 = add_comm_T_at (parity aF, bF);   (* add(parity a)b = add b(parity a) *)
    val s2Abs = Abs("z", natT, oeq (parity (add (parity aF) bF)) (parity (Bound 0)));
    val s2 = oeq_rw_T (s2Abs, add (parity aF) bF, add bF (parity aF)) comm1
                      (oeqRefl_T (parity (add (parity aF) bF)));
             (* parity(add(parity a)b) = parity(add b(parity a)) *)
    (* step3 : parity(add b (parity a)) = parity(add (parity b)(parity a))  [norm_l on (b, parity a)] *)
    val s3 = parity_norm_l_at (bF, parity aF);
    (* step4 : add (parity b)(parity a) = add (parity a)(parity b)  [comm] -> rewrite *)
    val comm2 = add_comm_T_at (parity bF, parity aF);  (* add(parity b)(parity a)=add(parity a)(parity b) *)
    val s4Abs = Abs("z", natT, oeq (parity (add (parity bF) (parity aF))) (parity (Bound 0)));
    val s4 = oeq_rw_T (s4Abs, add (parity bF)(parity aF), add (parity aF)(parity bF)) comm2
                      (oeqRefl_T (parity (add (parity bF)(parity aF))));
             (* parity(add(parity b)(parity a)) = parity(add(parity a)(parity b)) *)
    val chain = oeq_trans_T OF [oeq_trans_T OF [oeq_trans_T OF [s1, s2], s3], s4];
  in varify chain end;
val i_parity_add = jT (oeq (parity (add aV_pn bV_pn))
                           (parity (add (parity aV_pn) (parity bV_pn))));
val r_parity_add = checkT ("parity_add", parity_add, i_parity_add);

val () = if r_parity_bounded andalso r_parity_add then out "PARITY_INFRA_OK\n"
         else out "PARITY_INFRA_INCOMPLETE\n";

(* ============================================================================
   (T2) SUM_CONST : oeq (sumf (%i. c) n) (mult c (Suc n))
   sumf is inclusive 0..n (n+1 terms = Suc n).  By induction on n.
   ============================================================================ *)
val () = out "T2_SUMCONST_BEGIN\n";

val mult_Suc_right_T = varifyT mult_Suc_right;
(* mult_Suc_right_T : oeq (mult n (Suc m))(add n (mult n m)) ; multcand -> n, incr -> m *)
fun mult_Suc_right_T_at (multcand, incr) = beta_norm (Drule.infer_instantiate ctxtT
      [(("n",0), ctermT multcand),(("m",0), ctermT incr)] mult_Suc_right_T);
val mult_1_right_T = varifyT mult_1_right;
fun mult_1_right_T_at t = beta_norm (Drule.infer_instantiate ctxtT [(("n",0), ctermT t)] mult_1_right_T);

val sum_const =
  let
    val cF = Free("c", natT);
    val cabs = Abs("i", natT, cF);              (* %i. c  (closed) *)
    val kF = Free("n", natT);
    (* P z = oeq (sumf (%i.c) z)(mult c (Suc z)) *)
    val Pabs = Abs("z", natT, oeq (sumf cabs (Bound 0)) (mult cF (suc (Bound 0))));
    (* BASE n=0 : sumf (%i.c) 0 = (%i.c) 0 = c ; mult c (Suc 0) = c *)
    val baseThm =
      let
        val sf0 = sumf0_T cabs;                  (* sumf (%i.c) 0 = (%i.c) 0 = c [beta] *)
        (* RHS mult c (Suc 0) = c  via mult_1_right *)
        val m1 = mult_1_right_T_at cF;           (* mult c (Suc 0) = c *)
        (* sumf (%i.c) 0 = c = mult c (Suc 0) *)
      in oeq_trans_T OF [sf0, oeq_sym_T OF [m1]] end;  (* sumf(%i.c)0 = mult c (Suc 0) *)
    (* STEP n -> Suc n *)
    val xF = Free("x", natT);
    val ihP = jT (oeq (sumf cabs xF) (mult cF (suc xF)));
    val IH  = Thm.assume (ctermT ihP);
    val stepConcl =
      let
        val sfS = sumfSuc_T (cabs, xF);          (* sumf(%i.c)(Sx) = add(sumf(%i.c)x)((%i.c)(Sx)) [beta -> c] *)
        (* rewrite (sumf(%i.c)x) -> mult c (Suc x) on LHS of add via IH *)
        val aAbs = Abs("z", natT, oeq (add (sumf cabs xF) cF) (add (Bound 0) cF));
        val cgL = oeq_rw_T (aAbs, sumf cabs xF, mult cF (suc xF)) IH (oeqRefl_T (add (sumf cabs xF) cF));
                  (* add(sumf(%i.c)x)c = add(mult c (Suc x))c *)
        val lhsEq = oeq_trans_T OF [sfS, cgL];   (* sumf(%i.c)(Sx) = add(mult c (Suc x))c *)
        (* RHS : mult c (Suc(Suc x)) = add c (mult c (Suc x))  [mult_Suc_right]
                then commute = add(mult c (Suc x))c *)
        val msr = mult_Suc_right_T_at (cF, suc xF);  (* mult c (Suc(Suc x)) = add c (mult c (Suc x)) *)
        val comm = add_comm_T_at (cF, mult cF (suc xF)); (* add c(mult c(Suc x)) = add(mult c(Suc x))c *)
        val rhsEq = oeq_trans_T OF [msr, comm];  (* mult c (Suc(Suc x)) = add(mult c (Suc x))c *)
        (* combine : sumf(%i.c)(Sx) = add(...)c = mult c (Suc(Suc x)) *)
      in oeq_trans_T OF [lhsEq, oeq_sym_T OF [rhsEq]] end;
    val stepF = Thm.forall_intr (ctermT xF) (Thm.implies_intr (ctermT ihP) stepConcl);
    val run = nat_induct_T_run Pabs kF baseThm stepF;
  in varify run end;
val cV_sc = Var(("c",0), natT); val nV_sc = Var(("n",0), natT);
val i_sum_const = jT (oeq (sumf (Abs("i", natT, cV_sc)) nV_sc) (mult cV_sc (suc nV_sc)));
val r_sum_const = checkT ("sum_const", sum_const, i_sum_const);
val () = if r_sum_const then out "SUM_CONST_OK\n" else out "SUM_CONST_INCOMPLETE\n";

(* ============================================================================
   (T3) COUNTLE combinator lemmas
   ============================================================================ *)
val () = out "T3_COUNTLE_BEGIN\n";

(* le helpers on ctxtT *)
val zero_le_T     = varifyT zero_le;
val le_suc_mono_T = varifyT le_suc_mono;
fun zero_le_T_at t = beta_norm (Drule.infer_instantiate ctxtT [(("n",0), ctermT t)] zero_le_T);
fun le_suc_mono_T_at (mt, nt) h = Thm.implies_elim (beta_norm (Drule.infer_instantiate ctxtT
      [(("m",0), ctermT mt),(("n",0), ctermT nt)] le_suc_mono_T)) h;
fun le_trans_T_at (at, bt, ct) h1 h2 =
  Thm.implies_elim (Thm.implies_elim (beta_norm (Drule.infer_instantiate ctxtT
      [(("m",0), ctermT at),(("n",0), ctermT bt),(("k",0), ctermT ct)] le_trans_T)) h1) h2;
(* le_intro on ctxtT : from (hyp : oeq n (add m w)) build jT (le m n) *)
fun le_intro_T (mT, nT, w) hyp =
  let val LAbs = Abs ("p", natT, oeq nT (add mT (Bound 0)))
      val inst = beta_norm (Drule.infer_instantiate ctxtT
            [(("P",0), ctermT LAbs), (("a",0), ctermT w)] exI_T)
  in Thm.implies_elim inst hyp end;
(* le_self_suc : le n (Suc n)  via witness Suc 0, oeq (Suc n)(add n (Suc 0)) *)
val add_Suc_right_T = varifyT add_Suc_right;
fun addSr_T (mt,nt) = beta_norm (Drule.infer_instantiate ctxtT
      [(("m",0), ctermT mt),(("n",0), ctermT nt)] add_Suc_right_T);
fun le_self_suc_T nt =
  let
    val aSr = addSr_T (nt, ZeroC);              (* (n + Suc 0) = Suc(n+0) *)
    val a0r = add0r_T nt;                        (* (n+0) = n *)
    val sa0r = Succong_T a0r;                    (* Suc(n+0) = Suc n *)
    val sum = oeq_trans_T OF [aSr, sa0r];        (* (n + Suc 0) = Suc n *)
    val sum_s = oeq_sym_T OF [sum];              (* Suc n = (n + Suc 0) *)
  in le_intro_T (nt, suc nt, suc ZeroC) sum_s end;
(* le_refl on ctxtT *)
fun le_refl_T_at t = beta_norm (Drule.infer_instantiate ctxtT [(("n",0), ctermT t)] le_refl_T);

(* capture-safe rewrite-context builder : abstract a FRESH Free (handles inner
   binders that `le`/`Ex` introduce; a raw Bound 0 would be captured by them) *)
fun mkRwAbs holeFn =
  let val zz = Free("zz_rwcap", natT) in Term.lambda zz (holeFn zz) end;

(* cnt ground instantiators *)
fun cnt0_T Pt = beta_norm (Drule.infer_instantiate ctxtT [(("P",0), ctermT Pt)] cnt_0_T);
fun cntSucT_T (Pt, nt) hP = Thm.implies_elim (beta_norm (Drule.infer_instantiate ctxtT
      [(("P",0), ctermT Pt),(("n",0), ctermT nt)] cnt_Suc_t_T)) hP;
fun cntSucF_T (Pt, nt) hNP = Thm.implies_elim (beta_norm (Drule.infer_instantiate ctxtT
      [(("P",0), ctermT Pt),(("n",0), ctermT nt)] cnt_Suc_f_T)) hNP;

(* --- cnt_le : le (cnt P n) (Suc n)  by induction on n --- *)
val cnt_le =
  let
    val PF = Free("P", predNatT);
    val kF = Free("n", natT);
    (* de-Bruijn-safe : le introduces an Ex binder, so abstract a FRESH Free *)
    val zPF = Free("z_cle", natT);
    val Pabs = Term.lambda zPF (le (cnt PF zPF) (suc zPF));
    (* BASE n=0 : le (cnt P 0)(Suc 0) ; cnt P 0 = 0 ; le 0 (Suc 0) *)
    val baseThm =
      let
        val c0 = cnt0_T PF;                       (* cnt P 0 = 0 *)
        val z_le = zero_le_T_at (suc ZeroC);      (* le 0 (Suc 0) *)
        (* rewrite 0 -> cnt P 0 in le _ (Suc 0) *)
        val lAbs = mkRwAbs (fn z => le z (suc ZeroC));
      in oeq_rw_T (lAbs, ZeroC, cnt PF ZeroC) (oeq_sym_T OF [c0]) z_le end;
    (* STEP n -> Suc n *)
    val xF = Free("x", natT);
    val ihP = jT (le (cnt PF xF) (suc xF));
    val IH  = Thm.assume (ctermT ihP);
    val goalS = le (cnt PF (suc xF)) (suc (suc xF));
    val em = ex_middle_T_at (PF $ (suc xF));       (* Disj (P(Suc x)) (~ P(Suc x)) *)
    val cA =
      let
        val hP = Thm.assume (ctermT (jT (PF $ (suc xF))))
        val ceq = cntSucT_T (PF, xF) hP;           (* cnt P (Suc x) = Suc(cnt P x) *)
        val leSuc = le_suc_mono_T_at (cnt PF xF, suc xF) IH;  (* le (Suc(cnt P x))(Suc(Suc x)) *)
        (* rewrite Suc(cnt P x) -> cnt P (Suc x) *)
        val lAbs = mkRwAbs (fn z => le z (suc (suc xF)));
        val res = oeq_rw_T (lAbs, suc (cnt PF xF), cnt PF (suc xF)) (oeq_sym_T OF [ceq]) leSuc;
      in Thm.implies_intr (ctermT (jT (PF $ (suc xF)))) res end;
    val cB =
      let
        val hNP = Thm.assume (ctermT (jT (neg (PF $ (suc xF)))))
        val ceq = cntSucF_T (PF, xF) hNP;          (* cnt P (Suc x) = cnt P x *)
        val lss = le_self_suc_T (suc xF);          (* le (Suc x)(Suc(Suc x)) *)
        val leT = le_trans_T_at (cnt PF xF, suc xF, suc (suc xF)) IH lss; (* le (cnt P x)(Suc(Suc x)) *)
        val lAbs = mkRwAbs (fn z => le z (suc (suc xF)));
        val res = oeq_rw_T (lAbs, cnt PF xF, cnt PF (suc xF)) (oeq_sym_T OF [ceq]) leT;
      in Thm.implies_intr (ctermT (jT (neg (PF $ (suc xF))))) res end;
    val stepConcl = disjE_T_at (PF $ (suc xF), neg (PF $ (suc xF)), goalS) em cA cB;
    val stepF = Thm.forall_intr (ctermT xF) (Thm.implies_intr (ctermT ihP) stepConcl);
    val run = nat_induct_T_run Pabs kF baseThm stepF;
  in varify run end;
val PV_cl = Var(("P",0), predNatT); val nV_cl = Var(("n",0), natT);
val i_cnt_le = jT (le (cnt PV_cl nV_cl) (suc nV_cl));
val r_cnt_le = checkT ("cnt_le", cnt_le, i_cnt_le);

(* --- cnt_cong : P,Q agree on all positive indices ==> oeq (cnt P n)(cnt Q n) ---
   agreement carried as two meta-universal premises (forward + backward):
     (!!k. Imp (P(Suc k))(Q(Suc k)))  and  (!!k. Imp (Q(Suc k))(P(Suc k)))
   By induction on n. *)
val cnt_cong =
  let
    val PF = Free("P", predNatT); val QF = Free("Q", predNatT);
    val kAg = Free("kag", natT);
    val fwdProp = Logic.all kAg (jT (mkImp (PF $ (suc kAg)) (QF $ (suc kAg))));
    val bwdProp = Logic.all kAg (jT (mkImp (QF $ (suc kAg)) (PF $ (suc kAg))));
    val hFwd = Thm.assume (ctermT fwdProp);
    val hBwd = Thm.assume (ctermT bwdProp);
    fun fwdAt t = Thm.forall_elim (ctermT t) hFwd;   (* Imp (P(Suc t))(Q(Suc t)) *)
    fun bwdAt t = Thm.forall_elim (ctermT t) hBwd;   (* Imp (Q(Suc t))(P(Suc t)) *)

    val kF = Free("n", natT);
    val Pabs = Abs("z", natT, oeq (cnt PF (Bound 0)) (cnt QF (Bound 0)));
    (* BASE n=0 : cnt P 0 = 0 = cnt Q 0 *)
    val baseThm =
      let val cP0 = cnt0_T PF; val cQ0 = cnt0_T QF;
      in oeq_trans_T OF [cP0, oeq_sym_T OF [cQ0]] end;  (* cnt P 0 = cnt Q 0 *)
    (* STEP *)
    val xF = Free("x", natT);
    val ihP = jT (oeq (cnt PF xF) (cnt QF xF));
    val IH  = Thm.assume (ctermT ihP);
    val goalS = oeq (cnt PF (suc xF)) (cnt QF (suc xF));
    val em = ex_middle_T_at (PF $ (suc xF));
    val cA =
      let
        val hP = Thm.assume (ctermT (jT (PF $ (suc xF))))
        val cPeq = cntSucT_T (PF, xF) hP;          (* cnt P(Suc x) = Suc(cnt P x) *)
        val hQ = mp_T_at (PF $ (suc xF), QF $ (suc xF)) (fwdAt xF) hP;  (* Q(Suc x) *)
        val cQeq = cntSucT_T (QF, xF) hQ;          (* cnt Q(Suc x) = Suc(cnt Q x) *)
        val sc = Succong_T IH;                      (* Suc(cnt P x) = Suc(cnt Q x) *)
        val res = oeq_trans_T OF [oeq_trans_T OF [cPeq, sc], oeq_sym_T OF [cQeq]];
                  (* cnt P(Suc x) = cnt Q(Suc x) *)
      in Thm.implies_intr (ctermT (jT (PF $ (suc xF)))) res end;
    val cB =
      let
        val hNP = Thm.assume (ctermT (jT (neg (PF $ (suc xF)))))  (* Imp (P(Suc x)) oFalse *)
        val cPeq = cntSucF_T (PF, xF) hNP;          (* cnt P(Suc x) = cnt P x *)
        (* build ~Q(Suc x) : assume Q(Suc x) -> P(Suc x) [bwd] -> oFalse [hNP] *)
        val negQ =
          let
            val hq = Thm.assume (ctermT (jT (QF $ (suc xF))))
            val pq = mp_T_at (QF $ (suc xF), PF $ (suc xF)) (bwdAt xF) hq;  (* P(Suc x) *)
            val fls = mp_T_at (PF $ (suc xF), oFalseC) hNP pq;             (* oFalse *)
            val meta = Thm.implies_intr (ctermT (jT (QF $ (suc xF)))) fls; (* meta: Q(Suc x) ==> oFalse *)
          in impI_T_at (QF $ (suc xF), oFalseC) meta end;                  (* OBJECT Imp (Q(Suc x)) oFalse = neg(Q(Suc x)) *)
        val cQeq = cntSucF_T (QF, xF) negQ;         (* cnt Q(Suc x) = cnt Q x *)
        val res = oeq_trans_T OF [oeq_trans_T OF [cPeq, IH], oeq_sym_T OF [cQeq]];
                  (* cnt P(Suc x) = cnt Q(Suc x) *)
      in Thm.implies_intr (ctermT (jT (neg (PF $ (suc xF))))) res end;
    val stepConcl = disjE_T_at (PF $ (suc xF), neg (PF $ (suc xF)), goalS) em cA cB;
    val stepF = Thm.forall_intr (ctermT xF) (Thm.implies_intr (ctermT ihP) stepConcl);
    val run = nat_induct_T_run Pabs kF baseThm stepF;  (* oeq (cnt P n)(cnt Q n), with 2 meta hyps *)
    val d2 = Thm.implies_intr (ctermT bwdProp) run;
    val d1 = Thm.implies_intr (ctermT fwdProp) d2;
  in varify d1 end;
(* validate cnt_cong *)
val PV_cc = Var(("P",0), predNatT); val QV_cc = Var(("Q",0), predNatT);
val nV_cc = Var(("n",0), natT); val kV_cc = Var(("kag",0), natT);
val i_cnt_cong =
  Logic.mk_implies (
    Logic.all (Free("kag",natT)) (jT (mkImp (PV_cc $ (suc (Free("kag",natT)))) (QV_cc $ (suc (Free("kag",natT)))))),
    Logic.mk_implies (
      Logic.all (Free("kag",natT)) (jT (mkImp (QV_cc $ (suc (Free("kag",natT)))) (PV_cc $ (suc (Free("kag",natT)))))),
      jT (oeq (cnt PV_cc nV_cc) (cnt QV_cc nV_cc))));
val r_cnt_cong = checkT ("cnt_cong", cnt_cong, i_cnt_cong);

val () = if r_cnt_le andalso r_cnt_cong then out "COUNTLE_OK\n" else out "COUNTLE_INCOMPLETE\n";

(* ============================================================================
   (T4) RDIV BASICS : rdiv_zero  (oeq (rdiv 0 p) 0  for 0<p)
   ============================================================================ *)
val () = out "T4_RDIV_BEGIN\n";

(* div_mod_eq / rmod_lt / div_mod_unique instantiators on ctxtT *)
fun div_mod_eq_T_at (aT, pT) hpos =
  Thm.implies_elim (beta_norm (Drule.infer_instantiate ctxtT
        [(("a",0), ctermT aT), (("p",0), ctermT pT)] div_mod_eq_T)) hpos;
fun rmod_lt_T_at (aT, pT) hpos =
  Thm.implies_elim (beta_norm (Drule.infer_instantiate ctxtT
        [(("a",0), ctermT aT), (("p",0), ctermT pT)] rmod_lt_T)) hpos;
fun conjunct1_T_at (At, Bt) h =
  Thm.implies_elim (beta_norm (Drule.infer_instantiate ctxtT
        [(("A",0), ctermT At), (("B",0), ctermT Bt)] conjunct1_T)) h;
(* div_mod_unique instantiator : vars b,q1,q2,r1,r2 *)
fun div_mod_unique_T_at (bT, q1T, r1T, q2T, r2T) hEQ hLT1 hLT2 =
  let val inst = beta_norm (Drule.infer_instantiate ctxtT
        [(("b",0), ctermT bT),(("q1",0), ctermT q1T),(("r1",0), ctermT r1T),
         (("q2",0), ctermT q2T),(("r2",0), ctermT r2T)] div_mod_unique_T)
  in Thm.implies_elim (Thm.implies_elim (Thm.implies_elim inst hEQ) hLT1) hLT2 end;

val rdiv_zero =
  let
    val pF = Free("p", natT);
    val hpos = Thm.assume (ctermT (jT (lt ZeroC pF)));     (* 0 < p *)
    val q1 = rdiv ZeroC pF; val r1 = rmod ZeroC pF;
    val divEq = div_mod_eq_T_at (ZeroC, pF) hpos;          (* 0 = add(mult p q1) r1 *)
    val rltp  = rmod_lt_T_at (ZeroC, pF) hpos;             (* lt r1 p  = le (suc r1) p *)
    (* EQ : add(mult p q1) r1 = add(mult p 0) 0 *)
    val lhs0 = oeq_sym_T OF [divEq];                       (* add(mult p q1) r1 = 0 *)
    (* add(mult p 0) 0 = 0 : mult p 0 = 0, then add 0 0 = 0 *)
    val mp0 = mult0r_T pF;                                 (* mult p 0 = 0 *)
    val aAbs = Abs("z", natT, oeq (add (mult pF ZeroC) ZeroC) (add (Bound 0) ZeroC));
    val cg = oeq_rw_T (aAbs, mult pF ZeroC, ZeroC) mp0 (oeqRefl_T (add (mult pF ZeroC) ZeroC));
             (* add(mult p 0) 0 = add 0 0 *)
    val a00 = add0r_T ZeroC;                               (* add 0 0 = 0 *)
    val rhs0 = oeq_trans_T OF [cg, a00];                   (* add(mult p 0) 0 = 0 *)
    val EQ = oeq_trans_T OF [lhs0, oeq_sym_T OF [rhs0]];   (* add(mult p q1) r1 = add(mult p 0) 0 *)
    (* LT2 : le (suc 0) p = lt 0 p = hpos *)
    val hLT2 = hpos;                                       (* lt 0 p = le (suc 0) p *)
    val conj = div_mod_unique_T_at (pF, q1, r1, ZeroC, ZeroC) EQ rltp hLT2;
               (* Conj (oeq q1 0)(oeq r1 0) *)
    val res = conjunct1_T_at (oeq q1 ZeroC, oeq r1 ZeroC) conj;  (* oeq (rdiv 0 p) 0 *)
    val d1 = Thm.implies_intr (ctermT (jT (lt ZeroC pF))) res;
  in varify d1 end;
val pV_rz = Var(("p",0), natT);
val i_rdiv_zero = Logic.mk_implies (jT (lt ZeroC pV_rz), jT (oeq (rdiv ZeroC pV_rz) ZeroC));
val r_rdiv_zero = checkT ("rdiv_zero", rdiv_zero, i_rdiv_zero);
(* soundness probe : dropping 0<p must change the theorem *)
val probe_rdiv_zero = not ((Thm.prop_of rdiv_zero) aconv (jT (oeq (rdiv ZeroC pV_rz) ZeroC)));
val () = if r_rdiv_zero andalso probe_rdiv_zero then out "RDIV_BASICS_OK\n" else out "RDIV_BASICS_INCOMPLETE\n";

(* ============================================================================
   TOOLBOX SUMMARY + AXIOM AUDIT
   ============================================================================ *)
val () = out "TOOLBOX_AUDIT_BEGIN\n";
val allAxT = Theory.all_axioms_of thyT;
val () = out ("toolbox_axiom_count=" ^ Int.toString (length allAxT) ^ "\n");
val hasEMt = List.exists (fn (nm,_) => String.isSuffix "ex_middle" nm orelse nm = "ex_middle") allAxT;
val () = out ("toolbox_ex_middle_present=" ^ Bool.toString hasEMt ^ "\n");
val newAxNames = List.filter (fn nm =>
      String.isSubstring "parity" (String.map Char.toLower nm)
      orelse String.isSubstring "cnt" (String.map Char.toLower nm)) (map fst allAxT);
val () = out ("toolbox_new_axioms=[" ^ String.concatWith "," newAxNames ^ "]\n");
val () = out "TOOLBOX_AUDIT_END\n";

val toolboxAllOK = r_parity_bounded andalso r_parity_add andalso r_sum_const
                   andalso r_cnt_le andalso r_cnt_cong andalso r_rdiv_zero
                   andalso hasEMt;
val () = if toolboxAllOK then out "TOOLBOX_ALL_OK\n" else out "TOOLBOX_PARTIAL\n";
val () = out "EISENSTEIN_TOOLBOX_END\n";
