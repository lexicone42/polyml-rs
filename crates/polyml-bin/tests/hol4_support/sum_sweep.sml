(* sum_sweep.sml — Stage 6b: sweep the REAL sumScript.sml
   (1212 lines; quotientLib refs are attr-tags only) on /tmp/hol4_numsimps;
   export /tmp/hol4_sum. Same harness as relation_tail_sweep with:
...
structure boolLib = struct
  ...
  (* sumScript: val PAIR = new_specification("PAIR[simp]", ...) *)
...
val () = writeFile ("/tmp/asweep_rebind.sml",
  String.concatWith "\n" [
    ...
    ""]);
(* WF_LESS (cut WF tail) — sumScript needs prim_recTheory.WF_LESS *)
...
val lines =
    String.fields (fn c => c = #"\n")
                  (readFile (HOL ^ "/src/coretypes/sumScript.sml"));
...
(* re-runnable (pass 2 on the exported image): chunks whose name is already
   ...
   current theory is already "sum" (re-running it would WIPE the
...
(* post-filter patches. quotientLib is absent on our images — sumScript only
            ...
            | NONE =>
              let val filtered0 =
                      HOLSource.inputFile {quietOpen = false, print = fn _ => ()}
                                          "/tmp/asweep_src.sml"
                  val f1 =
                      if name = "HEADER" andalso Theory.current_theory () = "sum"
...
val () = pr ("SUMTHEORY_NAMES " ^ Int.toString (length (!btbl)) ^ "\n");
val () =
    let val os = TextIO.openOut "/tmp/sumTheory_gen.sml"
    in TextIO.output (os, "structure sumTheory = struct\n");
       app (fn (n, _) =>
               TextIO.output (os, "  val " ^ n ^ " = bt \"" ^ n ^ "\";\n"))
           (!btbl);
       (* name->thm table for later DB.fetch "sum" serving *)
       TextIO.output (os, "  val dbTable : (string * Thm.thm) list = !btbl;\n");
       TextIO.output (os, "end;\n"); TextIO.closeOut os
    end;
val () = (PolyML.use "/tmp/sumTheory_gen.sml"; pr "STRUCT_OK\n")
         handle e => pr ("STRUCT_FAIL :: " ^ exnMessage e ^ "\n");

val smoke = ref true;
fun need tag b = if b then pr ("OK " ^ tag ^ "\n")
                 else (smoke := false; pr ("MISSING " ^ tag ^ "\n"));
val () = need "sum-current" (Theory.current_theory () = "sum");
val () = need "ISL"  ((ignore (bt "ISL");  true) handle _ => false);
val () = need "ISR"  ((ignore (bt "ISR");  true) handle _ => false);
val () = need "sum_case_def" ((ignore (bt "sum_case_def"); true) handle _ => false);
val () = pr (if !smoke then "SUM_SMOKE_PASS\n" else "SUM_SMOKE_FAIL\n");
val () =
    if !smoke then
      (pr "EXPORTING /tmp/hol4_sum\n";
       PolyML.export ("/tmp/hol4_sum", PolyML.rootFunction);
       pr "SUM_SWEEP_DONE\n")
