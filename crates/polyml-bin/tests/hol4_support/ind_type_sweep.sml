(* arithmetic_sweep.sml — Stage 4: sweep the REAL ind_typeScript.sml
   (5759 lines, 524 Theorem blocks) per-theorem on /tmp/hol4_relation and
   export /tmp/hol4_ind_type. Same harness as relation_tail_sweep with:
...
infix 8 by;
(* ind_typeScript uses the modern tactic combinators >> >- \\ (and >| ) *)
infix >> >- \\ >|;
...
val () = writeFile ("/tmp/asweep_rebind.sml",
  String.concatWith "\n" [
    ...
    (* bare Induct (ind_typeScript uses `Induct THEN ...`): num induction on the
    ...
    (* ind_typeScript uses `theorem "invtri0_ind"` (lines 86,95) — top-level
    ...
    "infix >> >- \\\\ >|;",
    (* ind_typeScript uses ARITH_ss / REDUCE_ss unqualified (from open numSimps,
       which the header's numLib failure had aborted) *)
    "val ARITH_ss = numSimps.ARITH_ss;",
    "val REDUCE_ss = numSimps.REDUCE_ss;",
    ""]);
(* WF_LESS (cut WF tail) — ind_typeScript needs prim_recTheory.WF_LESS *)
val () = (PolyML.use ((HOL ^ "/../../crates/polyml-bin/tests/hol4_support")
...
val lines =
    String.fields (fn c => c = #"\n")
                  (readFile (HOL ^ "/src/datatype/ind_typeScript.sml"));
...
(* re-runnable (pass 2 on the exported image): chunks whose name is already
   ...
   current theory is already "ind_type" (re-running it would WIPE the
            ...
            | NONE =>
              let val filtered0 =
                      HOLSource.inputFile {quietOpen = false, print = fn _ => ()}
                                          "/tmp/asweep_src.sml"
                  val f1 =
                      if name = "HEADER" andalso Theory.current_theory () = "ind_type"
...
(* synthesize ind_typeTheory; smoke; export. *)
val all_named =
...
val () = pr ("INDTYPETHEORY_NAMES " ^ Int.toString (length (!btbl)) ^ "\n");
val () =
    let val os = TextIO.openOut "/tmp/ind_typeTheory_gen.sml"
    in TextIO.output (os, "structure ind_typeTheory = struct\n");
       app (fn (n, _) =>
               TextIO.output (os, "  val " ^ n ^ " = bt \"" ^ n ^ "\";\n"))
           (!btbl);
       (* name->thm table for later DB.fetch "ind_type" serving *)
       TextIO.output (os, "  val dbTable : (string * Thm.thm) list = !btbl;\n");
       TextIO.output (os, "end;\n"); TextIO.closeOut os
    end;
val () = (PolyML.use "/tmp/ind_typeTheory_gen.sml"; pr "STRUCT_OK\n")
         handle e => pr ("STRUCT_FAIL :: " ^ exnMessage e ^ "\n");

val smoke = ref true;
fun need tag b = if b then pr ("OK " ^ tag ^ "\n")
                 else (smoke := false; pr ("MISSING " ^ tag ^ "\n"));
val () = need "ind_type-current" (Theory.current_theory () = "ind_type");
val () = need "CONSTR_REC" ((ignore (bt "CONSTR_REC"); true) handle _ => false);
val () = need "CONSTR_IND" ((ignore (bt "CONSTR_IND"); true) handle _ => false);
val () = need "FCONS" ((ignore (bt "FCONS"); true) handle _ => false);
val () = pr (if !smoke then "INDTYPE_SMOKE_PASS\n" else "INDTYPE_SMOKE_FAIL\n");
val () =
    if !smoke then
      (pr "EXPORTING /tmp/hol4_ind_type\n";
       PolyML.export ("/tmp/hol4_ind_type", PolyML.rootFunction);
       pr "INDTYPE_SWEEP_DONE\n")
