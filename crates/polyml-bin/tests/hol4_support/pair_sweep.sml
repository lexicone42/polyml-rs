(* pair_sweep.sml — Stage 6a: sweep the REAL pairScript.sml
   (1212 lines; quotientLib refs are attr-tags only) on /tmp/hol4_numsimps;
   export /tmp/hol4_pair. Same harness as relation_tail_sweep with:
...
val () = writeFile ("/tmp/asweep_rebind.sml",
  String.concatWith "\n" [
    ...
    ""]);
(* WF_LESS (cut WF tail) — pairScript needs prim_recTheory.WF_LESS *)
...
val lines =
    String.fields (fn c => c = #"\n")
                  (readFile (HOL ^ "/src/coretypes/pairScript.sml"));
...
(* re-runnable (pass 2 on the exported image): chunks whose name is already
   ...
   current theory is already "pair" (re-running it would WIPE the
...
(* post-filter patches. quotientLib is absent on our images — pairScript only
            ...
            | NONE =>
              let val filtered0 =
                      HOLSource.inputFile {quietOpen = false, print = fn _ => ()}
                                          "/tmp/asweep_src.sml"
                  val f1 =
                      if name = "HEADER" andalso Theory.current_theory () = "pair"
...
val () = pr ("PAIRTHEORY_NAMES " ^ Int.toString (length (!btbl)) ^ "\n");
val () =
    let val os = TextIO.openOut "/tmp/pairTheory_gen.sml"
    in TextIO.output (os, "structure pairTheory = struct\n");
       app (fn (n, _) =>
               TextIO.output (os, "  val " ^ n ^ " = bt \"" ^ n ^ "\";\n"))
           (!btbl);
       (* name->thm table for later DB.fetch "pair" serving *)
       TextIO.output (os, "  val dbTable : (string * Thm.thm) list = !btbl;\n");
       TextIO.output (os, "end;\n"); TextIO.closeOut os
    end;
val () = (PolyML.use "/tmp/pairTheory_gen.sml"; pr "STRUCT_OK\n")
         handle e => pr ("STRUCT_FAIL :: " ^ exnMessage e ^ "\n");

val smoke = ref true;
fun need tag b = if b then pr ("OK " ^ tag ^ "\n")
                 else (smoke := false; pr ("MISSING " ^ tag ^ "\n"));
val () = need "pair-current" (Theory.current_theory () = "pair");
val () = need "PAIR_EQ"    ((ignore (bt "PAIR_EQ");    true) handle _ => false);
val () = need "FST"  ((ignore (bt "FST");  true) handle _ => false);
val () = need "SND" ((ignore (bt "SND"); true) handle _ => false);
val () = pr (if !smoke then "PAIR_SMOKE_PASS\n" else "PAIR_SMOKE_FAIL\n");
val () =
    if !smoke then
      (pr "EXPORTING /tmp/hol4_pair\n";
       PolyML.export ("/tmp/hol4_pair", PolyML.rootFunction);
       pr "PAIR_SWEEP_DONE\n")
