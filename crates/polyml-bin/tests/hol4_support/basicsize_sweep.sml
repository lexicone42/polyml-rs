(* sum_sweep.sml — Stage 6b: sweep the REAL basicSizeScript.sml
   (1212 lines; quotientLib refs are attr-tags only) on /tmp/hol4_numsimps;
   export /tmp/hol4_basicsize. Same harness as relation_tail_sweep with:
...
structure boolLib = struct
  ...
  (* basicSizeScript: val PAIR = new_specification("PAIR[simp]", ...) *)
...
val () = writeFile ("/tmp/asweep_rebind.sml",
  String.concatWith "\n" [
    ...
    ""]);
(* WF_LESS (cut WF tail) — basicSizeScript needs prim_recTheory.WF_LESS *)
...
val lines =
    String.fields (fn c => c = #"\n")
                  (readFile (HOL ^ "/src/num/theories/basicSizeScript.sml"));
...
(* re-runnable (pass 2 on the exported image): chunks whose name is already
   ...
   current theory is already "basicSize" (re-running it would WIPE the
...
(* post-filter patches. quotientLib is absent on our images — basicSizeScript only
            ...
            | NONE =>
              let val filtered0 =
                      HOLSource.inputFile {quietOpen = false, print = fn _ => ()}
                                          "/tmp/asweep_src.sml"
                  val f1 =
                      if name = "HEADER" andalso Theory.current_theory () = "basicSize"
...
val () = pr ("BASICSIZETHEORY_NAMES " ^ Int.toString (length (!btbl)) ^ "\n");
val () =
    let val os = TextIO.openOut "/tmp/basicSizeTheory_gen.sml"
    in TextIO.output (os, "structure basicSizeTheory = struct\n");
       app (fn (n, _) =>
               TextIO.output (os, "  val " ^ n ^ " = bt \"" ^ n ^ "\";\n"))
           (!btbl);
       (* name->thm table for later DB.fetch "basicSize" serving *)
       TextIO.output (os, "  val dbTable : (string * Thm.thm) list = !btbl;\n");
       TextIO.output (os, "end;\n"); TextIO.closeOut os
    end;
val () = (PolyML.use "/tmp/basicSizeTheory_gen.sml"; pr "STRUCT_OK\n")
         handle e => pr ("STRUCT_FAIL :: " ^ exnMessage e ^ "\n");

val smoke = ref true;
fun need tag b = if b then pr ("OK " ^ tag ^ "\n")
                 else (smoke := false; pr ("MISSING " ^ tag ^ "\n"));
val () = need "basicSize-current" (Theory.current_theory () = "basicSize");
val () = need "pair_size_def" ((ignore (bt "pair_size_def"); true) handle _ => false);
val () = need "sum_size_def"  ((ignore (bt "sum_size_def");  true) handle _ => false);
val () = need "option_size_def" ((ignore (bt "option_size_def"); true) handle _ => false);
val () = pr (if !smoke then "BASICSIZE_SMOKE_PASS\n" else "BASICSIZE_SMOKE_FAIL\n");
val () =
    if !smoke then
      (pr "EXPORTING /tmp/hol4_basicsize\n";
       PolyML.export ("/tmp/hol4_basicsize", PolyML.rootFunction);
       pr "BASICSIZE_SWEEP_DONE\n")
