(* arithmetic_sweep.sml — Stage 4: sweep the REAL numpairScript.sml
   (5759 lines, 524 Theorem blocks) per-theorem on /tmp/hol4_relation and
   export /tmp/hol4_numpair. Same harness as relation_tail_sweep with:
...
val lines =
    String.fields (fn c => c = #"\n")
                  (readFile (HOL ^ "/src/num/extra_theories/numpairScript.sml"));
...
(* re-runnable (pass 2 on the exported image): chunks whose name is already
   ...
   current theory is already "numpair" (re-running it would WIPE the
            ...
            | NONE =>
              let val filtered0 =
                      HOLSource.inputFile {quietOpen = false, print = fn _ => ()}
                                          "/tmp/asweep_src.sml"
                  val f1 =
                      if name = "HEADER" andalso Theory.current_theory () = "numpair"
...
val () = pr ("NUMPAIRTHEORY_NAMES " ^ Int.toString (length (!btbl)) ^ "\n");
val () =
    let val os = TextIO.openOut "/tmp/numpairTheory_gen.sml"
    in TextIO.output (os, "structure numpairTheory = struct\n");
       app (fn (n, _) =>
               TextIO.output (os, "  val " ^ n ^ " = bt \"" ^ n ^ "\";\n"))
           (!btbl);
       (* name->thm table for later DB.fetch "numpair" serving *)
       TextIO.output (os, "  val dbTable : (string * Thm.thm) list = !btbl;\n");
       TextIO.output (os, "end;\n"); TextIO.closeOut os
    end;
val () = (PolyML.use "/tmp/numpairTheory_gen.sml"; pr "STRUCT_OK\n")
...
val () =
    if !smoke then
      (pr "EXPORTING /tmp/hol4_numpair\n";
       PolyML.export ("/tmp/hol4_numpair", PolyML.rootFunction);
       pr "NUMPAIR_SWEEP_DONE\n")
