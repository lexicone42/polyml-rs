...
if [ "$MODE" = "full" ]; then
  ...
  for t in export_roundtrip_live real_math parsetree_introspect overflow isabelle_pure isabelle_pure_arbint \
           ...
           concurrency_exit concurrency_alloc_storm concurrency_mutex_hammer \
           concurrency_parallel concurrency_server concurrency_fuzz os_sleep_poll weak_refs rounding_dns ffi_memory ffi_calls \
           isabelle_parallel isabelle_parallel_proving isabelle_cassini_lattice \
