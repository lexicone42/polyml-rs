...
val () = print ("@@genprog_gc_pressure_126=" ^ (wrap (fn () => Bool.toString ((List.null (List.take (treeLeaves (Lf (let fun loop k acc = if k <= 0 then String.size acc else loop (k-1) (acc ^ Int.toString (k mod 100)) in loop (Int.abs (((~5) * (~8))) mod 500 + 300) "" end)) @ [0,0,0,0,0], 5)))))) ^ "\n");
