...
val () = print ("@@genprog_gc_pressure_113=" ^ (wrap (fn () => Int.toString ((let fun loop k acc = if k <= 0 then acc else loop (k-1) (acc + List.foldl (fn (a,b) => a+b) 0 (List.tabulate (k mod 100 + 50, fn j => j + k))) in loop (Int.abs ((Int.abs (Int.max (0, 6)))) mod 4000 + 3000) 0 end)))) ^ "\n");
