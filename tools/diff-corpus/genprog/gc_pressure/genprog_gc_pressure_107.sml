...
val () = print ("@@genprog_gc_pressure_107=" ^ (wrap (fn () => pr2s ((let val (a, b) = (let val xs = List.tabulate (Int.abs (((9 div (~5)) - (Int.abs 10))) mod 3000 + 1000, fn i => i mod 13) in (List.length xs, List.foldl (fn (a,b) => a + b) 0 xs) end) in (b, a) end)))) ^ "\n");
