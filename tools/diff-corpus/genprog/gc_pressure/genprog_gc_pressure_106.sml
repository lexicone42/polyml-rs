...
val () = print ("@@genprog_gc_pressure_106=" ^ (wrap (fn () => let val t = ((Lf (let fun loop k acc = if k <= 0 then String.size acc else loop (k-1) (acc ^ Int.toString (k mod 100)) in loop (Int.abs ((~ (Int.min (2, (~4))))) mod 500 + 300) "" end))) in pr2s (sumTree t, depthTree t) end)) ^ "\n");
