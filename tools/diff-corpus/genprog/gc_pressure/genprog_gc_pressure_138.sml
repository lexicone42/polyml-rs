...
val () = print ("@@genprog_gc_pressure_138=" ^ (wrap (fn () => let val t = ((Lf (sumTree (if (((~9) * (~1)) > (let fun loop k acc = if k <= 0 then acc else loop (k-1) (acc + List.foldl (fn (a,b) => a+b) 0 (List.tabulate (k mod 100 + 50, fn j => j + k))) in loop (Int.abs (4) mod 4000 + 3000) 0 end)) then (let val v1 = (let val dpth = Int.abs (7) mod 6 + 8 in sumTree (buildTree dpth) end) in (mapTree (fn x => (x + (~4))) (Lf (~4))) end) else (mapTree (fn x => ((~1) - x)) (Br ((Lf (~3)), 9, (Lf 5)))))))) in pr2s (sumTree t, depthTree t) end)) ^ "\n");
