...
val () = print ("@@genprog_gc_pressure_144=" ^ (wrap (fn () => Int.toString ((let fun f n acc = if n <= 0 then acc else f (n-1) (acc+n) in f (Int.abs ((let fun loop k acc = if k <= 0 then acc else loop (k-1) (acc + List.foldl (fn (a,b) => a+b) 0 (List.tabulate (k mod 100 + 50, fn j => j + k))) in loop (Int.abs ((Int.abs (4 + (~6)))) mod 4000 + 3000) 0 end)) mod 1000) 0 end)))) ^ "\n");
