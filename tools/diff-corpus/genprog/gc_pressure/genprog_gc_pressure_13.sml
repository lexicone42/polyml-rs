...
val () = print ("@@genprog_gc_pressure_13=" ^ (wrap (fn () => let val t = ((mirrorTree (Br ((mapTree (fn x => (Int.min ((Int.max ((~5), (~6))), (x div (~9))))) (buildTree (Int.abs (((~1) + 2)) mod 6 + 6))), (case (case (List.take (treeLeaves (Lf (~9)) @ [0,0,0,0,0], 5)) of [] => [] | (a :: rest) => rest @ [a]) of [] => 0 | [a] => a | (a :: b :: _) => a + b), (mirrorTree (if (6 <= 4) then (Lf (~10)) else (Lf (~5)))))))) in pr2s (sumTree t, depthTree t) end)) ^ "\n");
