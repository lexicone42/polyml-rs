...
val () = print ("@@genprog_gc_pressure_115=" ^ (wrap (fn () => Bool.toString (((if ((let val n = Int.abs ((Int.abs 5)) mod 4000 + 2000 in List.foldl (fn (a,b) => a + b) 0 (List.tabulate (n, (fn i => (0 * i)))) end) = (Int.max ((if false then 4 else 5), (case [(~4), 8, 9, 5] of [] => 0 | [a] => a | (a :: b :: _) => a + b)))) then (String.substring (String.concatWith "-" (List.map Int.toString (List.tabulate (Int.abs (((Int.min (7, (~5))) - ((~5) * 0))) mod 1000 + 500, fn i => i mod 10))), 0, 9)) else (((let val v1 = [3, (~8), (~3)] in "mqjqb" end) ^ (Int.toString 5)) ^ (Int.toString (Int.max (7, 4))))) = (Bool.toString (((1 - 10) = (#2 ((~3), 8))) andalso ((0 + 9) < (let val v2 = true in 3 end)))))))) ^ "\n");
