...
val () = print ("@@genprog_gc_pressure_118=" ^ (wrap (fn () => pr2s ((let val v1 = ((let val n = Int.abs (((Int.abs 1) div (Int.abs (~8)))) mod 1000 + 500 val rs = List.tabulate (n, fn i => ref (i * 3)) val () = List.app (fn r => r := !r + 1) rs in List.foldl (fn (r,acc) => acc + !r) 0 rs end), (#2 (let val v2 = (let val v3 = (Lf (~2)) in (6, 2) end) in (if false then v2 else v2) end))) in (let val p = (if (List.exists (fn x => ("8z" = "ayghn")) (if false then [] else [(~6), 1, (~6), (~2)])) then (if (if true then true else false) then (2, 2) else ((~10), 7)) else (let val xs = List.tabulate (Int.abs ((~1)) mod 3000 + 1000, fn i => i mod 13) in (List.length xs, List.foldl (fn (a,b) => a + b) 0 xs) end)) in (#2 p, #1 p) end) end)))) ^ "\n");
