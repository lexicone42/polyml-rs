...
val () = print ("@@genprog_gc_pressure_153=" ^ (wrap (fn () => il2s ((case (List.take (List.map (fn x => ((x mod (~4)) + (Int.max (5, 9)))) (List.tabulate (Int.abs (((~1) + (Int.max (2, (~5))))) mod 1500 + 500, fn i => i)), 4)) of [] => [] | (a :: rest) => rest @ [a])))) ^ "\n");
