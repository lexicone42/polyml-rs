...
val () = print ("@@genprog_dre_14=" ^ (wrap (fn () => il2s ((case (List.rev ((List.map (fn x => if x < 0 then raise E1 x else x + 1) ((List.rev [8, 3, (~6)]) handle E1 v1 => ((~5) :: [1, (~4), 0, 9]) | _ => ((List.map (fn x => if x < 0 then raise E1 x else x + 1) []) handle E1 _ => []))) handle E1 _ => [])) of [] => [] | _ :: t => t)))) ^ "\n");
