...
val () = print ("@@genprog_dre_114=" ^ (wrap (fn () => Bool.toString (((Int.toString ((treeProd (Br ((Leaf 6), (Leaf (~2))))) handle E1 v1 => (Int.abs (treeProd (Leaf (~9)))) | _ => (treeSum (Br ((Leaf 6), (Leaf (~10))))))) = (treeShow (let val v2 = (not ("ofxv" = "x")) in (((listToTree [(~6)]) handle E2 => (listToTree [2, (~8), 1, 0])) handle E1 v3 => (if false then (Leaf (~8)) else (Leaf 1)) | _ => (listToTree [0, 7, 4, (~7)])) end)))))) ^ "\n");
