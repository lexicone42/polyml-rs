...
val () = print ("@@genprog_dre_107=" ^ (wrap (fn () => pr2s ((let val v1 = (let val v2 = (Rgt (String.map Char.toUpper (case Non of Non => "none" | Som n => "some:" ^ Int.toString n))) in (Br (((Leaf (~10)) handle E1 v3 => (if true then (Leaf (~6)) else (Leaf 3)) | _ => (listToTree [(~6), (~3)])), (let val v4 = (case (Som 6) of Non => "none" | Som n => "some:" ^ Int.toString n) in (Br ((Leaf (~3)), (Leaf 3))) end))) end) in (let val t = (Leaf ((List.foldl (fn (a,b) => a + b) 0 [8, 2, 2]) * (1 + 7))) in (treeSum t, treeDepth t) end) end)))) ^ "\n");
