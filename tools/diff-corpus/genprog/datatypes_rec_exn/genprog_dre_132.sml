...
val () = print ("@@genprog_dre_132=" ^ (wrap (fn () => pr2s (((Int.min ((let val v1 = (((if true then (Rgt "5") else (Rgt "mg7j")) handle E1 v2 => raise E2) handle E2 => (if true then (Rgt "h2lr") else (Lft (~9))) | _ => (let val v3 = "85" in (Rgt "hv2") end)) in (((#1 ((~3), (~5))) handle E1 v4 => raise E2) handle E2 => (case Non of Non => 0 | Som 0 => 100 | Som n => n) | _ => ((3 handle E1 v5 => raise E2) handle E2 => 0 | _ => 4)) end), (case (let val v6 = (let val v7 = false in [] end) in (let val n = 1 in if n mod 2 = 0 then Lft n else Rgt (Int.toString n) end) end) of Lft n => n | Rgt s => String.size s))), (treeProd (listToTree (List.rev (treeFlat (Leaf (~3)))))))))) ^ "\n");
