...
val () = print ("@@genprog_dre_128=" ^ (wrap (fn () => pr2s ((let val t = (listToTree (((([1] @ [2]) handle E1 v1 => raise E2) handle E2 => (List.filter (fn x => x mod 2 = 0) [(~7)]) | _ => (case [9, 4] of [] => [] | _ :: t => t)) @ ((1 + (~2)) :: (List.map (fn x => (Int.max ((~5), x))) [(~3), 10, (~7), 4])))) in (treeSum t, treeDepth t) end)))) ^ "\n");
