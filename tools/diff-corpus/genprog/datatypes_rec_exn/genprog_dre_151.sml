...
val () = print ("@@genprog_dre_151=" ^ (wrap (fn () => (case ((let val n = (List.length ((let val v1 = (Bool.toString true) in (let val v2 = true in 3 end) end) :: ((treeProd (Leaf (~5))) :: (let val v3 = (Som 3) in [(~8)] end)))) in if n mod 2 = 0 then Lft n else Rgt (Int.toString n) end)) of Lft n => "L" ^ Int.toString n | Rgt s => "R" ^ s))) ^ "\n");
