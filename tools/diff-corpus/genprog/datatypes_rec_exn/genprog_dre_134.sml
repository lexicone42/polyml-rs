...
val () = print ("@@genprog_dre_134=" ^ (wrap (fn () => Int.toString ((case (if (List.all (fn x => x < 100) (List.rev ((List.map (fn x => if x < 0 then raise E1 x else x + 1) [3, 4, (~9), 2]) handle E1 _ => []))) then (Som (treeDepth (listToTree []))) else (if (List.exists (fn x => x > 0) (List.map (fn x => (#2 (10, (~9)))) [])) then (case (((Som 8) handle E1 v1 => raise E2) handle E2 => (Som 7) | _ => Non) of Non => Non | Som n => Som (n + 1)) else (if ((~5) = 8) then (treeFind (Leaf (~6))) else (if false then (Som (~2)) else Non)))) of Non => 0 | Som 0 => 100 | Som n => n)))) ^ "\n");
