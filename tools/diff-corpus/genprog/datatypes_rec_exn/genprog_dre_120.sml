...
val () = print ("@@genprog_dre_120=" ^ (wrap (fn () => Int.toString ((String.size (case ((Som (let val v1 = ((~7), (~4)) in (~1) end)) handle E1 v2 => (if (true handle E1 v3 => false | _ => true) then (let val v4 = [(~9), 6] in Non end) else Non) | _ => (treeFind (treeMap (fn x => ((case (Som 9) of Som n => n | Non => raise E1 (~1)) handle E1 k => k)) (Leaf (~6))))) of Non => "none" | Som n => "some:" ^ Int.toString n))))) ^ "\n");
