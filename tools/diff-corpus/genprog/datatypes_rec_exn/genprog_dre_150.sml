...
val () = print ("@@genprog_dre_150=" ^ (wrap (fn () => (case ((Som ((List.length (List.map (fn x => (case (Rgt "x1h") of Lft n => n | Rgt s => String.size s)) ([9] @ []))) mod (treeSum (((let val v1 = (Leaf (~8)) in (Leaf 5) end) handle E2 => (listToTree [5, 10, 0, (~10)])) handle E1 v2 => (listToTree [6, 1, 3]) | _ => (if false then (Leaf 2) else (Leaf (~6)))))))) of Non => "none" | Som n => "some:" ^ Int.toString n))) ^ "\n");
