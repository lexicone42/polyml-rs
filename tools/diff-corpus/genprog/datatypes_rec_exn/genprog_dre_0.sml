...
val () = print ("@@genprog_dre_0=" ^ (wrap (fn () => ((((String.map Char.toUpper ((case (Rgt "0wx") of Lft n => "L" ^ Int.toString n | Rgt s => "R" ^ s) handle E1 v1 => (let val v2 = (Lft (~7)) in "bf8u" end) | _ => (Bool.toString false))) ^ (String.map Char.toUpper (((if false then "n1e" else "60") handle E2 => (Int.toString (~10))) handle E1 v3 => (String.concatWith "-" (List.map Int.toString [(~5), 4])) | _ => (case Non of Non => "none" | Som n => "some:" ^ Int.toString n)))) ^ (case (treeFind (treeMap (fn x => ((~6) mod (~9))) ((Leaf (~7)) handle E1 v4 => (Leaf (~8)) | _ => (Leaf 8)))) of Non => "none" | Som n => "some:" ^ Int.toString n))))) ^ "\n");
