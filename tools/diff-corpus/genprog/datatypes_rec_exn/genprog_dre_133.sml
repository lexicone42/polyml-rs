...
val () = print ("@@genprog_dre_133=" ^ (wrap (fn () => Int.toString ((String.size (if (treeAny (let val v1 = (List.foldl (fn (a,b) => a + b) 0 []) in (Br ((Leaf 8), (Leaf (~5)))) end)) then (treeShow (Br ((let val v2 = [3] in (Leaf 1) end), (treeMap (fn x => ((case (Som 10) of Som n => n | Non => raise E1 (~1)) handle E1 k => k)) (Leaf 0))))) else (let val v3 = ((treeShow (Leaf 10)) = ("4qy" handle E1 v4 => "" | _ => "e")) in (String.map Char.toUpper (treeShow (Leaf (~10)))) end)))))) ^ "\n");
