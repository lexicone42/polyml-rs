...
val () = print ("@@genprog_dre_129=" ^ (wrap (fn () => Int.toString ((Int.max ((treeDepth (if (((case (Rgt "5") of Lft _ => true | Rgt _ => false) handle E1 v1 => raise E2) handle E2 => ("7i1d0" = "") | _ => (4 = 6)) then (Br ((treeMap (fn x => (x div x)) (Leaf 5)), ((Leaf 4) handle E1 v2 => (Leaf (~8)) | _ => (Leaf 6)))) else (listToTree ([6, 2, 0, 3] @ [3])))), ((((List.length [(~3)]) + (treeDepth (Leaf 6))) * ((Int.abs (~3)) div (~ 6))) + (case (if (true orelse false) then (if false then (Som 4) else (Som 6)) else (let val v3 = Non in Non end)) of Non => 0 | Som 0 => 100 | Som n => n))))))) ^ "\n");
