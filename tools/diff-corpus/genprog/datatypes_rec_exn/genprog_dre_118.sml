...
val () = print ("@@genprog_dre_118=" ^ (wrap (fn () => Bool.toString (((List.foldl (fn (a,b) => a + b) 0 ((List.map (fn x => if x < 0 then raise E1 x else x + 1) (case ((~10) :: [7]) of [] => [] | _ :: t => t)) handle E1 _ => [])) = ((case (let val v1 = ((~3) :: [8, (~7), (~7), 0]) in (Som (~6)) end) of Non => 0 | Som 0 => 100 | Som n => n) mod (((case ((Som 8) handle E1 v2 => Non | _ => (Som (~6))) of Som n => n | Non => raise E1 (~1)) handle E1 k => k) * ((let val v3 = 5 in 6 end) mod (case Non of Non => 0 | Som 0 => 100 | Som n => n)))))))) ^ "\n");
