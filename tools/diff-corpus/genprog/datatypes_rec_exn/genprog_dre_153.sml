...
val () = print ("@@genprog_dre_153=" ^ (wrap (fn () => Int.toString ((~ (List.foldl (fn (a,b) => a + b) 0 ((((List.map (fn x => if x < 0 then raise E1 x else x + 1) ((List.map (fn x => if x < 0 then raise E1 x else x + 1) [2, (~6)]) handle E1 _ => [])) handle E1 _ => []) handle E2 => (let val v2 = (let val v3 = (10, 6) in (Leaf (~6)) end) in ([7, 7, 10, 2] @ []) end)) handle E1 v1 => (List.rev ((List.map (fn x => if x < 0 then raise E1 x else x + 1) [10, 5, (~2)]) handle E1 _ => [])) | _ => (List.filter (fn x => x mod 2 = 0) (List.rev [(~8), 3])))))))) ^ "\n");
