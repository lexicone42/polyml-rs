...
val () = print ("@@genprog_dre_147=" ^ (wrap (fn () => Int.toString ((((treeProd (listToTree ([10, 7, (~9), (~4)] handle E1 v1 => [(~7), 1, (~8), (~3)] | _ => [(~6), 5, 2]))) + ((Int.max ((treeSum (Leaf 2)), ((case (Som 9) of Som n => n | Non => raise E1 (~1)) handle E1 k => k))) mod (Int.abs (#2 (1, 8))))) * (treeProd (if (((~6) > 10) andalso (false andalso true)) then (let val v2 = (case (Lft 5) of Lft _ => true | Rgt _ => false) in (listToTree [5, 0]) end) else (if (case (Som 2) of Non => false | Som _ => true) then (if true then (Leaf (~9)) else (Leaf 0)) else (let val v3 = "h68v" in (Leaf 2) end)))))))) ^ "\n");
