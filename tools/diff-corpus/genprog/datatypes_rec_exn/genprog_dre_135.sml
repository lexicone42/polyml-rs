...
val () = print ("@@genprog_dre_135=" ^ (wrap (fn () => (case ((let val n = ((if (if (false orelse false) then ((~2) <= (~4)) else (not false)) then (Int.abs ((~7) + 9)) else ((let val v1 = true in (~1) end) * (3 - 5))) - ((case (case (Som 6) of Non => Non | Som n => Som (n + 1)) of Non => 0 | Som 0 => 100 | Som n => n) + (Int.max (((~5) mod (~9)), (~ 9))))) in if n mod 2 = 0 then Lft n else Rgt (Int.toString n) end)) of Lft n => "L" ^ Int.toString n | Rgt s => "R" ^ s))) ^ "\n");
