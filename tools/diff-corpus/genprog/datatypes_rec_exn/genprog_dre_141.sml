...
val () = print ("@@genprog_dre_141=" ^ (wrap (fn () => ((if ((Int.toString (#2 (let val t = (Leaf 3) in (treeSum t, treeDepth t) end))) = (String.map Char.toUpper (String.concatWith "-" (List.map Int.toString (List.rev []))))) then (if ((Int.toString ((~3) - (~7))) = (Bool.toString (false orelse true))) then (Bool.toString ((String.substring ("vjea5", 0, 1)) = (if true then "ptbkv" else "5q"))) else ((String.map Char.toUpper (let val v1 = "z3ss0" in v1 end)) ^ ((case (Som 7) of Non => "none" | Som n => "some:" ^ Int.toString n) ^ ("rs p" ^ "mi")))) else (String.concatWith "-" (List.map Int.toString (((([7, (~3), 4, 10] handle E2 => [(~3)]) handle E1 v2 => [10, 10, 6, (~4)] | _ => [(~6)]) @ (List.filter (fn x => x mod 2 = 0) [7, 10])) handle E1 v3 => (List.filter (fn x => x mod 2 = 0) ((~1) :: [9, (~10), (~7), (~5)])) | _ => (List.rev (treeFlat (Leaf 8)))))))))) ^ "\n");
