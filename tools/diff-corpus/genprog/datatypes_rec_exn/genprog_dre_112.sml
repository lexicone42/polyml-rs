...
val () = print ("@@genprog_dre_112=" ^ (wrap (fn () => il2s ((case ((List.rev (treeFlat (((Leaf (~4)) handle E1 v1 => raise E2) handle E2 => (Leaf 2) | _ => (Leaf (~4))))) @ (let val v2 = ((#1 ((~7), 9)) :: ([] handle E1 v3 => [6, 1] | _ => [(~3), (~9)])) in (case ((List.map (fn x => if x < 0 then raise E1 x else x + 1) [(~5), (~10), (~5)]) handle E1 _ => []) of [] => [] | _ :: t => t) end)) of [] => [] | _ :: t => t)))) ^ "\n");
