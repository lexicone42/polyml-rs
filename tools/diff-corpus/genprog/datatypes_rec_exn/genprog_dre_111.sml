...
val () = print ("@@genprog_dre_111=" ^ (wrap (fn () => pr2s ((let val t = (let val v1 = (Leaf (String.size (Bool.toString true))) in (if (((List.exists (fn x => x > 0) [7, 2, (~9), (~5)]) handle E1 v2 => raise E2) handle E2 => (treeAny v1) | _ => ("m61" = "f8")) then (Leaf (#2 (7, (~6)))) else (let val v3 = (if false then (Rgt "yp") else (Rgt "")) in (listToTree [4, (~1), (~10)]) end)) end) in (treeSum t, treeDepth t) end)))) ^ "\n");
