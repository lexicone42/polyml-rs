...
val () = print ("@@genprog_dre_103=" ^ (wrap (fn () => ((Bool.toString ((List.foldl (fn (a,b) => a + b) 0 ((List.filter (fn x => x mod 2 = 0) []) handle E1 v1 => (([3, 1] handle E2 => [5, (~10)]) handle E1 v2 => [(~1), (~5)] | _ => [6, (~3), (~1)]) | _ => ((List.map (fn x => if x < 0 then raise E1 x else x + 1) [0]) handle E1 _ => []))) > (((List.foldl (fn (a,b) => a + b) 0 [(~3), (~10)]) - (4 div (~1))) * (case Non of Non => 0 | Som 0 => 100 | Som n => n))))))) ^ "\n");
