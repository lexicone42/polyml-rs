...
val () = print ("@@genprog_dre_148=" ^ (wrap (fn () => (case ((let val v1 = (if (List.all (fn x => x < 100) (List.filter (fn x => x mod 2 = 0) ([(~4), (~3), 3] @ [(~4), (~8), 1, (~2)]))) then (case (Lft (8 div (~5))) of Lft _ => true | Rgt _ => false) else (not ((("" = "2zkiy") handle E2 => ((~6) < 2)) handle E1 v2 => (if false then true else false) | _ => (false orelse false)))) in (let val n = (String.size (let val v3 = (v1 orelse v1) in (case (Rgt "az") of Lft n => "L" ^ Int.toString n | Rgt s => "R" ^ s) end)) in if n mod 2 = 0 then Lft n else Rgt (Int.toString n) end) end)) of Lft n => "L" ^ Int.toString n | Rgt s => "R" ^ s))) ^ "\n");
