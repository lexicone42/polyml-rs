...
val () = print ("@@genprog_dre_125=" ^ (wrap (fn () => il2s ((List.rev (((if (let val v1 = (let val v2 = "cdgr" in (Rgt "elij") end) in ((true handle E2 => false) handle E1 v3 => true | _ => true) end) then ((let val v4 = (Leaf 7) in [] end) @ (if false then [(~2)] else [])) else (((List.map (fn x => if x < 0 then raise E1 x else x + 1) [9, 5]) handle E1 _ => []) handle E1 v5 => (let val v7 = (1, 5) in [] end) | _ => (let val v6 = (Som (~4)) in [8, (~5), (~7)] end))) handle E1 v8 => raise E2) handle E2 => (case (([] @ [(~10)]) @ (if true then [0] else [(~6), (~5), (~7)])) of [] => [] | _ :: t => t) | _ => (if (((~5) < 9) andalso (if false then false else true)) then (List.rev (List.filter (fn x => x mod 2 = 0) [7, 9, 5, (~4)])) else (List.filter (fn x => x mod 2 = 0) (treeFlat (Leaf 1))))))))) ^ "\n");
