...
val () = print ("@@genprog_dre_142=" ^ (wrap (fn () => (case ((if ((Int.min ((case (let val v1 = (Lft (~8)) in (Lft (~5)) end) of Lft n => n | Rgt s => String.size s), (case (let val n = 1 in if n mod 2 = 0 then Lft n else Rgt (Int.toString n) end) of Lft n => n | Rgt s => String.size s))) < ((case (Som 2) of Non => 0 | Som 0 => 100 | Som n => n) * (Int.abs (case (Som 7) of Non => 0 | Som 0 => 100 | Som n => n)))) then ((case (treeFind (Leaf 3)) of Non => Non | Som n => Som (n + 1)) handle E1 v2 => (let val v4 = (treeMap (fn x => (#1 (6, 5))) ((Leaf (~10)) handle E1 v5 => (Leaf 9) | _ => (Leaf 3))) in (treeFind ((Leaf (~10)) handle E1 v6 => v4 | _ => v4)) end) | _ => (treeFind ((Leaf 4) handle E1 v3 => (if false then (Leaf 9) else (Leaf (~7))) | _ => (Leaf 1)))) else (treeFind (Br ((listToTree ((~1) :: [])), ((Leaf (~8)) handle E1 v7 => (if false then (Leaf 4) else (Leaf 3)) | _ => ((Leaf (~10)) handle E1 v8 => (Leaf 9) | _ => (Leaf (~6))))))))) of Non => "none" | Som n => "some:" ^ Int.toString n))) ^ "\n");
