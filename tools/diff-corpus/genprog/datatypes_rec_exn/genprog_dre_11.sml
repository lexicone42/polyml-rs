...
val () = print ("@@genprog_dre_11=" ^ (wrap (fn () => ((let val v1 = (not (((treeProd (Leaf (~8))) > (treeSum (Leaf 4))) orelse (let val v2 = (String.substring ("pq1", 0, 1)) in (List.null [9, (~8), 1, 9]) end))) in (String.substring ((Bool.toString (if (7 > 9) then (List.all (fn x => x < 100) [(~8), 1, 10]) else (if v1 then v1 else v1))), 0, 1)) end)))) ^ "\n");
