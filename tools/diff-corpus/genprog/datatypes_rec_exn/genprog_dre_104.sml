...
val () = print ("@@genprog_dre_104=" ^ (wrap (fn () => il2s ((case (List.map (fn x => (treeProd (listToTree (if false then [(~4)] else [])))) (List.rev (List.filter (fn x => x mod 2 = 0) (let val v1 = [] in v1 end)))) of [] => [] | _ :: t => t)))) ^ "\n");
