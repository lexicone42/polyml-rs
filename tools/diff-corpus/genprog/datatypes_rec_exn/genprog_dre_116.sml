...
val () = print ("@@genprog_dre_116=" ^ (wrap (fn () => Bool.toString ((let val v1 = (List.filter (fn x => x mod 2 = 0) ((List.map (fn x => if x < 0 then raise E1 x else x + 1) (if (0 < 10) then ((List.map (fn x => if x < 0 then raise E1 x else x + 1) []) handle E1 _ => []) else ((List.map (fn x => if x < 0 then raise E1 x else x + 1) [9, 3]) handle E1 _ => []))) handle E1 _ => [])) in ((Int.min ((case (let val v2 = Non in (Lft 5) end) of Lft n => n | Rgt s => String.size s), (case (let val n = (~8) in if n mod 2 = 0 then Lft n else Rgt (Int.toString n) end) of Lft n => n | Rgt s => String.size s))) < (#2 ((List.foldl (fn (a,b) => a + b) 0 []), (String.size "y1eg")))) end)))) ^ "\n");
