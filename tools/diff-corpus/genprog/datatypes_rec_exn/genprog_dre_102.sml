...
val () = print ("@@genprog_dre_102=" ^ (wrap (fn () => treeShow ((treeMap (fn x => (treeSum (listToTree (List.filter (fn x => x mod 2 = 0) (if false then [] else [7, (~6), (~4)]))))) (if (((List.null (List.map (fn x => (x div 7)) [(~2), (~6)])) handle E2 => (let val v2 = (true orelse true) in (case (Rgt "c1j") of Lft _ => true | Rgt _ => false) end)) handle E1 v1 => (case (case Non of Non => Non | Som n => Som (n + 1)) of Non => false | Som _ => true) | _ => (not (true orelse false))) then (let val v3 = (listToTree (8 :: [(~3)])) in (Leaf (Int.min ((~4), 0))) end) else (let val v4 = ((#2 ((~6), (~2))) <= (let val v5 = [(~3), 8, (~1), (~6)] in 8 end)) in (((if false then (Leaf 6) else (Leaf (~9))) handle E1 v6 => raise E2) handle E2 => (Leaf 4) | _ => (Leaf (~8))) end)))))) ^ "\n");
