...
val () = print ("@@genprog_dre_124=" ^ (wrap (fn () => Int.toString (((case (case (if ((true handle E1 v1 => raise E2) handle E2 => false | _ => true) then (case Non of Non => Non | Som n => Som (n + 1)) else Non) of Non => Non | Som n => Som (n + 1)) of Non => 0 | Som 0 => 100 | Som n => n) * ((case (let val n = (3 div 2) in if n mod 2 = 0 then Lft n else Rgt (Int.toString n) end) of Lft n => n | Rgt s => String.size s) * (let val v2 = (let val v3 = (listToTree [5, (~9)]) in (let val p = (1, (~2)) in (#2 p, #1 p) end) end) in ((let val v4 = true in (~9) end) div ((~1) * (~10))) end)))))) ^ "\n");
