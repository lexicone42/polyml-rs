...
val () = print ("@@genprog_dre_121=" ^ (wrap (fn () => pr2s ((let val v1 = (let val p = (let val v2 = (List.null (case [(~9), (~2), (~9)] of [] => [] | _ :: t => t)) in (let val t = (if true then (Leaf (~5)) else (Leaf (~3))) in (treeSum t, treeDepth t) end) end) in (#2 p, #1 p) end) in (((#1 (let val t = (Leaf (~5)) in (treeSum t, treeDepth t) end)) div (Int.max (((2 handle E2 => (~7)) handle E1 v3 => v3 | _ => (~4)), (String.size "9")))), (treeProd (if (6 = (~10)) then (listToTree [(~7), 1]) else (let val v4 = v1 in (Leaf (~1)) end)))) end)))) ^ "\n");
