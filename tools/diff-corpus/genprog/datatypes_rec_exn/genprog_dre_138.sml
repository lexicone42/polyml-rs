...
val () = print ("@@genprog_dre_138=" ^ (wrap (fn () => Int.toString ((Int.abs (let val v1 = (treeMap (fn x => (#2 (if true then (4, 0) else ((~2), 10)))) (Leaf (List.foldl (fn (a,b) => a + b) 0 [2, (~6), 1]))) in (if ((List.foldl (fn (a,b) => a + b) 0 [3, (~1), (~9), 10]) = (~ 4)) then ((case ((Som 9) handle E1 v2 => (Som (~7)) | _ => Non) of Som n => n | Non => raise E1 (~1)) handle E1 k => k) else (List.foldl (fn (a,b) => a + b) 0 (if false then [(~2), (~8)] else []))) end))))) ^ "\n");
