...
val () = print ("@@genprog_dre_13=" ^ (wrap (fn () => Int.toString ((List.length (if ((case (if false then (Rgt "0hh") else (Lft 9)) of Lft n => n | Rgt s => String.size s) = (case (treeFind (Leaf (~8))) of Non => 0 | Som 0 => 100 | Som n => n)) then (((case (((Som 7) handle E2 => Non) handle E1 v1 => (Som (~3)) | _ => (Som 10)) of Som n => n | Non => raise E1 (~1)) handle E1 k => k) :: (treeFlat (Br ((Leaf 1), (Leaf (~3)))))) else (List.map (fn x => (((5 handle E1 v2 => raise E2) handle E2 => (~10) | _ => 6) - (treeDepth (Leaf (~4))))) ((List.map (fn x => if x < 0 then raise E1 x else x + 1) ((~1) :: [2])) handle E1 _ => []))))))) ^ "\n");
