...
val () = print ("@@genprog_dre_126=" ^ (wrap (fn () => Int.toString ((List.length (let val v1 = (((treeSum (Leaf 4)) * (let val v2 = 1 in 3 end)) < (case Non of Non => 0 | Som 0 => 100 | Som n => n)) in (List.map (fn x => ((if v1 then (~10) else x) div (1 + x))) ((case (Rgt "o9") of Lft n => n | Rgt s => String.size s) :: (List.map (fn x => (treeDepth (Leaf (~1)))) [(~1), (~1)]))) end))))) ^ "\n");
