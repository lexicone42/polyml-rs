...
val () = print ("@@genprog_dre_122=" ^ (wrap (fn () => Int.toString ((let val v1 = ((List.null (((List.map (fn x => if x < 0 then raise E1 x else x + 1) [(~7)]) handle E1 _ => []) handle E1 v2 => ([] @ [10, (~8)]) | _ => (9 :: [(~1), (~5), 5]))) andalso (case (Som ((~9) handle E1 v3 => 1 | _ => (~2))) of Non => false | Som _ => true)) in (let val v4 = (if ((Int.max ((~10), (~6))) <= (case (Rgt " e") of Lft n => n | Rgt s => String.size s)) then (let val v5 = ((~10), (~1)) in (Som (~3)) end) else ((treeFind (Leaf (~10))) handle E1 v6 => Non | _ => Non)) in (((treeSum (Leaf 6)) + ((~1) + 2)) * (treeSum ((Leaf 9) handle E1 v7 => (Leaf (~6)) | _ => (Leaf (~10))))) end) end)))) ^ "\n");
