...
val () = print ("@@genprog_dre_137=" ^ (wrap (fn () => (case ((let val v1 = (List.all (fn x => x < 100) (List.map (fn x => (Int.max ((x mod 8), ((case Non of Som n => n | Non => raise E1 (~1)) handle E1 k => k)))) ((List.map (fn x => (#1 ((~4), (~5)))) [9]) @ (case [2, 9, (~4)] of [] => [] | _ :: t => t)))) in (treeFind (Br ((Leaf (List.length [(~3)])), ((Leaf 10) handle E1 v2 => (Br ((Leaf 9), (Leaf (~7)))) | _ => (Br ((Leaf 8), (Leaf (~6)))))))) end)) of Non => "none" | Som n => "some:" ^ Int.toString n))) ^ "\n");
